...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
        ...
        `,document.head.appendChild(q)}let c=document.createElement("div");c.className="cw-zen-container";let p=document.createElement("div");Object.assign(p.style,{display:"none"});let d=document.createElement("div");d.className="cw-screens-container",p.appendChild(d),c.innerHTML=`
    ...
    `;let x=c.querySelector(".cw-hero-grid"),g=c.querySelector(".cw-acc-container"),f=c.querySelector(".cw-results-container"),u=c.querySelector(".cw-search-input"),y=c.querySelector(".cw-status-bar"),v=c.querySelector(".cw-status-text"),V=c.querySelector(".cw-footer-icons");i.forEach(([q,T])=>{let M=s(T.name),j=document.createElement("div");j.className="cw-hero-card",j.id=`hero-${q}`,j.style.setProperty("--hero-color",M.color),j.innerHTML=`
        ...
        `,j.onclick=L=>{if(L.target.closest(".cw-step-btn"))return;let k=n[q]?n[q].count:0;B(q,k>0?-k:1,T)},j.querySelector(".minus").onclick=()=>B(q,-1,T),j.querySelector(".plus").onclick=()=>B(q,1,T),j.dataset.color=M.color,x.appendChild(j)});function P(q,T){let M=s(T.name),j=document.createElement("div");return j.className="cw-task-item",j.dataset.id=q,j.innerHTML=`
  ...
  `,u.onfocus=()=>{g.style.boxShadow="0 4px 12px rgba(26,115,232,0.15)",g.style.border="1px solid #1a73e8"},u.onblur=()=>{g.style.boxShadow="0 2px 6px rgba(0,0,0,0.04)",g.style.border="1px solid transparent"},g.appendChild(f),g.appendChild(u),x.appendChild(g),d.appendChild(x);let y=document.createElement("div");y.style.cssText="flex: 1; overflow-y: auto; padding: 0 24px 40px 24px; scroll-behavior: smooth;",d.appendChild(y);let v=null;function V(){if(v)return;v=document.createElement("div"),v.style.cssText=`
      ...
      `,b.title=C,b.dataset.key=k;let A=document.createElement("div");A.style.cssText="width: 24px; height: 24px; margin-bottom: 2px; transition: transform 0.2s;",A.innerHTML=O||De.tasks;let h=document.createElement("div");return h.style.cssText="font-size: 9px; font-weight: 600; opacity: 0.7; letter-spacing: 0.3px;",h.textContent=C,b.appendChild(A),b.appendChild(h),b.onmouseenter=()=>{n!==k&&!(k==="history"&&a)&&(b.style.background="#F1F3F4",A.style.transform="scale(1.1)")},b.onmouseleave=()=>{n!==k&&!(k==="history"&&a)&&(b.style.background="transparent",A.style.transform="scale(1)")},b}function q(k){let C=document.getElementById(`cat-anchor-${k}`);C&&(C.scrollIntoView({behavior:"smooth",block:"start"}),n=k,T())}function T(){Object.keys(Ve).forEach(C=>{let O=p.querySelector(`#cw-sidebar-btn-${C}`);if(O)if(n===C&&!a){let b=ct[C];O.style.background=b.bg,O.style.color=b.color,O.querySelector("div:first-child").style.transform="scale(1.1)"}else O.style.background="transparent",O.style.color=s.textSecondary,O.querySelector("div:first-child").style.transform="scale(1)"});let k=p.querySelector("#cw-sidebar-btn-history");k&&(a?(k.style.background="#3C4043",k.style.color="#FFFFFF"):(k.style.background="transparent",k.style.color=s.textSecondary))}function M(){if(y.innerHTML="",e.trim()!==""){let C=[];if(Object.entries(Ve).forEach(([b,A])=>{let h=A.links.filter(S=>S.name.toLowerCase().includes(e.toLowerCase())||S.desc.toLowerCase().includes(e.toLowerCase()));C.push(...h.map(S=>({...S,_cat:b})))}),C.length===0){y.innerHTML='<div style="text-align:center; padding: 60px; color:#999; font-size:13px;">Nada encontrado.</div>';return}let O=document.createElement("div");O.innerHTML="Resultados da busca",O.style.cssText="font-size:12px; font-weight:700; color:#5f6368; margin:20px 0 10px; text-transform:uppercase; letter-spacing:0.5px;",y.appendChild(O),C.forEach(b=>{let A=j(b,De[b._cat],!1,b._cat);y.appendChild(A)});return}Object.entries(Ve).forEach(([C,O])=>{let b=ct[C],A=document.createElement("div"),h=document.createElement("div");h.id=`cat-anchor-${C}`,h.style.cssText=`
          ...
          `,h.innerHTML=`
            ...
            ${O.label}
          `,A.appendChild(h);let S=document.createElement("div");S.style.cssText="display: grid; grid-template-columns: 1fr; gap: 8px;",O.links.forEach(E=>{let I=j(E,De[C],!1,C);S.appendChild(I)}),A.appendChild(S),y.appendChild(A)});let k=document.createElement("div");k.style.height="80px",y.appendChild(k)}function j(k,C,O,b){let A=document.createElement("div"),h=ct[b]||ct.history;A.style.cssText=`
