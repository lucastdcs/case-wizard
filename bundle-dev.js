...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
    ...
    `;let u=c.querySelector(".cw-hero-grid"),g=c.querySelector(".cw-acc-container"),h=c.querySelector(".cw-results-container"),S=c.querySelector(".cw-search-input"),f=c.querySelector(".cw-status-bar"),T=c.querySelector(".cw-status-text"),O=c.querySelector(".cw-footer-icons");function L(q,F){return N=>{if(N.target.closest(".cw-step-btn"))return;let V=a[q]?a[q].count:0;G(q,V>0?-V:1,F)}}i.forEach(([q,F])=>{let N=n(F.name),V=document.createElement("div");V.className="cw-hero-card",V.id=`hero-${q}`,V.style.setProperty("--hero-color",N.color),V.innerHTML=`
        ...
        `,V.onclick=L(q,F),V.querySelector(".minus").onclick=()=>G(q,-1,F),V.querySelector(".plus").onclick=()=>G(q,1,F),V.dataset.color=N.color,u.appendChild(V)});function _(q,F){let N=n(F.name),V=document.createElement("div");return V.className="cw-task-item",V.dataset.id=q,V.innerHTML=`
        ...
        `,V.onclick=L(q,F),V.querySelector(".minus").onclick=()=>G(q,-1,F),V.querySelector(".plus").onclick=()=>G(q,1,F),V}Object.entries(s).forEach(([q,F])=>{let N=document.createElement("div");N.className="cw-acc-group";let V=document.createElement("div");V.className="cw-acc-header",V.innerHTML=`
            <div class="cw-acc-title">
                <div class="cw-acc-dot" style="background:${F.brand.color}"></div>
                ${q}
            </div>
            <div class="cw-acc-icon">\u25BC</div>
        `,V.onclick=()=>{g.querySelectorAll(".cw-acc-group.open").forEach(D=>{D!==N&&D.classList.remove("open")}),N.classList.toggle("open")};let B=document.createElement("div");B.className="cw-acc-body",F.tasks.forEach(D=>{let Y=_(D.key,D);B.appendChild(Y)}),N.appendChild(V),N.appendChild(B),g.appendChild(N)});function G(q,F,N){a[q]||(a[q]={count:0,data:N,brand:n(N.name)}),a[q].count+=F,a[q].count<=0&&delete a[q],X(),oe(),t&&t()}function X(){i.forEach(([B])=>{let D=u.querySelector(`#hero-${B}`);if(!D)return;let Y=a[B];Y?(D.classList.add("active"),D.querySelector(".cw-step-val").textContent=Y.count,D.querySelector(".cw-step-val").style.color=D.dataset.color,D.classList.toggle("ts-success",Ft(B,o))):(D.classList.remove("active"),D.classList.remove("ts-success"))}),c.querySelectorAll(".cw-task-item").forEach(B=>{let D=B.dataset.id,Y=a[D];Y?(B.classList.add("selected"),B.querySelector(".cw-step-val").textContent=Y.count,B.classList.toggle("ts-success",Ft(D,o))):(B.classList.remove("selected"),B.classList.remove("ts-success"))});let F=Object.keys(a),N=0,V=[];if(F.forEach(B=>{let D=a[B];N+=D.count;for(let Y=0;Y<D.count;Y++)V.length<6&&V.push(D.brand)}),N>0){f.classList.add("visible");let B=N>1?"A\xE7\xF5es":"A\xE7\xE3o",D=N>1?"definidas":"definida";T.textContent=`${N} ${B} ${D}`,O.innerHTML="",V.forEach(Y=>{let E=document.createElement("div");E.className="cw-mini-icon",E.innerHTML=He[Y.icon]||He.default;let I=E.querySelector("svg");I&&(I.style.width="14px",I.style.height="14px"),O.appendChild(E)})}else f.classList.remove("visible")}S.addEventListener("input",q=>{let F=q.target.value.toLowerCase();if(F.length>0){g.style.display="none",h.style.display="block",h.innerHTML="";let N=!1;Object.entries(Fe).forEach(([V,B])=>{if(B.name.toLowerCase().includes(F)){N=!0;let D=_(V,B);a[V]&&(D.classList.add("selected"),D.querySelector(".cw-step-val").textContent=a[V].count),h.appendChild(D)}}),N||(h.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else g.style.display="block",h.style.display="none"});function oe(){d.innerHTML="";let q=Object.keys(a),F=!1;if(q.length===0){d.innerHTML=`<div class="cw-empty-state">${e("selecione_tarefas")}</div>`,b.style.display="none";return}let N=document.createElement("div");N.className="cw-info-banner",N.innerHTML=`
            <span style="font-size:14px">\u2139\uFE0F</span>
            <span>
                Os screenshots seguem as diretrizes atuais do Win Criteria e pol\xEDticas de Tag Support.
            </span>
        `,d.appendChild(N),q.forEach(V=>{let B=a[V].data,D=a[V].count,Y=a[V].brand,E=Ft(V,o),I=o.screenshotMode||"implementation",H=B.screenshots?.[I]||[];if(H.length>0||E){F=!0;for(let M=1;M<=D;M++){let J=document.createElement("div");J.className="cw-screen-card",E&&J.classList.add("ts-success"),J.style.setProperty("--brand-color",Y.color),J.style.setProperty("--brand-bg",Y.bg),J.style.setProperty("--brand-shadow",Y.color+"40");let ae=document.createElement("div");ae.className="cw-card-header";let te=document.createElement("div");te.className="cw-card-icon",te.innerHTML=He[Y.icon]||He.default;let j=document.createElement("div");j.style.cssText="flex:1; display:flex; align-items:center; gap:8px;";let U=document.createElement("input");U.className="cw-card-title-input",U.id=`name-${V}-${M}`,U.value=`${B.name}${D>1?" #"+M:""}`,U.title="Clique para renomear esta task";let K=document.createElement("span");if(K.className="cw-edit-hint",K.innerHTML="\u270E Renomear",j.appendChild(U),j.appendChild(K),ae.appendChild(te),ae.appendChild(j),J.appendChild(ae),E){let l=document.createElement("div");l.className="cw-ts-disclaimer-box",l.innerHTML=`
                <span>${e("ts_disclaimer")}</span>
                <button class="cw-btn-ts-force">${e("incluir_mesmo_assim")}</button>
            `,l.querySelector("button").onclick=()=>{o.toggleForcedScreenshot(V,!0)},J.appendChild(l)}else H.forEach((l,m)=>{let y=document.createElement("div");y.className="cw-input-group";let x=document.createElement("label");x.className="cw-input-label",x.textContent=l;let v=document.createElement("input");v.className="cw-input-field",v.id=`screen-${V}-${M}-${m}`,v.placeholder="Cole o link aqui...",v.setAttribute("autocomplete","off"),v.addEventListener("input",()=>{v.value.trim().length>5?v.classList.add("filled"):v.classList.remove("filled")});let A=document.createElement("div");A.className="cw-input-check",A.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>',y.appendChild(x),y.appendChild(v),y.appendChild(A),J.appendChild(y)});d.appendChild(J)}}}),b.style.display=F?"block":"none"}return{selectionElement:c,screenshotsElement:b,updateSubStatus:()=>oe(),getCheckedElements:()=>Object.keys(a).map(q=>({value:q,count:a[q].count})),setTaskCount:(q,F)=>{a[q]&&delete a[q],F>0&&Fe[q]&&G(q,F,Fe[q])},toggleTask:(q,F=!0)=>{let N=a[q];F&&!N?G(q,1,Fe[q]):!F&&N&&G(q,-N.count,Fe[q])},setLanguage:q=>{e=q;let F=c.querySelector(".js-hero-title");F&&(F.textContent=e("acesso_rapido"));let N=c.querySelector(".cw-search-input");N&&(N.placeholder=e("buscar_catalogo")),oe()},reset:()=>{for(let q in a)delete a[q];S.value="",g.style.display="block",h.style.display="none",X(),oe()}}}var ia={marginTop:"24px",marginBottom:"16px",padding:"20px",background:"#fff9e6",borderRadius:"20px",border:"1.5px solid #fbbc0540",display:"none",boxShadow:"0 4px 12px rgba(251, 188, 5, 0.05)"},sa={fontSize:"12px",color:"#b06000",marginTop:"8px",lineHeight:"1.4"},ra={width:"100%",padding:"12px 16px",borderRadius:"12px",border:"1.5px solid #dadce0",fontSize:"14px",marginBottom:"16px",boxSizing:"border-box",background:"#fff"},la={display:"flex",gap:"20px",marginBottom:"12px"};function po(t){let e=document.createElement("div");e.id="tag-support-container",Object.assign(e.style,ia);let o=document.createElement("label");o.className="js-ts-main-label",o.textContent=t("utilizou_tag_support"),Object.assign(o.style,St,{marginTop:"0"});let a=document.createElement("div");Object.assign(a.style,la);let n=document.createElement("input");n.type="radio",n.name="ts_usage_mod",n.value="Sim",Object.assign(n.style,At);let i=document.createElement("label");i.textContent="Sim";let s=document.createElement("div");Object.assign(s.style,{display:"flex",alignItems:"center"}),s.appendChild(n),s.appendChild(i);let r=document.createElement("input");r.type="radio",r.name="ts_usage_mod",r.value="N\xE3o",r.checked=!0,Object.assign(r.style,At);let c=document.createElement("label");c.textContent="N\xE3o";let b=document.createElement("div");Object.assign(b.style,{display:"flex",alignItems:"center"}),b.appendChild(r),b.appendChild(c),a.appendChild(s),a.appendChild(b);let d=document.createElement("div");d.style.display="block";let u=document.createElement("label");u.className="js-ts-reason-label",u.textContent=t("motivo_ts"),Object.assign(u.style,St,{fontSize:"12px"});let g=document.createElement("input");g.type="text",Object.assign(g.style,ra);let h=document.createElement("div");h.className="js-ts-warning",h.innerHTML=`\u26A0\uFE0F <strong>${t("lembre_preencher_form")}</strong> <a href="https://docs.google.com/forms/d/e/1FAIpQLSeP_JM8D-6qHa5ZC93aTzj38WiO5zx8nyrWNPvbZhjJj6CpkA/viewform" target="_blank" style="color:#e37400; text-decoration:underline;">Link aqui</a>`,Object.assign(h.style,sa),d.appendChild(u),d.appendChild(g),d.appendChild(h),e.appendChild(o),e.appendChild(a),e.appendChild(d),n.onchange=()=>{d.style.display="none",W.setTagSupportUsed(!0)},r.onchange=()=>{d.style.display="block",W.setTagSupportUsed(!1)};function S(L,_){if(e.style.display="none",!L||!_||_.length===0)return;_.some(X=>X==="ads_conversion_tracking"||X==="ads_enhanced_conversions")?e.style.display="block":(O(),W.setTagSupportUsed(!1))}function f(){if(e.style.display==="none")return"";let L=`<br><b>Utilizou Tag Support?</b> ${n.checked?"\u2705 Sim":"\u274C N\xE3o"}`;return r.checked&&g.value.trim()!==""&&(L+=`<br><b>Motivo:</b> ${g.value}`),L+="<br>",L}function T(L){t=L,o.textContent=t("utilizou_tag_support"),u.textContent=t("motivo_ts"),h.innerHTML=`\u26A0\uFE0F <strong>${t("lembre_preencher_form")}</strong> <a href="https://docs.google.com/forms/d/e/1FAIpQLSeP_JM8D-6qHa5ZC93aTzj38WiO5zx8nyrWNPvbZhjJj6CpkA/viewform" target="_blank" style="color:#b06000; text-decoration:underline;">Link aqui</a>`}function O(){e.style.display="none",r.checked=!0,n.checked=!1,d.style.display="block",g.value=""}return{element:e,updateVisibility:S,getOutput:f,setLanguage:T,reset:O}}var It="cw_notes_parking_lot",lt="cw_notes_emergency_save";var we={getAll:()=>{try{return JSON.parse(localStorage.getItem(It)||"[]")}catch{return[]}},save:t=>{let e=we.getAll(),o={id:Date.now().toString(),timestamp:new Date().toISOString(),...t};return e.unshift(o),e.length>5&&e.pop(),localStorage.setItem(It,JSON.stringify(e)),o},delete:t=>{let e=we.getAll();return e=e.filter(o=>o.id!==t),localStorage.setItem(It,JSON.stringify(e)),e},getCount:()=>we.getAll().length,saveEmergency:t=>{let e={timestamp:Date.now(),data:t};localStorage.setItem(lt,JSON.stringify(e))},getEmergency:()=>{try{let t=localStorage.getItem(lt);if(!t)return null;let e=JSON.parse(t);return Date.now()-e.timestamp>432e5?(localStorage.removeItem(lt),null):!e.data||!e.data.subStatus?null:e.data}catch{return null}},clearEmergency:()=>{localStorage.removeItem(lt)}};var ct=["lucaste","ricardogi"];var le={glassBg:"rgba(61, 61, 61, 0.77)",glassBorder:"rgba(255, 255, 255, 0.15)",glassActive:"rgba(79, 79, 79, 0.89)",glassHighlight:"rgba(255, 255, 255, 0.08)",iconIdle:"#c2c5c8ff",iconActive:"#FFFFFF",blue:"#8AB4F8",red:"#F28B82",purple:"#C58AF9",green:"#81C995",orange:"#F9AB00",teal:"#00BFA5",pink:"#F48FB1",gray:"#9AA0A6"};function dt(t){let e=document.getElementById("cw-btn-notes");if(!e)return;let o=e.querySelector(".cw-dot-dirty");t?o||(o=document.createElement("div"),o.className="cw-dot-dirty",e.appendChild(o)):o&&o.remove()}function uo(t){let e="cw-command-center-style";if(!document.getElementById(e)){let f=document.createElement("style");f.id=e,f.innerHTML=`
    ...
    `;let n=document.createElement("div");n.className="cw-focus-backdrop",document.body.appendChild(n),document.body.appendChild(a);let i=(f,T)=>{R.playClick(),a.querySelector(`.${f}`).classList.toggle("active"),T()};if(a.querySelector(".notes").onclick=f=>{f.stopPropagation(),i("notes",t.toggleNotes)},a.querySelector(".bauform").onclick=f=>{f.stopPropagation(),i("bauform",t.toggleBAUForm)},a.querySelector(".email").onclick=f=>{f.stopPropagation(),i("email",t.toggleEmail)},a.querySelector(".script").onclick=f=>{f.stopPropagation(),i("script",t.toggleScript)},a.querySelector(".links").onclick=f=>{f.stopPropagation(),i("links",t.toggleLinks)},a.querySelector(".library").onclick=f=>{f.stopPropagation(),i("library",t.toggleLibrary)},a.querySelector(".timezone").onclick=f=>{f.stopPropagation(),i("timezone",t.toggleTimezone)},a.querySelector(".configs").onclick=f=>{f.stopPropagation(),i("configs",t.toggleConfigs)},a.querySelector(".broadcast").onclick=f=>{f.stopPropagation(),i("broadcast",()=>{let T=f.currentTarget.querySelector(".cw-badge");T&&T.remove(),t.broadcastControl&&t.broadcastControl.toggle()})},a.querySelectorAll(".cw-btn").forEach(f=>{f.addEventListener("mouseenter",()=>R.playHover())}),t.broadcastControl&&t.broadcastControl.hasUnread){let f=document.createElement("div");f.className="cw-badge",a.querySelector(".broadcast").appendChild(f)}let s=null;a.onmouseleave=()=>{a.querySelector(".cw-btn.active")||a.classList.contains("processing-center")||(s=setTimeout(()=>{a.classList.add("collapsed")},3e3))},a.onmouseenter=()=>{s&&clearTimeout(s)},(async function(){let T=()=>{let L=fe();if(L){let _=L.split("@")[0].toLowerCase();if(ct.includes(_)){let G=a.querySelector("#cw-admin-tag");G&&G.classList.add("visible")}}else setTimeout(T,2e3)};T(),await ne(2800),a.classList.add("docked"),await ne(300);let O=a.querySelectorAll(".cw-btn");a.querySelectorAll(".cw-sep").forEach(L=>L.classList.add("visible"));for(let L=0;L<O.length;L++)O[L].classList.add("popped"),await ne(40);await ne(100),a.classList.add("system-check"),await ne(100),a.classList.add("system-ready"),setTimeout(()=>a.classList.remove("system-ready"),400)})();let r=!1,c,b,d,u,g=3;a.onmousedown=f=>{if(f.target.closest("button"))return;f.preventDefault(),c=f.clientX,b=f.clientY;let T=a.getBoundingClientRect();d=T.left,u=T.top,document.addEventListener("mousemove",h),document.addEventListener("mouseup",S)};function h(f){let T=f.clientX-c,O=f.clientY-b;!r&&Math.sqrt(T*T+O*O)>g&&(r=!0,a.classList.add("dragging"),a.style.transition="none",s&&clearTimeout(s)),r&&(a.style.left=`${d+T}px`,a.style.top=`${u+O}px`,a.style.right="auto",a.style.bottom="auto",a.style.transform="none")}function S(f){if(document.removeEventListener("mousemove",h),document.removeEventListener("mouseup",S),r){r=!1,a.classList.remove("dragging");let T=window.innerWidth,O=window.innerHeight,L=a.getBoundingClientRect(),_=L.left+L.width/2,G;_<T/2?(G=24,a.classList.remove("side-right"),a.classList.add("side-left")):(G=T-L.width-24,a.classList.remove("side-left"),a.classList.add("side-right"));let X=Be(L.top,24,O-L.height-24);setTimeout(()=>{a.style.setProperty("transition","left 0.3s cubic-bezier(0.4, 0, 0.2, 1), top 0.3s cubic-bezier(0.4, 0, 0.2, 1)","important"),a.style.left=`${G}px`,a.style.top=`${X}px`,a.style.bottom="auto",a.style.transform=""},10),setTimeout(()=>{a.style.transition="",a.style.removeProperty("transition")},700)}else{let T=a.querySelector(".cw-btn.active"),O=f.target.closest("button");if(a.classList.contains("collapsed")){let L=a.getBoundingClientRect(),_=window.innerHeight,G=L.top>_/2;if(a.style.setProperty("transition","none","important"),G){let X=_-L.bottom;a.style.top="auto",a.style.bottom=`${X}px`}else a.style.bottom="auto",a.style.top=`${L.top}px`;a.offsetWidth,a.style.removeProperty("transition"),a.classList.remove("collapsed"),R.playGenieOpen()}else!T&&!O&&(a.classList.add("collapsed"),R.playGenieOpen());O&&(O.style.transform="scale(0.9)",setTimeout(()=>O.style.transform="",150))}}}function Ze(){let t=document.querySelector(".cw-pill"),e=document.querySelector(".cw-focus-backdrop");if(!t)return()=>{};t.classList.remove("collapsed"),window._CW_ABORT_PROCESS=!1;let o=document.createElement("div");o.className="cw-center-stage",o.innerHTML=`
...
`.replace(/\n/g,"<br>");Qe(q);let F=await ut();F?(F.innerText.trim()===""&&(F.innerHTML=""),document.execCommand("insertHTML",!1,q),pt(F),Q("Nota gerada e inserida!")):Q("Copiado! Abra uma nota para colar.")},e}function Ee(t,e="info"){let o={info:"background: #e8f0fe; color: #1a73e8; padding: 2px 5px; border-radius: 3px;",warn:"background: #fef7e0; color: #b06000; padding: 2px 5px; border-radius: 3px;",error:"background: #fce8e6; color: #c5221f; padding: 2px 5px; border-radius: 3px;",success:"background: #e6f4ea; color: #137333; padding: 2px 5px; border-radius: 3px;"};console.log(`%c[EMAIL-BOT] ${t}`,o[e]||o.info)}function mt(t,e){if(!t)return;let o=`cw-warning-${t.id||Math.random().toString(36).substr(2,9)}`,a=document.getElementById(o);a&&a.remove();let n=t.getBoundingClientRect(),i=document.createElement("div");i.id=o,i.style.cssText=`
    ...
    `,document.head.appendChild(t)}function be(t,e,o){let a=document.getElementById(o);if(!e)return;let n=e.getAttribute("data-moved")==="true",i={x:0,y:0};if(a){let d=a.getBoundingClientRect();i.x=d.left+d.width/2,i.y=d.top+d.height/2}let s,r;if(!n)s=window.innerWidth/2,r=window.innerHeight/2;else{let d=e.getBoundingClientRect();s=d.left+d.width/2,r=d.top+d.height/2,s===0&&r===0&&(s=window.innerWidth/2,r=window.innerHeight/2)}let c=i.x-s,b=i.y-r;t?(R.playGenieOpen(),e.style.transition="none",e.style.opacity="0",e.style.pointerEvents="auto",n?e.style.transform=`translate(${c}px, ${b}px) scale(0.05)`:e.style.transform=`translate(calc(-50% + ${c}px), calc(-50% + ${b}px)) scale(0.05)`,e.offsetWidth,requestAnimationFrame(()=>{e.classList.add("open"),a&&a.classList.add("active"),e.style.transition="opacity 0.4s ease-out, transform 0.5s cubic-bezier(0.19, 1, 0.22, 1)",e.style.opacity="1",n?e.style.transform="translate(0, 0) scale(1)":e.style.transform="translate(-50%, -50%) scale(1)"}),typeof ho=="function"&&ho(e,o)):(R.playSwoosh(),e.style.transition="opacity 0.25s ease, transform 0.3s cubic-bezier(0.5, 0, 1, 1)",e.style.pointerEvents="none",requestAnimationFrame(()=>{e.style.opacity="0",n?e.style.transform=`translate(${c}px, ${b}px) scale(0.1)`:e.style.transform=`translate(calc(-50% + ${c}px), calc(-50% + ${b}px)) scale(0.1)`}),setTimeout(()=>{e.classList.remove("open"),a&&a.classList.remove("active"),e.style.transition="",e.style.transform=""},300),typeof qt=="function"&&qt(e))}function ho(t,e){qt(t);let o=a=>{if(!t.classList.contains("open"))return;let n=t.contains(a.target),i=document.querySelector(".cw-pill"),s=i&&i.contains(a.target);n?(t.classList.remove("idle"),t.style.zIndex="2147483648"):s||(t.classList.add("idle"),t.style.zIndex="2147483646")};t._idleHandler=o,document.addEventListener("mousedown",o)}function qt(t){t._idleHandler&&(document.removeEventListener("mousedown",t._idleHandler),t._idleHandler=null)}function yo(){let t="v4.0.0",{popup:e,content:o,header:a,animRefs:n,credit:i}=no(t,V),s=po(x),r=co(()=>{M(),W.setActiveTasks(r.getCheckedElements())},x,W),c=document.createElement("div");c.style.display="none";let b=lo((p,C)=>{J(p,C)});c.appendChild(b);let d=document.createElement("div");d.id="evidence-container",Object.assign(d.style,{display:"none",marginTop:"16px",padding:"16px",background:$.bgInput,border:`1px solid ${$.border}`,borderRadius:me.medium,boxShadow:Me.subtle});let u=document.createElement("div");u.className="cw-section-title",u.textContent=x("evidencias_contato"),d.appendChild(u);let g={},h=(p,C)=>{let k=document.createElement("div");k.style.marginBottom="12px";let P=document.createElement("label");P.textContent=C,P.setAttribute("for",p),P.style.cssText=`display: block; font-size: 11px; font-weight: 700; color: ${$.textSub}; margin-bottom: 6px; text-transform: uppercase;`;let z=document.createElement("input");return z.type="text",z.id=p,z.className="cw-input",z.placeholder="https://screenshot.googleplex.com/...",z.style.marginBottom="0",k.appendChild(P),k.appendChild(z),g[p]=z,k};d.appendChild(h("evidence-l1",x("ligacao_1"))),d.appendChild(h("evidence-l2",x("ligacao_2"))),d.appendChild(h("evidence-msg",x("mensagem_am")));let S=mo({onSaveCurrent:async()=>{let p=await l();return K(),p},onLoadDraft:p=>{y(p)},t:p=>x(p)}),f=D(),T=Y(),O=document.createElement("div"),L=A(),_=ae(S,x);o.appendChild(f),o.appendChild(T),o.appendChild(L),o.appendChild(c),o.appendChild(O),o.appendChild(d),r.selectionElement.style.display="none",r.screenshotsElement.style.display="none";let G=document.createElement("button");G.id="manual-task-toggle",G.textContent=x("gostaria_de_adicionar_uma_task")||"Gostaria de adicionar uma task",G.style.cssText=`display: none; width: 100%; padding: 14px; border: 2px dashed ${$.primary}; background: ${$.surface}; color: ${$.primary}; border-radius: ${me.medium}; cursor: pointer; font-size: 13px; font-weight: 700; margin-top: 8px; transition: all 0.2s ${de}; text-transform: uppercase; letter-spacing: 0.5px;`,G.onmouseenter=()=>{G.style.background=$.primaryBg},G.onmouseleave=()=>{G.style.background=$.surface},G.onclick=()=>{r.selectionElement.style.display="block",r.screenshotsElement.style.display="block",G.style.display="none"},o.appendChild(G),o.appendChild(r.selectionElement),o.appendChild(s.element),o.appendChild(r.screenshotsElement),o.appendChild(_);let X=document.createElement("div");X.style.display="none",X.style.flexGrow="1",X.style.minHeight="0",X.style.overflow="hidden";let oe=bo(()=>B());oe.style.height="100%",X.appendChild(oe),e.insertBefore(X,i);let q=a.lastElementChild;q&&(q.insertBefore(S.historyBtnWrapper,q.firstChild),q.insertBefore(v(),q.firstChild)),e.appendChild(S.drawer);let F=null;W.subscribe(p=>{w(p),N(),p.isDirty?(F&&clearTimeout(F),F=setTimeout(async()=>{let C=await l(!0);C.subStatus?we.saveEmergency(C):we.clearEmergency(),p.isDirty=!1},2e3)):F&&(clearTimeout(F),F=null)});function N(){let p=we.getCount()>0,C=!!W.currentSubStatus;dt(p||C)}function V(){W.visible=!W.visible,be(W.visible,e,"cw-btn-notes")}function B(){W.isSplitView=!W.isSplitView,W.isSplitView?(o.style.display="none",X.style.display="flex",X.style.flexDirection="column",n.googleLine&&(n.googleLine.style.background="linear-gradient(to right, #8e24aa, #7b1fa2)")):(o.style.display="flex",X.style.display="none",n.googleLine&&(n.googleLine.style.background="linear-gradient(to right, #4285F4, #EA4335, #FBBC05, #34A853)"))}function D(){let p=document.createElement("div");if(p.innerHTML=`
        ...
        `,!document.getElementById("cw-segmented-styles")){let k=document.createElement("style");k.id="cw-segmented-styles",k.innerHTML=`
            ...
            `,document.head.appendChild(k)}let C=(k,P)=>{let ee=p.querySelector(`#${k}`).querySelector(".cw-segmented-indicator");ee&&(ee.style.transform=`translateX(${P*100}%) translateX(${P*2}px)`)};return p.querySelectorAll("#lang-selector button").forEach((k,P)=>{k.onclick=()=>{W.setLanguage(k.dataset.lang),p.querySelectorAll("#lang-selector button").forEach(z=>z.classList.remove("active")),k.classList.add("active"),C("lang-selector",P),R.playHover(),W.currentSubStatus&&H(W.currentSubStatus)}}),p.querySelectorAll("#type-selector button").forEach((k,P)=>{k.onclick=()=>{W.setCaseType(k.dataset.type),p.querySelectorAll("#type-selector button").forEach(z=>z.classList.remove("active")),k.classList.add("active"),C("type-selector",P),R.playHover(),W.currentSubStatus&&H(W.currentSubStatus)}}),p.querySelectorAll("#portugal-selector button").forEach((k,P)=>{k.onclick=()=>{W.setPortugalCase(k.dataset.val==="true"),p.querySelectorAll("#portugal-selector button").forEach(z=>z.classList.remove("active")),k.classList.add("active"),C("portugal-selector",P),R.playHover(),W.currentSubStatus&&H(W.currentSubStatus)}}),p}function Y(){let p=document.createElement("div");p.className="cw-status-section",p.style.cssText="display: flex; flex-direction: column; gap: 8px;",p.innerHTML=`
        ...
        `;let C=p.querySelector("#main-status-select"),k=p.querySelector("#sub-status-select");return C.onchange=()=>{W.setStatus(C.value),I(C.value,k),W.setSubStatus(""),H("")},k.onchange=()=>{W.setSubStatus(k.value),H(k.value)},p}function E(){return d.style.display==="none"?null:{l1:g["evidence-l1"]?.value.trim()||"",l2:g["evidence-l2"]?.value.trim()||"",msg:g["evidence-msg"]?.value.trim()||""}}function I(p,C){if(C.innerHTML=`<option value="">${x("select_substatus")}</option>`,!p){C.disabled=!0;return}for(let k in Oe)if(Oe[k].status===p){let P=document.createElement("option");P.value=k,P.textContent=Oe[k].name,C.appendChild(P)}C.disabled=!1}function H(p){let C=Oe[p],k=p==="NI_Attempted_Contact"||C&&C.name&&C.name.toLowerCase().includes("attempted contact");if(b.render&&b.render(p,W.currentCaseType),!p){d.style.display="none",g["evidence-l1"]&&(g["evidence-l1"].value=""),g["evidence-l2"]&&(g["evidence-l2"].value=""),g["evidence-msg"]&&(g["evidence-msg"].value=""),c.style.display="none",O.style.display="none";let pe=document.getElementById("manual-task-toggle");pe&&(pe.style.display="none"),r.selectionElement.style.display="none",r.screenshotsElement.style.display="none",L.style.display="flex",L.style.opacity="1",_.style.display="none";return}k?d.style.display="block":(d.style.display="none",g["evidence-l1"]&&(g["evidence-l1"].value=""),g["evidence-l2"]&&(g["evidence-l2"].value=""),g["evidence-msg"]&&(g["evidence-msg"].value="")),L.style.opacity="0",setTimeout(()=>{W.currentSubStatus&&(L.style.display="none")},400),_.style.display="grid",C&&C.templateFields&&W.setActiveFields(C.templateFields),U(),kt(p,O,W),O.style.display="block",c.style.display="block";let P=p.startsWith("SO_"),z=p==="NI_Awaiting_Validation",ee=document.getElementById("manual-task-toggle");P||z?(r.selectionElement.style.display="block",ee.style.display="none"):(r.selectionElement.style.display="none",r.screenshotsElement.style.display="none",ee.style.display="block");let Z=p==="SO_Education_Only"?"education":"implementation";W.setScreenshotMode(Z),W.currentCaseType==="lm"?W.toggleFieldExclusion("field-ON_CALL",!0):W.toggleFieldExclusion("field-ON_CALL",!1),r.updateSubStatus(p),M();let se=document.getElementById("email-automation-toggle-row");se&&(se.style.display=Ye[p]?"flex":"none")}function M(){let p=r.getCheckedElements().map(C=>C.value);s.updateVisibility(W.currentSubStatus,p)}function J(p,C){let k=st[p];if(k){for(let P in k)if(P==="linkedTask")r.toggleTask(k.linkedTask,C);else if(P==="activeTasks")k.activeTasks.forEach(z=>{C?r.setTaskCount(z.value,z.count):r.setTaskCount(z.value,0)});else if(P.startsWith("field-")){let z=P,ee=k[P],Z=document.getElementById(z);if(Z){let se=We.includes(z.replace("field-",""));if(C)if(se){let pe=Z.value.trim();pe.includes(ee.trim())||(Z.value=pe?pe+`
`+ee.trim():ee.trim())}else Z.value=ee;else if(se){let pe=Z.value.trim(),Ie=ee.trim();pe.includes(Ie)&&(Z.value=pe.replace(Ie,"").trim().replace(/\n{3,}/g,`

`))}else Z.value.trim()===ee.trim()&&(Z.value="");W.updateField(z,Z.value),Z.dispatchEvent(new Event("input"))}}}}function ae(p,C){let k=document.createElement("div");if(k.className="cw-actions-section",k.style.cssText=`
            display: grid;
        ...
        `;let z=p.parkButton;z.classList.add("js-btn-park"),z.style.cssText="width: 100%; margin: 0; border-radius: 10px; height: 34px; font-weight: 600; font-size: 11.5px;";let ee=document.createElement("button");ee.className="cw-btn-secondary js-btn-reset",ee.textContent=C("limpar"),ee.style.cssText=`width: 100%; height: 34px; background: ${$.surface}; color: ${$.textSub}; border: 1px solid ${$.border}; border-radius: 10px; font-weight: 600; cursor: pointer; font-size: 11.5px;`,ee.onclick=()=>K();let Z=document.createElement("button");Z.className="cw-btn-secondary js-btn-copy",Z.textContent=C("copiar"),Z.style.cssText=`width: 100%; height: 34px; background: ${$.surface}; color: ${$.primary}; border: 1px solid ${$.primary}; border-radius: 10px; font-weight: 600; cursor: pointer; font-size: 11.5px;`,Z.onclick=()=>te();let se=document.createElement("button");return se.className="cw-btn-primary js-btn-generate",se.textContent=C("preencher"),se.style.cssText=`width: 100%; height: 38px; background: ${$.primary}; color: #fff; border: none; border-radius: 10px; font-weight: 600; cursor: pointer; grid-column: span 2; font-size: 12.5px; box-shadow: 0 4px 10px rgba(26, 115, 232, 0.2); margin-top: 0px;`,se.onclick=()=>j(),k.appendChild(P),k.appendChild(z),k.appendChild(ee),k.appendChild(Z),k.appendChild(se),k}async function te(){if(!W.currentSubStatus){Q(x("select_substatus"),{error:!0});return}let p=Tt(W,r,s,E());p?(Qe(p),Q(x("copiado_sucesso")),R.playClick()):Q(x("select_substatus"),{error:!0})}async function j(){if(!W.currentSubStatus){Q(x("select_substatus"),{error:!0});return}let p=Tt(W,r,s,E());Qe(p),V();let C=Ze(),k=await ut();if(k){k.focus(),document.execCommand("insertHTML",!1,p),pt(k);let P=document.getElementById("email-automation-checkbox");(!P||P.checked)&&W.currentSubStatus&&Ye[W.currentSubStatus]&&await bt(Ye[W.currentSubStatus]),Q(x("inserido_copiado")),R.playSuccess(),K()}C()}function U(){if(W.currentSubStatus){if(W.currentCaseType==="lm")W.removeField("ON_CALL");else{let p=Oe[W.currentSubStatus];p&&p.templateFields.includes("ON_CALL")&&W.addFieldAt("ON_CALL",1)}W.isPortugalCase?(W.addFieldAt("CASO_PORTUGAL",1),W.addFieldAt("CONSENTIU_GRAVACAO",2)):(W.removeField("CASO_PORTUGAL"),W.removeField("CONSENTIU_GRAVACAO"))}}function K(){W.reset(),r.reset(),s.reset(),N(),we.clearEmergency(),o.querySelectorAll("select").forEach(C=>C.value=""),o.querySelector("#sub-status-select").disabled=!0;let p=document.getElementById("email-automation-toggle-row");p&&(p.style.display="none"),O.innerHTML="",c.style.display="none",L.style.display="flex",L.style.opacity="1",_.style.display="none",document.getElementById("manual-task-toggle")&&(document.getElementById("manual-task-toggle").style.display="none"),r.selectionElement.style.display="none",r.screenshotsElement.style.display="none",d.style.display="none",g["evidence-l1"]&&(g["evidence-l1"].value=""),g["evidence-l2"]&&(g["evidence-l2"].value=""),g["evidence-msg"]&&(g["evidence-msg"].value="")}async function l(p=!1){let C={};O.querySelectorAll("input, textarea, select").forEach(Z=>{(Z.id.startsWith("field-")||Z.id==="consent-select")&&(C[Z.id]=Z.value)});let k="Cliente",P="---";if(!p)try{let Z=await ke();k=Z.advertiserName,P=Z.cid}catch(Z){console.warn("Erro ao coletar pageData:",Z)}let z=r.getCheckedElements().map(Z=>({key:Z.value,count:Z.count})),ee=z.map(Z=>{let se=Fe[Z.key];return se?se.name:Z.key});return{currentCaseType:W.currentCaseType,currentLang:W.currentLang,isPortugalCase:W.isPortugalCase,consent:W.consent,tagSupportUsed:W.tagSupportUsed,forcedScreenshots:[...W.forcedScreenshots],excludedFields:[...W.excludedFields],activeFields:W.activeFields,status:W.currentStatus,subStatus:W.currentSubStatus,formData:C,activeTasks:z,summaryTags:ee,clientName:k,cid:P,timestamp:new Date().toISOString()}}let m=p=>new Promise(C=>setTimeout(C,p));async function y(p){W.setLanguage(p.currentLang||"pt"),W.setCaseType(p.currentCaseType||"bau"),W.setPortugalCase(p.isPortugalCase||!1),W.setConsent(p.consent||!1),W.setExcludedFields(p.excludedFields||[]),p.activeFields&&W.setActiveFields(p.activeFields);let C=o.querySelector(`#lang-selector button[data-lang="${W.currentLang}"]`);C&&C.classList.add("active"),o.querySelectorAll("#lang-selector button").forEach(z=>{z!==C&&z.classList.remove("active")});let k=o.querySelector(`#type-selector button[data-type="${W.currentCaseType}"]`);k&&k.classList.add("active"),o.querySelectorAll("#type-selector button").forEach(z=>{z!==k&&z.classList.remove("active")});let P=o.querySelector(`#portugal-selector button[data-val="${W.isPortugalCase}"]`);if(P&&P.classList.add("active"),o.querySelectorAll("#portugal-selector button").forEach(z=>{z!==P&&z.classList.remove("active")}),p.status){let z=o.querySelector("#main-status-select");z.value=p.status,W.setStatus(p.status);let ee=o.querySelector("#sub-status-select");if(I(p.status,ee),await m(50),p.subStatus){if(ee.value=p.subStatus,W.setSubStatus(p.subStatus),H(p.subStatus),await m(100),p.tagSupportUsed!==void 0){W.setTagSupportUsed(p.tagSupportUsed);let Z=s.element.querySelector('input[value="Sim"]'),se=s.element.querySelector('input[value="N\xE3o"]');p.tagSupportUsed&&Z?Z.checked=!0:se&&(se.checked=!0),s.element.querySelector("div:last-child").style.display=p.tagSupportUsed?"none":"block"}p.forcedScreenshots&&W.setForcedScreenshots(p.forcedScreenshots);for(let Z in p.formData){let se=document.getElementById(Z);se&&(se.value=p.formData[Z],W.updateField(Z,se.value))}p.activeTasks&&(p.activeTasks.forEach(Z=>r.setTaskCount(Z.key,Z.count)),W.setActiveTasks(r.getCheckedElements()))}}W.isDirty=!1}function x(p){return Te[W.currentLang]?.[p]||Te.pt?.[p]||p}function v(){let p=document.createElement("div");return p.innerHTML='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="16 3 21 3 21 8"></polyline><line x1="4" y1="20" x2="21" y2="3"></line><polyline points="21 16 21 21 16 21"></polyline><line x1="15" y1="15" x2="21" y2="21"></line><line x1="4" y1="4" x2="9" y2="9"></line></svg>',p.style.cssText="width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; border-radius: 50%; cursor: pointer; color: #9AA0A6; transition: all 0.2s;",p.onclick=C=>{C.stopPropagation(),B()},p.title="Alternar para Split & Transfer",p}function A(){let p=document.createElement("div");return p.id="notes-empty-state",p.style.cssText=`
            display: flex;
            flex-direction: column;
            align-items: center;
            justify-content: center;
    ...
    `,document.head.appendChild(b),Object.assign(c.style,ye,{width:"850px",height:"650px",display:"none",flexDirection:"column",fontFamily:"'-apple-system', 'BlinkMacSystemFont', 'Segoe UI', 'Roboto', 'Helvetica Neue', 'Arial', sans-serif",borderRadius:"12px",overflow:"hidden"});let d=ge(c,"Email Assistant",t,"Refatora\xE7\xE3o completa do m\xF3dulo de e-mail para uma experi\xEAncia moderna e eficiente.",{popup:c},()=>Y()),u=document.createElement("div");Object.assign(u.style,{display:"flex",flex:"1",overflow:"hidden",backgroundColor:r.bgApp});let g=document.createElement("div");Object.assign(g.style,{width:"320px",backgroundColor:"#EFEFF0",borderRight:`1px solid ${r.borderSubtle}`,display:"flex",flexDirection:"column",flexShrink:"0"});let h=document.createElement("div");Object.assign(h.style,{padding:"16px",borderBottom:`1px solid ${r.borderSubtle}`,position:"relative"});let S=document.createElement("input");S.placeholder="Buscar templates...",Object.assign(S.style,{width:"100%",padding:"10px 14px 10px 36px",borderRadius:"10px",border:"1.5px solid transparent",backgroundColor:"#E3E3E8",fontSize:"15px",outline:"none",boxSizing:"border-box",color:r.textPrimary,backgroundImage:`url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="%238A8A8E" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>')`,backgroundRepeat:"no-repeat",backgroundPosition:"12px center",transition:"all 0.2s ease-in-out"}),S.onfocus=()=>{S.style.backgroundColor="#FFFFFF",S.style.borderColor=r.primary,S.style.boxShadow="0 0 0 4px rgba(0, 122, 255, 0.1)",S.style.transform="scale(1.02)"},S.onblur=()=>{S.style.backgroundColor="#E3E3E8",S.style.borderColor="transparent",S.style.boxShadow="none",S.style.transform="scale(1)"};let f=document.createElement("div");f.id="email-template-list",Object.assign(f.style,{flex:"1",overflowY:"auto",padding:"8px",scrollBehavior:"smooth"});let T=document.createElement("div");T.innerHTML="\u2715",Object.assign(T.style,{position:"absolute",right:"26px",top:"50%",transform:"translateY(-50%)",fontSize:"10px",color:"#fff",cursor:"pointer",display:"none",backgroundColor:"#C7C7CC",width:"16px",height:"16px",borderRadius:"50%",textAlign:"center",lineHeight:"16px",fontWeight:"bold"}),T.onclick=()=>{S.value="",n="",T.style.display="none",I(),S.focus()},h.appendChild(S),h.appendChild(T),g.appendChild(h),g.appendChild(f);let O=document.createElement("div");Object.assign(O.style,{flex:"1",display:"flex",flexDirection:"column",overflow:"hidden",backgroundColor:r.bgApp,transition:"all 0.3s cubic-bezier(0.25, 1, 0.5, 1)"});let L=document.createElement("div");Object.assign(L.style,{padding:"20px",borderBottom:`1px solid ${r.borderSubtle}`,backgroundColor:r.bgSurface,maxHeight:"250px",overflowY:"auto",display:"none"});let _=document.createElement("div");Object.assign(_.style,{flex:"1",display:"flex",flexDirection:"column",padding:"20px",backgroundColor:r.bgApp,overflow:"hidden"});let G=document.createElement("div");Object.assign(G.style,{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px"});let X=document.createElement("span");X.textContent="Preview do E-mail",Object.assign(X.style,{fontSize:"12px",fontWeight:"600",color:r.textSecondary,textTransform:"uppercase",letterSpacing:"0.5px"});let oe=document.createElement("div");Object.assign(oe.style,{display:"flex",gap:"8px"});let q=(te,j=!1)=>{let U=document.createElement("button");return U.textContent=te,Object.assign(U.style,{padding:"8px 14px",borderRadius:"10px",border:j?"none":`1.5px solid ${r.primary}`,background:j?r.primary:"transparent",color:j?"#fff":r.primary,fontSize:"13px",fontWeight:"600",cursor:"pointer",transition:"all 0.2s cubic-bezier(0.25, 1, 0.5, 1)",boxShadow:j?"0 4px 12px rgba(0, 122, 255, 0.3)":"none"}),U.onmouseenter=()=>{j?(U.style.backgroundColor="#0062CC",U.style.transform="translateY(-1px)",U.style.boxShadow="0 6px 16px rgba(0, 122, 255, 0.4)"):U.style.backgroundColor="rgba(0, 122, 255, 0.05)"},U.onmouseleave=()=>{j?(U.style.backgroundColor=r.primary,U.style.transform="translateY(0)",U.style.boxShadow="0 4px 12px rgba(0, 122, 255, 0.3)"):(U.style.backgroundColor="transparent",U.style.transform="translateY(0)")},U.onmousedown=()=>U.style.transform="scale(0.94)",U.onmouseup=()=>U.style.transform="scale(1)",U},F=q("Copiar HTML"),N=q("Preencher no CRM",!0),V=q("Smart CR");V.style.borderColor="#E67E22",V.style.color="#E67E22",V.style.display="none",oe.appendChild(V),oe.appendChild(F),oe.appendChild(N),G.appendChild(X),G.appendChild(oe);let B=document.createElement("div");B.contentEditable="true",Object.assign(B.style,{flex:"1",backgroundColor:r.bgSurface,border:`1px solid ${r.borderSubtle}`,borderRadius:"8px",padding:"20px",fontSize:"15px",lineHeight:"1.6",color:r.textPrimary,overflowY:"auto",outline:"none",boxShadow:"inset 0 1px 2px rgba(0,0,0,0.02)"}),_.appendChild(G),_.appendChild(B),ae(),O.appendChild(L),O.appendChild(_),u.appendChild(g),u.appendChild(O),c.appendChild(d),c.appendChild(u);let D=document.createElement("div");Object.assign(D.style,Le),c.appendChild(D),qe(c,D),document.body.appendChild(c);function Y(){e=!e,e?(c.style.display="flex",Zt(c),o.length===0&&E()):c.style.display="none",be(e,c,"cw-btn-email")}async function E(){f.innerHTML='<div style="padding: 20px; text-align: center; color: #5f6368;">Carregando...</div>',o=await wo.getTemplates(),I()}function I(){f.innerHTML="";let te=o.filter(m=>m.name.toLowerCase().includes(n.toLowerCase())||m.category.toLowerCase().includes(n.toLowerCase())),j=Object.entries(Ye).filter(([m,y])=>y&&(m.toLowerCase().includes(n.toLowerCase())||y.toLowerCase().includes(n.toLowerCase()))).map(([m,y])=>({id:m,name:m.replace(/_/g," "),category:"\u26A1 Smart CRs",code:y,isSmartCR:!0})),U=Se.getSnippets("email").filter(m=>m.title.toLowerCase().includes(n.toLowerCase())||m.subject&&m.subject.toLowerCase().includes(n.toLowerCase())).map(m=>{let y=[],x=m.content.match(/\[([^\]]+)\]/g);return x&&[...new Set(x)].forEach(v=>{y.push({key:v,label:v.replace("[","").replace("]",""),type:v.toLowerCase().includes("data")?"date":"text",auto:v.toLowerCase().includes("nome")&&v.toLowerCase().includes("seu")?"agentName":null})}),{id:m.id||`snippet-${Math.random()}`,name:m.title,category:"\u{1F464} Pessoal",subject:m.subject||"Sem Assunto",template:m.content,placeholders:y}}),K=[...te,...j,...U];if(K.length===0){f.innerHTML=`
                ...
                </div>`;return}[...new Set(K.map(m=>m.category))].sort((m,y)=>m.localeCompare(y)).forEach(m=>{let y=s.has(m)||n.length>0,x=K.filter(k=>k.category===m),v=document.createElement("div");Object.assign(v.style,{padding:"12px 16px 12px 24px",fontSize:"11px",fontWeight:"700",color:r.textSecondary,textTransform:"uppercase",letterSpacing:"0.8px",position:"sticky",top:"-8px",backgroundColor:"rgba(239, 239, 240, 0.9)",zIndex:"10",backdropFilter:"blur(20px)",margin:"0 -8px 8px -8px",borderBottom:`0.5px solid ${r.borderSubtle}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between",userSelect:"none",transition:"background-color 0.2s ease"}),v.onmouseenter=()=>v.style.backgroundColor="rgba(230, 230, 232, 0.9)",v.onmouseleave=()=>v.style.backgroundColor="rgba(239, 239, 240, 0.9)";let A=document.createElement("span");A.textContent=m,v.appendChild(A);let w=document.createElement("span");w.textContent=x.length,Object.assign(w.style,{backgroundColor:"rgba(0, 0, 0, 0.05)",padding:"2px 8px",borderRadius:"10px",fontSize:"10px",color:r.textSecondary});let p=document.createElement("span");p.innerHTML=y?"\u{10012A}":"\u{10012B}",p.innerHTML=y?"\u25BE":"\u25B8",p.style.marginLeft="8px",p.style.transition="transform 0.3s ease";let C=document.createElement("div");C.style.display="flex",C.style.alignItems="center",C.appendChild(w),C.appendChild(p),v.appendChild(C),v.onclick=()=>{s.has(m)?s.delete(m):s.add(m),I()},f.appendChild(v),y&&x.forEach(k=>{let P=a&&a.id===k.id,z=document.createElement("div");if(Object.assign(z.style,{padding:"12px 14px",fontSize:"14px",cursor:"pointer",transition:"all 0.3s cubic-bezier(0.25, 1, 0.5, 1)",borderRadius:"10px",color:r.textPrimary,margin:"4px 6px",display:"flex",alignItems:"center",gap:"12px",backgroundColor:P?r.primary:r.bgSurface,boxShadow:P?"0 4px 12px rgba(0, 122, 255, 0.3)":"0 1px 2px rgba(0,0,0,0.05)",border:P?"none":`1px solid ${r.borderSubtle}`,position:"relative",overflow:"hidden"}),P){let se=document.createElement("div");Object.assign(se.style,{position:"absolute",left:"0",top:"0",bottom:"0",width:"4px",backgroundColor:"#fff",borderRadius:"0 4px 4px 0"}),z.appendChild(se)}let ee=document.createElement("span");ee.innerHTML=k.isSmartCR?"\u26A1":k.category==="\u{1F464} Pessoal"?"\u{1F464}":"\u{1F4C4}",ee.style.fontSize="12px",ee.style.opacity="0.7",ee.style.flexShrink="0",z.appendChild(ee);let Z=document.createElement("span");Z.textContent=k.name,Z.style.overflow="hidden",Z.style.textOverflow="ellipsis",Z.style.whiteSpace="nowrap",Z.style.flex="1",z.appendChild(Z),P&&(z.style.color="#fff",z.style.fontWeight="600",ee.style.opacity="1"),z.onmouseenter=()=>{P||(z.style.backgroundColor="#f8f8f9",z.style.transform="translateY(-1px) scale(1.01)",z.style.boxShadow="0 4px 8px rgba(0,0,0,0.08)",z.style.borderColor="rgba(0, 122, 255, 0.2)")},z.onmouseleave=()=>{P||(z.style.backgroundColor=r.bgSurface,z.style.transform="translateY(0) scale(1)",z.style.boxShadow="0 1px 2px rgba(0,0,0,0.05)",z.style.borderColor=r.borderSubtle)},z.onmousedown=()=>{z.style.transform=P?"scale(0.97)":"scale(0.98)"},z.onmouseup=()=>{z.style.transform=P?"scale(1)":"translateY(-1px) scale(1.01)"},z.onclick=()=>{M(k)},f.appendChild(z)})})}let H=null;async function M(te){a?.id!==te.id&&(a=te,H&&clearTimeout(H),O.style.opacity="0",O.style.transform="translateY(5px)",H=setTimeout(()=>{V.style.display=te.isSmartCR?"block":"none",N.style.display=te.isSmartCR?"none":"block",F.style.display=te.isSmartCR?"none":"block",I(),J(),ae(),O.style.opacity="1",O.style.transform="translateY(0)",H=null},150))}function J(){if(L.innerHTML="",!a||a.isSmartCR){a?.isSmartCR?(L.style.display="block",L.innerHTML=`<div style="padding: 12px; font-size: 13px; color: #856404; background: #FFF3CD; border: 1px solid #FFEEBA; border-radius: 8px; display: flex; align-items: center; gap: 8px;">
 ...
 D) Nombre del sitio web.`,"autenticar la cuenta del anunciante en el cases, si corresponde.","T\xE9rminos y condiciones.","Informar las Task solicitadas y AM.","Cerrar contenido sensible.","Confirmaci\xF3n de copia de seguridad y acceso de administrador a las herramientas.","Resumen de llamada."],fim:["Ofrecer ayuda adicional.","Dejar de compartir la pantalla.","Pasos siguientes (Si se le har\xE1 seguimiento al caso).","Encuesta de Satisfacci\xF3n.","Informar al cliente que el equipo de QA ir\xE1 a realizar pruebas en los siguientes d\xEDas."]}};var ie={bgApp:"#F5F5F7",bgSurface:"#FFFFFF",borderSubtle:"rgba(0, 0, 0, 0.07)",primary:"#007AFF",primaryBg:"rgba(0, 122, 255, 0.1)",textPrimary:"#1D1D1F",textSecondary:"#6E6E73",danger:"#D93025",dangerBg:"#FCE8E6",success:"#34A853",successBg:"#E6F4EA"},ca={inicio:{PT:"Abertura",ES:"Apertura"},meio:{PT:"Implementa\xE7\xE3o (Tag Support)",ES:"Implementaci\xF3n"},fim:{PT:"Fechamento",ES:"Cierre"}};function da(){if(document.getElementById("csa-styles-v2"))return;let t=document.createElement("style");t.id="csa-styles-v2",t.textContent=`
    ...
    `,document.head.appendChild(t)}function Ao(){let t="v3.1.0";da();let e={},o="PT",a="BAU",n=!1,i=document.createElement("div");i.id="call-script-popup",i.classList.add("cw-module-window"),Object.assign(i.style,ye,{right:"auto",left:"50%",width:"420px",height:"700px",display:"flex",flexDirection:"column",transform:"translateX(-50%) scale(0.05)"});let s={popup:i,googleLine:null},r=null;function c(){n&&ke().then(y=>{let x=i.querySelector("#cw-ctx-name"),v=i.querySelector("#cw-ctx-cid"),A=i.querySelector("#cw-ctx-email");if(x&&(x.textContent=y.advertiserName||"Cliente Desconhecido"),v){let w=y.cid||"---";v.textContent!==w&&(v.textContent=w)}if(A){let w=y.clientEmail||"N\xE3o encontrado";A.textContent!==w&&(A.textContent=w,A.title=w)}})}function b(){ke().then(y=>{let x=new Date().toLocaleDateString("pt-BR"),v=i.querySelector("#cw-am-message-area"),A=i.querySelector("#cw-am-review-container"),w=`Ol\xE1. Bom dia!
...
CID: ${y.cid||"---"}
Case ID: ${y.caseId||"---"}
E-mail: ${y.clientEmail||"---"}`;v&&(v.value=w),A&&(A.classList.add("visible"),A.scrollIntoView({behavior:"smooth",block:"end"}))})}function d(){n=!n,be(n,i,"cw-btn-script"),n?(c(),r||(r=setInterval(c,2e3))):r&&(clearInterval(r),r=null)}let u=ge(i,"Call Script",t,"Guia interativo para condu\xE7\xE3o de chamadas.",s,()=>{d()});i.appendChild(u);let g=document.createElement("div");g.className="csa-context-banner",g.innerHTML=`
  ...
  `;let h=g.querySelector("#csa-toggle-options"),S=g.querySelector("#csa-options-content"),f=g.querySelector("#csa-options-arrow"),T=!1;h.onclick=()=>{T=!T,f.classList.toggle("expanded",T),S.classList.toggle("expanded",T),R.playClick()};let O=g.querySelector("#cw-pill-message"),L=g.querySelector("#cw-am-copy-final"),_=g.querySelector("#cw-am-message-area");O.addEventListener("click",()=>{b()}),L.addEventListener("click",()=>{_.value&&(navigator.clipboard.writeText(_.value),Q("Mensagem copiada!"),R.playSuccess(),L.classList.add("copied-flash"),L.textContent="Copiado!",setTimeout(()=>{L.classList.remove("copied-flash"),L.textContent="Copiar Mensagem Final"},2e3))});let G=(y,x)=>{let v=g.querySelector(y),A=g.querySelector(x);v.onclick=()=>{let w=A.textContent;!w||w.includes("---")||w.includes("N\xE3o encontrado")||(navigator.clipboard.writeText(w),R.playSuccess(),v.classList.add("copied"),setTimeout(()=>v.classList.remove("copied"),1500))}};i.appendChild(g);let X=document.createElement("div");X.className="csa-progress-container";let oe=document.createElement("div");oe.className="csa-progress-fill",X.appendChild(oe),i.appendChild(X);let q=document.createElement("div");q.id="csa-content",q.className="csa-content-area",i.appendChild(q);let F=document.createElement("div");F.className="csa-footer";let N=document.createElement("span");N.className="csa-credit",N.textContent="by lucaste@";let V=document.createElement("button");V.className="csa-reset-btn",V.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"></path><path d="M3 3v5h5"></path></svg> Resetar Script',V.onclick=()=>{for(let y in e)delete e[y];K()},F.appendChild(N),F.appendChild(V),i.appendChild(F);let B=document.createElement("div");B.className="csa-controls";let D=document.createElement("div");D.className="csa-segmented-control",D.innerHTML=`
  ...
  `,f.onfocus=()=>{h.style.boxShadow="0 4px 12px rgba(26,115,232,0.15)",h.style.border="1px solid #1a73e8"},f.onblur=()=>{h.style.boxShadow="0 2px 6px rgba(0,0,0,0.04)",h.style.border="1px solid transparent"},h.appendChild(S),h.appendChild(f),g.appendChild(h),u.appendChild(g);let T=document.createElement("div");T.style.cssText="flex: 1; overflow-y: auto; padding: 0 24px 40px 24px; scroll-behavior: smooth;",u.appendChild(T);let O=null;function L(){if(O)return;O=document.createElement("div"),O.style.cssText=`
          position: absolute; bottom: 0; left: 0; width: 100%; height: 100%;
          background: rgba(255,255,255,0.98); z-index: 20;
          display: flex; flex-direction: column;
          transform: translateY(100%); transition: transform 0.3s cubic-bezier(0.2, 0.8, 0.2, 1);
          box-shadow: 0 -4px 20px rgba(0,0,0,0.1);
      `;let D=document.createElement("div");D.style.cssText="padding: 16px 24px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #F1F3F4;",D.innerHTML='<span style="font-size: 16px; font-weight: 700; color: #202124;">\u{1F552} Recentes</span>';let Y=document.createElement("button");Y.innerHTML='<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>',Y.style.cssText="background: none; border: none; cursor: pointer; color: #5f6368;",Y.onclick=()=>{G(),n=!1,F()},D.appendChild(Y),O.appendChild(D);let E=document.createElement("div");E.id="cw-history-list",E.style.cssText="flex: 1; overflow-y: auto; padding: 20px; background: #F8F9FA;",O.appendChild(E),u.appendChild(O)}function _(){O||L();let D=O.querySelector("#cw-history-list");D.innerHTML="";let Y=pa();Y.length===0?D.innerHTML='<div style="text-align: center; color: #999; margin-top: 60px; font-size:13px;">Nada por aqui ainda.</div>':Y.forEach(E=>{let I=V(E,Ge[E._originalCat],!0,E._originalCat);D.appendChild(I)}),requestAnimationFrame(()=>O.style.transform="translateY(0)")}function G(){O&&(O.style.transform="translateY(100%)")}function X(){d.innerHTML="";let D=oe("history","Recentes",Ge.history);D.id="cw-sidebar-btn-history",D.onclick=()=>{R.playClick(),n=!n,n?_():G(),F()},d.appendChild(D);let Y=document.createElement("div");Y.style.cssText="width: 32px; height: 1px; background: rgba(0,0,0,0.08); margin: 4px 0;",d.appendChild(Y),Object.keys(et).forEach(E=>{let I=et[E],H=oe(E,I.label,Ge[E]);H.id=`cw-sidebar-btn-${E}`,H.onclick=()=>{R.playClick(),n&&(n=!1,G()),q(E)},d.appendChild(H)})}function oe(D,Y,E){let I=document.createElement("div");I.style.cssText=`
          width: 56px; height: 56px; border-radius: 16px;
          display: flex; flex-direction: column; align-items: center; justify-content: center;
          cursor: pointer; color: ${i.textSecondary}; 
          transition: all 0.2s cubic-bezier(0.2, 0.0, 0.2, 1);
          position: relative;
      `,I.title=Y,I.dataset.key=D;let H=document.createElement("div");H.style.cssText="width: 24px; height: 24px; margin-bottom: 2px; transition: transform 0.2s;",H.innerHTML=E||Ge.tasks;let M=document.createElement("div");return M.style.cssText="font-size: 9px; font-weight: 600; opacity: 0.7; letter-spacing: 0.3px;",M.textContent=Y,I.appendChild(H),I.appendChild(M),I.onmouseenter=()=>{a!==D&&!(D==="history"&&n)&&(I.style.background="#F1F3F4",H.style.transform="scale(1.1)")},I.onmouseleave=()=>{a!==D&&!(D==="history"&&n)&&(I.style.background="transparent",H.style.transform="scale(1)")},I}function q(D){let Y=document.getElementById(`cat-anchor-${D}`);Y&&(Y.scrollIntoView({behavior:"smooth",block:"start"}),a=D,F())}function F(){Object.keys(et).forEach(Y=>{let E=d.querySelector(`#cw-sidebar-btn-${Y}`);if(E)if(a===Y&&!n){let I=ft[Y];E.style.background=I.bg,E.style.color=I.color,E.querySelector("div:first-child").style.transform="scale(1.1)"}else E.style.background="transparent",E.style.color=i.textSecondary,E.querySelector("div:first-child").style.transform="scale(1)"});let D=d.querySelector("#cw-sidebar-btn-history");D&&(n?(D.style.background="#3C4043",D.style.color="#FFFFFF"):(D.style.background="transparent",D.style.color=i.textSecondary))}function N(){if(T.innerHTML="",e.trim()!==""){let Y=[];if(Object.entries(et).forEach(([I,H])=>{let M=H.links.filter(J=>J.name.toLowerCase().includes(e.toLowerCase())||J.desc.toLowerCase().includes(e.toLowerCase()));Y.push(...M.map(J=>({...J,_cat:I})))}),Y.length===0){T.innerHTML='<div style="text-align:center; padding: 60px; color:#999; font-size:13px;">Nada encontrado.</div>';return}let E=document.createElement("div");E.innerHTML="Resultados da busca",E.style.cssText="font-size:12px; font-weight:700; color:#5f6368; margin:20px 0 10px; text-transform:uppercase; letter-spacing:0.5px;",T.appendChild(E),Y.forEach(I=>{let H=V(I,Ge[I._cat],!1,I._cat);T.appendChild(H)});return}Object.entries(et).forEach(([Y,E])=>{let I=ft[Y],H=document.createElement("div"),M=document.createElement("div");M.id=`cat-anchor-${Y}`,M.style.cssText=`
              display: flex; align-items: center; gap: 8px;
          ...
          `,M.innerHTML=`
            ...
            ${E.label}
          `,H.appendChild(M);let J=document.createElement("div");J.style.cssText="display: grid; grid-template-columns: 1fr; gap: 8px;",E.links.forEach(ae=>{let te=V(ae,Ge[Y],!1,Y);J.appendChild(te)}),H.appendChild(J),T.appendChild(H)});let D=document.createElement("div");D.style.height="80px",T.appendChild(D)}function V(D,Y,E,I){let H=document.createElement("div"),M=ft[I]||ft.history;H.style.cssText=`
      ...
      `;let J=document.createElement("div");J.style.cssText=`
          ...
          display: flex; align-items: center; justify-content: center;
          flex-shrink: 0; transition: all 0.2s;
      `,J.innerHTML=Y||Ge.tasks;let ae=J.querySelector("svg");ae&&(ae.style.width="22px",ae.style.height="22px");let te=document.createElement("div");te.style.cssText="flex: 1; display: flex; flex-direction: column; gap: 2px; overflow: hidden;";let j=document.createElement("div");j.style.cssText=`font-size: 14px; font-weight: 600; color: ${i.textPrimary}; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;`,j.textContent=D.name;let U=document.createElement("div");U.style.cssText=`font-size: 12px; color: ${i.textSecondary}; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;`,U.textContent=D.desc,te.appendChild(j),te.appendChild(U);let K=document.createElement("div");return K.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>',K.style.cssText=`
          width: 32px; height: 32px; border-radius: 8px;
          display: flex; align-items: center; justify-content: center;
          color: #9AA0A6; transition: all 0.2s; opacity: 0;
      `,K.title="Copiar URL",H.onmouseenter=()=>{H.style.transform="translateY(-2px)",H.style.boxShadow="0 8px 20px rgba(0,0,0,0.08)",H.style.borderColor="rgba(0,0,0,0.05)",H.style.borderLeft=`4px solid ${M.color}`,K.style.opacity="1",K.style.background="#F1F3F4"},H.onmouseleave=()=>{H.style.transform="translateY(0)",H.style.boxShadow="0 1px 3px rgba(0,0,0,0.05)",H.style.border="1px solid transparent",K.style.opacity="0",K.style.background="transparent"},H.onclick=()=>{!E&&I&&ko(D,I),window.open(D.url,"_blank")},K.onclick=l=>{l.stopPropagation(),R.playClick(),navigator.clipboard.writeText(D.url),!E&&I&&ko(D,I),Q("Link copiado!")},H.appendChild(J),H.appendChild(te),H.appendChild(K),H}f.addEventListener("input",D=>{e=D.target.value,N()});function B(){o=!o,be(o,s,"cw-btn-links")}return document.body.appendChild(s),X(),N(),B}var ze=[];function Dt(t){ze=t}var ua=60*1e3;window._cwIsAdmin=!1;window._cwCurrentUser=null;function Fo(){let t="v4.9",e=!1,o=null,a=null;function n(E){if(!E)return"";try{let I=new Date(E);return isNaN(I.getTime())?String(E):I.toLocaleString("pt-BR",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}).replace(","," \xE0s")}catch{return String(E)}}if(!document.getElementById("cw-broadcast-hd-css")){let E=document.createElement("style");E.id="cw-broadcast-hd-css",E.innerHTML=`
        @keyframes cw-pulse {
      ...
      `,document.head.appendChild(E)}let i={feedContainer:{padding:"20px 24px 80px 24px",overflowY:"auto",flexGrow:"1",background:"#F8F9FA",display:"flex",flexDirection:"column",gap:"20px"},card:{background:"#FFFFFF",borderRadius:"16px",border:"1px solid rgba(0,0,0,0.12)",boxShadow:"0 4px 12px rgba(60,64,67,0.08)",overflow:"hidden",transition:"all 0.3s ease",position:"relative",width:"100%",boxSizing:"border-box",flexShrink:"0"},cardHistory:{background:"#FFFFFF",borderRadius:"16px",border:"1px solid rgba(0,0,0,0.05)",boxShadow:"none",opacity:"0.6",filter:"grayscale(0.8)",marginBottom:"16px",flexShrink:"0",width:"100%",boxSizing:"border-box",position:"relative"},cardHeader:{padding:"16px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"1px solid #F1F3F4"},typeTag:{display:"flex",alignItems:"center",gap:"6px",fontSize:"11px",fontWeight:"700",textTransform:"uppercase",letterSpacing:"0.6px",padding:"4px 8px",borderRadius:"6px"},dateTag:{fontSize:"11px",color:"#5f6368",fontWeight:"500"},cardContent:{padding:"16px 20px 20px 20px"},msgTitle:{fontSize:"16px",fontWeight:"700",color:"#202124",marginBottom:"8px",lineHeight:"1.4"},msgBody:{fontSize:"14px",color:"#3c4043",lineHeight:"1.6",whiteSpace:"pre-wrap",wordBreak:"break-word"},msgMeta:{fontSize:"11px",color:"#9aa0a6",marginTop:"12px",display:"flex",alignItems:"center",gap:"6px"},dismissBtn:{width:"28px",height:"28px",borderRadius:"50%",border:"1px solid rgba(0,0,0,0.1)",background:"#fff",color:"#5f6368",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.2s ease",marginLeft:"12px"},bauContainer:{margin:"16px 24px 0 24px",padding:"16px",background:"#F3E8FD",border:"1px solid #D8B4FE",borderRadius:"16px",display:"flex",flexDirection:"column",gap:"12px",boxShadow:"0 4px 12px rgba(147, 51, 234, 0.1)"},bauHeader:{display:"flex",alignItems:"center",justifyContent:"space-between",gap:"2px"},bauLabel:{fontSize:"11px",fontWeight:"800",color:"#7E22CE",textTransform:"uppercase",letterSpacing:"0.8px"},liveIndicator:{display:"flex",alignItems:"center",gap:"8px"},pulseDot:{width:"8px",height:"8px",borderRadius:"50%",background:"#9333EA",boxShadow:"0 0 0 0 rgba(147, 51, 234, 0.7)",animation:"cw-pulse 2s infinite"},bauSlotRow:{display:"flex",alignItems:"center",gap:"10px",padding:"8px 12px",background:"rgba(255,255,255,0.5)",borderRadius:"8px",marginBottom:"4px"},bauFlag:{fontSize:"18px",lineHeight:"1"},bauDate:{fontSize:"16px",fontWeight:"700",color:"#581C87",letterSpacing:"-0.5px"},emptyState:{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"60px 0",color:"#BDC1C6",gap:"16px",textAlign:"center"},historyDivider:{display:"flex",alignItems:"center",justifyContent:"center",margin:"20px 0",cursor:"pointer",color:"#1a73e8",fontSize:"13px",fontWeight:"500",gap:"8px",padding:"8px 16px",borderRadius:"20px",background:"#E8F0FE"},historyContainer:{display:"none",flexDirection:"column",gap:"16px",opacity:"0.8"}},s={critical:{color:"#991B1B",bg:"#FEF2F2",icon:'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round"><path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>'},info:{color:"#1E40AF",bg:"#EFF6FF",icon:'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="16" x2="12" y2="12"></line><line x1="12" y1="8" x2="12.01" y2="8"></line></svg>'},success:{color:"#166534",bg:"#F0FDF4",icon:'<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>'}};function r(E){if(!E||typeof E!="string")return"";let I=E;return I=I.replace(/(https?:\/\/[^\s]+)/g,'<a href="$1" target="_blank" style="color:#1967d2; text-decoration:none; font-weight:500;">$1</a>'),I=I.replace(/\*\*(.*?)\*\*/g,"<b>$1</b>"),I=I.replace(/_(.*?)_/g,"<i>$1</i>"),I=I.replace(/\n/g,"<br>"),I=Qt(I),I}let c=document.createElement("div");c.id="broadcast-popup",c.classList.add("cw-module-window"),Object.assign(c.style,ye,{right:"auto",left:"50%",width:"420px",height:"680px",display:"flex",flexDirection:"column",transform:"translateX(-50%) scale(0.05)",backgroundColor:"#FAFAFA",overflow:"hidden"});let b={popup:c,googleLine:null};function d(){if(e=!e,be(e,c,"cw-btn-broadcast"),e){let E=document.getElementById("cw-btn-broadcast");E&&E.classList.remove("has-new"),oe()}}let u=ge(c,"Central de Avisos",t,"Comunica\xE7\xE3o oficial da opera\xE7\xE3o.",b,()=>d()),g=u.querySelector(".cw-header-actions")||u.lastElementChild,h=null;function S(){let E=null;try{E=fe()}catch{console.warn("TechSol: Auth Pending")}if(E){let I=E.split("@")[0].toLowerCase(),H=ct.includes(I);if(window._cwIsAdmin=H,window._cwCurrentUser=I,H&&g&&!g.querySelector("#cw-admin-btn")){let M=document.createElement("div");M.id="cw-admin-btn",M.className="cw-btn-interactive",M.innerHTML='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>',Object.assign(M.style,{width:"32px",height:"32px",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",color:"#1a73e8",background:"rgba(26, 115, 232, 0.1)",marginRight:"8px"}),M.title="Novo Aviso",M.onclick=J=>{J.stopPropagation(),O()},g.insertBefore(M,g.firstChild),h||T(),F()}}else window._cwAdminRetries||(window._cwAdminRetries=0),window._cwAdminRetries<5&&(window._cwAdminRetries++,setTimeout(S,2e3))}if(g){let E=document.createElement("button");E.textContent="Limpar",E.className="cw-btn-interactive",Object.assign(E.style,{fontSize:"12px",color:"#1a73e8",background:"transparent",border:"none",padding:"8px",fontWeight:"600"}),E.onclick=I=>{I.stopPropagation(),R.playSuccess();let H=ze.map(M=>M.id);localStorage.setItem("cw_read_broadcasts",JSON.stringify(H)),F(),q()},g.insertBefore(E,g.firstChild)}c.appendChild(u);let f=document.createElement("div");f.id="cw-update-status",f.style.cssText="padding: 8px; text-align: center; font-size: 11px; color: #5f6368; background: #FAFAFA; border-bottom: 1px solid transparent; font-weight:500; display:none;",c.appendChild(f);function T(){h=document.createElement("div"),h.className="cw-editor-overlay",h.innerHTML=`
      ...
      `,h.querySelectorAll('input[name="cw-bc-type"]').forEach(M=>{M.addEventListener("change",()=>{h.querySelectorAll(".cw-radio-option").forEach(J=>J.classList.remove("checked")),M.parentElement.classList.add("checked")})}),setTimeout(()=>{let M=h.querySelector(".cw-radio-option.info");M&&M.classList.add("checked")},100);let E=h.querySelector("#cw-bc-cancel"),I=h.querySelector("#cw-bc-close-x"),H=h.querySelector("#cw-bc-send");E.onclick=L,I.onclick=L,H.onclick=_,c.appendChild(h)}function O(E=null){if(!h)return;let I=h.querySelector("#cw-editor-title-label"),H=h.querySelector("#cw-bc-title"),M=h.querySelector("#cw-bc-text"),J=h.querySelector("#cw-bc-send");if(E){a=E.id,I.textContent="Editar Aviso",H.value=E.title||"",M.value=E.text||"",J.textContent="Salvar Altera\xE7\xF5es";let ae=E.type||"info",te=h.querySelector(`input[name="cw-bc-type"][value="${ae}"]`);te&&te.click()}else{a=null,I.textContent="Novo Aviso",H.value="",M.value="",J.textContent="Publicar";let ae=h.querySelector('input[name="cw-bc-type"][value="info"]');ae&&ae.click()}h.classList.add("active"),setTimeout(()=>H.focus(),300)}function L(){h&&h.classList.remove("active"),a=null}async function _(){let E=h.querySelector("#cw-bc-send"),I=h.querySelector("#cw-bc-title"),H=h.querySelector("#cw-bc-text"),M=h.querySelector('input[name="cw-bc-type"]:checked'),J=M?M.value:"info";if(!I.value.trim()||!H.value.trim()){Q("Preencha todos os campos!",{error:!0});return}E.textContent="Salvando...",E.style.opacity="0.7";let ae=!1;a?ae=await ce.updateBroadcast(a,{title:I.value,text:H.value,type:J}):ae=await ce.sendBroadcast({title:I.value,text:H.value,type:J,author:window._cwCurrentUser||"admin"}),ae?(Q(a?"Atualizado!":"Publicado!"),R.playSuccess(),L(),setTimeout(()=>oe(),1500)):(Q("Erro ao salvar. Verifique a conex\xE3o.",{error:!0}),E.textContent=a?"Salvar Altera\xE7\xF5es":"Publicar",E.style.opacity="1")}async function G(E){if(await ve("Confirma a exclus\xE3o deste aviso?",{danger:!0}))if(await ce.deleteBroadcast(E)){Q("Aviso removido."),R.playClick();let M=ze.findIndex(J=>J.id===E);M>-1&&ze.splice(M,1),F(),setTimeout(()=>oe(),1500)}else Q("Erro ao excluir.",{error:!0})}let X=document.createElement("div");X.className="cw-nice-scroll",Object.assign(X.style,i.feedContainer),c.appendChild(X);async function oe(){e&&(f.style.display="block",f.innerHTML="\u{1F504} Sincronizando...");try{let E=await ce.fetchData();E&&E.broadcast&&(Dt(E.broadcast),q(),e&&(F(),f.innerHTML='<span style="color:#137333">\u2713 Atualizado</span>',setTimeout(()=>{f.style.display="none"},1500)))}catch{e&&(f.innerHTML="\u26A0\uFE0F Offline")}}function q(){let E=document.getElementById("cw-btn-broadcast");if(!E)return;let I=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]");if(ze.some(M=>!I.includes(M.id))){if(E.classList.add("has-new"),!E.querySelector(".cw-badge")){let M=document.createElement("div");M.className="cw-badge",Object.assign(M.style,{position:"absolute",top:"8px",right:"8px",width:"8px",height:"8px",backgroundColor:"#d93025",borderRadius:"50%",border:"1px solid #fff",zIndex:"10"}),E.appendChild(M)}}else{E.classList.remove("has-new");let M=E.querySelector(".cw-badge");M&&M.remove()}}function F(){X.innerHTML="";let E=c.querySelector("#cw-bau-widget");E&&E.remove();let I=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]"),H=[...ze].sort((j,U)=>{let K=new Date(j.date).getTime()||0;return(new Date(U.date).getTime()||0)-K}),M=H.findIndex(j=>j.title&&j.title.toLowerCase().includes("disponibilidade bau"));if(M!==-1){let j=H[M];H.splice(M,1);let U=document.createElement("div");U.id="cw-bau-widget",Object.assign(U.style,i.bauContainer);let K=[],l=(j.text||"").split(`
`),m=/\d{1,2}\/\d{1,2}/,y="\u{1F4C5}";if(l.forEach(p=>{/🇧🇷|🇵🇹|PT|BR|BRASIL|BRAZIL|PORTUGAL|LISBOA/i.test(p)?y="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM|ESPANHA|SPAIN|MEXICO|MÉXICO/i.test(p)&&(y="\u{1F1EA}\u{1F1F8}");let C=p.match(m);if(C){let k=C[0],P=y;/🇧🇷|🇵🇹|PT|BR/i.test(p)?P="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM/i.test(p)&&(P="\u{1F1EA}\u{1F1F8}"),K.some(ee=>ee.flag===P&&ee.date===k)||K.push({flag:P,date:k})}}),K.length===0){let p=(j.text||"").match(/\d{1,2}\/\d{1,2}/g);p&&[...new Set(p)].forEach(C=>K.push({flag:"\u{1F4C5}",date:C}))}let x="",v='<button id="cw-bau-toggle-btn" class="cw-btn-interactive" style="background:rgba(255,255,255,0.7); border:1px solid rgba(139, 92, 246, 0.4); border-radius:12px; padding:8px 12px; color:#6D28D9; font-size:12px; font-weight:600;">Detalhes</button>';window._cwIsAdmin&&(v=`
              ...
              `),K.length>0?x=`
                  <div style="display:flex; justify-content:space-between; align-items:center;">
                      <div style="flex:1; display:flex; gap:8px;">${K.map(C=>`
                  <div style="${Ne(i.bauSlotRow)}; margin-bottom:0; flex:1; justify-content:center;">
                      <span style="${Ne(i.bauFlag)}">${C.flag}</span>
                      <span style="${Ne(i.bauDate)}">${C.date}</span>
                  </div>
              `).join("")}</div>
          ...
          `,f.after(U);let A=U.querySelector("#cw-bau-toggle-btn"),w=U.querySelector("#cw-bau-full");if(A&&w&&(A.onclick=()=>{let p=w.style.display==="none";w.style.display=p?"block":"none",A.textContent=p?"Ocultar":"Detalhes"}),window._cwIsAdmin){let p=U.querySelector(".cw-bau-edit");p&&(p.onclick=()=>O(j))}}let J=H.sort((j,U)=>{let K=I.includes(j.id),l=I.includes(U.id);return K===l?0:K?1:-1});if(J.length===0&&!M){let j=document.createElement("div");Object.assign(j.style,i.emptyState),j.innerHTML=`
           ...
           `,X.appendChild(j)}let ae=J.filter(j=>!I.includes(j.id)),te=J.filter(j=>I.includes(j.id));if(ae.forEach(j=>X.appendChild(N(j,!1))),te.length>0){let j=document.createElement("div");Object.assign(j.style,i.historyDivider),j.innerHTML=`<span>Hist\xF3rico (${te.length})</span><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>`;let U=document.createElement("div");Object.assign(U.style,i.historyContainer),te.forEach(l=>U.appendChild(N(l,!0)));let K=!1;j.onclick=()=>{R.playClick(),K=!K,U.style.display=K?"flex":"none",j.querySelector("svg").style.transform=K?"rotate(180deg)":"rotate(0deg)"},X.appendChild(j),X.appendChild(U)}}function N(E,I){let H=document.createElement("div");Object.assign(H.style,I?i.cardHistory:i.card);let M=s[E.type]||s.info,J=document.createElement("div");Object.assign(J.style,i.cardHeader);let ae=document.createElement("div");Object.assign(ae.style,i.typeTag,{color:M.color,background:M.bg}),ae.innerHTML=`${M.icon} <span>${E.type}</span>`;let te=document.createElement("span");if(Object.assign(te.style,i.dateTag),te.textContent=n(E.date),J.appendChild(ae),I)J.appendChild(te);else{let m=document.createElement("button");m.className="cw-btn-interactive",Object.assign(m.style,i.dismissBtn),m.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"></polyline></svg>',m.onmouseenter=()=>{m.style.color="#1e8e3e",m.style.background="#e6f4ea",m.style.borderColor="#1e8e3e"},m.onmouseleave=()=>{m.style.color="#5f6368",m.style.background="#fff",m.style.borderColor="rgba(0,0,0,0.1)"},m.onclick=y=>{y.stopPropagation(),R.playClick(),H.style.transform="translateX(20px)",H.style.opacity="0",setTimeout(()=>{let x=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]");x.push(E.id),localStorage.setItem("cw_read_broadcasts",JSON.stringify(x)),F(),q()},200)},J.appendChild(m)}let j=document.createElement("div");Object.assign(j.style,i.cardContent);let U=document.createElement("div");Object.assign(U.style,i.msgTitle),U.textContent=E.title;let K=document.createElement("div");Object.assign(K.style,i.msgBody),K.innerHTML=r(E.text);let l=document.createElement("div");if(Object.assign(l.style,i.msgMeta),l.innerHTML=`Publicado por <b>${E.author||"Sistema"}</b>`,I||(l.innerHTML+=` \u2022 ${n(E.date)}`),j.appendChild(U),j.appendChild(K),j.appendChild(l),H.appendChild(J),H.appendChild(j),window._cwIsAdmin){let m=document.createElement("div");m.className="cw-card-actions";let y=document.createElement("button");y.className="cw-action-btn edit",y.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg> Editar',y.onclick=()=>O(E);let x=document.createElement("button");x.className="cw-action-btn delete",x.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg> Excluir',x.onclick=()=>G(E.id),m.appendChild(y),m.appendChild(x),H.appendChild(m)}return H}let V=ce.getCachedBroadcasts();V.length>0&&(Dt(V),F()),setTimeout(S,500),oe(),o||(o=setInterval(oe,ua));let B=document.createElement("div");Object.assign(B.style,Le),B.className="no-drag",c.appendChild(B),qe(c,B),document.body.appendChild(c);let D=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]"),Y=ze.some(E=>!D.includes(E.id));return{toggle:d,hasUnread:Y}}function Io(){if(localStorage.getItem("cw_onboarding_seen_v1"))return;let t=[{icon:"\u{1F680}",title:"Bem-vindo ao TechSol Suite",text:"Sua nova central de opera\xE7\xF5es para maximizar produtividade e padroniza\xE7\xE3o no CRM."},{icon:"\u{1F4DD}",title:"Notas Autom\xE1ticas",text:"Gere notas de caso (BAU/LM) perfeitas em segundos. Selecione o Status, as Tasks e deixe o wizard escrever o texto t\xE9cnico para voc\xEA."},{icon:"\u26A1",title:"Quick Email & Scripts",text:"Responda e-mails com templates inteligentes que detectam o contexto e use scripts de chamada interativos que guiam seu atendimento."},{icon:"\u{1F4E2}",title:"Fique Informado",text:"O m\xF3dulo Broadcast traz avisos importantes e disponibilidade BAU direto na sua tela, sem precisar abrir planilhas externas."},{icon:"\u2705",title:"Tudo Pronto!",text:"Explore o Menu Flutuante para come\xE7ar. Bom trabalho!",isLast:!0}],e=0,o={overlay:{position:"fixed",top:0,left:0,width:"100vw",height:"100vh",backgroundColor:"rgba(0,0,0,0.6)",backdropFilter:"blur(4px)",zIndex:"2147483647",display:"flex",alignItems:"center",justifyContent:"center",opacity:"0",transition:"opacity 0.3s ease"},card:{width:"380px",background:"#fff",borderRadius:"24px",padding:"32px",textAlign:"center",position:"relative",boxShadow:"0 20px 50px rgba(0,0,0,0.3)",fontFamily:"'Google Sans', Roboto, sans-serif",transform:"translateY(20px)",transition:"all 0.4s cubic-bezier(0.19, 1, 0.22, 1)"},icon:{fontSize:"48px",marginBottom:"20px",display:"block"},title:{fontSize:"22px",fontWeight:"700",color:"#202124",marginBottom:"12px"},text:{fontSize:"15px",color:"#5f6368",lineHeight:"1.6",marginBottom:"32px"},dotsContainer:{display:"flex",justifyContent:"center",gap:"8px",marginBottom:"24px"},dot:{width:"8px",height:"8px",borderRadius:"50%",background:"#dadce0",transition:"all 0.3s"},dotActive:{background:"#1a73e8",width:"24px",borderRadius:"4px"},btnContainer:{display:"flex",justifyContent:"space-between",alignItems:"center"},btn:{padding:"10px 24px",borderRadius:"20px",border:"none",cursor:"pointer",fontSize:"14px",fontWeight:"600",transition:"background 0.2s"},btnSkip:{background:"transparent",color:"#5f6368"},btnNext:{background:"#1a73e8",color:"#fff",boxShadow:"0 4px 12px rgba(26,115,232,0.3)"}},a=document.createElement("div");Object.assign(a.style,o.overlay);let n=document.createElement("div");Object.assign(n.style,o.card);let i=document.createElement("div");Object.assign(i.style,o.icon);let s=document.createElement("div");Object.assign(s.style,o.title);let r=document.createElement("div");Object.assign(r.style,o.text);let c=document.createElement("div");Object.assign(c.style,o.dotsContainer);let b=document.createElement("div");Object.assign(b.style,o.btnContainer);let d=document.createElement("button");d.textContent="Pular",Object.assign(d.style,o.btn,o.btnSkip),d.onmouseover=()=>d.style.color="#202124",d.onmouseout=()=>d.style.color="#5f6368";let u=document.createElement("button");u.textContent="Pr\xF3ximo",Object.assign(u.style,o.btn,o.btnNext),u.onmouseover=()=>u.style.transform="scale(1.05)",u.onmouseout=()=>u.style.transform="scale(1)",b.appendChild(d),b.appendChild(u),n.appendChild(i),n.appendChild(s),n.appendChild(r),n.appendChild(c),n.appendChild(b),a.appendChild(n),document.body.appendChild(a);function g(S){let f=t[S];i.textContent=f.icon,s.textContent=f.title,r.textContent=f.text,c.innerHTML="",t.forEach((T,O)=>{let L=document.createElement("div");Object.assign(L.style,o.dot),O===S&&Object.assign(L.style,o.dotActive),c.appendChild(L)}),f.isLast?(d.style.display="none",u.textContent="Come\xE7ar \u{1F680}",u.style.width="100%"):(d.style.display="block",u.textContent="Pr\xF3ximo",u.style.width="auto")}function h(){localStorage.setItem("cw_onboarding_seen_v1","true"),a.style.opacity="0",n.style.transform="translateY(20px)",setTimeout(()=>a.remove(),400),R.playSuccess(),Q("Tudo pronto! Use o menu flutuante.")}u.onclick=()=>{R.playClick(),e<t.length-1?(e++,g(e)):h()},d.onclick=async()=>{await ve("Pular o tutorial?")&&h()},g(0),requestAnimationFrame(()=>{a.style.opacity="1",n.style.transform="translateY(0)"})}var Lo={version:"v5.1",title:"Atualiza\xE7\xE3o: v5.1 - Produtividade Blindada \u{1F6E1}\uFE0F",slides:[{icon:"\u{1F17F}\uFE0F",title:"Estacionamento de Casos",text:"Interrup\xE7\xE3o urgente? Agora voc\xEA pode 'Estacionar' seu atendimento atual (Notas + Tasks) com um clique e retomar depois exatamente de onde parou."},{icon:"\u{1F6DF}",title:"Sistema 'Airbag'",text:"Caiu a internet? Fechou a aba sem querer? O TechSol agora possui Auto-Save de emerg\xEAncia a cada 5 segundos. Seu texto est\xE1 salvo, sempre."},{icon:"\u{1F7E0}",title:"Indicador de Progresso",text:"Nunca mais esque\xE7a uma nota aberta. Um indicador laranja ('Dirty State') avisa na P\xEDlula principal se h\xE1 trabalho n\xE3o salvo/estacionado."},{icon:"\u{1F50D}",title:"Time Zone Pro",text:"O m\xF3dulo de fusos hor\xE1rios ganhou superpoderes: nova barra de pesquisa global, filtros r\xE1pidos por regi\xE3o e corre\xE7\xE3o de visualiza\xE7\xE3o."},{icon:"\u{1F916}",title:"Leitura de BAU Aprimorada",text:"O sistema de Broadcast agora \xE9 mais inteligente ao ler avisos de disponibilidade, detectando datas e bandeiras mesmo quando quebradas em v\xE1rias linhas."},{icon:"\u{1F3A8}",title:"Refinamento Visual",text:"Bot\xF5es padronizados, sombras suavizadas e micro-intera\xE7\xF5es t\xE1teis em todo o sistema para uma experi\xEAncia mais fluida e profissional."}]};function qo(t){let e=localStorage.getItem("cw_last_version");if(!e){localStorage.setItem("cw_last_version",t);return}e!==t&&ma(t)}function ma(t){let e=Lo.slides,o=0,a={overlay:{position:"fixed",top:0,left:0,width:"100vw",height:"100vh",backgroundColor:"rgba(0,0,0,0.7)",backdropFilter:"blur(5px)",zIndex:"2147483647",display:"flex",alignItems:"center",justifyContent:"center",opacity:"0",transition:"opacity 0.3s ease"},card:{width:"400px",background:"#fff",borderRadius:"24px",padding:"32px",textAlign:"center",position:"relative",boxShadow:"0 24px 60px rgba(0,0,0,0.4)",fontFamily:"'Google Sans', Roboto, sans-serif",transform:"translateY(30px)",transition:"all 0.4s cubic-bezier(0.19, 1, 0.22, 1)"},badge:{display:"inline-block",padding:"4px 12px",borderRadius:"12px",background:"#E8F0FE",color:"#1967D2",fontSize:"11px",fontWeight:"700",textTransform:"uppercase",marginBottom:"16px",letterSpacing:"0.5px"},icon:{fontSize:"42px",marginBottom:"16px",display:"block"},title:{fontSize:"20px",fontWeight:"700",color:"#202124",marginBottom:"8px"},text:{fontSize:"14px",color:"#5f6368",lineHeight:"1.5",marginBottom:"32px",minHeight:"42px"},dotsContainer:{display:"flex",justifyContent:"center",gap:"6px",marginBottom:"24px"},dot:{width:"6px",height:"6px",borderRadius:"50%",background:"#dadce0",transition:"all 0.3s"},dotActive:{background:"#1a73e8",width:"18px",borderRadius:"4px"},btn:{width:"100%",padding:"12px",borderRadius:"12px",border:"none",cursor:"pointer",fontSize:"14px",fontWeight:"600",transition:"all 0.2s",background:"#1a73e8",color:"#fff",boxShadow:"0 4px 12px rgba(26,115,232,0.3)"}},n=document.createElement("div");Object.assign(n.style,a.overlay);let i=document.createElement("div");Object.assign(i.style,a.card);let s=document.createElement("div");Object.assign(s.style,a.badge),s.textContent=`Atualiza\xE7\xE3o ${t}`;let r=document.createElement("div");Object.assign(r.style,a.icon);let c=document.createElement("div");Object.assign(c.style,a.title);let b=document.createElement("div");Object.assign(b.style,a.text);let d=document.createElement("div");Object.assign(d.style,a.dotsContainer);let u=document.createElement("button");Object.assign(u.style,a.btn),u.onmouseover=()=>u.style.transform="scale(1.02)",u.onmouseout=()=>u.style.transform="scale(1)",i.appendChild(s),i.appendChild(r),i.appendChild(c),i.appendChild(b),i.appendChild(d),i.appendChild(u),n.appendChild(i),document.body.appendChild(n);function g(S){let f=e[S];r.textContent=f.icon,c.textContent=f.title,b.textContent=f.text,d.innerHTML="",e.forEach((T,O)=>{let L=document.createElement("div");Object.assign(L.style,a.dot),O===S&&Object.assign(L.style,a.dotActive),d.appendChild(L)}),S===e.length-1?u.textContent="Entendi, vamos l\xE1! \u{1F44D}":u.textContent="Pr\xF3ximo"}function h(){localStorage.setItem("cw_last_version",t),n.style.opacity="0",i.style.transform="translateY(30px)",setTimeout(()=>n.remove(),400),R.playSuccess(),Q(`TechSol atualizado para ${t}!`)}u.onclick=()=>{R.playClick(),o<e.length-1?(o++,g(o)):h()},g(0),requestAnimationFrame(()=>{n.style.opacity="1",i.style.transform="translateY(0)"})}var Oo="cw_timezone_pinned",Mt=[{id:"pt",name:"Portugal",flag:"\u{1F1F5}\u{1F1F9}",zone:"Europe/Lisbon",label:"Lisboa",region:"eu"},{id:"es",name:"Espanha",flag:"\u{1F1EA}\u{1F1F8}",zone:"Europe/Madrid",label:"Madrid",region:"eu"},{id:"ar",name:"Argentina",flag:"\u{1F1E6}\u{1F1F7}",zone:"America/Argentina/Buenos_Aires",label:"Buenos Aires",region:"sa"},{id:"bo",name:"Bol\xEDvia",flag:"\u{1F1E7}\u{1F1F4}",zone:"America/La_Paz",label:"La Paz",region:"sa"},{id:"cl",name:"Chile",flag:"\u{1F1E8}\u{1F1F1}",zone:"America/Santiago",label:"Santiago",region:"sa"},{id:"co",name:"Col\xF4mbia",flag:"\u{1F1E8}\u{1F1F4}",zone:"America/Bogota",label:"Bogot\xE1",region:"sa"},{id:"ec",name:"Equador",flag:"\u{1F1EA}\u{1F1E8}",zone:"America/Guayaquil",label:"Guayaquil",region:"sa"},{id:"py",name:"Paraguai",flag:"\u{1F1F5}\u{1F1FE}",zone:"America/Asuncion",label:"Assun\xE7\xE3o",region:"sa"},{id:"pe",name:"Peru",flag:"\u{1F1F5}\u{1F1EA}",zone:"America/Lima",label:"Lima",region:"sa"},{id:"uy",name:"Uruguai",flag:"\u{1F1FA}\u{1F1FE}",zone:"America/Montevideo",label:"Montevid\xE9u",region:"sa"},{id:"ve",name:"Venezuela",flag:"\u{1F1FB}\u{1F1EA}",zone:"America/Caracas",label:"Caracas",region:"sa"},{id:"mx",name:"M\xE9xico",flag:"\u{1F1F2}\u{1F1FD}",zone:"America/Mexico_City",label:"CDMX",region:"na"},{id:"cr",name:"Costa Rica",flag:"\u{1F1E8}\u{1F1F7}",zone:"America/Costa_Rica",label:"San Jos\xE9",region:"na"},{id:"sv",name:"El Salvador",flag:"\u{1F1F8}\u{1F1FB}",zone:"America/El_Salvador",label:"San Salvador",region:"na"},{id:"gt",name:"Guatemala",flag:"\u{1F1EC}\u{1F1F9}",zone:"America/Guatemala",label:"C. da Guatemala",region:"na"},{id:"hn",name:"Honduras",flag:"\u{1F1ED}\u{1F1F3}",zone:"America/Tegucigalpa",label:"Tegucigalpa",region:"na"},{id:"ni",name:"Nicar\xE1gua",flag:"\u{1F1F3}\u{1F1EE}",zone:"America/Managua",label:"Man\xE1gua",region:"na"},{id:"pa",name:"Panam\xE1",flag:"\u{1F1F5}\u{1F1E6}",zone:"America/Panama",label:"C. do Panam\xE1",region:"na"},{id:"do",name:"Rep. Dominicana",flag:"\u{1F1E9}\u{1F1F4}",zone:"America/Santo_Domingo",label:"Santo Domingo",region:"na"},{id:"pr",name:"Porto Rico",flag:"\u{1F1F5}\u{1F1F7}",zone:"America/Puerto_Rico",label:"San Juan",region:"na"}],ga=[{id:"all",label:"Todos"},{id:"sa",label:"Am\xE9rica do Sul"},{id:"na",label:"Norte & Central"},{id:"eu",label:"Europa"}];function _o(){let t="v2.2 Pro",e=!1,o=null,a="mx",n=JSON.parse(localStorage.getItem(Oo)||"[]"),i="",s="all",r=new Date;r.setHours(14,0,0,0);let c={bg:"#F8F9FA",surface:"#FFFFFF",primary:"#1A73E8",primaryBg:"#E8F0FE",text:"#202124",textSub:"#5F6368",border:"#DADCE0",success:"#1E8E3E",successBg:"#E6F4EA",warning:"#E37400",warningBg:"#FEF7E0",error:"#D93025",errorBg:"#FCE8E6"},b={container:{display:"flex",flexDirection:"column",height:"100%",background:c.bg,fontFamily:"'Google Sans', Roboto, sans-serif"},tabHeader:{display:"flex",background:c.surface,borderBottom:`1px solid ${c.border}`,padding:"8px 16px 0 16px"},tabBtn:{flex:1,padding:"12px",textAlign:"center",cursor:"pointer",fontSize:"13px",fontWeight:"500",color:c.textSub,borderBottom:"3px solid transparent",transition:"all 0.2s ease",userSelect:"none"},tabActive:{color:c.primary,borderBottomColor:c.primary,fontWeight:"600"},toolbar:{padding:"12px 16px 8px 16px",background:c.bg,display:"flex",flexDirection:"column",gap:"12px",borderBottom:"1px solid rgba(0,0,0,0.03)"},searchInputWrapper:{position:"relative",width:"100%"},searchInput:{width:"100%",boxSizing:"border-box",padding:"10px 12px 10px 38px",borderRadius:"10px",border:"1px solid transparent",background:"#FFFFFF",fontSize:"14px",color:c.text,outline:"none",boxShadow:"0 1px 3px rgba(0,0,0,0.05)",transition:"all 0.2s",fontFamily:"'Google Sans', Roboto, sans-serif"},searchIcon:{position:"absolute",left:"12px",top:"50%",transform:"translateY(-50%)",width:"16px",height:"16px",color:"#9AA0A6",pointerEvents:"none"},chipsRow:{display:"flex",gap:"8px",overflowX:"auto",paddingBottom:"4px",scrollbarWidth:"none",msOverflowStyle:"none"},chip:{whiteSpace:"nowrap",padding:"6px 12px",borderRadius:"16px",fontSize:"12px",fontWeight:"500",cursor:"pointer",border:`1px solid ${c.border}`,background:c.surface,color:c.textSub,transition:"all 0.2s"},chipActive:{background:c.primaryBg,color:c.primary,borderColor:c.primaryBg,fontWeight:"600"},listContainer:{padding:"16px 16px 40px 16px",overflowY:"auto",flex:1,display:"flex",flexDirection:"column",gap:"12px",scrollbarWidth:"none"},hubCard:{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px 20px",background:c.surface,borderRadius:"16px",border:"1px solid transparent",boxShadow:"0 2px 6px rgba(60,64,67,0.05)",transition:"transform 0.2s cubic-bezier(0.25, 0.8, 0.25, 1), box-shadow 0.2s ease",cursor:"pointer",position:"relative"},hubCardPinned:{borderLeft:`4px solid ${c.primary}`,paddingLeft:"16px"},plannerWrapper:{padding:"24px",display:"flex",flexDirection:"column",gap:"24px",flex:1,overflowY:"auto"},timeComparisonRow:{display:"flex",gap:"16px",alignItems:"stretch"},timeCard:{flex:1,padding:"20px",borderRadius:"20px",background:c.surface,border:`1px solid ${c.border}`,display:"flex",flexDirection:"column",alignItems:"center",gap:"8px",boxShadow:"0 4px 12px rgba(60,64,67,0.05)"},timelineContainer:{position:"relative",height:"60px",marginTop:"16px",userSelect:"none"},timelineTrack:{position:"absolute",top:"26px",left:"0",right:"0",height:"6px",borderRadius:"3px",background:"#E0E0E0",overflow:"hidden"},dayZone:{position:"absolute",top:"0",bottom:"0",left:"37.5%",width:"37.5%",background:"rgba(52, 168, 83, 0.3)",pointerEvents:"none"},hdInput:{fontSize:"28px",fontWeight:"700",color:c.text,border:"none",background:"transparent",width:"100%",textAlign:"center",outline:"none",fontFamily:"'Google Sans', sans-serif",cursor:"text"},statusBadge:{padding:"8px 16px",borderRadius:"50px",fontSize:"13px",fontWeight:"600",display:"inline-flex",alignItems:"center",gap:"8px",marginTop:"16px",alignSelf:"center",transition:"background-color 0.3s"}},d=document.createElement("div");d.id="timezone-popup",d.classList.add("cw-module-window"),Object.assign(d.style,ye,{right:"100px",width:"450px",height:"720px",overflow:"hidden",borderRadius:"24px"});let g=ge(d,"Time Zone Traveler",t,"Monitoramento global e planejamento de chamadas.",{popup:d},()=>H());d.appendChild(g);let h=document.createElement("div");Object.assign(h.style,b.container),d.appendChild(h);let S=document.createElement("div");Object.assign(S.style,b.tabHeader);let f=document.createElement("div");f.textContent="Monitoramento",Object.assign(f.style,b.tabBtn,b.tabActive);let T=document.createElement("div");T.textContent="Planejador",Object.assign(T.style,b.tabBtn),S.appendChild(f),S.appendChild(T),h.appendChild(S);let O=document.createElement("div");Object.assign(O.style,b.toolbar);let L=document.createElement("div");Object.assign(L.style,b.searchInputWrapper);let _=document.createElement("div");_.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>',Object.assign(_.style,b.searchIcon);let G=document.createElement("input");G.placeholder="Buscar cidade ou pa\xEDs...",Object.assign(G.style,b.searchInput),G.onfocus=()=>{G.style.boxShadow="0 2px 8px rgba(26,115,232,0.15)",G.style.borderColor="rgba(26,115,232,0.3)"},G.onblur=()=>{G.style.boxShadow="0 1px 3px rgba(0,0,0,0.05)",G.style.borderColor="transparent"},G.oninput=M=>{i=M.target.value.toLowerCase(),D()},L.appendChild(_),L.appendChild(G),O.appendChild(L);let X=document.createElement("div");Object.assign(X.style,b.chipsRow),ga.forEach(M=>{let J=document.createElement("div");J.textContent=M.label,J.id=`tz-filter-${M.id}`,Object.assign(J.style,b.chip),M.id===s&&Object.assign(J.style,b.chipActive),J.onclick=()=>{R.playClick(),s=M.id,Array.from(X.children).forEach(ae=>{Object.assign(ae.style,b.chip)}),Object.assign(J.style,b.chipActive),D()},X.appendChild(J)}),O.appendChild(X),h.appendChild(O);let oe=document.createElement("div");Object.assign(oe.style,b.listContainer);let q=document.createElement("style");q.textContent="#timezone-popup ::-webkit-scrollbar { display: none; }",h.appendChild(q);let F=document.createElement("div");Object.assign(F.style,b.plannerWrapper,{display:"none"}),h.appendChild(oe),h.appendChild(F),f.onclick=()=>N("live"),T.onclick=()=>N("plan");function N(M){R.playClick(),M==="live"?(Object.assign(f.style,b.tabActive),Object.assign(T.style,b.tabBtn),T.style.borderBottomColor="transparent",oe.style.display="flex",O.style.display="flex",F.style.display="none",E()):(Object.assign(T.style,b.tabActive),Object.assign(f.style,b.tabBtn),f.style.borderBottomColor="transparent",F.style.display="flex",oe.style.display="none",O.style.display="none",I(),Y())}function V(M){return M>=9&&M<17?{color:c.success,bg:c.successBg,label:"Aberto",icon:"\u{1F7E2}"}:M>=8&&M<9?{color:c.warning,bg:c.warningBg,label:"Abrindo",icon:"\u{1F7E1}"}:M>=17&&M<19?{color:c.warning,bg:c.warningBg,label:"Fechando",icon:"\u{1F7E1}"}:{color:c.textSub,bg:"#F1F3F4",label:"Fechado",icon:"\u{1F534}"}}function B(M){n.includes(M)?n=n.filter(J=>J!==M):n.push(M),localStorage.setItem(Oo,JSON.stringify(n)),D(),R.playClick()}function D(){oe.innerHTML="";let M=new Date,J=Mt.filter(te=>{let j=te.name.toLowerCase().includes(i)||te.label.toLowerCase().includes(i),U=s==="all"||te.region===s;return j&&U});if(J.sort((te,j)=>{let U=n.includes(te.id),K=n.includes(j.id);return U&&!K?-1:!U&&K?1:te.name.localeCompare(j.name)}),J.length===0){oe.innerHTML=`
            ...
            `,y.onmouseenter=()=>{y.style.transform="translateY(-2px)",y.style.boxShadow="0 6px 12px rgba(60,64,67,0.1)"},y.onmouseleave=()=>{y.style.transform="translateY(0)",y.style.boxShadow="0 2px 6px rgba(60,64,67,0.05)"};let A=y.querySelector(".cw-pin-btn");A.onmouseenter=()=>{A.style.backgroundColor="#F1F3F4"},A.onmouseleave=()=>{A.style.backgroundColor="transparent"},A.onclick=w=>{w.stopPropagation(),B(te.id)},y.onclick=()=>{a=te.id,N("plan")},oe.appendChild(y)});let ae=document.createElement("div");ae.style.height="20px",ae.style.width="100%",oe.appendChild(ae)}function Y(){F.innerHTML="";let M=document.createElement("div"),J=document.createElement("label");J.textContent="Onde est\xE1 o cliente?",J.style.cssText="display:block; font-size:12px; font-weight:700; color:#5F6368; margin-bottom:8px; text-transform:uppercase; letter-spacing:0.5px;";let ae=document.createElement("select");Object.assign(ae.style,Jt),ae.style.padding="14px",[...Mt].sort((ee,Z)=>ee.name.localeCompare(Z.name)).forEach(ee=>{let Z=document.createElement("option");Z.value=ee.id,Z.textContent=`${ee.flag} ${ee.name} (${ee.zone})`,ee.id===a&&(Z.selected=!0),ae.appendChild(Z)}),ae.onchange=ee=>{a=ee.target.value,z(),R.playClick()},M.appendChild(J),M.appendChild(ae),F.appendChild(M);let j=document.createElement("div");Object.assign(j.style,b.timeComparisonRow);let U=document.createElement("div");Object.assign(U.style,b.timeCard),U.style.backgroundColor="#F8FAFF",U.style.borderColor="#E8F0FE",U.innerHTML=`
    ...
    `,document.head.appendChild(t)}function Mo(){let t="v2.0",e=!1,o="general",a="",n=null,i=null;ba();let s=document.createElement("div");s.id="library-popup",s.classList.add("cw-module-window"),Object.assign(s.style,ye,{right:"auto",left:"50%",width:"620px",height:"680px",maxHeight:"90vh",transform:"translateX(-50%) scale(0.05)"});let c=ge(s,"Minha Biblioteca",t,"Gerencie seus snippets, textos e templates.",{popup:s},()=>K());s.appendChild(c);let b=document.createElement("div");b.className="cw-lib-container",s.appendChild(b);let d=document.createElement("div");d.className="cw-lib-toolbar";let u=document.createElement("div");u.className="cw-lib-search-wrap";let g=document.createElement("div");g.className="cw-lib-search-icon",g.innerHTML=xe.search;let h=document.createElement("input");h.className="cw-lib-search no-drag",h.placeholder="Buscar por t\xEDtulo ou conte\xFAdo...",h.type="text";let S=document.createElement("div");S.className="cw-lib-search-clear cw-tactile",S.innerHTML=xe.clear,u.append(g,h,S);let f=document.createElement("div");f.className="cw-lib-tabs",Do.forEach(l=>{let m=document.createElement("div");m.className="cw-lib-tab"+(l.id===o?" active":""),m.id=`lib-tab-${l.id}`,m.innerHTML=`${l.icon}<span>${l.label}</span>`,m.onmouseenter=()=>R.playHover(),m.onclick=()=>D(l.id),f.appendChild(m)}),d.append(u,f),b.appendChild(d);let T=document.createElement("div");T.className="cw-lib-grid",b.appendChild(T);let O=document.createElement("div");O.className="cw-lib-fab cw-tactile",O.title="Novo item",O.innerHTML=xe.add,O.onclick=()=>J(),b.appendChild(O);let L=document.createElement("div");L.className="cw-lib-sheet";let _=document.createElement("div");_.className="cw-lib-sheet-handle";let G=document.createElement("div");G.className="cw-lib-sheet-head";let X=document.createElement("div");X.className="cw-lib-sheet-back no-drag",X.innerHTML=xe.back,X.title="Cancelar",X.onclick=ae;let oe=document.createElement("span");oe.className="cw-lib-sheet-title",oe.textContent="Novo Item",G.append(X,oe);let q=document.createElement("div");q.className="cw-lib-sheet-body";let F=document.createElement("div");F.className="cw-lib-sheet-foot";let N=document.createElement("button");N.className="cw-lib-save-btn no-drag",N.textContent="Salvar",N.onclick=te,F.appendChild(N);let V=document.createElement("div");V.className="cw-lib-loading",V.innerHTML='<div class="cw-lib-spinner"></div><div class="cw-lib-loading-text">Salvando...</div>',L.append(_,G,q,F,V),b.appendChild(L);let B=document.createElement("div");Object.assign(B.style,Le),B.className="no-drag",s.appendChild(B),qe(s,B),document.body.appendChild(s),document.addEventListener("mousedown",l=>{i&&!i.contains(l.target)&&Y()});function D(l){R.playClick(),o=l,Do.forEach(m=>{document.getElementById(`lib-tab-${m.id}`).classList.toggle("active",m.id===l)}),I()}function Y(){if(i){let l=i.querySelector(".cw-lib-menu");l&&l.classList.remove("open"),i=null}}function E(l,m){return m?`${l.title} ${l.content}`.toLowerCase().includes(m):!0}function I(){Y(),T.innerHTML="";let l=a.trim().toLowerCase(),m=Se.getSnippets(o).filter(y=>E(y,l));if(m.length===0){let y=document.createElement("div");y.className="cw-lib-empty";let x=l.length>0;y.innerHTML=`
            ...
            `,T.appendChild(y);return}m.forEach(y=>T.appendChild(H(y)))}function H(l){let m=document.createElement("div");m.className="cw-lib-card"+(l.isCode?" is-code":"");let y=l.content,x="";if(l.isRich){let p=document.createElement("div");p.innerHTML=l.content;let C=!!p.querySelector("img");y=p.innerText.substring(0,200),C&&(x=`<span class="cw-lib-media-tag">${xe.media} M\xEDdia</span>`)}let v=[l.isCode?'<span class="cw-lib-badge code">CODE</span>':"",o==="email"?'<span class="cw-lib-badge template">TEMPLATE</span>':""].join("");m.innerHTML=`
        ...
        `,m.querySelector(".cw-act-copy").onclick=p=>{p.stopPropagation(),R.playClick(),M(l)};let A=m.querySelector(".cw-act-more"),w=m.querySelector(".cw-lib-menu");return A.onclick=p=>{p.stopPropagation(),R.playClick();let C=w.classList.contains("open");Y(),C||(w.classList.add("open"),i=m)},m.querySelector(".cw-act-edit").onclick=p=>{p.stopPropagation(),R.playClick(),Y(),J(l)},m.querySelector(".cw-act-del").onclick=async p=>{p.stopPropagation(),R.playClick(),Y(),await ve(`Excluir "${l.title}"?`)&&(Se.delete(l.id),I(),Q("Item exclu\xEDdo."))},m}function M(l){if(l.isRich){let m=new Blob([l.content],{type:"text/html"}),y=document.createElement("div");y.innerHTML=l.content;let x=new Blob([y.innerText],{type:"text/plain"});navigator.clipboard.write([new ClipboardItem({"text/html":m,"text/plain":x})])}else navigator.clipboard.writeText(l.content);Q("Copiado!")}function J(l=null){n=l?l.id:null,q.innerHTML="",q.appendChild(j("title","T\xEDtulo / Nome",l?l.title:"")),o==="email"&&q.appendChild(j("subject","Assunto do Email",l?l.subject:""));let m="Conte\xFAdo";o==="email"&&(m="Corpo do Email (HTML)"),o==="note"&&(m="Texto da Nota"),q.appendChild(j("content",m,l?l.content:"",{isRich:!0,isCode:l?l.isCode:!1})),oe.textContent=l?"Editar Item":"Novo Item",N.textContent=l?"Salvar Altera\xE7\xF5es":"Salvar",L.classList.add("open"),setTimeout(()=>{let y=q.querySelector("input");y&&y.focus()},350)}function ae(){R.playSwoosh(),L.classList.remove("open"),setTimeout(()=>{n=null},300)}async function te(){V.classList.add("active"),N.disabled=!0;try{let l=q.querySelector("#cw-lib-inp-title"),m=q.querySelector("#cw-lib-inp-content"),y=l.value.trim(),x=m.contentEditable==="true"?m.innerHTML:m.value.trim(),v=m.getAttribute("data-is-code")==="true";if(!y||!x||x==="<br>"){Q("Preencha t\xEDtulo e conte\xFAdo.",{error:!0});return}let A={id:n,type:o,title:y,content:x,isCode:v,isRich:m.contentEditable==="true"};if(o==="email"){let p=q.querySelector("#cw-lib-inp-subject").value.trim();if(!p){Q("Assunto \xE9 obrigat\xF3rio para emails.",{error:!0});return}A.subject=p}let w=await Se.save(A);if(w===!1){Q("N\xE3o foi poss\xEDvel salvar: usu\xE1rio n\xE3o identificado. Recarregue a p\xE1gina e tente de novo.",{error:!0});return}I(),ae(),w.synced===!1?Q("Salvo localmente \u2014 sem conex\xE3o com a nuvem no momento.",{error:!0}):(Q("Salvo e sincronizado!"),R.playSuccess())}catch(l){console.error("Erro ao salvar item da biblioteca:",l),Q("Erro ao salvar item.",{error:!0})}finally{V.classList.remove("active"),N.disabled=!1}}function j(l,m,y,x={}){let v=document.createElement("div");v.className="cw-lib-field";let A=document.createElement("label");A.className="cw-lib-label",A.textContent=m,v.appendChild(A);let w;if(x.isRich){let p=document.createElement("div");p.className="cw-lib-toolbar-mini",p.innerHTML=`
            ...
            `,w=document.createElement("div"),w.className="cw-lib-input cw-lib-editable",w.contentEditable="true",w.innerHTML=y||"",x.isCode&&(w.style.fontFamily="'Roboto Mono', monospace",w.style.background="#F8F9FA",w.setAttribute("data-is-code","true"),p.querySelector(".cw-tb-code").classList.add("active")),p.querySelectorAll(".cw-lib-tb-btn").forEach(C=>{C.onmouseenter=()=>R.playHover(),C.onmousedown=()=>R.playClick()}),p.querySelector(".cw-tb-bold").onclick=()=>{document.execCommand("bold"),w.focus()},p.querySelector(".cw-tb-italic").onclick=()=>{document.execCommand("italic"),w.focus()},p.querySelector(".cw-tb-code").onclick=C=>{let P=!(w.getAttribute("data-is-code")==="true");w.setAttribute("data-is-code",String(P)),w.style.fontFamily=P?"'Roboto Mono', monospace":"inherit",w.style.background=P?"#F8F9FA":"#fff",C.currentTarget.classList.toggle("active",P),w.focus()},p.querySelector(".cw-tb-img").onclick=async()=>{let C=await oo("Cole a URL da imagem:");C&&(document.execCommand("insertImage",!1,C),w.querySelectorAll("img").forEach(k=>{k.style.maxWidth="100%",k.style.borderRadius="8px"}))},w.onpaste=C=>{let k=(C.clipboardData||C.originalEvent.clipboardData).items;for(let P of k)if(P.kind==="file"&&P.type.startsWith("image/")){C.preventDefault();let z=new FileReader;z.onload=ee=>{document.execCommand("insertHTML",!1,`<img src="${ee.target.result}" style="max-width:100%;border-radius:8px;margin:8px 0;display:block;">`)},z.readAsDataURL(P.getAsFile())}},v.appendChild(p)}else w=document.createElement("input"),w.className="cw-lib-input",w.type="text",w.value=y||"";return w.id=`cw-lib-inp-${l}`,v.appendChild(w),v}function U(l){let m=document.createElement("div");return m.textContent=l||"",m.innerHTML}h.addEventListener("input",l=>{a=l.target.value,S.classList.toggle("visible",a.length>0),I()}),S.onclick=()=>{h.value="",a="",S.classList.remove("visible"),I(),h.focus()};function K(){e=!e,be(e,s,"cw-btn-library"),e?(document.body.style.overflow="hidden",I()):(document.body.style.overflow="",Y())}return K}function No(){let t="v1.0",e=!1,o={bg:"#F8F9FA",surface:"#FFFFFF",primary:"#1A73E8",text:"#202124",textSub:"#5F6368",border:"#DADCE0"},a="cw-configs-styles";if(!document.getElementById(a)){let S=document.createElement("style");S.id=a,S.innerHTML=`
    ...
    `;let u=d.querySelector("#cw-config-sound-toggle");u.onchange=S=>{R.setMuted(!S.target.checked),S.target.checked&&R.playClick()},r.appendChild(d);let g=document.createElement("div");g.className="cw-configs-section",g.innerHTML=`
  ...
  `,document.head.appendChild(t)};var Ue={steps:[{id:0,title:"Selecione o tipo de atendimento",isBranching:!0},{id:1,title:"Contexto e Valida\xE7\xE3o",fields:[{id:"advName",name:"advName",label:"Nome do Anunciante",type:"text",placeholder:"Nome do Anunciante",required:!0,isSmart:!0},{id:"cid",name:"cid",label:"CID",type:"text",placeholder:"000-000-0000",required:!0,isSmart:!0,tooltip:"Use o formato 000-000-0000 ou 10 d\xEDgitos",validation:{regex:"^(\\d{3}-\\d{3}-\\d{4}|\\d{10})$",error:"Formato de CID incorreto"}},{id:"amName",name:"amName",label:"Account Manager (AM)",type:"text",placeholder:"Nome do AM",required:!0,isSmart:!0},{id:"website",name:"website",label:"Website",type:"text",placeholder:"https://www.exemplo.com",required:!0,isSmart:!0},{id:"seId",name:"seId",label:"Speakeasy ID (SE ID)",type:"text-with-button",placeholder:"Speakeasy ID",required:!1,isSmart:!0,button:{id:"bau-top-se-search",icon:"wand",title:"Buscar ID automaticamente"}}]},{id:2,title:"Tasks",fields:[{id:"reason",name:"reason",label:"O que deve ser feito em BAU",type:"textarea",placeholder:"Descreva as a\xE7\xF5es esperadas...",required:!0,style:{minHeight:"80px"}},{id:"taskType",name:"taskType",label:"Tasks para BAU (Selecione 1 ou mais)",type:"checkbox-grid",required:!0,tooltip:"Selecione os tipos de implementa\xE7\xE3o t\xE9cnica",options:["Ads Conversion Tracking","Ads Dynamic Remarketing","Ads Enhanced Conversions","Ads Website Call Conversion","Ads Remarketing","Analytics Cross Domain Tracking","Analytics E-Commerce Tracking","Analytics Enhanced E-Commerce Tracking","Analytics Event Tracking","Analytics Health Check","Analytics Remarketing","Analytics Setup","Fix GA4 implementation","Consent Mode","Fix Sitewide Tagging (OGT & CT)","Google Tag Manager Installation","Customer Match"]}]},{id:3,title:"Justificativa e Agendamento",fields:[{id:"nonImplementationReason",name:"nonImplementationReason",label:"Motivo da N\xE3o Implementa\xE7\xE3o (Justificativa BAU)",type:"select",required:!0,options:[{value:"",text:"Selecione um motivo..."},{value:"Tempo da consultoria esgotado",text:"Tempo da consultoria esgotado"},{value:"Solicita\xE7\xE3o de reagendamento pelo anunciante",text:"Solicita\xE7\xE3o de reagendamento pelo anunciante"},{value:"Falta de acessos ou backup do site",text:"Falta de acessos ou backup do site"},{value:"Anunciante indispon\xEDvel ou n\xE3o preparado",text:"Anunciante indispon\xEDvel ou n\xE3o preparado"},{value:"Implementa\xE7\xE3o parcial (nem todas as tasks conclu\xEDdas)",text:"Implementa\xE7\xE3o parcial (nem todas as tasks conclu\xEDdas)"},{value:"Solicita\xE7\xE3o de tarefas (tasks) adicionais",text:"Solicita\xE7\xE3o de tarefas (tasks) adicionais"},{value:"Necessidade de novas altera\xE7\xF5es (fase de acompanhamento)",text:"Necessidade de novas altera\xE7\xF5es (fase de acompanhamento)"},{value:"Retorno de contato ap\xF3s prazo de 14 dias expirado",text:"Retorno de contato ap\xF3s prazo de 14 dias expirado"}]},{id:"description",name:"description",label:"Justificativa / Descri\xE7\xE3o",type:"textarea",placeholder:"Descreva detalhadamente o que precisa ser feito...",required:!0},{id:"availability",name:"availability",label:"Disponibilidade (m\xEDnimo 1 op\xE7\xE3o)",type:"datetime-group",required:!0,fields:[{name:"availability_1",label:"Op\xE7\xE3o 1 (Prioridade)",required:!0},{name:"availability_2",label:"Op\xE7\xE3o 2 (Opcional)",required:!1},{name:"availability_3",label:"Op\xE7\xE3o 3 (Opcional)",required:!1}]}]},{id:4,title:"Confirma\xE7\xE3o",isConfirmation:!0},{id:5,title:"Solicitar Descarte",fields:[{id:"caseId",name:"caseId",label:"Case ID",type:"text",placeholder:"Case ID",required:!0,isSmart:!0},{id:"language",name:"language",label:"Idioma",type:"text",placeholder:"Idioma",required:!0,isSmart:!0},{id:"seId",name:"seId",label:"Speakeasy ID (SE ID)",type:"text",placeholder:"Speakeasy ID",required:!0,isSmart:!0},{id:"description",name:"description",label:"Descri\xE7\xE3o",type:"textarea",placeholder:"Descreva o motivo do descarte...",required:!0},{id:"discardReason",name:"reason",label:"Motivo do Descarte",type:"select",required:!0,groups:[{label:"Live Appointments",options:[{value:"Caso Filho gerado no atendimento",text:"Caso Filho gerado no atendimento"},{value:"3\xAA Tentativa de contato sem sucesso",text:"3\xAA Tentativa de contato sem sucesso"}]},{label:"Live Meet",options:[{value:"Apenas o AM presente",text:"Apenas o AM presente"},{value:"Estouro de tempo para conclus\xE3o",text:"Estouro de tempo para conclus\xE3o"},{value:"Gera\xE7\xE3o de caso BAU (Reagendamento)",text:"Gera\xE7\xE3o de caso BAU (Reagendamento)"}]}]}]}]};var ue={add:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>',back:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z"/></svg>',wand:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M7.5 5.6L10 7 8.6 4.5 10 2 7.5 3.4 5 2l1.4 2.5L5 7zm12 9.8L17 14l1.4 2.5L17 19l2.5-1.4L22 19l-1.4-2.5L22 14zM22 2l-2.5 1.4L17 2l1.4 2.5L17 7l2.5-1.4L22 7l-1.4-2.5zm-7.63 5.29c-.39-.39-1.02-.39-1.41 0L1.29 18.96c-.39.39-.39 1.02 0 1.41l2.34 2.34c.39.39 1.02.39 1.41 0L16.7 11.05c.39-.39.39-1.02 0-1.41l-2.33-2.35zm-1.03 5.41l-2.12-2.12 2.44-2.44 2.12 2.12-2.44 2.44z"/></svg>',send:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/></svg>',check:'<svg width="64" height="64" viewBox="0 0 24 24" fill="none"><path class="bau-check-path" d="M5 13l4 4L19 7" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/></svg>',folder:'<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M10 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2h-8l-2-2z"/></svg>',empty:'<svg width="64" height="64" viewBox="0 0 24 24" fill="currentColor"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5v-3h3.56c.69 1.19 1.97 2 3.44 2s2.75-.81 3.44-2H19v3zm0-5h-4.99c0 1.1-.9 2-2 2s-2-.9-2-2H5V5h14v9z"/></svg>',refresh:'<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M17.65 6.35C16.2 4.9 14.21 4 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08c-.82 2.33-3.04 4-5.65 4-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z"/></svg>',expand:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M16.59 8.59L12 13.17 7.41 8.59 6 10l6 6 6-6z"/></svg>',edit:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg>'};function Ro(t){switch(t){case"PENDING_TL_CREATION":return{text:"Aguardando TL",class:"status-yellow",aura:"status-yellow-aura"};case"CREATED":return{text:"Aprovado / Criado",class:"status-green",aura:"status-green-aura"};case"DISCARDED":return{text:"Descartado pelo TL",class:"status-red",aura:"status-red-aura"};case"CANCELED_BY_AGENT":return{text:"Cancelado",class:"status-gray",aura:""};default:return{text:t||"Pendente",class:"status-gray",aura:""}}}function Bo(t){let e=document.createElement("div");if(e.className="bau-dynamic-input",e.id=`wrapper-${t.id}`,t.label){let a=document.createElement("label");a.className="bau-label",a.textContent=t.label,t.tooltip&&a.setAttribute("data-tooltip",t.tooltip),e.appendChild(a)}let o;switch(t.type){case"textarea":o=document.createElement("textarea"),o.style.minHeight="80px",e.appendChild(o);break;case"select":o=document.createElement("select"),t.groups?t.groups.forEach(s=>{let r=document.createElement("optgroup");r.label=s.label,s.options.forEach(c=>{let b=document.createElement("option");b.value=c.value,b.textContent=c.text,r.appendChild(b)}),o.appendChild(r)}):t.options&&t.options.forEach(s=>{let r=document.createElement("option");r.value=s.value,r.textContent=s.text,o.appendChild(r)}),e.appendChild(o);break;case"checkbox-grid":return o=document.createElement("div"),o.className="bau-tasks-grid",t.options.forEach(s=>{let r=document.createElement("label");r.className="bau-task-item",r.innerHTML=`<input type="checkbox" name="${t.name}" value="${s}"><span>${s}</span>`,r.addEventListener("click",c=>{c.preventDefault();let b=r.querySelector("input");b.checked=!b.checked,r.classList.toggle("active",b.checked),R.playClick()}),o.appendChild(r)}),e.appendChild(o),e;case"datetime-group":o=document.createElement("div"),o.className="bau-availability-container",t.fields.forEach(s=>{let r=document.createElement("div");r.className="bau-availability-field",r.innerHTML=`
    ...
    `,f.appendChild(T);let O=l=>{T.classList.toggle("active",l)},L=document.createElement("div");L.className="bau-progress-indicator",f.appendChild(L);let _=document.createElement("form");_.id="bau-escalation-form",_.noValidate=!0,f.appendChild(_),Ue.steps.forEach(l=>{let m=document.createElement("div");if(m.className="bau-step"+(l.id===a?" active":""),m.id=`bau-step-${l.id}`,l.isBranching)m.innerHTML=`
            ...
            `,m.querySelector("#bau-opt-full").onclick=()=>{n="BAU",a=1,_.querySelectorAll(".bau-highlight-panel").forEach(y=>y.classList.remove("discard-theme")),I(),R.playClick()},m.querySelector("#bau-opt-discard").onclick=()=>{n="DISCARD",a=5,_.querySelectorAll(".bau-highlight-panel").forEach(y=>y.classList.add("discard-theme")),I(),R.playClick()};else if(l.isConfirmation)m.innerHTML=`
                <div class="bau-card">
            ...
            `;else{let y=document.createElement("div");if(y.className="bau-card",l.id===1||l.id===5){y.innerHTML=`
                ...
                `;let x=y.querySelector(".bau-dynamic-inputs-container");l.fields.forEach(A=>{x.appendChild(Bo(A))});let v=y.querySelector("#wrapper-cid");if(v){let A=document.createElement("div");A.id="bau-cid-error",A.className="bau-cid-error-hint",A.style.display="none",A.textContent="Formato de CID incorreto",v.appendChild(A)}}else l.fields.forEach(x=>{y.appendChild(Bo(x))});m.appendChild(y)}_.appendChild(m)});let G=document.createElement("div");G.className="bau-footer";let X=document.createElement("button");X.type="button",X.id="bau-step-back-btn",X.className="bau-btn-secondary",X.textContent="Voltar";let oe=document.createElement("button");oe.type="button",oe.id="bau-step-next-btn",oe.className="bau-btn-primary",oe.textContent="Pr\xF3ximo";let q=document.createElement("button");q.type="submit",q.className="bau-btn-submit",q.innerHTML=`${ue.send} Enviar para o TL`,q.style.display="none",G.appendChild(X),G.appendChild(oe),G.appendChild(q),_.appendChild(G),d.appendChild(h);let F=document.createElement("div");F.id="bau-view-success",F.className="bau-view bau-success-view",F.innerHTML=`
    ...
    `,d.appendChild(F),document.body.appendChild(c);function N(l){e=l,c.querySelectorAll(".bau-view").forEach(A=>A.classList.remove("active"));let m=c.querySelector(`#bau-view-${l}`);m&&m.classList.add("active");let y=b.querySelector(".cw-module-header-title")||b.querySelector("h2"),x=b.querySelector(".cw-module-header-subtitle")||b.querySelector("p");y&&(l==="form"?y.textContent=i?`Editando Caso #${s}`:"Novo Caso BAU":y.textContent="BAU Central"),x&&(x.textContent=l==="form"?"Preencha os detalhes abaixo":"Dashboard de Casos BAU");let v=_.querySelector(".bau-btn-submit");v&&(v.innerHTML=i?`${ue.send} Salvar Altera\xE7\xF5es`:`${ue.send} Enviar para o TL`)}function V(){let l=c.querySelector("#bau-case-list-container"),m=c.querySelector("#bau-dashboard-metrics");m&&(m.innerHTML=`
        ...
        `).join(""))}async function B(){let l=c.querySelector("#bau-case-list-container"),m=c.querySelector("#bau-dashboard-metrics");if(!(!l||!m)){V();try{let y=await Gt();if(!Array.isArray(y))throw new Error("API response is not a valid array");E(y)}catch(y){console.error("Critical Error loading BAU cases:",y),m&&(m.innerHTML=""),l.innerHTML=`
            ...
            `,c.querySelector("#bau-retry-btn")?.addEventListener("click",()=>B()),Q("Erro ao carregar Dashboard. Verifique sua conex\xE3o.",{error:!0})}}}function D(l){if(!l)return;let m=Ro(l.status),y=(v,A)=>{navigator.clipboard.writeText(v).then(()=>{Q("Copiado para a \xE1rea de transfer\xEAncia!"),R.playClick();let w=A.style.color;A.style.color="#1E8E3E",setTimeout(()=>{A.style.color=w},800)})};u.innerHTML=`
        ...
        `}function E(l){let m=c.querySelector("#bau-case-list-container"),y=c.querySelector("#bau-dashboard-metrics");if(!m||!y)return;let x=Array.isArray(l)?l.filter(Boolean):[];if(x.length===0){y.innerHTML=`
            ...
            `,c.querySelector("#bau-refresh-dashboard")?.addEventListener("click",()=>B());return}let v=x.filter(k=>k.status==="PENDING_TL_CREATION").length,A=x.filter(k=>k.status==="CREATED").length;y.innerHTML=`
        ...
        `;let w=y.querySelector("#bau-refresh-dashboard");w?.addEventListener("click",async()=>{w.classList.contains("spinning")||(w.classList.add("spinning"),R.playClick(),await B(),setTimeout(()=>w.classList.remove("spinning"),1e3))}),m.innerHTML="";let p=x.slice(0,5),C=x.slice(5);if(p.forEach(k=>{let P=Y(k),z=document.createElement("div");z.innerHTML=P;let ee=z.firstElementChild;ee.addEventListener("click",se=>{se.target.closest(".bau-case-edit-btn")||D(k)});let Z=ee.querySelector(".bau-case-edit-btn");Z&&(Z.onclick=se=>{se.stopPropagation(),j(k)}),m.appendChild(ee)}),C.length>0){let k=document.createElement("li");k.className="bau-accordion-container";let P=document.createElement("button");P.className="bau-accordion-toggle",P.innerHTML=`${ue.expand} <span>Mostrar ${C.length} casos mais antigos</span>`;let z=document.createElement("ul");z.className="bau-case-list bau-accordion-content",z.style.display="none",C.forEach(ee=>{let Z=Y(ee),se=document.createElement("div");se.innerHTML=Z;let pe=se.firstElementChild;pe.addEventListener("click",Ce=>{Ce.target.closest(".bau-case-edit-btn")||D(ee)});let Ie=pe.querySelector(".bau-case-edit-btn");Ie&&(Ie.onclick=Ce=>{Ce.stopPropagation(),j(ee)}),z.appendChild(pe)}),P.addEventListener("click",()=>{let ee=z.style.display==="none";z.style.display=ee?"block":"none",P.classList.toggle("expanded",ee),P.querySelector("span").textContent=ee?"Esconder casos mais antigos":`Mostrar ${C.length} casos mais antigos`,R.playClick()}),k.appendChild(P),k.appendChild(z),m.appendChild(k)}}function I(){let l=n==="BAU"?[1,2,3,4]:[5,4];_.querySelectorAll(".bau-step").forEach(x=>{let v=parseInt(x.id.replace("bau-step-","")),A=v===a,w=l.includes(v)||v===0;x.classList.toggle("active",A),x.style.display=A?"block":"none",x.querySelectorAll("input, select, textarea").forEach(p=>{p.disabled=!w})});let m=a===0;if(L.style.display=m?"none":"flex",!m){L.innerHTML="";let x=n==="BAU"?[1,2,3,4]:[5,4];x.forEach((v,A)=>{let w=document.createElement("div"),p=v===a,C=x.indexOf(a),k=A<C;w.className=`bau-progress-step ${p?"active":k?"completed":""}`,w.textContent=A+1,L.appendChild(w)})}let y=a===4;X.style.display=a>0?"inline-block":"none",oe.style.display=!m&&!y?"inline-block":"none",q.style.display=y?"flex":"none",y&&te()}function H(l){let m=Ue.steps.find(y=>y.id===l);if(!m||!m.fields||m.isConfirmation)return!0;for(let y of m.fields){let x=_.querySelector(`#bau-step-${l} #wrapper-${y.id}`);if(!(x&&x.style.display==="none")&&y.validation){let v=_.querySelector(`#bau-step-${l} [name="${y.name}"]`);if(v&&v.offsetParent!==null&&v.value.trim())if(new RegExp(y.validation.regex).test(v.value.trim())){v.classList.remove("invalid-cid");let w=_.querySelector("#bau-cid-error");w&&(w.style.display="none")}else{console.warn(`Validation failed for field "${y.name}" in step ${l}: Regex mismatch.`),Q(`Erro: ${y.validation.error}`,{error:!0}),v.classList.add("invalid-cid");let w=_.querySelector("#bau-cid-error");return w&&(w.style.display="flex"),!1}}}return!0}function M(l){if(!_.querySelector(`#bau-step-${l}`))return!1;let y=Ue.steps.find(v=>v.id===l);if(!y||!y.fields||y.isConfirmation)return!0;let x=!0;for(let v of y.fields){let A=_.querySelector(`#bau-step-${l} #wrapper-${v.id}`);if(!(A&&A.style.display==="none")&&v.required){let w=!0,p="";if(v.type==="checkbox-grid")_.querySelector(`#bau-step-${l} input[name="${v.name}"]:checked`)||(p="No option selected in checkbox-grid",Q(`Erro: Selecione pelo menos uma op\xE7\xE3o para "${v.label}".`,{error:!0}),w=!1);else if(v.type==="datetime-group"){let C=_.querySelector(`#bau-step-${l} input[name="${v.fields[0].name}"]`);if(!C||C.offsetParent===null)continue;C.value.trim()||(p="Datetime group first field is empty",Q(`Erro: O campo "${v.fields[0].label}" \xE9 obrigat\xF3rio.`,{error:!0}),w=!1)}else{let C=_.querySelector(`#bau-step-${l} [name="${v.name}"]`);if(!C||C.offsetParent===null)continue;C.value.trim()||(p="Field is empty",Q(`Erro: O campo '${v.label}' \xE9 obrigat\xF3rio.`,{error:!0}),w=!1)}if(!w){console.warn(`Validation failed for required field "${v.name}" in step ${l}: ${p}`),x=!1;break}}}return x}oe.addEventListener("click",()=>{if(H(a)&&M(a)){n==="BAU"?a++:a===5?a=4:a++,I();let l=c.querySelector(".bau-content");l&&(l.scrollTop=0),R.playClick()}}),X.addEventListener("click",()=>{a>0&&(n==="BAU"?a--:a===4?a=5:a===5?a=0:a--,I(),R.playClick())});async function J(){let l=await ke()||{};(!l.amName||l.amName==="N/A")&&(l.amName=l.internalEmail||"N/A"),o=l,_.querySelectorAll(".bau-vital-highlights").forEach(x=>{let v=[{label:"Anunciante",value:l.advName},{label:"CID",value:l.cid},{label:"Website",value:l.website||l.site},{label:"Case ID",value:l.caseId}];x.innerHTML=v.map(A=>{let w=A.value&&A.value!=="N/A"&&A.value!=="undefined"&&A.value!=="null"?A.value:"N\xE3o capturado";return`
                    <div class="bau-highlight-item">
                        <span class="bau-highlight-label">${A.label}</span>
                        <span class="bau-highlight-value">${w}</span>
                    </div>
                `}).join("")}),Ue.steps.forEach(x=>{x.fields&&x.fields.forEach(v=>{if(v.isSmart){let A=l[v.id];v.id==="language"&&l.userProfile?.defaultLanguage&&(A=l.userProfile.defaultLanguage);let w=_.querySelector(`#bau-step-${x.id} [name="${v.name}"]`),p=_.querySelector(`#bau-step-${x.id} #wrapper-${v.id}`);if(w&&(w.value=A&&A!=="N/A"?A:"",v.id==="language"&&A&&A!=="N/A"&&(w.readOnly=!0,w.style.background="#F1F3F4",w.style.cursor="not-allowed")),p){let C=A&&A!==""&&A!=="N/A"&&A!=="undefined"&&A!=="null";v.id==="language"?p.style.display="block":p.style.display=C?"none":"block"}}})}),_.querySelectorAll(".bau-all-data").forEach(x=>{let v=[{label:"Anunciante",value:l.advName},{label:"CID",value:l.cid},{label:"AM",value:l.amName},{label:"SE ID",value:l.seId},{label:"Site",value:l.website||l.site},{label:"Email",value:l.email},{label:"Timezone",value:l.timezone},{label:"Case ID",value:l.caseId},{label:"Programa",value:l.salesProgram},{label:"Idioma",value:l.language}];x.innerHTML=`
            ...
            `})}c.querySelector("#bau-top-se-search")?.addEventListener("click",l=>{l.preventDefault(),rt("bau-form-seId")});let ae=c.querySelector("#bau-form-cid");ae&&ae.addEventListener("input",()=>H(1));function te(){let l=new FormData(_),m=Object.fromEntries(l.entries()),y=c.querySelector("#bau-confirmation-details");if(y){if(n==="BAU"){let x=l.getAll("taskType"),v=x.length>0?x.join(", "):"Nenhuma";y.innerHTML=`
            ...
            `;y.querySelectorAll(".bau-confirm-value-input").forEach(x=>{x.addEventListener("input",v=>{let A=v.target.dataset.field,w=v.target.dataset.step;if(!A||!w)return;let p=_.querySelector(`#bau-step-${w} [name="${A}"]`);p&&(p.value=v.target.value,A==="cid"&&H(1))})})}}async function j(l){if(!await ve("Aten\xE7\xE3o: Para editar as informa\xE7\xF5es, voc\xEA deve estar com a p\xE1gina deste Caso espec\xEDfico aberta no sistema. Caso contr\xE1rio, os dados capturados estar\xE3o incorretos.",{confirmText:"Estou na p\xE1gina correta"}))return;O(!0),U(),i=!0,s=l.id,n=l.status==="PENDING_TL_DISCARD"||l.reason&&!l.task?"DISCARD":"BAU",N("form"),await J(),o={...o,advName:l.advName||o.advName,cid:l.cid||o.cid,caseId:l.caseId||o.caseId,seId:l.seId||o.seId,site:l.site||l.website||o.site||o.website,email:l.advEmail||o.email,timezone:l.timezone||o.timezone,language:l.language||o.language,amName:l.amName||o.amName,salesProgram:l.salesProgram||o.salesProgram};let y=l.availability?l.availability.split("|").map(x=>x.trim()):[];_.querySelectorAll("input, select, textarea").forEach(x=>{let v=x.name,w={advEmail:"advEmail",website:"site",site:"site"}[v]||v;if(v==="taskType"){let p=(l.task||l.taskType||"").split(",").map(C=>C.trim());x.type==="checkbox"&&(x.checked=p.includes(x.value),x.closest(".bau-task-item")?.classList.toggle("active",x.checked))}else if(v.startsWith("availability_")){let p=parseInt(v.split("_")[1])-1,C=y[p];if(C&&x.type==="datetime-local")try{let k=new Date(C);if(!isNaN(k.getTime())){let P=new Date(k.getTime()-k.getTimezoneOffset()*6e4).toISOString().slice(0,16);x.value=P}}catch{}}else l[w]!==void 0?x.value=l[w]:v==="reason"?x.value=l.reason:v==="description"?x.value=l.description:v==="nonImplementationReason"&&(x.value=l.nonImplementationReason||"")}),a=n==="BAU"?1:5,I(),R.playClick(),setTimeout(()=>O(!1),500)}_.onsubmit=async l=>{l.preventDefault();let m=n==="BAU"?[1,2,3]:[5];for(let p of m)if(!Ue.steps.find(k=>k.id===p)?.isConfirmation&&(!H(p)||!M(p))){console.warn(`Form submission blocked by validation failure in step ${p}`),a=p,I();return}let y=c.querySelector(".bau-btn-submit");y.disabled=!0,y.innerHTML="Enviando...";let x=new FormData(_),v=Object.fromEntries(x.entries()),A=o||{},w={...A,...v,requestType:n};if(v.advEmail?w.advEmail=v.advEmail:A.email&&(w.advEmail=A.email),v.website?w.website=v.website:A.website?w.website=A.website:A.site&&(w.website=A.site),n==="BAU"){let p=x.getAll("taskType"),C=[v.availability_1,v.availability_2,v.availability_3].filter(k=>k&&k.trim()!=="").join(" | ");w.taskType=p.join(", "),w.availability=C,i?(v.nonImplementationReason?w.nonImplementationReason=v.nonImplementationReason:delete w.nonImplementationReason,v.description?w.description=v.description:delete w.description):(w.nonImplementationReason=v.nonImplementationReason||"",w.description=v.description||"",w.nonImplementationReason||console.warn("Aviso: Campo 'Justificativa' (nonImplementationReason) est\xE1 saindo vazio."),w.description||console.warn("Aviso: Campo 'Descri\xE7\xE3o detalhada' (description) est\xE1 saindo vazio."))}else w.reason=v.reason,i?(v.description?w.description=v.description:delete w.description,delete w.taskType,delete w.availability,delete w.nonImplementationReason):(w.taskType="",w.availability="",w.nonImplementationReason="",w.description=v.description||"");try{i?await jt(s,w):await Ht(w,A.agentEmail||"anon"),R.playSuccess();let p=c.querySelector(".bau-success-title");p&&(i?p.textContent="Caso atualizado com sucesso!":p.textContent=n==="DISCARD"?"Caso enviado para descarte com sucesso!":"Caso enviado com sucesso!"),N("success")}catch(p){Q("Erro: "+(p.message||"Erro desconhecido"),{error:!0}),console.error("Payload que tentou enviar:",w)}finally{y.disabled=!1,y.innerHTML=`${ue.send} Enviar para o TL`}};function U(){_.reset(),a=0,n="BAU",i=!1,s=null,I(),_.querySelectorAll(".bau-task-item.active").forEach(m=>m.classList.remove("active"));let l=_.querySelector('[name="language"]');l&&(l.readOnly=!1,l.style.background="",l.style.cursor="")}c.querySelector("#bau-new-case-btn").addEventListener("click",()=>{U(),N("form"),J()}),c.querySelector("#bau-form-back-btn").addEventListener("click",()=>N("dashboard")),c.querySelector("#bau-success-back-btn").addEventListener("click",()=>N("dashboard"));async function K(){t=!t,c.style.display=t?"flex":"none",t&&(N("dashboard"),B()),be(t,c,"cw-btn-bauform")}return I(),K}function fa(){if(window.techSolInitialized){Ct();return}window.techSolInitialized=!0;let t="v5.2";console.log(`\u{1F680} TechSol Suite Initializing (${t})...`);try{Wt();try{R.initGlobalListeners(),R.playStartup()}catch(d){console.warn("\xC1udio bloqueado:",d)}ce.fetchTips(),Ct();let e=yo(),o=Co(),a=Ao(),n=To(),i=_o(),s=Mo(),r=No(),c=$o(),b=Fo();uo({toggleNotes:e,toggleEmail:o,toggleScript:a,toggleLinks:n,toggleTimezone:i,toggleLibrary:s,toggleConfigs:r,toggleBAUForm:c,broadcastControl:b}),setTimeout(()=>{ce.logEvent("App","Start","Session Start"),Io(),setTimeout(()=>{qo(t)},500)},2500)}catch(e){console.error("Erro fatal na inicializa\xE7\xE3o:",e),Q("Erro cr\xEDtico ao iniciar o Case Wizard.",{error:!0})}}fa();})();
