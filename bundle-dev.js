...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
        ...
        `,document.head.appendChild(M)}let d=document.createElement("div");d.className="cw-zen-container";let c=document.createElement("div");Object.assign(c.style,{display:"none"});let p=document.createElement("div");p.className="cw-screens-container",c.appendChild(p),d.innerHTML=`
    ...
    `;let x=d.querySelector(".cw-hero-grid"),b=d.querySelector(".cw-acc-container"),g=d.querySelector(".cw-results-container"),m=d.querySelector(".cw-search-input"),w=d.querySelector(".cw-status-bar"),C=d.querySelector(".cw-status-text"),H=d.querySelector(".cw-footer-icons");s.forEach(([M,F])=>{let R=i(F.name),U=document.createElement("div");U.className="cw-hero-card",U.id=`hero-${M}`,U.style.setProperty("--hero-color",R.color),U.innerHTML=`
        ...
        `,U.onclick=_=>{if(_.target.closest(".cw-step-btn"))return;let k=a[M]?a[M].count:0;B(M,k>0?-k:1,F)},U.querySelector(".minus").onclick=()=>B(M,-1,F),U.querySelector(".plus").onclick=()=>B(M,1,F),U.dataset.color=R.color,x.appendChild(U)});function j(M,F){let R=i(F.name),U=document.createElement("div");return U.className="cw-task-item",U.dataset.id=M,U.innerHTML=`
        ...
        `,U.onclick=_=>{if(_.target.closest(".cw-step-btn"))return;let k=a[M]?a[M].count:0;B(M,k>0?-k:1,F)},U.querySelector(".minus").onclick=()=>B(M,-1,F),U.querySelector(".plus").onclick=()=>B(M,1,F),U}Object.entries(r).forEach(([M,F])=>{let R=document.createElement("div");R.className="cw-acc-group";let U=document.createElement("div");U.className="cw-acc-header",U.innerHTML=`
            <div class="cw-acc-title">
                <div class="cw-acc-dot" style="background:${F.brand.color}"></div>
                ${M}
            </div>
            <div class="cw-acc-icon">\u25BC</div>
        `,U.onclick=()=>{b.querySelectorAll(".cw-acc-group.open").forEach(k=>{k!==R&&k.classList.remove("open")}),R.classList.toggle("open")};let _=document.createElement("div");_.className="cw-acc-body",F.tasks.forEach(k=>{let E=j(k.key,k);_.appendChild(E)}),R.appendChild(U),R.appendChild(_),b.appendChild(R)});function B(M,F,R){a[M]||(a[M]={count:0,data:R,brand:i(R.name)}),a[M].count+=F,a[M].count<=0&&delete a[M],Z(),J(),t&&t()}function Z(){let M=o.tagSupportUsed;s.forEach(([k])=>{let E=x.querySelector(`#hero-${k}`);if(!E)return;let L=a[k];L?(E.classList.add("active"),E.querySelector(".cw-step-val").textContent=L.count,E.querySelector(".cw-step-val").style.color=E.dataset.color,M&&(k==="ads_conversion_tracking"||k==="ads_enhanced_conversions")&&!o.forcedScreenshots.has(k)?E.classList.add("ts-success"):E.classList.remove("ts-success")):(E.classList.remove("active"),E.classList.remove("ts-success"))}),d.querySelectorAll(".cw-task-item").forEach(k=>{let E=k.dataset.id,L=a[E];L?(k.classList.add("selected"),k.querySelector(".cw-step-val").textContent=L.count,M&&(E==="ads_conversion_tracking"||E==="ads_enhanced_conversions")&&!o.forcedScreenshots.has(E)?k.classList.add("ts-success"):k.classList.remove("ts-success")):(k.classList.remove("selected"),k.classList.remove("ts-success"))});let R=Object.keys(a),U=0,_=[];if(R.forEach(k=>{let E=a[k];U+=E.count;for(let L=0;L<E.count;L++)_.length<6&&_.push(E.brand)}),U>0){w.classList.add("visible");let k=U>1?"A\xE7\xF5es":"A\xE7\xE3o",E=U>1?"definidas":"definida";C.textContent=`${U} ${k} ${E}`,H.innerHTML="",_.forEach(L=>{let f=document.createElement("div");f.className="cw-mini-icon",f.innerHTML=ze[L.icon]||ze.default;let A=f.querySelector("svg");A&&(A.style.width="14px",A.style.height="14px"),H.appendChild(f)})}else w.classList.remove("visible")}m.addEventListener("input",M=>{let F=M.target.value.toLowerCase();if(F.length>0){b.style.display="none",g.style.display="block",g.innerHTML="";let R=!1;Object.entries(Ce).forEach(([U,_])=>{if(_.name.toLowerCase().includes(F)){R=!0;let k=j(U,_);a[U]&&(k.classList.add("selected"),k.querySelector(".cw-step-val").textContent=a[U].count),g.appendChild(k)}}),R||(g.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else b.style.display="block",g.style.display="none"});function J(){p.innerHTML="";let M=Object.keys(a),F=!1;if(M.length===0){p.innerHTML=`<div class="cw-empty-state">${e("selecione_tarefas")}</div>`,c.style.display="none";return}let R=o.tagSupportUsed,U=document.createElement("div");U.className="cw-info-banner",U.innerHTML=`
            <span style="font-size:14px">\u2139\uFE0F</span>
            <span>
                Os screenshots seguem as diretrizes atuais do Win Criteria e pol\xEDticas de Tag Support.
            </span>
        `,p.appendChild(U),M.forEach(_=>{let k=a[_].data,E=a[_].count,L=a[_].brand,A=R&&(_==="ads_conversion_tracking"||_==="ads_enhanced_conversions")&&!o.forcedScreenshots.has(_),I=o.screenshotMode||"implementation",q=k.screenshots?.[I]||[];if(q.length>0||A){F=!0;for(let D=1;D<=E;D++){let P=document.createElement("div");P.className="cw-screen-card",A&&P.classList.add("ts-success"),P.style.setProperty("--brand-color",L.color),P.style.setProperty("--brand-bg",L.bg),P.style.setProperty("--brand-shadow",L.color+"40");let Y=document.createElement("div");Y.className="cw-card-header";let y=document.createElement("div");y.className="cw-card-icon",y.innerHTML=ze[L.icon]||ze.default;let V=document.createElement("div");V.style.cssText="flex:1; display:flex; align-items:center; gap:8px;";let S=document.createElement("input");S.className="cw-card-title-input",S.id=`name-${_}-${D}`,S.value=`${k.name}${E>1?" #"+D:""}`,S.title="Clique para renomear esta task";let T=document.createElement("span");if(T.className="cw-edit-hint",T.innerHTML="\u270E Renomear",V.appendChild(S),V.appendChild(T),Y.appendChild(y),Y.appendChild(V),P.appendChild(Y),A){let u=document.createElement("div");u.className="cw-ts-disclaimer-box",u.innerHTML=`
                <span>${e("ts_disclaimer")}</span>
                <button class="cw-btn-ts-force">${e("incluir_mesmo_assim")}</button>
            `,u.querySelector("button").onclick=()=>{o.toggleForcedScreenshot(_,!0)},P.appendChild(u)}else q.forEach((u,v)=>{let h=document.createElement("div");h.className="cw-input-group";let N=document.createElement("label");N.className="cw-input-label",N.textContent=u;let O=document.createElement("input");O.className="cw-input-field",O.id=`screen-${_}-${D}-${v}`,O.placeholder="Cole o link aqui...",O.setAttribute("autocomplete","off"),O.addEventListener("input",()=>{O.value.trim().length>5?O.classList.add("filled"):O.classList.remove("filled")});let X=document.createElement("div");X.className="cw-input-check",X.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>',h.appendChild(N),h.appendChild(O),h.appendChild(X),P.appendChild(h)});p.appendChild(P)}}}),c.style.display=F?"block":"none"}return{selectionElement:d,screenshotsElement:c,updateSubStatus:()=>J(),getCheckedElements:()=>Object.keys(a).map(M=>({value:M,count:a[M].count})),setTaskCount:(M,F)=>{a[M]&&delete a[M],F>0&&Ce[M]&&B(M,F,Ce[M])},toggleTask:(M,F=!0)=>{let R=a[M];F&&!R?B(M,1,Ce[M]):!F&&R&&B(M,-R.count,Ce[M])},setLanguage:M=>{e=M;let F=d.querySelector(".js-hero-title");F&&(F.textContent=e("acesso_rapido"));let R=d.querySelector(".cw-search-input");R&&(R.placeholder=e("buscar_catalogo")),J()},reset:()=>{for(let M in a)delete a[M];m.value="",b.style.display="block",g.style.display="none",Z(),J()}}}var ea={marginTop:"24px",marginBottom:"16px",padding:"20px",background:"#fff9e6",borderRadius:"20px",border:"1.5px solid #fbbc0540",display:"none",boxShadow:"0 4px 12px rgba(251, 188, 5, 0.05)"},ta={fontSize:"12px",color:"#b06000",marginTop:"8px",lineHeight:"1.4"},oa={width:"100%",padding:"12px 16px",borderRadius:"12px",border:"1.5px solid #dadce0",fontSize:"14px",marginBottom:"16px",boxSizing:"border-box",background:"#fff"},aa={display:"flex",gap:"20px",marginBottom:"12px"};function io(t){let e=document.createElement("div");e.id="tag-support-container",Object.assign(e.style,ea);let o=document.createElement("label");o.className="js-ts-main-label",o.textContent=t("utilizou_tag_support"),Object.assign(o.style,Ct,{marginTop:"0"});let a=document.createElement("div");Object.assign(a.style,aa);let n=document.createElement("input");n.type="radio",n.name="ts_usage_mod",n.value="Sim",Object.assign(n.style,At);let i=document.createElement("label");i.textContent="Sim";let s=document.createElement("div");Object.assign(s.style,{display:"flex",alignItems:"center"}),s.appendChild(n),s.appendChild(i);let r=document.createElement("input");r.type="radio",r.name="ts_usage_mod",r.value="N\xE3o",r.checked=!0,Object.assign(r.style,At);let l=document.createElement("label");l.textContent="N\xE3o";let d=document.createElement("div");Object.assign(d.style,{display:"flex",alignItems:"center"}),d.appendChild(r),d.appendChild(l),a.appendChild(s),a.appendChild(d);let c=document.createElement("div");c.style.display="block";let p=document.createElement("label");p.className="js-ts-reason-label",p.textContent=t("motivo_ts"),Object.assign(p.style,Ct,{fontSize:"12px"});let x=document.createElement("input");x.type="text",Object.assign(x.style,oa);let b=document.createElement("div");b.className="js-ts-warning",b.innerHTML=`\u26A0\uFE0F <strong>${t("lembre_preencher_form")}</strong> <a href="https://docs.google.com/forms/d/e/1FAIpQLSeP_JM8D-6qHa5ZC93aTzj38WiO5zx8nyrWNPvbZhjJj6CpkA/viewform" target="_blank" style="color:#e37400; text-decoration:underline;">Link aqui</a>`,Object.assign(b.style,ta),c.appendChild(p),c.appendChild(x),c.appendChild(b),e.appendChild(o),e.appendChild(a),e.appendChild(c),n.onchange=()=>{c.style.display="none",Promise.resolve().then(()=>(Xe(),at)).then(H=>H.notesState.setTagSupportUsed(!0))},r.onchange=()=>{c.style.display="block",Promise.resolve().then(()=>(Xe(),at)).then(H=>H.notesState.setTagSupportUsed(!1))};function g(H,j){if(e.style.display="none",!H||!j||j.length===0)return;j.some(Z=>Z==="ads_conversion_tracking"||Z==="ads_enhanced_conversions")?e.style.display="block":(C(),Promise.resolve().then(()=>(Xe(),at)).then(Z=>Z.notesState.setTagSupportUsed(!1)))}function m(){if(e.style.display==="none")return"";let H=`<br><b>Utilizou Tag Support?</b> ${n.checked?"\u2705 Sim":"\u274C N\xE3o"}`;return r.checked&&x.value.trim()!==""&&(H+=`<br><b>Motivo:</b> ${x.value}`),H+="<br>",H}function w(H){t=H,o.textContent=t("utilizou_tag_support"),p.textContent=t("motivo_ts"),b.innerHTML=`\u26A0\uFE0F <strong>${t("lembre_preencher_form")}</strong> <a href="https://docs.google.com/forms/d/e/1FAIpQLSeP_JM8D-6qHa5ZC93aTzj38WiO5zx8nyrWNPvbZhjJj6CpkA/viewform" target="_blank" style="color:#b06000; text-decoration:underline;">Link aqui</a>`}function C(){e.style.display="none",r.checked=!0,n.checked=!1,c.style.display="block",x.value=""}return{element:e,updateVisibility:g,getOutput:m,setLanguage:w,reset:C}}var Ft="cw_notes_parking_lot",rt="cw_notes_emergency_save";var ge={getAll:()=>{try{return JSON.parse(localStorage.getItem(Ft)||"[]")}catch{return[]}},save:t=>{let e=ge.getAll(),o={id:Date.now().toString(),timestamp:new Date().toISOString(),...t};return e.unshift(o),e.length>5&&e.pop(),localStorage.setItem(Ft,JSON.stringify(e)),o},delete:t=>{let e=ge.getAll();return e=e.filter(o=>o.id!==t),localStorage.setItem(Ft,JSON.stringify(e)),e},getCount:()=>ge.getAll().length,saveEmergency:t=>{let e={timestamp:Date.now(),data:t};localStorage.setItem(rt,JSON.stringify(e))},getEmergency:()=>{try{let t=localStorage.getItem(rt);if(!t)return null;let e=JSON.parse(t);return Date.now()-e.timestamp>432e5?(localStorage.removeItem(rt),null):!e.data||!e.data.subStatus?null:e.data}catch{return null}},clearEmergency:()=>{localStorage.removeItem(rt)}};var lt=["lucaste","ricardogi"];var ie={glassBg:"rgba(61, 61, 61, 0.77)",glassBorder:"rgba(255, 255, 255, 0.15)",glassActive:"rgba(79, 79, 79, 0.89)",glassHighlight:"rgba(255, 255, 255, 0.08)",iconIdle:"#c2c5c8ff",iconActive:"#FFFFFF",blue:"#8AB4F8",red:"#F28B82",purple:"#C58AF9",green:"#81C995",orange:"#F9AB00",teal:"#00BFA5",pink:"#F48FB1",gray:"#9AA0A6"},ct=t=>new Promise(e=>setTimeout(e,t));function dt(t){let e=document.getElementById("cw-btn-notes");if(!e)return;let o=e.querySelector(".cw-dot-dirty");t?o||(o=document.createElement("div"),o.className="cw-dot-dirty",e.appendChild(o)):o&&o.remove()}function so(t){let e="cw-command-center-style";if(!document.getElementById(e)){let m=document.createElement("style");m.id=e,m.innerHTML=`
    ...
    `;let n=document.createElement("div");n.className="cw-focus-backdrop",document.body.appendChild(n),document.body.appendChild(a);let i=(m,w)=>{G.playClick(),a.querySelector(`.${m}`).classList.toggle("active"),w()};if(a.querySelector(".notes").onclick=m=>{m.stopPropagation(),i("notes",t.toggleNotes)},a.querySelector(".bauform").onclick=m=>{m.stopPropagation(),i("bauform",t.toggleBAUForm)},a.querySelector(".email").onclick=m=>{m.stopPropagation(),i("email",t.toggleEmail)},a.querySelector(".script").onclick=m=>{m.stopPropagation(),i("script",t.toggleScript)},a.querySelector(".links").onclick=m=>{m.stopPropagation(),i("links",t.toggleLinks)},a.querySelector(".library").onclick=m=>{m.stopPropagation(),i("library",t.toggleLibrary)},a.querySelector(".timezone").onclick=m=>{m.stopPropagation(),i("timezone",t.toggleTimezone)},a.querySelector(".configs").onclick=m=>{m.stopPropagation(),i("configs",t.toggleConfigs)},a.querySelector(".broadcast").onclick=m=>{m.stopPropagation(),i("broadcast",()=>{let w=m.currentTarget.querySelector(".cw-badge");w&&w.remove(),t.broadcastControl&&t.broadcastControl.toggle()})},a.querySelectorAll(".cw-btn").forEach(m=>{m.addEventListener("mouseenter",()=>G.playHover())}),t.broadcastControl&&t.broadcastControl.hasUnread){let m=document.createElement("div");m.className="cw-badge",a.querySelector(".broadcast").appendChild(m)}let s=null;a.onmouseleave=()=>{a.querySelector(".cw-btn.active")||a.classList.contains("processing-center")||(s=setTimeout(()=>{a.classList.add("collapsed")},3e3))},a.onmouseenter=()=>{s&&clearTimeout(s)},(async function(){let w=()=>{let H=he();if(H){let j=H.split("@")[0].toLowerCase();if(lt.includes(j)){let B=a.querySelector("#cw-admin-tag");B&&B.classList.add("visible")}}else setTimeout(w,2e3)};w(),await ct(2800),a.classList.add("docked"),await ct(300);let C=a.querySelectorAll(".cw-btn");a.querySelectorAll(".cw-sep").forEach(H=>H.classList.add("visible"));for(let H=0;H<C.length;H++)C[H].classList.add("popped"),await ct(90);await ct(200),a.classList.add("system-check")})();let r=!1,l,d,c,p,x=3;a.onmousedown=m=>{if(m.target.closest("button"))return;m.preventDefault(),l=m.clientX,d=m.clientY;let w=a.getBoundingClientRect();c=w.left,p=w.top,document.addEventListener("mousemove",b),document.addEventListener("mouseup",g)};function b(m){let w=m.clientX-l,C=m.clientY-d;!r&&Math.sqrt(w*w+C*C)>x&&(r=!0,a.classList.add("dragging"),a.style.transition="none",s&&clearTimeout(s)),r&&(a.style.left=`${c+w}px`,a.style.top=`${p+C}px`,a.style.right="auto",a.style.bottom="auto",a.style.transform="none")}function g(m){if(document.removeEventListener("mousemove",b),document.removeEventListener("mouseup",g),r){r=!1,a.classList.remove("dragging");let w=window.innerWidth,C=window.innerHeight,H=a.getBoundingClientRect(),j=H.left+H.width/2,B;j<w/2?(B=24,a.classList.remove("side-right"),a.classList.add("side-left")):(B=w-H.width-24,a.classList.remove("side-left"),a.classList.add("side-right"));let Z=Math.max(24,Math.min(H.top,C-H.height-24));setTimeout(()=>{a.style.setProperty("transition","left 0.6s cubic-bezier(0.34, 1.56, 0.64, 1), top 0.6s cubic-bezier(0.34, 1.56, 0.64, 1)","important"),a.style.left=`${B}px`,a.style.top=`${Z}px`,a.style.bottom="auto",a.style.transform=""},10),setTimeout(()=>{a.style.transition="",a.style.removeProperty("transition")},700)}else{let w=a.querySelector(".cw-btn.active"),C=m.target.closest("button");if(a.classList.contains("collapsed")){let H=a.getBoundingClientRect(),j=window.innerHeight,B=H.top>j/2;if(a.style.setProperty("transition","none","important"),B){let Z=j-H.bottom;a.style.top="auto",a.style.bottom=`${Z}px`}else a.style.bottom="auto",a.style.top=`${H.top}px`;a.offsetWidth,a.style.removeProperty("transition"),a.classList.remove("collapsed"),G.playGenieOpen()}else!w&&!C&&(a.classList.add("collapsed"),G.playGenieOpen());C&&(C.style.transform="scale(0.9)",setTimeout(()=>C.style.transform="",150))}}}function Je(){let t=document.querySelector(".cw-pill"),e=document.querySelector(".cw-focus-backdrop");if(!t)return()=>{};t.classList.remove("collapsed"),window._CW_ABORT_PROCESS=!1;let o=document.createElement("div");o.className="cw-center-stage",o.innerHTML=`
...
`.replace(/\n/g,"<br>");Ze(M);let F=await mt();F?(F.innerText.trim()===""&&(F.innerHTML=""),document.execCommand("insertHTML",!1,M),ut(F),K("Nota gerada e inserida!")):K("Copiado! Abra uma nota para colar.")},e}var be=t=>new Promise(e=>setTimeout(e,t));function ye(t,e="info"){let o={info:"background: #e8f0fe; color: #1a73e8; padding: 2px 5px; border-radius: 3px;",warn:"background: #fef7e0; color: #b06000; padding: 2px 5px; border-radius: 3px;",error:"background: #fce8e6; color: #c5221f; padding: 2px 5px; border-radius: 3px;",success:"background: #e6f4ea; color: #137333; padding: 2px 5px; border-radius: 3px;"};console.log(`%c[EMAIL-BOT] ${t}`,o[e]||o.info)}function ke(t){if(!t)return;let e={bubbles:!0,cancelable:!0,view:window};["mouseover","mousedown","mouseup","click"].forEach(o=>t.dispatchEvent(new MouseEvent(o,e)))}function gt(t,e){if(!t)return;let o=`cw-warning-${t.id||Math.random().toString(36).substr(2,9)}`,a=document.getElementById(o);a&&a.remove();let n=t.getBoundingClientRect(),i=document.createElement("div");i.id=o,i.style.cssText=`
    ...
    `,document.head.appendChild(t)}function ue(t,e,o){let a=document.getElementById(o);if(!e)return;let n=e.getAttribute("data-moved")==="true",i={x:0,y:0};if(a){let c=a.getBoundingClientRect();i.x=c.left+c.width/2,i.y=c.top+c.height/2}let s,r;if(!n)s=window.innerWidth/2,r=window.innerHeight/2;else{let c=e.getBoundingClientRect();s=c.left+c.width/2,r=c.top+c.height/2,s===0&&r===0&&(s=window.innerWidth/2,r=window.innerHeight/2)}let l=i.x-s,d=i.y-r;t?(G.playGenieOpen(),e.style.transition="none",e.style.opacity="0",e.style.pointerEvents="auto",n?e.style.transform=`translate(${l}px, ${d}px) scale(0.05)`:e.style.transform=`translate(calc(-50% + ${l}px), calc(-50% + ${d}px)) scale(0.05)`,e.offsetWidth,requestAnimationFrame(()=>{e.classList.add("open"),a&&a.classList.add("active"),e.style.transition="opacity 0.4s ease-out, transform 0.5s cubic-bezier(0.19, 1, 0.22, 1)",e.style.opacity="1",n?e.style.transform="translate(0, 0) scale(1)":e.style.transform="translate(-50%, -50%) scale(1)"}),typeof go=="function"&&go(e,o)):(G.playSwoosh(),e.style.transition="opacity 0.25s ease, transform 0.3s cubic-bezier(0.5, 0, 1, 1)",e.style.pointerEvents="none",requestAnimationFrame(()=>{e.style.opacity="0",n?e.style.transform=`translate(${l}px, ${d}px) scale(0.1)`:e.style.transform=`translate(calc(-50% + ${l}px), calc(-50% + ${d}px)) scale(0.1)`}),setTimeout(()=>{e.classList.remove("open"),a&&a.classList.remove("active"),e.style.transition="",e.style.transform=""},300),typeof Lt=="function"&&Lt(e))}function go(t,e){Lt(t);let o=a=>{if(!t.classList.contains("open"))return;let n=t.contains(a.target),i=document.querySelector(".cw-pill"),s=i&&i.contains(a.target);n?(t.classList.remove("idle"),t.style.zIndex="2147483648"):s||(t.classList.add("idle"),t.style.zIndex="2147483646")};t._idleHandler=o,document.addEventListener("mousedown",o)}function Lt(t){t._idleHandler&&(document.removeEventListener("mousedown",t._idleHandler),t._idleHandler=null)}function bo(){let t="v4.0.0",{popup:e,content:o,header:a,animRefs:n,credit:i}=Qt(t,J),s=io(y),r=no(()=>{k(),W.setActiveTasks(r.getCheckedElements())},y,W),l=document.createElement("div");l.style.display="none";let d=ao((u,v)=>{E(u,v)});l.appendChild(d);let c=ro({onSaveCurrent:async()=>{let u=await D();return q(),u},onLoadDraft:u=>{Y(u)},t:u=>y(u)}),p=F(),x=R(),b=document.createElement("div"),g=S(),m=L(c,y);o.appendChild(p),o.appendChild(x),o.appendChild(g),o.appendChild(l),o.appendChild(b),r.selectionElement.style.display="none",r.screenshotsElement.style.display="none";let w=document.createElement("button");w.id="manual-task-toggle",w.textContent=y("gostaria_de_adicionar_uma_task")||"Gostaria de adicionar uma task",w.style.cssText=`display: none; width: 100%; padding: 14px; border: 2px dashed ${$.primary}; background: ${$.surface}; color: ${$.primary}; border-radius: ${de.medium}; cursor: pointer; font-size: 13px; font-weight: 700; margin-top: 8px; transition: all 0.2s ${se}; text-transform: uppercase; letter-spacing: 0.5px;`,w.onmouseenter=()=>{w.style.background=$.primaryBg},w.onmouseleave=()=>{w.style.background=$.surface},w.onclick=()=>{r.selectionElement.style.display="block",r.screenshotsElement.style.display="block",w.style.display="none"},o.appendChild(w),o.appendChild(r.selectionElement),o.appendChild(s.element),o.appendChild(r.screenshotsElement),o.appendChild(m);let C=document.createElement("div");C.style.display="none",C.style.flexGrow="1",C.style.minHeight="0",C.style.overflow="hidden";let H=po(()=>M());H.style.height="100%",C.appendChild(H),e.insertBefore(C,i);let j=a.lastElementChild;j&&(j.insertBefore(c.historyBtnWrapper,j.firstChild),j.insertBefore(V(),j.firstChild)),e.appendChild(c.drawer);let B=null;W.subscribe(u=>{T(u),Z(),u.isDirty?(B&&clearTimeout(B),B=setTimeout(async()=>{let v=await D(!0);v.subStatus?ge.saveEmergency(v):ge.clearEmergency(),u.isDirty=!1},2e3)):B&&(clearTimeout(B),B=null)});function Z(){let u=ge.getCount()>0,v=!!W.currentSubStatus;dt(u||v)}function J(){W.visible=!W.visible,ue(W.visible,e,"cw-btn-notes")}function M(){W.isSplitView=!W.isSplitView,W.isSplitView?(o.style.display="none",C.style.display="flex",C.style.flexDirection="column",n.googleLine&&(n.googleLine.style.background="linear-gradient(to right, #8e24aa, #7b1fa2)")):(o.style.display="flex",C.style.display="none",n.googleLine&&(n.googleLine.style.background="linear-gradient(to right, #4285F4, #EA4335, #FBBC05, #34A853)"))}function F(){let u=document.createElement("div");if(u.innerHTML=`
        ...
        `,!document.getElementById("cw-segmented-styles")){let h=document.createElement("style");h.id="cw-segmented-styles",h.innerHTML=`
            ...
            `,document.head.appendChild(h)}let v=(h,N)=>{let X=u.querySelector(`#${h}`).querySelector(".cw-segmented-indicator");X&&(X.style.transform=`translateX(${N*100}%) translateX(${N*2}px)`)};return u.querySelectorAll("#lang-selector button").forEach((h,N)=>{h.onclick=()=>{W.setLanguage(h.dataset.lang),u.querySelectorAll("#lang-selector button").forEach(O=>O.classList.remove("active")),h.classList.add("active"),v("lang-selector",N),G.playHover(),W.currentSubStatus&&_(W.currentSubStatus)}}),u.querySelectorAll("#type-selector button").forEach((h,N)=>{h.onclick=()=>{W.setCaseType(h.dataset.type),u.querySelectorAll("#type-selector button").forEach(O=>O.classList.remove("active")),h.classList.add("active"),v("type-selector",N),G.playHover(),W.currentSubStatus&&_(W.currentSubStatus)}}),u.querySelectorAll("#portugal-selector button").forEach((h,N)=>{h.onclick=()=>{W.setPortugalCase(h.dataset.val==="true"),u.querySelectorAll("#portugal-selector button").forEach(O=>O.classList.remove("active")),h.classList.add("active"),v("portugal-selector",N),G.playHover(),W.currentSubStatus&&_(W.currentSubStatus)}}),u}function R(){let u=document.createElement("div");u.className="cw-status-section",u.style.cssText="display: flex; flex-direction: column; gap: 8px;",u.innerHTML=`
        ...
        `;let v=u.querySelector("#main-status-select"),h=u.querySelector("#sub-status-select");return v.onchange=()=>{W.setStatus(v.value),U(v.value,h),W.setSubStatus(""),_("")},h.onchange=()=>{W.setSubStatus(h.value),_(h.value)},u}function U(u,v){if(v.innerHTML=`<option value="">${y("select_substatus")}</option>`,!u){v.disabled=!0;return}for(let h in Te)if(Te[h].status===u){let N=document.createElement("option");N.value=h,N.textContent=Te[h].name,v.appendChild(N)}v.disabled=!1}function _(u){if(d.render&&d.render(u,W.currentCaseType),!u){l.style.display="none",b.style.display="none",document.getElementById("manual-task-toggle").style.display="none",r.selectionElement.style.display="none",r.screenshotsElement.style.display="none",g.style.display="flex",g.style.opacity="1",m.style.display="none";return}g.style.opacity="0",setTimeout(()=>{W.currentSubStatus&&(g.style.display="none")},400),m.style.display="grid";let v=Te[u];v&&v.templateFields&&W.setActiveFields(v.templateFields),I(),Tt(u,b,W),b.style.display="block",l.style.display="block";let h=u.startsWith("SO_"),N=u==="NI_Awaiting_Validation",O=document.getElementById("manual-task-toggle");h||N?(r.selectionElement.style.display="block",O.style.display="none"):(r.selectionElement.style.display="none",r.screenshotsElement.style.display="none",O.style.display="block");let X=u==="SO_Education_Only"?"education":"implementation";W.setScreenshotMode(X),W.currentCaseType==="lm"?W.toggleFieldExclusion("field-ON_CALL",!0):W.toggleFieldExclusion("field-ON_CALL",!1),r.updateSubStatus(u),k();let z=document.getElementById("email-automation-toggle-row");z&&(z.style.display=$e[u]?"flex":"none")}function k(){let u=r.getCheckedElements().map(v=>v.value);s.updateVisibility(W.currentSubStatus,u)}function E(u,v){let h=it[u];if(h){for(let N in h)if(N==="linkedTask")r.toggleTask(h.linkedTask,v);else if(N==="activeTasks")h.activeTasks.forEach(O=>{v?r.setTaskCount(O.value,O.count):r.setTaskCount(O.value,0)});else if(N.startsWith("field-")){let O=N,X=h[N],z=document.getElementById(O);if(z){let ee=je.includes(O.replace("field-",""));if(v)if(ee){let oe=z.value.trim();oe.includes(X.trim())||(z.value=oe?oe+`
`+X.trim():X.trim())}else z.value=X;else if(ee){let oe=z.value.trim(),Q=X.trim();oe.includes(Q)&&(z.value=oe.replace(Q,"").trim().replace(/\n{3,}/g,`

`))}else z.value.trim()===X.trim()&&(z.value="");W.updateField(O,z.value),z.dispatchEvent(new Event("input"))}}}}function L(u,v){let h=document.createElement("div");if(h.className="cw-actions-section",h.style.cssText=`
            display: grid;
        ...
        `,!document.getElementById("cw-actions-hover-styles")){let oe=document.createElement("style");oe.id="cw-actions-hover-styles",oe.innerHTML=`
            ...
            `,document.head.appendChild(oe)}let N=document.createElement("div");N.id="email-automation-toggle-row",N.style.cssText="grid-column: span 2; display: none; align-items: center; justify-content: center; padding-bottom: 6px; border-bottom: 1px solid rgba(0,0,0,0.05); margin-bottom: 2px;",N.innerHTML=`
        ...
        `;let O=u.parkButton;O.classList.add("js-btn-park"),O.style.cssText="width: 100%; margin: 0; border-radius: 10px; height: 34px; font-weight: 600; font-size: 11.5px;";let X=document.createElement("button");X.className="cw-btn-secondary js-btn-reset",X.textContent=v("limpar"),X.style.cssText=`width: 100%; height: 34px; background: ${$.surface}; color: ${$.textSub}; border: 1px solid ${$.border}; border-radius: 10px; font-weight: 600; cursor: pointer; font-size: 11.5px;`,X.onclick=()=>q();let z=document.createElement("button");z.className="cw-btn-secondary js-btn-copy",z.textContent=v("copiar"),z.style.cssText=`width: 100%; height: 34px; background: ${$.surface}; color: ${$.primary}; border: 1px solid ${$.primary}; border-radius: 10px; font-weight: 600; cursor: pointer; font-size: 11.5px;`,z.onclick=()=>f();let ee=document.createElement("button");return ee.className="cw-btn-primary js-btn-generate",ee.textContent=v("preencher"),ee.style.cssText=`width: 100%; height: 38px; background: ${$.primary}; color: #fff; border: none; border-radius: 10px; font-weight: 600; cursor: pointer; grid-column: span 2; font-size: 12.5px; box-shadow: 0 4px 10px rgba(26, 115, 232, 0.2); margin-top: 0px;`,ee.onclick=()=>A(),h.appendChild(N),h.appendChild(O),h.appendChild(X),h.appendChild(z),h.appendChild(ee),h}async function f(){if(!W.currentSubStatus){K(y("select_substatus"),{error:!0});return}let u=kt(W,r,s);u?(Ze(u),K(y("copiado_sucesso")),G.playClick()):K(y("select_substatus"),{error:!0})}async function A(){if(!W.currentSubStatus){K(y("select_substatus"),{error:!0});return}let u=kt(W,r,s);Ze(u),J();let v=Je(),h=await mt();if(h){h.focus(),document.execCommand("insertHTML",!1,u),ut(h);let N=document.getElementById("email-automation-checkbox");(!N||N.checked)&&W.currentSubStatus&&$e[W.currentSubStatus]&&await ft($e[W.currentSubStatus]),K(y("inserido_copiado")),G.playSuccess(),q()}v()}function I(){if(W.currentSubStatus){if(W.currentCaseType==="lm")W.removeField("ON_CALL");else{let u=Te[W.currentSubStatus];u&&u.templateFields.includes("ON_CALL")&&W.addFieldAt("ON_CALL",1)}W.isPortugalCase?(W.addFieldAt("CASO_PORTUGAL",1),W.addFieldAt("CONSENTIU_GRAVACAO",2)):(W.removeField("CASO_PORTUGAL"),W.removeField("CONSENTIU_GRAVACAO"))}}function q(){W.reset(),r.reset(),s.reset(),Z(),ge.clearEmergency(),o.querySelectorAll("select").forEach(v=>v.value=""),o.querySelector("#sub-status-select").disabled=!0;let u=document.getElementById("email-automation-toggle-row");u&&(u.style.display="none"),b.innerHTML="",l.style.display="none",g.style.display="flex",g.style.opacity="1",m.style.display="none",document.getElementById("manual-task-toggle")&&(document.getElementById("manual-task-toggle").style.display="none"),r.selectionElement.style.display="none",r.screenshotsElement.style.display="none"}async function D(u=!1){let v={};b.querySelectorAll("input, textarea, select").forEach(z=>{(z.id.startsWith("field-")||z.id==="consent-select")&&(v[z.id]=z.value)});let h="Cliente",N="---";if(!u)try{let z=await we();h=z.advertiserName,N=z.cid}catch(z){console.warn("Erro ao coletar pageData:",z)}let O=r.getCheckedElements().map(z=>({key:z.value,count:z.count})),X=O.map(z=>{let ee=Ce[z.key];return ee?ee.name:z.key});return{currentCaseType:W.currentCaseType,currentLang:W.currentLang,isPortugalCase:W.isPortugalCase,consent:W.consent,tagSupportUsed:W.tagSupportUsed,forcedScreenshots:[...W.forcedScreenshots],excludedFields:[...W.excludedFields],activeFields:W.activeFields,status:W.currentStatus,subStatus:W.currentSubStatus,formData:v,activeTasks:O,summaryTags:X,clientName:h,cid:N,timestamp:new Date().toISOString()}}let P=u=>new Promise(v=>setTimeout(v,u));async function Y(u){W.setLanguage(u.currentLang||"pt"),W.setCaseType(u.currentCaseType||"bau"),W.setPortugalCase(u.isPortugalCase||!1),W.setConsent(u.consent||!1),W.setExcludedFields(u.excludedFields||[]),u.activeFields&&W.setActiveFields(u.activeFields);let v=o.querySelector(`#lang-selector button[data-lang="${W.currentLang}"]`);v&&v.classList.add("active"),o.querySelectorAll("#lang-selector button").forEach(O=>{O!==v&&O.classList.remove("active")});let h=o.querySelector(`#type-selector button[data-type="${W.currentCaseType}"]`);h&&h.classList.add("active"),o.querySelectorAll("#type-selector button").forEach(O=>{O!==h&&O.classList.remove("active")});let N=o.querySelector(`#portugal-selector button[data-val="${W.isPortugalCase}"]`);if(N&&N.classList.add("active"),o.querySelectorAll("#portugal-selector button").forEach(O=>{O!==N&&O.classList.remove("active")}),u.status){let O=o.querySelector("#main-status-select");O.value=u.status,W.setStatus(u.status);let X=o.querySelector("#sub-status-select");if(U(u.status,X),await P(50),u.subStatus){if(X.value=u.subStatus,W.setSubStatus(u.subStatus),_(u.subStatus),await P(100),u.tagSupportUsed!==void 0){W.setTagSupportUsed(u.tagSupportUsed);let z=s.element.querySelector('input[value="Sim"]'),ee=s.element.querySelector('input[value="N\xE3o"]');u.tagSupportUsed&&z?z.checked=!0:ee&&(ee.checked=!0),s.element.querySelector("div:last-child").style.display=u.tagSupportUsed?"none":"block"}u.forcedScreenshots&&W.setForcedScreenshots(u.forcedScreenshots);for(let z in u.formData){let ee=document.getElementById(z);ee&&(ee.value=u.formData[z],W.updateField(z,ee.value))}u.activeTasks&&(u.activeTasks.forEach(z=>r.setTaskCount(z.key,z.count)),W.setActiveTasks(r.getCheckedElements()))}}W.isDirty=!1}function y(u){return Se[W.currentLang]?.[u]||Se.pt?.[u]||u}function V(){let u=document.createElement("div");return u.innerHTML='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="16 3 21 3 21 8"></polyline><line x1="4" y1="20" x2="21" y2="3"></line><polyline points="21 16 21 21 16 21"></polyline><line x1="15" y1="15" x2="21" y2="21"></line><line x1="4" y1="4" x2="9" y2="9"></line></svg>',u.style.cssText="width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; border-radius: 50%; cursor: pointer; color: #9AA0A6; transition: all 0.2s;",u.onclick=v=>{v.stopPropagation(),M()},u.title="Alternar para Split & Transfer",u}function S(){let u=document.createElement("div");return u.id="notes-empty-state",u.style.cssText=`
            display: flex;
            flex-direction: column;
            align-items: center;
            justify-content: center;
    ...
    `,document.head.appendChild(d),Object.assign(l.style,me,{width:"850px",height:"650px",display:"none",flexDirection:"column",fontFamily:"'-apple-system', 'BlinkMacSystemFont', 'Segoe UI', 'Roboto', 'Helvetica Neue', 'Arial', sans-serif",borderRadius:"12px",overflow:"hidden"});let c=pe(l,"Email Assistant",t,"Refatora\xE7\xE3o completa do m\xF3dulo de e-mail para uma experi\xEAncia moderna e eficiente.",{popup:l},()=>E()),p=document.createElement("div");Object.assign(p.style,{display:"flex",flex:"1",overflow:"hidden",backgroundColor:r.bgApp});let x=document.createElement("div");Object.assign(x.style,{width:"320px",backgroundColor:"#EFEFF0",borderRight:`1px solid ${r.borderSubtle}`,display:"flex",flexDirection:"column",flexShrink:"0"});let b=document.createElement("div");Object.assign(b.style,{padding:"16px",borderBottom:`1px solid ${r.borderSubtle}`,position:"relative"});let g=document.createElement("input");g.placeholder="Buscar templates...",Object.assign(g.style,{width:"100%",padding:"10px 14px 10px 36px",borderRadius:"10px",border:"1.5px solid transparent",backgroundColor:"#E3E3E8",fontSize:"15px",outline:"none",boxSizing:"border-box",color:r.textPrimary,backgroundImage:`url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="%238A8A8E" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>')`,backgroundRepeat:"no-repeat",backgroundPosition:"12px center",transition:"all 0.2s ease-in-out"}),g.onfocus=()=>{g.style.backgroundColor="#FFFFFF",g.style.borderColor=r.primary,g.style.boxShadow="0 0 0 4px rgba(0, 122, 255, 0.1)",g.style.transform="scale(1.02)"},g.onblur=()=>{g.style.backgroundColor="#E3E3E8",g.style.borderColor="transparent",g.style.boxShadow="none",g.style.transform="scale(1)"};let m=document.createElement("div");m.id="email-template-list",Object.assign(m.style,{flex:"1",overflowY:"auto",padding:"8px",scrollBehavior:"smooth"});let w=document.createElement("div");w.innerHTML="\u2715",Object.assign(w.style,{position:"absolute",right:"26px",top:"50%",transform:"translateY(-50%)",fontSize:"10px",color:"#fff",cursor:"pointer",display:"none",backgroundColor:"#C7C7CC",width:"16px",height:"16px",borderRadius:"50%",textAlign:"center",lineHeight:"16px",fontWeight:"bold"}),w.onclick=()=>{g.value="",n="",w.style.display="none",f(),g.focus()},b.appendChild(g),b.appendChild(w),x.appendChild(b),x.appendChild(m);let C=document.createElement("div");Object.assign(C.style,{flex:"1",display:"flex",flexDirection:"column",overflow:"hidden",backgroundColor:r.bgApp,transition:"all 0.3s cubic-bezier(0.25, 1, 0.5, 1)"});let H=document.createElement("div");Object.assign(H.style,{padding:"20px",borderBottom:`1px solid ${r.borderSubtle}`,backgroundColor:r.bgSurface,maxHeight:"250px",overflowY:"auto",display:"none"});let j=document.createElement("div");Object.assign(j.style,{flex:"1",display:"flex",flexDirection:"column",padding:"20px",backgroundColor:r.bgApp,overflow:"hidden"});let B=document.createElement("div");Object.assign(B.style,{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px"});let Z=document.createElement("span");Z.textContent="Preview do E-mail",Object.assign(Z.style,{fontSize:"12px",fontWeight:"600",color:r.textSecondary,textTransform:"uppercase",letterSpacing:"0.5px"});let J=document.createElement("div");Object.assign(J.style,{display:"flex",gap:"8px"});let M=(P,Y=!1)=>{let y=document.createElement("button");return y.textContent=P,Object.assign(y.style,{padding:"8px 14px",borderRadius:"10px",border:Y?"none":`1.5px solid ${r.primary}`,background:Y?r.primary:"transparent",color:Y?"#fff":r.primary,fontSize:"13px",fontWeight:"600",cursor:"pointer",transition:"all 0.2s cubic-bezier(0.25, 1, 0.5, 1)",boxShadow:Y?"0 4px 12px rgba(0, 122, 255, 0.3)":"none"}),y.onmouseenter=()=>{Y?(y.style.backgroundColor="#0062CC",y.style.transform="translateY(-1px)",y.style.boxShadow="0 6px 16px rgba(0, 122, 255, 0.4)"):y.style.backgroundColor="rgba(0, 122, 255, 0.05)"},y.onmouseleave=()=>{Y?(y.style.backgroundColor=r.primary,y.style.transform="translateY(0)",y.style.boxShadow="0 4px 12px rgba(0, 122, 255, 0.3)"):(y.style.backgroundColor="transparent",y.style.transform="translateY(0)")},y.onmousedown=()=>y.style.transform="scale(0.94)",y.onmouseup=()=>y.style.transform="scale(1)",y},F=M("Copiar HTML"),R=M("Preencher no CRM",!0),U=M("Smart CR");U.style.borderColor="#E67E22",U.style.color="#E67E22",U.style.display="none",J.appendChild(U),J.appendChild(F),J.appendChild(R),B.appendChild(Z),B.appendChild(J);let _=document.createElement("div");_.contentEditable="true",Object.assign(_.style,{flex:"1",backgroundColor:r.bgSurface,border:`1px solid ${r.borderSubtle}`,borderRadius:"8px",padding:"20px",fontSize:"15px",lineHeight:"1.6",color:r.textPrimary,overflowY:"auto",outline:"none",boxShadow:"inset 0 1px 2px rgba(0,0,0,0.02)"}),j.appendChild(B),j.appendChild(_),D(),C.appendChild(H),C.appendChild(j),p.appendChild(x),p.appendChild(C),l.appendChild(c),l.appendChild(p);let k=document.createElement("div");Object.assign(k.style,Ee),l.appendChild(k),Ae(l,k),document.body.appendChild(l);function E(){e=!e,e?(l.style.display="flex",Wt(l),o.length===0&&L()):l.style.display="none",ue(e,l,"cw-btn-email")}async function L(){m.innerHTML='<div style="padding: 20px; text-align: center; color: #5f6368;">Carregando...</div>',o=await xo.getTemplates(),f()}function f(){m.innerHTML="";let P=o.filter(T=>T.name.toLowerCase().includes(n.toLowerCase())||T.category.toLowerCase().includes(n.toLowerCase())),Y=Object.entries($e).filter(([T,u])=>u&&(T.toLowerCase().includes(n.toLowerCase())||u.toLowerCase().includes(n.toLowerCase()))).map(([T,u])=>({id:T,name:T.replace(/_/g," "),category:"\u26A1 Smart CRs",code:u,isSmartCR:!0})),y=xe.getSnippets("email").filter(T=>T.title.toLowerCase().includes(n.toLowerCase())||T.subject&&T.subject.toLowerCase().includes(n.toLowerCase())).map(T=>{let u=[],v=T.content.match(/\[([^\]]+)\]/g);return v&&[...new Set(v)].forEach(h=>{u.push({key:h,label:h.replace("[","").replace("]",""),type:h.toLowerCase().includes("data")?"date":"text",auto:h.toLowerCase().includes("nome")&&h.toLowerCase().includes("seu")?"agentName":null})}),{id:T.id||`snippet-${Math.random()}`,name:T.title,category:"\u{1F464} Pessoal",subject:T.subject||"Sem Assunto",template:T.content,placeholders:u}}),V=[...P,...Y,...y];if(V.length===0){m.innerHTML=`
                ...
                </div>`;return}[...new Set(V.map(T=>T.category))].sort((T,u)=>T.localeCompare(u)).forEach(T=>{let u=s.has(T)||n.length>0,v=V.filter(ee=>ee.category===T),h=document.createElement("div");Object.assign(h.style,{padding:"12px 16px 12px 24px",fontSize:"11px",fontWeight:"700",color:r.textSecondary,textTransform:"uppercase",letterSpacing:"0.8px",position:"sticky",top:"-8px",backgroundColor:"rgba(239, 239, 240, 0.9)",zIndex:"10",backdropFilter:"blur(20px)",margin:"0 -8px 8px -8px",borderBottom:`0.5px solid ${r.borderSubtle}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between",userSelect:"none",transition:"background-color 0.2s ease"}),h.onmouseenter=()=>h.style.backgroundColor="rgba(230, 230, 232, 0.9)",h.onmouseleave=()=>h.style.backgroundColor="rgba(239, 239, 240, 0.9)";let N=document.createElement("span");N.textContent=T,h.appendChild(N);let O=document.createElement("span");O.textContent=v.length,Object.assign(O.style,{backgroundColor:"rgba(0, 0, 0, 0.05)",padding:"2px 8px",borderRadius:"10px",fontSize:"10px",color:r.textSecondary});let X=document.createElement("span");X.innerHTML=u?"\u{10012A}":"\u{10012B}",X.innerHTML=u?"\u25BE":"\u25B8",X.style.marginLeft="8px",X.style.transition="transform 0.3s ease";let z=document.createElement("div");z.style.display="flex",z.style.alignItems="center",z.appendChild(O),z.appendChild(X),h.appendChild(z),h.onclick=()=>{s.has(T)?s.delete(T):s.add(T),f()},m.appendChild(h),u&&v.forEach(ee=>{let oe=a&&a.id===ee.id,Q=document.createElement("div");if(Object.assign(Q.style,{padding:"12px 14px",fontSize:"14px",cursor:"pointer",transition:"all 0.3s cubic-bezier(0.25, 1, 0.5, 1)",borderRadius:"10px",color:r.textPrimary,margin:"4px 6px",display:"flex",alignItems:"center",gap:"12px",backgroundColor:oe?r.primary:r.bgSurface,boxShadow:oe?"0 4px 12px rgba(0, 122, 255, 0.3)":"0 1px 2px rgba(0,0,0,0.05)",border:oe?"none":`1px solid ${r.borderSubtle}`,position:"relative",overflow:"hidden"}),oe){let re=document.createElement("div");Object.assign(re.style,{position:"absolute",left:"0",top:"0",bottom:"0",width:"4px",backgroundColor:"#fff",borderRadius:"0 4px 4px 0"}),Q.appendChild(re)}let ne=document.createElement("span");ne.innerHTML=ee.isSmartCR?"\u26A1":ee.category==="\u{1F464} Pessoal"?"\u{1F464}":"\u{1F4C4}",ne.style.fontSize="12px",ne.style.opacity="0.7",ne.style.flexShrink="0",Q.appendChild(ne);let te=document.createElement("span");te.textContent=ee.name,te.style.overflow="hidden",te.style.textOverflow="ellipsis",te.style.whiteSpace="nowrap",te.style.flex="1",Q.appendChild(te),oe&&(Q.style.color="#fff",Q.style.fontWeight="600",ne.style.opacity="1"),Q.onmouseenter=()=>{oe||(Q.style.backgroundColor="#f8f8f9",Q.style.transform="translateY(-1px) scale(1.01)",Q.style.boxShadow="0 4px 8px rgba(0,0,0,0.08)",Q.style.borderColor="rgba(0, 122, 255, 0.2)")},Q.onmouseleave=()=>{oe||(Q.style.backgroundColor=r.bgSurface,Q.style.transform="translateY(0) scale(1)",Q.style.boxShadow="0 1px 2px rgba(0,0,0,0.05)",Q.style.borderColor=r.borderSubtle)},Q.onmousedown=()=>{Q.style.transform=oe?"scale(0.97)":"scale(0.98)"},Q.onmouseup=()=>{Q.style.transform=oe?"scale(1)":"translateY(-1px) scale(1.01)"},Q.onclick=()=>{I(ee)},m.appendChild(Q)})})}let A=null;async function I(P){a?.id!==P.id&&(a=P,A&&clearTimeout(A),C.style.opacity="0",C.style.transform="translateY(5px)",A=setTimeout(()=>{U.style.display=P.isSmartCR?"block":"none",R.style.display=P.isSmartCR?"none":"block",F.style.display=P.isSmartCR?"none":"block",f(),q(),D(),C.style.opacity="1",C.style.transform="translateY(0)",A=null},150))}function q(){if(H.innerHTML="",!a||a.isSmartCR){a?.isSmartCR?(H.style.display="block",H.innerHTML=`<div style="padding: 12px; font-size: 13px; color: #856404; background: #FFF3CD; border: 1px solid #FFEEBA; border-radius: 8px; display: flex; align-items: center; gap: 8px;">
                ...
                </div>`):H.style.display="none";return}let P=a.placeholders&&a.placeholders.length>0;if(H.style.display=P?"block":"none",!P)return;let Y=document.createElement("div");Object.assign(Y.style,{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"16px"}),(a.placeholders||[]).forEach(y=>{let V=document.createElement("div"),S=document.createElement("label");S.textContent=y.label,Object.assign(S.style,{display:"block",fontSize:"11px",fontWeight:"700",color:r.textSecondary,marginBottom:"8px",textTransform:"uppercase",letterSpacing:"0.5px"});let T=document.createElement("input");T.type=y.type||"text",T.dataset.key=y.key,Object.assign(T.style,{width:"100%",padding:"10px 12px",borderRadius:"8px",border:`1.5px solid ${r.borderSubtle}`,backgroundColor:"#FBFBFD",fontSize:"14px",boxSizing:"border-box",transition:"all 0.2s ease",outline:"none"}),T.onfocus=()=>{T.style.borderColor=r.primary,T.style.backgroundColor="#FFFFFF",T.style.boxShadow="0 0 0 4px rgba(0, 122, 255, 0.1)"},T.onblur=()=>{T.style.borderColor=r.borderSubtle,T.style.backgroundColor="#FBFBFD",T.style.boxShadow="none"},y.auto==="agentName"&&(T.value=Ye().split(" ")[0]),T.addEventListener("input",D),V.appendChild(S),V.appendChild(T),Y.appendChild(V)}),H.appendChild(Y)}function D(){if(!a){_.innerHTML=`
...
CID: ${v.cid||"---"}
Case ID: ${v.caseId||"---"}
E-mail: ${v.clientEmail||"---"}`;N&&(N.value=X),O&&(O.style.display="block",O.style.maxHeight="300px",O.style.opacity="1",O.scrollIntoView({behavior:"smooth",block:"end"}))})}function b(){r=!r,ue(r,l,"cw-btn-script"),r?(p(),c||(c=setInterval(p,2e3))):c&&(clearInterval(c),c=null)}let g=pe(l,"Call Script",t,"Guia interativo para condu\xE7\xE3o de chamadas.",d,()=>{b()});l.appendChild(g);let m=document.createElement("div");Object.assign(m.style,a.contextBanner),m.innerHTML=`
  ...
  `;let w=m.querySelector("#csa-toggle-options"),C=m.querySelector("#csa-options-content"),H=m.querySelector("#csa-options-arrow"),j=!1;w.onclick=()=>{j=!j,H.style.transform=j?"rotate(180deg)":"rotate(0deg)",C.style.maxHeight=j?"400px":"0",C.style.opacity=j?"1":"0",C.style.marginTop=j?"8px":"0",G.playClick()};let B=m.querySelector("#cw-pill-message"),Z=m.querySelector("#cw-am-copy-final"),J=m.querySelector("#cw-am-message-area");B.onmouseenter=()=>{B.style.borderColor="#007AFF",B.style.boxShadow="0 2px 8px rgba(0,0,0,0.05)"},B.onmouseleave=()=>{B.style.borderColor="#DADCE0",B.style.boxShadow="0 1px 2px rgba(0,0,0,0.02)"},B.addEventListener("click",()=>{x()}),Z.addEventListener("click",()=>{J.value&&(navigator.clipboard.writeText(J.value),K("Mensagem copiada!"),G.playSuccess(),Z.style.background="#34A853",Z.textContent="Copiado!",setTimeout(()=>{Z.style.background="#1A73E8",Z.textContent="Copiar Mensagem Final"},2e3))});let M=(v,h)=>{let N=m.querySelector(v),O=m.querySelector(h);N.onclick=()=>{let X=O.textContent;!X||X.includes("---")||X.includes("N\xE3o encontrado")||(navigator.clipboard.writeText(X),G.playSuccess(),N.classList.add("copied"),setTimeout(()=>N.classList.remove("copied"),1500))}};l.appendChild(m);let F=document.createElement("div");Object.assign(F.style,a.progressBarContainer);let R=document.createElement("div");R.className="csa-progress-fill",Object.assign(R.style,a.progressBarFill),F.appendChild(R),l.appendChild(F);let U=document.createElement("div");U.id="csa-content",Object.assign(U.style,a.contentArea),l.appendChild(U);let _=document.createElement("div");Object.assign(_.style,a.footer);let k=document.createElement("span");k.textContent="by lucaste@",Object.assign(k.style,{fontSize:"10px",color:"#bdc1c6"});let E=document.createElement("button");E.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"></path><path d="M3 3v5h5"></path></svg> Resetar Script',Object.assign(E.style,a.resetBtn),E.onmouseenter=()=>E.style.background="#fce8e6",E.onmouseleave=()=>E.style.background="transparent",E.onclick=()=>{E.style.transform="scale(0.9)",setTimeout(()=>E.style.transform="scale(1)",150);for(let v in n)delete n[v];S()},_.appendChild(k),_.appendChild(E),l.appendChild(_);let L=document.createElement("div");Object.assign(L.style,{display:"flex",flexDirection:"column",gap:"8px",marginBottom:"16px"});let f=document.createElement("div");f.className="csa-segmented-control",f.innerHTML=`
  ...
  `,m.onfocus=()=>{b.style.boxShadow="0 4px 12px rgba(26,115,232,0.15)",b.style.border="1px solid #1a73e8"},m.onblur=()=>{b.style.boxShadow="0 2px 6px rgba(0,0,0,0.04)",b.style.border="1px solid transparent"},b.appendChild(g),b.appendChild(m),x.appendChild(b),p.appendChild(x);let w=document.createElement("div");w.style.cssText="flex: 1; overflow-y: auto; padding: 0 24px 40px 24px; scroll-behavior: smooth;",p.appendChild(w);let C=null;function H(){if(C)return;C=document.createElement("div"),C.style.cssText=`
      ...
      `,f.title=E,f.dataset.key=k;let A=document.createElement("div");A.style.cssText="width: 24px; height: 24px; margin-bottom: 2px; transition: transform 0.2s;",A.innerHTML=L||Be.tasks;let I=document.createElement("div");return I.style.cssText="font-size: 9px; font-weight: 600; opacity: 0.7; letter-spacing: 0.3px;",I.textContent=E,f.appendChild(A),f.appendChild(I),f.onmouseenter=()=>{a!==k&&!(k==="history"&&n)&&(f.style.background="#F1F3F4",A.style.transform="scale(1.1)")},f.onmouseleave=()=>{a!==k&&!(k==="history"&&n)&&(f.style.background="transparent",A.style.transform="scale(1)")},f}function M(k){let E=document.getElementById(`cat-anchor-${k}`);E&&(E.scrollIntoView({behavior:"smooth",block:"start"}),a=k,F())}function F(){Object.keys(Qe).forEach(E=>{let L=c.querySelector(`#cw-sidebar-btn-${E}`);if(L)if(a===E&&!n){let f=xt[E];L.style.background=f.bg,L.style.color=f.color,L.querySelector("div:first-child").style.transform="scale(1.1)"}else L.style.background="transparent",L.style.color=i.textSecondary,L.querySelector("div:first-child").style.transform="scale(1)"});let k=c.querySelector("#cw-sidebar-btn-history");k&&(n?(k.style.background="#3C4043",k.style.color="#FFFFFF"):(k.style.background="transparent",k.style.color=i.textSecondary))}function R(){if(w.innerHTML="",e.trim()!==""){let E=[];if(Object.entries(Qe).forEach(([f,A])=>{let I=A.links.filter(q=>q.name.toLowerCase().includes(e.toLowerCase())||q.desc.toLowerCase().includes(e.toLowerCase()));E.push(...I.map(q=>({...q,_cat:f})))}),E.length===0){w.innerHTML='<div style="text-align:center; padding: 60px; color:#999; font-size:13px;">Nada encontrado.</div>';return}let L=document.createElement("div");L.innerHTML="Resultados da busca",L.style.cssText="font-size:12px; font-weight:700; color:#5f6368; margin:20px 0 10px; text-transform:uppercase; letter-spacing:0.5px;",w.appendChild(L),E.forEach(f=>{let A=U(f,Be[f._cat],!1,f._cat);w.appendChild(A)});return}Object.entries(Qe).forEach(([E,L])=>{let f=xt[E],A=document.createElement("div"),I=document.createElement("div");I.id=`cat-anchor-${E}`,I.style.cssText=`
          ...
          `,I.innerHTML=`
            ...
            ${L.label}
          `,A.appendChild(I);let q=document.createElement("div");q.style.cssText="display: grid; grid-template-columns: 1fr; gap: 8px;",L.links.forEach(D=>{let P=U(D,Be[E],!1,E);q.appendChild(P)}),A.appendChild(q),w.appendChild(A)});let k=document.createElement("div");k.style.height="80px",w.appendChild(k)}function U(k,E,L,f){let A=document.createElement("div"),I=xt[f]||xt.history;A.style.cssText=`
      ...
      `;let q=document.createElement("div");q.style.cssText=`
          ...
          display: flex; align-items: center; justify-content: center;
          flex-shrink: 0; transition: all 0.2s;
      `,q.innerHTML=E||Be.tasks;let D=q.querySelector("svg");D&&(D.style.width="22px",D.style.height="22px");let P=document.createElement("div");P.style.cssText="flex: 1; display: flex; flex-direction: column; gap: 2px; overflow: hidden;";let Y=document.createElement("div");Y.style.cssText=`font-size: 14px; font-weight: 600; color: ${i.textPrimary}; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;`,Y.textContent=k.name;let y=document.createElement("div");y.style.cssText=`font-size: 12px; color: ${i.textSecondary}; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;`,y.textContent=k.desc,P.appendChild(Y),P.appendChild(y);let V=document.createElement("div");return V.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>',V.style.cssText=`
          width: 32px; height: 32px; border-radius: 8px;
          display: flex; align-items: center; justify-content: center;
          color: #9AA0A6; transition: all 0.2s; opacity: 0;
      `,V.title="Copiar URL",A.onmouseenter=()=>{A.style.transform="translateY(-2px)",A.style.boxShadow="0 8px 20px rgba(0,0,0,0.08)",A.style.borderColor="rgba(0,0,0,0.05)",A.style.borderLeft=`4px solid ${I.color}`,V.style.opacity="1",V.style.background="#F1F3F4"},A.onmouseleave=()=>{A.style.transform="translateY(0)",A.style.boxShadow="0 1px 3px rgba(0,0,0,0.05)",A.style.border="1px solid transparent",V.style.opacity="0",V.style.background="transparent"},A.onclick=()=>{!L&&f&&So(k,f),window.open(k.url,"_blank")},V.onclick=S=>{S.stopPropagation(),G.playClick(),navigator.clipboard.writeText(k.url),!L&&f&&So(k,f),K("Link copiado!")},A.appendChild(q),A.appendChild(P),A.appendChild(V),A}m.addEventListener("input",k=>{e=k.target.value,R()});function _(){o=!o,ue(o,s,"cw-btn-links")}return document.body.appendChild(s),Z(),R(),_}var qe=[];function qt(t){qe=t}var ia=60*1e3;window._cwIsAdmin=!1;window._cwCurrentUser=null;function Eo(){let t="v4.9",e=!1,o=null,a=null;function n(f){if(!f)return"";try{let A=new Date(f);return isNaN(A.getTime())?String(f):A.toLocaleString("pt-BR",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}).replace(","," \xE0s")}catch{return String(f)}}if(!document.getElementById("cw-broadcast-hd-css")){let f=document.createElement("style");f.id="cw-broadcast-hd-css",f.innerHTML=`
        @keyframes cw-pulse {
      ...
      `,g.querySelectorAll('input[name="cw-bc-type"]').forEach(q=>{q.addEventListener("change",()=>{g.querySelectorAll(".cw-radio-option").forEach(D=>D.classList.remove("checked")),q.parentElement.classList.add("checked")})}),setTimeout(()=>{let q=g.querySelector(".cw-radio-option.info");q&&q.classList.add("checked")},100);let f=g.querySelector("#cw-bc-cancel"),A=g.querySelector("#cw-bc-close-x"),I=g.querySelector("#cw-bc-send");f.onclick=j,A.onclick=j,I.onclick=B,d.appendChild(g)}function H(f=null){if(!g)return;let A=g.querySelector("#cw-editor-title-label"),I=g.querySelector("#cw-bc-title"),q=g.querySelector("#cw-bc-text"),D=g.querySelector("#cw-bc-send");if(f){a=f.id,A.textContent="Editar Aviso",I.value=f.title||"",q.value=f.text||"",D.textContent="Salvar Altera\xE7\xF5es";let P=f.type||"info",Y=g.querySelector(`input[name="cw-bc-type"][value="${P}"]`);Y&&Y.click()}else{a=null,A.textContent="Novo Aviso",I.value="",q.value="",D.textContent="Publicar";let P=g.querySelector('input[name="cw-bc-type"][value="info"]');P&&P.click()}g.classList.add("active"),setTimeout(()=>I.focus(),300)}function j(){g&&g.classList.remove("active"),a=null}async function B(){let f=g.querySelector("#cw-bc-send"),A=g.querySelector("#cw-bc-title"),I=g.querySelector("#cw-bc-text"),q=g.querySelector('input[name="cw-bc-type"]:checked'),D=q?q.value:"info";if(!A.value.trim()||!I.value.trim()){K("Preencha todos os campos!",{error:!0});return}f.textContent="Salvando...",f.style.opacity="0.7";let P=!1;a?P=await le.updateBroadcast(a,{title:A.value,text:I.value,type:D}):P=await le.sendBroadcast({title:A.value,text:I.value,type:D,author:window._cwCurrentUser||"admin"}),P?(K(a?"Atualizado!":"Publicado!"),G.playSuccess(),j(),setTimeout(()=>M(),1500)):(K("Erro ao salvar. Verifique a conex\xE3o.",{error:!0}),f.textContent=a?"Salvar Altera\xE7\xF5es":"Publicar",f.style.opacity="1")}async function Z(f){if(await fe("Confirma a exclus\xE3o deste aviso?",{danger:!0}))if(await le.deleteBroadcast(f)){K("Aviso removido."),G.playClick();let q=qe.findIndex(D=>D.id===f);q>-1&&qe.splice(q,1),R(),setTimeout(()=>M(),1500)}else K("Erro ao excluir.",{error:!0})}let J=document.createElement("div");J.className="cw-nice-scroll",Object.assign(J.style,i.feedContainer),d.appendChild(J);async function M(){e&&(w.style.display="block",w.innerHTML="\u{1F504} Sincronizando...");try{let f=await le.fetchData();f&&f.broadcast&&(qt(f.broadcast),F(),e&&(R(),w.innerHTML='<span style="color:#137333">\u2713 Atualizado</span>',setTimeout(()=>{w.style.display="none"},1500)))}catch{e&&(w.innerHTML="\u26A0\uFE0F Offline")}}function F(){let f=document.getElementById("cw-btn-broadcast");if(!f)return;let A=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]");if(qe.some(q=>!A.includes(q.id))){if(f.classList.add("has-new"),!f.querySelector(".cw-badge")){let q=document.createElement("div");q.className="cw-badge",Object.assign(q.style,{position:"absolute",top:"8px",right:"8px",width:"8px",height:"8px",backgroundColor:"#d93025",borderRadius:"50%",border:"1px solid #fff",zIndex:"10"}),f.appendChild(q)}}else{f.classList.remove("has-new");let q=f.querySelector(".cw-badge");q&&q.remove()}}function R(){J.innerHTML="";let f=d.querySelector("#cw-bau-widget");f&&f.remove();let A=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]"),I=[...qe].sort((y,V)=>{let S=new Date(y.date).getTime()||0;return(new Date(V.date).getTime()||0)-S}),q=I.findIndex(y=>y.title&&y.title.toLowerCase().includes("disponibilidade bau"));if(q!==-1){let y=I[q];I.splice(q,1);let V=document.createElement("div");V.id="cw-bau-widget",Object.assign(V.style,i.bauContainer);let S=[],T=(y.text||"").split(`
`),u=/\d{1,2}\/\d{1,2}/,v="\u{1F4C5}";if(T.forEach(z=>{/🇧🇷|🇵🇹|PT|BR|BRASIL|BRAZIL|PORTUGAL|LISBOA/i.test(z)?v="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM|ESPANHA|SPAIN|MEXICO|MÉXICO/i.test(z)&&(v="\u{1F1EA}\u{1F1F8}");let ee=z.match(u);if(ee){let oe=ee[0],Q=v;/🇧🇷|🇵🇹|PT|BR/i.test(z)?Q="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM/i.test(z)&&(Q="\u{1F1EA}\u{1F1F8}"),S.some(te=>te.flag===Q&&te.date===oe)||S.push({flag:Q,date:oe})}}),S.length===0){let z=(y.text||"").match(/\d{1,2}\/\d{1,2}/g);z&&[...new Set(z)].forEach(ee=>S.push({flag:"\u{1F4C5}",date:ee}))}let h="",N='<button id="cw-bau-toggle-btn" class="cw-btn-interactive" style="background:rgba(255,255,255,0.7); border:1px solid rgba(139, 92, 246, 0.4); border-radius:12px; padding:8px 12px; color:#6D28D9; font-size:12px; font-weight:600;">Detalhes</button>';window._cwIsAdmin&&(N=`
              ...
              `),S.length>0?h=`
                  <div style="display:flex; justify-content:space-between; align-items:center;">
                      <div style="flex:1; display:flex; gap:8px;">${S.map(ee=>`
          ...
          `,w.after(V);let O=V.querySelector("#cw-bau-toggle-btn"),X=V.querySelector("#cw-bau-full");if(O&&X&&(O.onclick=()=>{let z=X.style.display==="none";X.style.display=z?"block":"none",O.textContent=z?"Ocultar":"Detalhes"}),window._cwIsAdmin){let z=V.querySelector(".cw-bau-edit");z&&(z.onclick=()=>H(y))}}let D=I.sort((y,V)=>{let S=A.includes(y.id),T=A.includes(V.id);return S===T?0:S?1:-1});if(D.length===0&&!q){let y=document.createElement("div");Object.assign(y.style,i.emptyState),y.innerHTML=`
           ...
           `,J.appendChild(y)}let P=D.filter(y=>!A.includes(y.id)),Y=D.filter(y=>A.includes(y.id));if(P.forEach(y=>J.appendChild(U(y,!1))),Y.length>0){let y=document.createElement("div");Object.assign(y.style,i.historyDivider),y.innerHTML=`<span>Hist\xF3rico (${Y.length})</span><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>`;let V=document.createElement("div");Object.assign(V.style,i.historyContainer),Y.forEach(T=>V.appendChild(U(T,!0)));let S=!1;y.onclick=()=>{G.playClick(),S=!S,V.style.display=S?"flex":"none",y.querySelector("svg").style.transform=S?"rotate(180deg)":"rotate(0deg)"},J.appendChild(y),J.appendChild(V)}}function U(f,A){let I=document.createElement("div");Object.assign(I.style,A?i.cardHistory:i.card);let q=s[f.type]||s.info,D=document.createElement("div");Object.assign(D.style,i.cardHeader);let P=document.createElement("div");Object.assign(P.style,i.typeTag,{color:q.color,background:q.bg}),P.innerHTML=`${q.icon} <span>${f.type}</span>`;let Y=document.createElement("span");if(Object.assign(Y.style,i.dateTag),Y.textContent=n(f.date),D.appendChild(P),A)D.appendChild(Y);else{let u=document.createElement("button");u.className="cw-btn-interactive",Object.assign(u.style,i.dismissBtn),u.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"></polyline></svg>',u.onmouseenter=()=>{u.style.color="#1e8e3e",u.style.background="#e6f4ea",u.style.borderColor="#1e8e3e"},u.onmouseleave=()=>{u.style.color="#5f6368",u.style.background="#fff",u.style.borderColor="rgba(0,0,0,0.1)"},u.onclick=v=>{v.stopPropagation(),G.playClick(),I.style.transform="translateX(20px)",I.style.opacity="0",setTimeout(()=>{let h=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]");h.push(f.id),localStorage.setItem("cw_read_broadcasts",JSON.stringify(h)),R(),F()},200)},D.appendChild(u)}let y=document.createElement("div");Object.assign(y.style,i.cardContent);let V=document.createElement("div");Object.assign(V.style,i.msgTitle),V.textContent=f.title;let S=document.createElement("div");Object.assign(S.style,i.msgBody),S.innerHTML=l(f.text);let T=document.createElement("div");if(Object.assign(T.style,i.msgMeta),T.innerHTML=`Publicado por <b>${f.author||"Sistema"}</b>`,A||(T.innerHTML+=` \u2022 ${n(f.date)}`),y.appendChild(V),y.appendChild(S),y.appendChild(T),I.appendChild(D),I.appendChild(y),window._cwIsAdmin){let u=document.createElement("div");u.className="cw-card-actions";let v=document.createElement("button");v.className="cw-action-btn edit",v.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg> Editar',v.onclick=()=>H(f);let h=document.createElement("button");h.className="cw-action-btn delete",h.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg> Excluir',h.onclick=()=>Z(f.id),u.appendChild(v),u.appendChild(h),I.appendChild(u)}return I}let _=le.getCachedBroadcasts();_.length>0&&(qt(_),R()),setTimeout(m,500),M(),o||(o=setInterval(M,ia));let k=document.createElement("div");Object.assign(k.style,Ee),k.className="no-drag",d.appendChild(k),Ae(d,k),document.body.appendChild(d);let E=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]"),L=qe.some(f=>!E.includes(f.id));return{toggle:p,hasUnread:L}}function Ao(){if(localStorage.getItem("cw_onboarding_seen_v1"))return;let t=[{icon:"\u{1F680}",title:"Bem-vindo ao TechSol Suite",text:"Sua nova central de opera\xE7\xF5es para maximizar produtividade e padroniza\xE7\xE3o no CRM."},{icon:"\u{1F4DD}",title:"Notas Autom\xE1ticas",text:"Gere notas de caso (BAU/LM) perfeitas em segundos. Selecione o Status, as Tasks e deixe o wizard escrever o texto t\xE9cnico para voc\xEA."},{icon:"\u26A1",title:"Quick Email & Scripts",text:"Responda e-mails com templates inteligentes que detectam o contexto e use scripts de chamada interativos que guiam seu atendimento."},{icon:"\u{1F4E2}",title:"Fique Informado",text:"O m\xF3dulo Broadcast traz avisos importantes e disponibilidade BAU direto na sua tela, sem precisar abrir planilhas externas."},{icon:"\u2705",title:"Tudo Pronto!",text:"Explore o Menu Flutuante para come\xE7ar. Bom trabalho!",isLast:!0}],e=0,o={overlay:{position:"fixed",top:0,left:0,width:"100vw",height:"100vh",backgroundColor:"rgba(0,0,0,0.6)",backdropFilter:"blur(4px)",zIndex:"2147483647",display:"flex",alignItems:"center",justifyContent:"center",opacity:"0",transition:"opacity 0.3s ease"},card:{width:"380px",background:"#fff",borderRadius:"24px",padding:"32px",textAlign:"center",position:"relative",boxShadow:"0 20px 50px rgba(0,0,0,0.3)",fontFamily:"'Google Sans', Roboto, sans-serif",transform:"translateY(20px)",transition:"all 0.4s cubic-bezier(0.19, 1, 0.22, 1)"},icon:{fontSize:"48px",marginBottom:"20px",display:"block"},title:{fontSize:"22px",fontWeight:"700",color:"#202124",marginBottom:"12px"},text:{fontSize:"15px",color:"#5f6368",lineHeight:"1.6",marginBottom:"32px"},dotsContainer:{display:"flex",justifyContent:"center",gap:"8px",marginBottom:"24px"},dot:{width:"8px",height:"8px",borderRadius:"50%",background:"#dadce0",transition:"all 0.3s"},dotActive:{background:"#1a73e8",width:"24px",borderRadius:"4px"},btnContainer:{display:"flex",justifyContent:"space-between",alignItems:"center"},btn:{padding:"10px 24px",borderRadius:"20px",border:"none",cursor:"pointer",fontSize:"14px",fontWeight:"600",transition:"background 0.2s"},btnSkip:{background:"transparent",color:"#5f6368"},btnNext:{background:"#1a73e8",color:"#fff",boxShadow:"0 4px 12px rgba(26,115,232,0.3)"}},a=document.createElement("div");Object.assign(a.style,o.overlay);let n=document.createElement("div");Object.assign(n.style,o.card);let i=document.createElement("div");Object.assign(i.style,o.icon);let s=document.createElement("div");Object.assign(s.style,o.title);let r=document.createElement("div");Object.assign(r.style,o.text);let l=document.createElement("div");Object.assign(l.style,o.dotsContainer);let d=document.createElement("div");Object.assign(d.style,o.btnContainer);let c=document.createElement("button");c.textContent="Pular",Object.assign(c.style,o.btn,o.btnSkip),c.onmouseover=()=>c.style.color="#202124",c.onmouseout=()=>c.style.color="#5f6368";let p=document.createElement("button");p.textContent="Pr\xF3ximo",Object.assign(p.style,o.btn,o.btnNext),p.onmouseover=()=>p.style.transform="scale(1.05)",p.onmouseout=()=>p.style.transform="scale(1)",d.appendChild(c),d.appendChild(p),n.appendChild(i),n.appendChild(s),n.appendChild(r),n.appendChild(l),n.appendChild(d),a.appendChild(n),document.body.appendChild(a);function x(g){let m=t[g];i.textContent=m.icon,s.textContent=m.title,r.textContent=m.text,l.innerHTML="",t.forEach((w,C)=>{let H=document.createElement("div");Object.assign(H.style,o.dot),C===g&&Object.assign(H.style,o.dotActive),l.appendChild(H)}),m.isLast?(c.style.display="none",p.textContent="Come\xE7ar \u{1F680}",p.style.width="100%"):(c.style.display="block",p.textContent="Pr\xF3ximo",p.style.width="auto")}function b(){localStorage.setItem("cw_onboarding_seen_v1","true"),a.style.opacity="0",n.style.transform="translateY(20px)",setTimeout(()=>a.remove(),400),G.playSuccess(),K("Tudo pronto! Use o menu flutuante.")}p.onclick=()=>{G.playClick(),e<t.length-1?(e++,x(e)):b()},c.onclick=async()=>{await fe("Pular o tutorial?")&&b()},x(0),requestAnimationFrame(()=>{a.style.opacity="1",n.style.transform="translateY(0)"})}var To={version:"v5.1",title:"Atualiza\xE7\xE3o: v5.1 - Produtividade Blindada \u{1F6E1}\uFE0F",slides:[{icon:"\u{1F17F}\uFE0F",title:"Estacionamento de Casos",text:"Interrup\xE7\xE3o urgente? Agora voc\xEA pode 'Estacionar' seu atendimento atual (Notas + Tasks) com um clique e retomar depois exatamente de onde parou."},{icon:"\u{1F6DF}",title:"Sistema 'Airbag'",text:"Caiu a internet? Fechou a aba sem querer? O TechSol agora possui Auto-Save de emerg\xEAncia a cada 5 segundos. Seu texto est\xE1 salvo, sempre."},{icon:"\u{1F7E0}",title:"Indicador de Progresso",text:"Nunca mais esque\xE7a uma nota aberta. Um indicador laranja ('Dirty State') avisa na P\xEDlula principal se h\xE1 trabalho n\xE3o salvo/estacionado."},{icon:"\u{1F50D}",title:"Time Zone Pro",text:"O m\xF3dulo de fusos hor\xE1rios ganhou superpoderes: nova barra de pesquisa global, filtros r\xE1pidos por regi\xE3o e corre\xE7\xE3o de visualiza\xE7\xE3o."},{icon:"\u{1F916}",title:"Leitura de BAU Aprimorada",text:"O sistema de Broadcast agora \xE9 mais inteligente ao ler avisos de disponibilidade, detectando datas e bandeiras mesmo quando quebradas em v\xE1rias linhas."},{icon:"\u{1F3A8}",title:"Refinamento Visual",text:"Bot\xF5es padronizados, sombras suavizadas e micro-intera\xE7\xF5es t\xE1teis em todo o sistema para uma experi\xEAncia mais fluida e profissional."}]};function ko(t){let e=localStorage.getItem("cw_last_version");if(!e){localStorage.setItem("cw_last_version",t);return}e!==t&&sa(t)}function sa(t){let e=To.slides,o=0,a={overlay:{position:"fixed",top:0,left:0,width:"100vw",height:"100vh",backgroundColor:"rgba(0,0,0,0.7)",backdropFilter:"blur(5px)",zIndex:"2147483647",display:"flex",alignItems:"center",justifyContent:"center",opacity:"0",transition:"opacity 0.3s ease"},card:{width:"400px",background:"#fff",borderRadius:"24px",padding:"32px",textAlign:"center",position:"relative",boxShadow:"0 24px 60px rgba(0,0,0,0.4)",fontFamily:"'Google Sans', Roboto, sans-serif",transform:"translateY(30px)",transition:"all 0.4s cubic-bezier(0.19, 1, 0.22, 1)"},badge:{display:"inline-block",padding:"4px 12px",borderRadius:"12px",background:"#E8F0FE",color:"#1967D2",fontSize:"11px",fontWeight:"700",textTransform:"uppercase",marginBottom:"16px",letterSpacing:"0.5px"},icon:{fontSize:"42px",marginBottom:"16px",display:"block"},title:{fontSize:"20px",fontWeight:"700",color:"#202124",marginBottom:"8px"},text:{fontSize:"14px",color:"#5f6368",lineHeight:"1.5",marginBottom:"32px",minHeight:"42px"},dotsContainer:{display:"flex",justifyContent:"center",gap:"6px",marginBottom:"24px"},dot:{width:"6px",height:"6px",borderRadius:"50%",background:"#dadce0",transition:"all 0.3s"},dotActive:{background:"#1a73e8",width:"18px",borderRadius:"4px"},btn:{width:"100%",padding:"12px",borderRadius:"12px",border:"none",cursor:"pointer",fontSize:"14px",fontWeight:"600",transition:"all 0.2s",background:"#1a73e8",color:"#fff",boxShadow:"0 4px 12px rgba(26,115,232,0.3)"}},n=document.createElement("div");Object.assign(n.style,a.overlay);let i=document.createElement("div");Object.assign(i.style,a.card);let s=document.createElement("div");Object.assign(s.style,a.badge),s.textContent=`Atualiza\xE7\xE3o ${t}`;let r=document.createElement("div");Object.assign(r.style,a.icon);let l=document.createElement("div");Object.assign(l.style,a.title);let d=document.createElement("div");Object.assign(d.style,a.text);let c=document.createElement("div");Object.assign(c.style,a.dotsContainer);let p=document.createElement("button");Object.assign(p.style,a.btn),p.onmouseover=()=>p.style.transform="scale(1.02)",p.onmouseout=()=>p.style.transform="scale(1)",i.appendChild(s),i.appendChild(r),i.appendChild(l),i.appendChild(d),i.appendChild(c),i.appendChild(p),n.appendChild(i),document.body.appendChild(n);function x(g){let m=e[g];r.textContent=m.icon,l.textContent=m.title,d.textContent=m.text,c.innerHTML="",e.forEach((w,C)=>{let H=document.createElement("div");Object.assign(H.style,a.dot),C===g&&Object.assign(H.style,a.dotActive),c.appendChild(H)}),g===e.length-1?p.textContent="Entendi, vamos l\xE1! \u{1F44D}":p.textContent="Pr\xF3ximo"}function b(){localStorage.setItem("cw_last_version",t),n.style.opacity="0",i.style.transform="translateY(30px)",setTimeout(()=>n.remove(),400),G.playSuccess(),K(`TechSol atualizado para ${t}!`)}p.onclick=()=>{G.playClick(),o<e.length-1?(o++,x(o)):b()},x(0),requestAnimationFrame(()=>{n.style.opacity="1",i.style.transform="translateY(0)"})}var Fo="cw_timezone_pinned",_t=[{id:"pt",name:"Portugal",flag:"\u{1F1F5}\u{1F1F9}",zone:"Europe/Lisbon",label:"Lisboa",region:"eu"},{id:"es",name:"Espanha",flag:"\u{1F1EA}\u{1F1F8}",zone:"Europe/Madrid",label:"Madrid",region:"eu"},{id:"ar",name:"Argentina",flag:"\u{1F1E6}\u{1F1F7}",zone:"America/Argentina/Buenos_Aires",label:"Buenos Aires",region:"sa"},{id:"bo",name:"Bol\xEDvia",flag:"\u{1F1E7}\u{1F1F4}",zone:"America/La_Paz",label:"La Paz",region:"sa"},{id:"cl",name:"Chile",flag:"\u{1F1E8}\u{1F1F1}",zone:"America/Santiago",label:"Santiago",region:"sa"},{id:"co",name:"Col\xF4mbia",flag:"\u{1F1E8}\u{1F1F4}",zone:"America/Bogota",label:"Bogot\xE1",region:"sa"},{id:"ec",name:"Equador",flag:"\u{1F1EA}\u{1F1E8}",zone:"America/Guayaquil",label:"Guayaquil",region:"sa"},{id:"py",name:"Paraguai",flag:"\u{1F1F5}\u{1F1FE}",zone:"America/Asuncion",label:"Assun\xE7\xE3o",region:"sa"},{id:"pe",name:"Peru",flag:"\u{1F1F5}\u{1F1EA}",zone:"America/Lima",label:"Lima",region:"sa"},{id:"uy",name:"Uruguai",flag:"\u{1F1FA}\u{1F1FE}",zone:"America/Montevideo",label:"Montevid\xE9u",region:"sa"},{id:"ve",name:"Venezuela",flag:"\u{1F1FB}\u{1F1EA}",zone:"America/Caracas",label:"Caracas",region:"sa"},{id:"mx",name:"M\xE9xico",flag:"\u{1F1F2}\u{1F1FD}",zone:"America/Mexico_City",label:"CDMX",region:"na"},{id:"cr",name:"Costa Rica",flag:"\u{1F1E8}\u{1F1F7}",zone:"America/Costa_Rica",label:"San Jos\xE9",region:"na"},{id:"sv",name:"El Salvador",flag:"\u{1F1F8}\u{1F1FB}",zone:"America/El_Salvador",label:"San Salvador",region:"na"},{id:"gt",name:"Guatemala",flag:"\u{1F1EC}\u{1F1F9}",zone:"America/Guatemala",label:"C. da Guatemala",region:"na"},{id:"hn",name:"Honduras",flag:"\u{1F1ED}\u{1F1F3}",zone:"America/Tegucigalpa",label:"Tegucigalpa",region:"na"},{id:"ni",name:"Nicar\xE1gua",flag:"\u{1F1F3}\u{1F1EE}",zone:"America/Managua",label:"Man\xE1gua",region:"na"},{id:"pa",name:"Panam\xE1",flag:"\u{1F1F5}\u{1F1E6}",zone:"America/Panama",label:"C. do Panam\xE1",region:"na"},{id:"do",name:"Rep. Dominicana",flag:"\u{1F1E9}\u{1F1F4}",zone:"America/Santo_Domingo",label:"Santo Domingo",region:"na"},{id:"pr",name:"Porto Rico",flag:"\u{1F1F5}\u{1F1F7}",zone:"America/Puerto_Rico",label:"San Juan",region:"na"}],ra=[{id:"all",label:"Todos"},{id:"sa",label:"Am\xE9rica do Sul"},{id:"na",label:"Norte & Central"},{id:"eu",label:"Europa"}];function Oo(){let t="v2.2 Pro",e=!1,o=null,a="mx",n=JSON.parse(localStorage.getItem(Fo)||"[]"),i="",s="all",r=new Date;r.setHours(14,0,0,0);let l={bg:"#F8F9FA",surface:"#FFFFFF",primary:"#1A73E8",primaryBg:"#E8F0FE",text:"#202124",textSub:"#5F6368",border:"#DADCE0",success:"#1E8E3E",successBg:"#E6F4EA",warning:"#E37400",warningBg:"#FEF7E0",error:"#D93025",errorBg:"#FCE8E6"},d={container:{display:"flex",flexDirection:"column",height:"100%",background:l.bg,fontFamily:"'Google Sans', Roboto, sans-serif"},tabHeader:{display:"flex",background:l.surface,borderBottom:`1px solid ${l.border}`,padding:"8px 16px 0 16px"},tabBtn:{flex:1,padding:"12px",textAlign:"center",cursor:"pointer",fontSize:"13px",fontWeight:"500",color:l.textSub,borderBottom:"3px solid transparent",transition:"all 0.2s ease",userSelect:"none"},tabActive:{color:l.primary,borderBottomColor:l.primary,fontWeight:"600"},toolbar:{padding:"12px 16px 8px 16px",background:l.bg,display:"flex",flexDirection:"column",gap:"12px",borderBottom:"1px solid rgba(0,0,0,0.03)"},searchInputWrapper:{position:"relative",width:"100%"},searchInput:{width:"100%",boxSizing:"border-box",padding:"10px 12px 10px 38px",borderRadius:"10px",border:"1px solid transparent",background:"#FFFFFF",fontSize:"14px",color:l.text,outline:"none",boxShadow:"0 1px 3px rgba(0,0,0,0.05)",transition:"all 0.2s",fontFamily:"'Google Sans', Roboto, sans-serif"},searchIcon:{position:"absolute",left:"12px",top:"50%",transform:"translateY(-50%)",width:"16px",height:"16px",color:"#9AA0A6",pointerEvents:"none"},chipsRow:{display:"flex",gap:"8px",overflowX:"auto",paddingBottom:"4px",scrollbarWidth:"none",msOverflowStyle:"none"},chip:{whiteSpace:"nowrap",padding:"6px 12px",borderRadius:"16px",fontSize:"12px",fontWeight:"500",cursor:"pointer",border:`1px solid ${l.border}`,background:l.surface,color:l.textSub,transition:"all 0.2s"},chipActive:{background:l.primaryBg,color:l.primary,borderColor:l.primaryBg,fontWeight:"600"},listContainer:{padding:"16px 16px 40px 16px",overflowY:"auto",flex:1,display:"flex",flexDirection:"column",gap:"12px",scrollbarWidth:"none"},hubCard:{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px 20px",background:l.surface,borderRadius:"16px",border:"1px solid transparent",boxShadow:"0 2px 6px rgba(60,64,67,0.05)",transition:"transform 0.2s cubic-bezier(0.25, 0.8, 0.25, 1), box-shadow 0.2s ease",cursor:"pointer",position:"relative"},hubCardPinned:{borderLeft:`4px solid ${l.primary}`,paddingLeft:"16px"},plannerWrapper:{padding:"24px",display:"flex",flexDirection:"column",gap:"24px",flex:1,overflowY:"auto"},timeComparisonRow:{display:"flex",gap:"16px",alignItems:"stretch"},timeCard:{flex:1,padding:"20px",borderRadius:"20px",background:l.surface,border:`1px solid ${l.border}`,display:"flex",flexDirection:"column",alignItems:"center",gap:"8px",boxShadow:"0 4px 12px rgba(60,64,67,0.05)"},timelineContainer:{position:"relative",height:"60px",marginTop:"16px",userSelect:"none"},timelineTrack:{position:"absolute",top:"26px",left:"0",right:"0",height:"6px",borderRadius:"3px",background:"#E0E0E0",overflow:"hidden"},dayZone:{position:"absolute",top:"0",bottom:"0",left:"37.5%",width:"37.5%",background:"rgba(52, 168, 83, 0.3)",pointerEvents:"none"},hdInput:{fontSize:"28px",fontWeight:"700",color:l.text,border:"none",background:"transparent",width:"100%",textAlign:"center",outline:"none",fontFamily:"'Google Sans', sans-serif",cursor:"text"},statusBadge:{padding:"8px 16px",borderRadius:"50px",fontSize:"13px",fontWeight:"600",display:"inline-flex",alignItems:"center",gap:"8px",marginTop:"16px",alignSelf:"center",transition:"background-color 0.3s"}},c=document.createElement("div");c.id="timezone-popup",c.classList.add("cw-module-window"),Object.assign(c.style,me,{right:"100px",width:"450px",height:"720px",overflow:"hidden",borderRadius:"24px"});let x=pe(c,"Time Zone Traveler",t,"Monitoramento global e planejamento de chamadas.",{popup:c},()=>A());c.appendChild(x);let b=document.createElement("div");Object.assign(b.style,d.container),c.appendChild(b);let g=document.createElement("div");Object.assign(g.style,d.tabHeader);let m=document.createElement("div");m.textContent="Monitoramento",Object.assign(m.style,d.tabBtn,d.tabActive);let w=document.createElement("div");w.textContent="Planejador",Object.assign(w.style,d.tabBtn),g.appendChild(m),g.appendChild(w),b.appendChild(g);let C=document.createElement("div");Object.assign(C.style,d.toolbar);let H=document.createElement("div");Object.assign(H.style,d.searchInputWrapper);let j=document.createElement("div");j.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>',Object.assign(j.style,d.searchIcon);let B=document.createElement("input");B.placeholder="Buscar cidade ou pa\xEDs...",Object.assign(B.style,d.searchInput),B.onfocus=()=>{B.style.boxShadow="0 2px 8px rgba(26,115,232,0.15)",B.style.borderColor="rgba(26,115,232,0.3)"},B.onblur=()=>{B.style.boxShadow="0 1px 3px rgba(0,0,0,0.05)",B.style.borderColor="transparent"},B.oninput=I=>{i=I.target.value.toLowerCase(),k()},H.appendChild(j),H.appendChild(B),C.appendChild(H);let Z=document.createElement("div");Object.assign(Z.style,d.chipsRow),ra.forEach(I=>{let q=document.createElement("div");q.textContent=I.label,q.id=`tz-filter-${I.id}`,Object.assign(q.style,d.chip),I.id===s&&Object.assign(q.style,d.chipActive),q.onclick=()=>{G.playClick(),s=I.id,Array.from(Z.children).forEach(D=>{Object.assign(D.style,d.chip)}),Object.assign(q.style,d.chipActive),k()},Z.appendChild(q)}),C.appendChild(Z),b.appendChild(C);let J=document.createElement("div");Object.assign(J.style,d.listContainer);let M=document.createElement("style");M.textContent="#timezone-popup ::-webkit-scrollbar { display: none; }",b.appendChild(M);let F=document.createElement("div");Object.assign(F.style,d.plannerWrapper,{display:"none"}),b.appendChild(J),b.appendChild(F),m.onclick=()=>R("live"),w.onclick=()=>R("plan");function R(I){G.playClick(),I==="live"?(Object.assign(m.style,d.tabActive),Object.assign(w.style,d.tabBtn),w.style.borderBottomColor="transparent",J.style.display="flex",C.style.display="flex",F.style.display="none",L()):(Object.assign(w.style,d.tabActive),Object.assign(m.style,d.tabBtn),m.style.borderBottomColor="transparent",F.style.display="flex",J.style.display="none",C.style.display="none",f(),E())}function U(I){return I>=9&&I<17?{color:l.success,bg:l.successBg,label:"Aberto",icon:"\u{1F7E2}"}:I>=8&&I<9?{color:l.warning,bg:l.warningBg,label:"Abrindo",icon:"\u{1F7E1}"}:I>=17&&I<19?{color:l.warning,bg:l.warningBg,label:"Fechando",icon:"\u{1F7E1}"}:{color:l.textSub,bg:"#F1F3F4",label:"Fechado",icon:"\u{1F534}"}}function _(I){n.includes(I)?n=n.filter(q=>q!==I):n.push(I),localStorage.setItem(Fo,JSON.stringify(n)),k(),G.playClick()}function k(){J.innerHTML="";let I=new Date,q=_t.filter(P=>{let Y=P.name.toLowerCase().includes(i)||P.label.toLowerCase().includes(i),y=s==="all"||P.region===s;return Y&&y});if(q.sort((P,Y)=>{let y=n.includes(P.id),V=n.includes(Y.id);return y&&!V?-1:!y&&V?1:P.name.localeCompare(Y.name)}),q.length===0){J.innerHTML=`
            ...
            `,u.onmouseenter=()=>{u.style.transform="translateY(-2px)",u.style.boxShadow="0 6px 12px rgba(60,64,67,0.1)"},u.onmouseleave=()=>{u.style.transform="translateY(0)",u.style.boxShadow="0 2px 6px rgba(60,64,67,0.05)"};let N=u.querySelector(".cw-pin-btn");N.onmouseenter=()=>{N.style.backgroundColor="#F1F3F4"},N.onmouseleave=()=>{N.style.backgroundColor="transparent"},N.onclick=O=>{O.stopPropagation(),_(P.id)},u.onclick=()=>{a=P.id,R("plan")},J.appendChild(u)});let D=document.createElement("div");D.style.height="20px",D.style.width="100%",J.appendChild(D)}function E(){F.innerHTML="";let I=document.createElement("div"),q=document.createElement("label");q.textContent="Onde est\xE1 o cliente?",q.style.cssText="display:block; font-size:12px; font-weight:700; color:#5F6368; margin-bottom:8px; text-transform:uppercase; letter-spacing:0.5px;";let D=document.createElement("select");Object.assign(D.style,Ut),D.style.padding="14px",[..._t].sort((ne,te)=>ne.name.localeCompare(te.name)).forEach(ne=>{let te=document.createElement("option");te.value=ne.id,te.textContent=`${ne.flag} ${ne.name} (${ne.zone})`,ne.id===a&&(te.selected=!0),D.appendChild(te)}),D.onchange=ne=>{a=ne.target.value,Q(),G.playClick()},I.appendChild(q),I.appendChild(D),F.appendChild(I);let Y=document.createElement("div");Object.assign(Y.style,d.timeComparisonRow);let y=document.createElement("div");Object.assign(y.style,d.timeCard),y.style.backgroundColor="#F8FAFF",y.style.borderColor="#E8F0FE",y.innerHTML=`
        ...
        `,Y.appendChild(y),Y.appendChild(V),F.appendChild(Y);let S=document.createElement("div");S.id="cw-planner-status",Object.assign(S.style,d.statusBadge),F.appendChild(S);let T=document.createElement("div");Object.assign(T.style,{padding:"0 4px",marginTop:"12px"});let u=document.createElement("div");u.textContent="Arraste para simular o hor\xE1rio:",u.style.cssText="font-size:12px; color:#5F6368; text-align:center; margin-bottom:12px;";let v=document.createElement("div");Object.assign(v.style,d.timelineContainer);let h=document.createElement("div");Object.assign(h.style,d.timelineTrack);let N=document.createElement("div");Object.assign(N.style,d.dayZone),h.appendChild(N);let O=document.createElement("input");O.type="range",O.min="0",O.max="1439",O.step="15",O.style.cssText="position:absolute; top:20px; left:0; width:100%; -webkit-appearance:none; background:transparent; z-index:2; cursor:pointer;";let X=document.createElement("div");X.style.cssText="position:absolute; top:36px; width:100%; display:flex; justify-content:space-between; font-size:10px; font-weight:600; color:#9AA0A6; padding:0 2px;",X.innerHTML="<span>00h</span><span>06h</span><span>12h</span><span>18h</span><span>24h</span>",v.appendChild(h),v.appendChild(O),v.appendChild(X),T.appendChild(u),T.appendChild(v),F.appendChild(T);let z=y.querySelector("#cw-time-input-br"),ee=V.querySelector("#cw-time-display-client"),oe=V.querySelector("#cw-client-label");function Q(){let ne=_t.find(Me=>Me.id===a);oe.textContent=`${ne.flag} ${ne.label} (${ne.zone})`;let te=r.getHours(),re=r.getMinutes(),Ve=`${String(te).padStart(2,"0")}:${String(re).padStart(2,"0")}`;z.value=Ve,O.value=te*60+re;let et=r.toLocaleTimeString("pt-BR",{timeZone:ne.zone,hour:"2-digit",minute:"2-digit"});ee.textContent=et;let Ie=parseInt(et.split(":")[0]);Ie>=9&&Ie<17?(S.style.background=l.successBg,S.style.color=l.success,S.innerHTML='<span style="font-size:16px">\u2705</span> Hor\xE1rio Comercial Ideal'):Ie>=8&&Ie<9||Ie>=17&&Ie<19?(S.style.background=l.warningBg,S.style.color=l.warning,S.innerHTML='<span style="font-size:16px">\u26A0\uFE0F</span> Hor\xE1rio Limite (Aten\xE7\xE3o)'):(S.style.background=l.errorBg,S.style.color=l.error,S.innerHTML='<span style="font-size:16px">\u26D4</span> Fora de Hor\xE1rio')}O.oninput=ne=>{let te=parseInt(ne.target.value);r.setHours(Math.floor(te/60)),r.setMinutes(te%60),Q()},z.oninput=ne=>{let[te,re]=ne.target.value.split(":");te&&re&&(r.setHours(parseInt(te)),r.setMinutes(parseInt(re)),Q())},Q()}function L(){k(),o||(o=setInterval(k,6e4))}function f(){o&&(clearInterval(o),o=null)}function A(){e=!e,ue(e,c,"cw-btn-timezone"),e?R("live"):f()}return document.body.appendChild(c),A}function Lo(){let t="v1.1",e=!1,o="general",a=null,n=null;if(!document.getElementById("cw-lib-styles")){let E=document.createElement("style");E.id="cw-lib-styles",E.innerHTML=`
        ...
        `,document.head.appendChild(E)}let i={bg:"#F0F2F5",surface:"#FFFFFF",primary:"#007AFF",primaryBg:"rgba(0, 122, 255, 0.1)",text:"#1C1C1E",textSub:"#8E8E93",border:"rgba(0, 0, 0, 0.08)",danger:"#FF3B30"},s={container:{display:"flex",flexDirection:"column",height:"100%",background:i.bg,fontFamily:"'Google Sans', Roboto, sans-serif"},tabHeader:{display:"flex",padding:"12px 16px 0 16px",background:i.surface,borderBottom:`1px solid ${i.border}`},tabBtn:{flex:1,padding:"12px",textAlign:"center",cursor:"pointer",fontSize:"13px",fontWeight:"500",color:i.textSub,borderBottom:"3px solid transparent",transition:"all 0.3s cubic-bezier(0.4, 0, 0.2, 1)",userSelect:"none"},tabActive:{color:i.primary,borderBottomColor:i.primary,fontWeight:"600"},listContainer:{flex:1,overflowY:"auto",padding:"16px",display:"flex",flexDirection:"column",gap:"12px"},emptyState:{padding:"40px 20px",textAlign:"center",color:"#BDC1C6",display:"flex",flexDirection:"column",alignItems:"center",gap:"10px"},card:{background:i.surface,borderRadius:"16px",padding:"16px",border:`1px solid ${i.border}`,boxShadow:"0 4px 12px rgba(0,0,0,0.05)",transition:"all 0.4s cubic-bezier(0.165, 0.84, 0.44, 1)",cursor:"default",position:"relative",overflow:"hidden"},cardHeader:{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"8px"},cardTitle:{fontSize:"14px",fontWeight:"600",color:i.text},cardPreview:{fontSize:"12px",color:i.textSub,lineHeight:"1.5",display:"-webkit-box",webkitLineClamp:"3",webkitBoxOrient:"vertical",overflow:"hidden"},cardActions:{display:"flex",justifyContent:"flex-end",gap:"8px",marginTop:"12px",paddingTop:"12px",borderTop:`1px dashed ${i.border}`},actionBtn:{padding:"6px 12px",borderRadius:"6px",fontSize:"12px",fontWeight:"600",cursor:"pointer",border:"none",background:"transparent",transition:"background 0.2s"},fab:{position:"absolute",bottom:"24px",right:"24px",width:"56px",height:"56px",borderRadius:"16px",background:i.primary,color:"#FFF",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 12px rgba(26, 115, 232, 0.4)",cursor:"pointer",transition:"transform 0.2s",zIndex:10},editorOverlay:{position:"absolute",top:0,left:0,width:"100%",height:"100%",background:"rgba(255, 255, 255, 0.85)",backdropFilter:"blur(25px) saturate(180%)",webkitBackdropFilter:"blur(25px) saturate(180%)",zIndex:20,transform:"translateY(100%)",transition:"transform 0.5s cubic-bezier(0.34, 1.56, 0.64, 1)",display:"flex",flexDirection:"column"},editorHeader:{padding:"16px 24px",background:i.surface,borderBottom:`1px solid ${i.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"},editorBody:{flex:1,padding:"24px",overflowY:"auto"},inputGroup:{marginBottom:"20px"},label:{display:"block",fontSize:"12px",fontWeight:"700",color:i.textSub,marginBottom:"8px",textTransform:"uppercase"},input:{width:"100%",padding:"12px",borderRadius:"8px",border:`1px solid ${i.border}`,fontSize:"14px",fontFamily:"inherit",outline:"none",background:i.surface,boxSizing:"border-box"}},r=document.createElement("div");r.id="library-popup",r.classList.add("cw-module-window"),Object.assign(r.style,me,{right:"auto",left:"50%",width:"400px",height:"600px",transform:"translateX(-50%) scale(0.05)",overflow:"hidden"});let d=pe(r,"Minha Biblioteca",t,"Gerencie seus snippets, textos e templates.",{popup:r},()=>k());r.appendChild(d);let c=document.createElement("div");Object.assign(c.style,s.container),r.appendChild(c);let p=document.createElement("div");Object.assign(p.style,s.tabHeader);let x=[{id:"general",label:"Geral",icon:"\u{1F4CB}"},{id:"note",label:"Notas",icon:"\u{1F4DD}"},{id:"email",label:"Emails",icon:"\u{1F4E7}"}];x.forEach(E=>{let L=document.createElement("div");L.innerHTML=`${E.icon} ${E.label}`,L.id=`lib-tab-${E.id}`,Object.assign(L.style,s.tabBtn),E.id===o&&Object.assign(L.style,s.tabActive),L.onmouseenter=()=>G.playHover(),L.onclick=()=>Z(E.id),p.appendChild(L)}),c.appendChild(p);let b=document.createElement("div");Object.assign(b.style,s.listContainer),c.appendChild(b);let g=document.createElement("div");g.className="cw-fab cw-tactile",Object.assign(g.style,s.fab),g.innerHTML='<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>',g.onmouseenter=()=>g.style.transform="scale(1.1)",g.onmouseleave=()=>g.style.transform="scale(1)",g.onclick=()=>M(),c.appendChild(g),a=document.createElement("div"),Object.assign(a.style,s.editorOverlay);let m=document.createElement("div");Object.assign(m.style,s.editorHeader),m.innerHTML='<span style="font-weight:700; font-size:16px;">Novo Item</span>';let w=document.createElement("button");w.innerHTML="Cancelar",w.style.cssText="background:none; border:none; color:#5f6368; font-weight:600; cursor:pointer;",w.onclick=F,m.appendChild(w),a.appendChild(m);let C=document.createElement("div");Object.assign(C.style,s.editorBody),a.appendChild(C);let H=document.createElement("div");H.style.cssText="padding:16px 24px; border-top:1px solid #DADCE0; background:#FFF; display:flex; justify-content:flex-end;";let j=document.createElement("button");j.textContent="Salvar",j.style.cssText="padding:10px 24px; background:#1a73e8; color:white; border:none; border-radius:20px; font-weight:600; cursor:pointer; box-shadow:0 2px 5px rgba(26,115,232,0.3);",j.onclick=R,H.appendChild(j),a.appendChild(H),c.appendChild(a);let B=document.createElement("div");Object.assign(B.style,Ee),B.className="no-drag",r.appendChild(B),Ae(r,B),document.body.appendChild(r);function Z(E){G.playClick(),o=E,x.forEach(L=>{let f=document.getElementById(`lib-tab-${L.id}`);L.id===E?Object.assign(f.style,s.tabActive):Object.assign(f.style,s.tabBtn)}),J()}function J(){b.innerHTML="";let E=xe.getSnippets(o);if(E.length===0){b.innerHTML=`
            ...
            `;return}E.forEach(L=>{let f=document.createElement("div");f.className="cw-lib-card",Object.assign(f.style,s.card),L.isCode&&(f.style.borderLeft=`4px solid ${i.primary}`,f.style.background="rgba(0, 122, 255, 0.02)");let A=L.content;if(L.isRich){let I=document.createElement("div");I.innerHTML=L.content;let q=I.querySelector("img");A=I.innerText.substring(0,150)+(I.innerText.length>150?"...":""),q&&(A="\u{1F5BC}\uFE0F [Cont\xE9m Imagens] "+A)}f.innerHTML=`
                <div style="${_(s.cardHeader)}">
                    <div style="${_(s.cardTitle)}">${L.title}</div>
                ...
                <div style="${_(s.cardPreview)}; ${L.isCode?"font-family:'Roboto Mono', monospace; font-size:11px;":""}">${A}</div>
                <div style="${_(s.cardActions)}">
                    <button class="cw-act-copy cw-tactile" title="Copiar" style="${_(s.actionBtn)}; color:#007AFF; display:flex; align-items:center; gap:4px;">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>
                        <span>Copiar</span>
                    </button>
                    <button class="cw-act-edit cw-tactile" title="Editar" style="${_(s.actionBtn)}; color:#8E8E93; display:flex; align-items:center; gap:4px;">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                        <span>Editar</span>
                    </button>
                    <button class="cw-act-del cw-tactile" title="Excluir" style="${_(s.actionBtn)}; color:#FF3B30; display:flex; align-items:center; gap:4px;">
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path><line x1="10" y1="11" x2="10" y2="17"></line><line x1="14" y1="11" x2="14" y2="17"></line></svg>
                        <span>Excluir</span>
                    </button>
                </div>
            `,f.onmouseenter=()=>{G.playHover()},f.querySelector(".cw-act-copy").onclick=I=>{if(I.stopPropagation(),G.playClick(),L.isRich){let q=new Blob([L.content],{type:"text/html"}),D=document.createElement("div");D.style.whiteSpace="pre-wrap",D.innerHTML=L.content;let P=new Blob([D.innerText],{type:"text/plain"}),Y=[new ClipboardItem({"text/html":q,"text/plain":P})];navigator.clipboard.write(Y)}else navigator.clipboard.writeText(L.content);K("Copiado!")},f.querySelector(".cw-act-edit").onclick=I=>{I.stopPropagation(),G.playClick(),M(L)},f.querySelector(".cw-act-del").onclick=async I=>{I.stopPropagation(),G.playClick(),await fe("Excluir este item?")&&(xe.delete(L.id),J(),K("Item exclu\xEDdo."))},b.appendChild(f)})}function M(E=null){n=E?E.id:null,C.innerHTML="",C.appendChild(U("title","T\xEDtulo / Nome",E?E.title:"")),o==="email"&&C.appendChild(U("subject","Assunto do Email",E?E.subject:""));let L="Conte\xFAdo";o==="email"&&(L="Corpo do Email (HTML)"),o==="note"&&(L="Texto da Nota (Reason)"),C.appendChild(U("content",L,E?E.content:"",{isRich:!0,isCode:E?E.isCode:!1})),m.querySelector("span").textContent=E?"Editar Item":"Novo Item",a.style.transform="translateY(0)",setTimeout(()=>{let f=C.querySelector("input");f&&f.focus()},300)}function F(){a.style.transform="translateY(100%)",setTimeout(()=>n=null,300)}async function R(){let E=C.querySelector("#cw-inp-title"),L=C.querySelector("#cw-inp-content"),f=E.value.trim(),A=L.contentEditable==="true"?L.innerHTML:L.value.trim(),I=L.getAttribute("data-is-code")==="true";if(!f||!A||A==="<br>"){K("Preencha t\xEDtulo e conte\xFAdo.",{error:!0});return}let q={id:n,type:o,title:f,content:A,isCode:I,isRich:L.contentEditable==="true"};if(o==="email"){let D=C.querySelector("#cw-inp-subject").value.trim();if(!D){K("Assunto \xE9 obrigat\xF3rio para emails.",{error:!0});return}q.subject=D}j.textContent="Salvando...",await xe.save(q),j.textContent="Salvar",F(),J(),K("Salvo com sucesso!"),G.playSuccess()}function U(E,L,f,A={}){let I=document.createElement("div");Object.assign(I.style,s.inputGroup);let q=document.createElement("label");q.textContent=L,Object.assign(q.style,s.label);let D;if(A.isRich){let P=document.createElement("div");P.style.cssText="display:flex; gap:6px; margin-bottom:12px; background:rgba(241, 243, 244, 0.8); padding:6px; border-radius:12px; border:1px solid #DADCE0; backdrop-filter: blur(10px);",P.innerHTML=`
            ...
            `,D=document.createElement("div"),D.contentEditable="true",Object.assign(D.style,s.input,{minHeight:"180px",maxHeight:"350px",overflowY:"auto",whiteSpace:"pre-wrap",lineHeight:"1.6",outline:"none"}),D.innerHTML=f||"",A.isCode&&(D.style.fontFamily="'Roboto Mono', monospace",D.style.backgroundColor="#F8F9FA",D.setAttribute("data-is-code","true")),P.querySelectorAll(".cw-toolbar-btn").forEach(Y=>{Y.onmouseenter=()=>G.playHover(),Y.onmousedown=()=>G.playClick()}),P.querySelector(".cw-tb-bold").onclick=()=>{document.execCommand("bold"),D.focus()},P.querySelector(".cw-tb-italic").onclick=()=>{document.execCommand("italic"),D.focus()},P.querySelector(".cw-tb-code").onclick=Y=>{let V=!(D.getAttribute("data-is-code")==="true");D.setAttribute("data-is-code",V),D.style.fontFamily=V?"'Roboto Mono', monospace":"inherit",D.style.backgroundColor=V?"rgba(0, 122, 255, 0.03)":i.surface,V?Y.currentTarget.classList.add("active"):Y.currentTarget.classList.remove("active"),D.focus()},P.querySelector(".cw-tb-img").onclick=async()=>{let Y=await Jt("Cole a URL da imagem:");Y&&(document.execCommand("insertImage",!1,Y),D.querySelectorAll("img").forEach(V=>{V.style.maxWidth="100%",V.style.borderRadius="8px"}))},D.onpaste=Y=>{let y=(Y.clipboardData||Y.originalEvent.clipboardData).items;for(let V of y)if(V.kind==="file"&&V.type.startsWith("image/")){Y.preventDefault();let S=V.getAsFile(),T=new FileReader;T.onload=u=>{let v=`<img src="${u.target.result}" style="max-width:100%; border-radius:8px; margin:8px 0; display:block;">`;document.execCommand("insertHTML",!1,v)},T.readAsDataURL(S)}},I.appendChild(q),I.appendChild(P)}else D=document.createElement("input"),D.type="text",Object.assign(D.style,s.input),D.value=f||"",I.appendChild(q);return D.id=`cw-inp-${E}`,D.onfocus=()=>{D.style.borderColor=i.primary,D.style.boxShadow=`0 0 0 2px ${i.primaryBg}`},D.onblur=()=>{D.style.borderColor=i.border,D.style.boxShadow="none"},I.appendChild(D),I}function _(E){return Object.entries(E).map(([L,f])=>`${L.replace(/[A-Z]/g,A=>"-"+A.toLowerCase())}:${f}`).join(";")}function k(){e=!e,ue(e,r,"cw-btn-library"),e&&J()}return k}function Io(){let t="v1.0",e=!1,o={bg:"#F8F9FA",surface:"#FFFFFF",primary:"#1A73E8",text:"#202124",textSub:"#5F6368",border:"#DADCE0"},a="cw-configs-styles";if(!document.getElementById(a)){let x=document.createElement("style");x.id=a,x.innerHTML=`
    ...
    `,r.appendChild(c);function p(){e=!e,ue(e,n,"cw-btn-configs"),e&&G.playClick()}return document.body.appendChild(n),p}var qo="cubic-bezier(0.4, 0, 0.2, 1)",Ss=`all 0.3s ${qo}`,_o=()=>{if(document.getElementById("bau-form-global-styles"))return;let t=document.createElement("style");t.id="bau-form-global-styles",t.textContent=`
  ...
  `,document.head.appendChild(t)};var _e={add:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>',back:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z"/></svg>',wand:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M7.5 5.6L10 7 8.6 4.5 10 2 7.5 3.4 5 2l1.4 2.5L5 7zm12 9.8L17 14l1.4 2.5L17 19l2.5-1.4L22 19l-1.4-2.5L22 14zM22 2l-2.5 1.4L17 2l1.4 2.5L17 7l2.5-1.4L22 7l-1.4-2.5zm-7.63 5.29c-.39-.39-1.02-.39-1.41 0L1.29 18.96c-.39.39-.39 1.02 0 1.41l2.34 2.34c.39.39 1.02.39 1.41 0L16.7 11.05c.39-.39.39-1.02 0-1.41l-2.33-2.35zm-1.03 5.41l-2.12-2.12 2.44-2.44 2.12 2.12-2.44 2.44z"/></svg>',send:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/></svg>',check:'<svg width="64" height="64" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>',folder:'<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M10 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2h-8l-2-2z"/></svg>',empty:'<svg width="64" height="64" viewBox="0 0 24 24" fill="currentColor"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5v-3h3.56c.69 1.19 1.97 2 3.44 2s2.75-.81 3.44-2H19v3zm0-5h-4.99c0 1.1-.9 2-2 2s-2-.9-2-2H5V5h14v9z"/></svg>'};function Mo(){_o();let t=!1,e="dashboard",o=null,a=1,n=4,i=document.createElement("div");i.id="bau-form-popup",i.className="bau-popup cw-module-window",i.style.display="none";let s=pe(i,"BAU Central","v2.0.0","Dashboard de Casos BAU",{},()=>V());i.appendChild(s);let r=document.createElement("div");r.className="bau-view-container",i.appendChild(r);let l=document.createElement("div");l.id="bau-view-dashboard",l.className="bau-view active",l.innerHTML=`
    ...
    `,g.appendChild(m);let w=document.createElement("div");w.className="bau-card bau-context-card",w.innerHTML=`
        <div id="bau-all-data"></div>
    `,g.appendChild(w),b.appendChild(g);let C=document.createElement("div");C.className="bau-step",C.id="bau-step-2";let H=document.createElement("div");H.className="bau-card";let j=["Ads Conversion Tracking","Ads Dynamic Remarketing","Ads Enhanced Conversions","Ads Website Call Conversion","Ads Remarketing","Analytics Cross Domain Tracking","Analytics E-Commerce Tracking","Analytics Enhanced E-Commerce Tracking","Analytics Event Tracking","Analytics Health Check","Analytics Remarketing","Analytics Setup","Fix GA4 implementation","Consent Mode","Fix Sitewide Tagging (OGT & CT)","Google Tag Manager Installation","Customer Match"];H.innerHTML=`
        ...
        <div class="bau-tasks-grid" id="bau-tasks-container">
            ${j.map(S=>`
                <label class="bau-task-item">
                    <input type="checkbox" name="taskType" value="${S}">
                    <span>${S}</span>
                </label>
            `).join("")}
        </div>
    `,H.querySelectorAll(".bau-task-item").forEach(S=>{let T=S.querySelector("input");S.addEventListener("click",()=>{T.checked=!T.checked,S.classList.toggle("active",T.checked),G.playClick()})}),C.appendChild(H),b.appendChild(C);let B=document.createElement("div");B.className="bau-step",B.id="bau-step-3";let Z=document.createElement("div");Z.className="bau-card",Z.innerHTML=`
        ...
        <select name="nonImplementationReason" required class="bau-select">
            ...
            ${["Tempo da consultoria esgotado","Solicita\xE7\xE3o de reagendamento pelo anunciante","Falta de acessos ou backup do site","Anunciante indispon\xEDvel ou n\xE3o preparado","Implementa\xE7\xE3o parcial (nem todas as tasks conclu\xEDdas)","Solicita\xE7\xE3o de tarefas (tasks) adicionais","Necessidade de novas altera\xE7\xF5es (fase de acompanhamento)","Retorno de contato ap\xF3s prazo de 14 dias expirado"].map(S=>`<option value="${S}">${S}</option>`).join("")}
        </select>
    ...
    `,b.appendChild(J);let M=document.createElement("div");M.className="bau-footer";let F=document.createElement("button");F.type="button",F.id="bau-step-back-btn",F.className="bau-btn-secondary",F.textContent="Voltar";let R=document.createElement("button");R.type="button",R.id="bau-step-next-btn",R.className="bau-btn-primary",R.textContent="Pr\xF3ximo";let U=document.createElement("button");U.type="submit",U.className="bau-btn-submit",U.innerHTML=`${_e.send} Enviar para o TL`,U.style.display="none",b.appendChild(F),b.appendChild(R),b.appendChild(U),b.appendChild(M),r.appendChild(d);let _=document.createElement("div");_.id="bau-view-success",_.className="bau-view",_.innerHTML=`
    ...
    `,r.appendChild(_),document.body.appendChild(i);function k(S){e=S,i.querySelectorAll(".bau-view").forEach(h=>h.classList.remove("active"));let T=i.querySelector(`#bau-view-${S}`);T&&T.classList.add("active");let u=s.querySelector(".cw-module-header-title")||s.querySelector("h2"),v=s.querySelector(".cw-module-header-subtitle")||s.querySelector("p");u&&(u.textContent=S==="form"?"Novo Caso BAU":"BAU Central"),v&&(v.textContent=S==="form"?"Preencha os detalhes abaixo":"Dashboard de Casos BAU")}async function E(){let S=i.querySelector("#bau-case-list-container");if(S){S.innerHTML="<p>Carregando casos...</p>";try{let T=await Rt();L(T)}catch{S.innerHTML='<p class="bau-error-text">Erro ao carregar casos.</p>'}}}function L(S){let T=i.querySelector("#bau-case-list-container"),u=i.querySelector("#bau-dashboard-metrics");if(!T)return;let v=O=>{switch(O){case"PENDING_TL_CREATION":return{text:"Aguardando TL",class:"status-yellow"};case"CREATED":return{text:"Aprovado / Criado",class:"status-green"};case"DISCARDED":return{text:"Descartado pelo TL",class:"status-red"};case"CANCELED_BY_AGENT":return{text:"Cancelado",class:"status-gray"};default:return{text:O||"Pendente",class:"status-gray"}}};if(!S||S.length===0){u&&(u.innerHTML=""),T.innerHTML=`
            ...
            `;return}let h=S.filter(O=>O.status==="PENDING_TL_CREATION").length,N=S.filter(O=>O.status==="CREATED").length;u&&(u.innerHTML=`
            ...
            `),T.innerHTML=S.map(O=>{let X=v(O.status),z=O.date?new Date(O.date).toLocaleDateString("pt-BR"):"";return`
                <li class="bau-case-card" data-case-id="${O.id}">
                    <div class="bau-case-main">
                        <div class="bau-case-icon">${_e.folder}</div>
                        <div class="bau-case-info">
                            <div class="bau-case-header">
                                <h3 class="bau-case-title">${O.advName||"Nome indefinido"}</h3>
                                <span class="bau-case-date">${z}</span>
                            </div>
                            <p class="bau-case-details">CID: ${O.cid||"N/A"} \u2022 Motivo: ${O.reason||"N/A"}</p>
                        </div>
                    </div>
                    <span class="bau-case-status-badge ${X.class}">${X.text}</span>
                </li>
            `}).join("")}function f(){b.querySelectorAll(".bau-step").forEach((S,T)=>{S.classList.toggle("active",T+1===a)}),x.innerHTML="";for(let S=1;S<=n;S++){let T=document.createElement("div");T.className=`bau-progress-step ${S===a?"active":S<a?"completed":""}`,T.textContent=S,x.appendChild(T)}F.style.display=a>1?"inline-block":"none",R.style.display=a<n?"inline-block":"none",U.style.display=a===n?"flex":"none",a===4&&q()}function A(S){let T=i.querySelector(`#bau-step-${S}`);if(!T)return!0;if(S===2)return b.querySelector('textarea[name="reason"]').value.trim()?b.querySelector('input[name="taskType"]:checked')?!0:(K("Erro: Selecione pelo menos uma Task.",{error:!0}),!1):(K("Erro: A descri\xE7\xE3o do que deve ser feito \xE9 obrigat\xF3ria.",{error:!0}),!1);for(let u of T.querySelectorAll("[required]"))if(!u.value.trim())return K(`Erro: O campo '${u.name||u.placeholder}' \xE9 obrigat\xF3rio.`,{error:!0}),!1;return!0}R.onclick=()=>{A(a)&&(a++,f(),G.playClick())},F.onclick=()=>{a>1&&(a--,f(),G.playClick())};async function I(){let S=await we()||{};o=S,["advName","cid","amName","seId"].forEach(v=>{let h=S[v],N=b.querySelector(`[name="${v}"]`),O=i.querySelector(`#wrapper-${v}`);if(N&&(N.value=h&&h!=="N/A"?h:""),O){let X=h&&h!==""&&h!=="N/A"&&h!=="undefined"&&h!=="null";O.style.display=X?"none":"block"}});let u=i.querySelector("#bau-all-data");if(u){let v=[{label:"Anunciante",value:S.advName},{label:"CID",value:S.cid},{label:"AM",value:S.amName},{label:"SE ID",value:S.seId},{label:"Site",value:S.site},{label:"Email",value:S.email},{label:"Timezone",value:S.timezone},{label:"Case ID",value:S.caseId},{label:"Programa",value:S.salesProgram},{label:"Idioma",value:S.language}];u.innerHTML=`
                <div class="bau-context-badges-grid">
                    ${v.filter(h=>h.value&&h.value!=="N/A"&&h.value!=="---"&&h.value!=="undefined"&&h.value!=="null").map(h=>`
                            <div class="bau-context-badge">
                                <span class="bau-badge-label">${h.label}:</span>
                                <span class="bau-badge-value">${h.value}</span>
                            </div>
                        `).join("")}
                </div>
            `}}i.querySelector("#bau-top-se-search").onclick=S=>{S.preventDefault(),st("bau-context-se-id-input")};function q(){let S=new FormData(b),T=Object.fromEntries(S.entries()),u=S.getAll("taskType"),v=i.querySelector("#bau-confirmation-details");if(!v)return;let h=u.length>0?u.join(", "):"Nenhuma";v.innerHTML=`
            ...
            <div class="bau-confirm-row"><span class="bau-confirm-label">Tasks:</span><span class="bau-confirm-value">${h}</span></div>
