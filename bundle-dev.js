...
`))}else D.value.trim()===U.trim()&&(D.value="");W.updateField(M,D.value),D.dispatchEvent(new Event("input"))}}}}function O(l,h){let v=document.createElement("div");if(v.className="cw-actions-section",v.style.cssText=`
        ...
        `;let M=l.parkButton;M.classList.add("js-btn-park"),M.style.cssText="width: 100%; margin: 0; border-radius: 10px; height: 34px; font-weight: 600; font-size: 11.5px;";let U=document.createElement("button");U.className="cw-btn-secondary js-btn-reset",U.textContent=h("limpar"),U.style.cssText=`width: 100%; height: 34px; background: ${P.surface}; color: ${P.textSub}; border: 1px solid ${P.border}; border-radius: 10px; font-weight: 600; cursor: pointer; font-size: 11.5px;`,U.onclick=()=>q();let D=document.createElement("button");D.className="cw-btn-secondary js-btn-copy",D.textContent=h("copiar"),D.style.cssText=`width: 100%; height: 34px; background: ${P.surface}; color: ${P.primary}; border: 1px solid ${P.primary}; border-radius: 10px; font-weight: 600; cursor: pointer; font-size: 11.5px;`,D.onclick=()=>f();let Q=document.createElement("button");return Q.className="cw-btn-primary js-btn-generate",Q.textContent=h("preencher"),Q.style.cssText=`width: 100%; height: 38px; background: ${P.primary}; color: #fff; border: none; border-radius: 10px; font-weight: 600; cursor: pointer; grid-column: span 2; font-size: 12.5px; box-shadow: 0 4px 10px rgba(26, 115, 232, 0.2); margin-top: 0px;`,Q.onclick=()=>A(),v.appendChild(k),v.appendChild(M),v.appendChild(U),v.appendChild(D),v.appendChild(Q),v}async function f(){if(!W.currentSubStatus){K(y("select_substatus"),{error:!0});return}let l=kt(W,r,s);l?(Ze(l),K(y("copiado_sucesso")),j.playClick()):K(y("select_substatus"),{error:!0})}async function A(){if(!W.currentSubStatus){K(y("select_substatus"),{error:!0});return}let l=kt(W,r,s);Ze(l),Z();let h=Je(),v=await mt();if(v){v.focus(),document.execCommand("insertHTML",!1,l),ut(v);let k=document.getElementById("email-automation-checkbox");(!k||k.checked)&&W.currentSubStatus&&$e[W.currentSubStatus]&&await ft($e[W.currentSubStatus]),K(y("inserido_copiado")),j.playSuccess(),q()}h()}function L(){if(W.currentSubStatus){if(W.currentCaseType==="lm")W.removeField("ON_CALL");else{let l=Te[W.currentSubStatus];l&&l.templateFields.includes("ON_CALL")&&W.addFieldAt("ON_CALL",1)}W.isPortugalCase?(W.addFieldAt("CASO_PORTUGAL",1),W.addFieldAt("CONSENTIU_GRAVACAO",2)):(W.removeField("CASO_PORTUGAL"),W.removeField("CONSENTIU_GRAVACAO"))}}function q(){W.reset(),r.reset(),s.reset(),J(),ge.clearEmergency(),o.querySelectorAll("select").forEach(h=>h.value=""),o.querySelector("#sub-status-select").disabled=!0;let l=document.getElementById("email-automation-toggle-row");l&&(l.style.display="none"),b.innerHTML="",c.style.display="none",g.style.display="flex",g.style.opacity="1",m.style.display="none",document.getElementById("manual-task-toggle")&&(document.getElementById("manual-task-toggle").style.display="none"),r.selectionElement.style.display="none",r.screenshotsElement.style.display="none"}async function N(l=!1){let h={};b.querySelectorAll("input, textarea, select").forEach(D=>{(D.id.startsWith("field-")||D.id==="consent-select")&&(h[D.id]=D.value)});let v="Cliente",k="---";if(!l)try{let D=await we();v=D.advertiserName,k=D.cid}catch(D){console.warn("Erro ao coletar pageData:",D)}let M=r.getCheckedElements().map(D=>({key:D.value,count:D.count})),U=M.map(D=>{let Q=Ce[D.key];return Q?Q.name:D.key});return{currentCaseType:W.currentCaseType,currentLang:W.currentLang,isPortugalCase:W.isPortugalCase,consent:W.consent,tagSupportUsed:W.tagSupportUsed,forcedScreenshots:[...W.forcedScreenshots],excludedFields:[...W.excludedFields],activeFields:W.activeFields,status:W.currentStatus,subStatus:W.currentSubStatus,formData:h,activeTasks:M,summaryTags:U,clientName:v,cid:k,timestamp:new Date().toISOString()}}let B=l=>new Promise(h=>setTimeout(h,l));async function Y(l){W.setLanguage(l.currentLang||"pt"),W.setCaseType(l.currentCaseType||"bau"),W.setPortugalCase(l.isPortugalCase||!1),W.setConsent(l.consent||!1),W.setExcludedFields(l.excludedFields||[]),l.activeFields&&W.setActiveFields(l.activeFields);let h=o.querySelector(`#lang-selector button[data-lang="${W.currentLang}"]`);h&&h.classList.add("active"),o.querySelectorAll("#lang-selector button").forEach(M=>{M!==h&&M.classList.remove("active")});let v=o.querySelector(`#type-selector button[data-type="${W.currentCaseType}"]`);v&&v.classList.add("active"),o.querySelectorAll("#type-selector button").forEach(M=>{M!==v&&M.classList.remove("active")});let k=o.querySelector(`#portugal-selector button[data-val="${W.isPortugalCase}"]`);if(k&&k.classList.add("active"),o.querySelectorAll("#portugal-selector button").forEach(M=>{M!==k&&M.classList.remove("active")}),l.status){let M=o.querySelector("#main-status-select");M.value=l.status,W.setStatus(l.status);let U=o.querySelector("#sub-status-select");if(G(l.status,U),await B(50),l.subStatus){if(U.value=l.subStatus,W.setSubStatus(l.subStatus),I(l.subStatus),await B(100),l.tagSupportUsed!==void 0){W.setTagSupportUsed(l.tagSupportUsed);let D=s.element.querySelector('input[value="Sim"]'),Q=s.element.querySelector('input[value="N\xE3o"]');l.tagSupportUsed&&D?D.checked=!0:Q&&(Q.checked=!0),s.element.querySelector("div:last-child").style.display=l.tagSupportUsed?"none":"block"}l.forcedScreenshots&&W.setForcedScreenshots(l.forcedScreenshots);for(let D in l.formData){let Q=document.getElementById(D);Q&&(Q.value=l.formData[D],W.updateField(D,Q.value))}l.activeTasks&&(l.activeTasks.forEach(D=>r.setTaskCount(D.key,D.count)),W.setActiveTasks(r.getCheckedElements()))}}W.isDirty=!1}function y(l){return Se[W.currentLang]?.[l]||Se.pt?.[l]||l}function V(){let l=document.createElement("div");return l.innerHTML='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="16 3 21 3 21 8"></polyline><line x1="4" y1="20" x2="21" y2="3"></line><polyline points="21 16 21 21 16 21"></polyline><line x1="15" y1="15" x2="21" y2="21"></line><line x1="4" y1="4" x2="9" y2="9"></line></svg>',l.style.cssText="width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; border-radius: 50%; cursor: pointer; color: #9AA0A6; transition: all 0.2s;",l.onclick=h=>{h.stopPropagation(),_()},l.title="Alternar para Split & Transfer",l}function X(){let l=document.createElement("div");return l.id="notes-empty-state",l.style.cssText=`
            display: flex;
            flex-direction: column;
            align-items: center;
            justify-content: center;
    ...
    `,document.head.appendChild(p),Object.assign(c.style,me,{width:"850px",height:"650px",display:"none",flexDirection:"column",fontFamily:"'-apple-system', 'BlinkMacSystemFont', 'Segoe UI', 'Roboto', 'Helvetica Neue', 'Arial', sans-serif",borderRadius:"12px",overflow:"hidden"});let d=pe(c,"Email Assistant",t,"Refatora\xE7\xE3o completa do m\xF3dulo de e-mail para uma experi\xEAncia moderna e eficiente.",{popup:c},()=>C()),u=document.createElement("div");Object.assign(u.style,{display:"flex",flex:"1",overflow:"hidden",backgroundColor:r.bgApp});let x=document.createElement("div");Object.assign(x.style,{width:"320px",backgroundColor:"#EFEFF0",borderRight:`1px solid ${r.borderSubtle}`,display:"flex",flexDirection:"column",flexShrink:"0"});let b=document.createElement("div");Object.assign(b.style,{padding:"16px",borderBottom:`1px solid ${r.borderSubtle}`,position:"relative"});let g=document.createElement("input");g.placeholder="Buscar templates...",Object.assign(g.style,{width:"100%",padding:"10px 14px 10px 36px",borderRadius:"10px",border:"1.5px solid transparent",backgroundColor:"#E3E3E8",fontSize:"15px",outline:"none",boxSizing:"border-box",color:r.textPrimary,backgroundImage:`url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="%238A8A8E" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>')`,backgroundRepeat:"no-repeat",backgroundPosition:"12px center",transition:"all 0.2s ease-in-out"}),g.onfocus=()=>{g.style.backgroundColor="#FFFFFF",g.style.borderColor=r.primary,g.style.boxShadow="0 0 0 4px rgba(0, 122, 255, 0.1)",g.style.transform="scale(1.02)"},g.onblur=()=>{g.style.backgroundColor="#E3E3E8",g.style.borderColor="transparent",g.style.boxShadow="none",g.style.transform="scale(1)"};let m=document.createElement("div");m.id="email-template-list",Object.assign(m.style,{flex:"1",overflowY:"auto",padding:"8px",scrollBehavior:"smooth"});let S=document.createElement("div");S.innerHTML="\u2715",Object.assign(S.style,{position:"absolute",right:"26px",top:"50%",transform:"translateY(-50%)",fontSize:"10px",color:"#fff",cursor:"pointer",display:"none",backgroundColor:"#C7C7CC",width:"16px",height:"16px",borderRadius:"50%",textAlign:"center",lineHeight:"16px",fontWeight:"bold"}),S.onclick=()=>{g.value="",n="",S.style.display="none",f(),g.focus()},b.appendChild(g),b.appendChild(S),x.appendChild(b),x.appendChild(m);let E=document.createElement("div");Object.assign(E.style,{flex:"1",display:"flex",flexDirection:"column",overflow:"hidden",backgroundColor:r.bgApp,transition:"all 0.3s cubic-bezier(0.25, 1, 0.5, 1)"});let H=document.createElement("div");Object.assign(H.style,{padding:"20px",borderBottom:`1px solid ${r.borderSubtle}`,backgroundColor:r.bgSurface,maxHeight:"250px",overflowY:"auto",display:"none"});let z=document.createElement("div");Object.assign(z.style,{flex:"1",display:"flex",flexDirection:"column",padding:"20px",backgroundColor:r.bgApp,overflow:"hidden"});let $=document.createElement("div");Object.assign($.style,{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px"});let J=document.createElement("span");J.textContent="Preview do E-mail",Object.assign(J.style,{fontSize:"12px",fontWeight:"600",color:r.textSecondary,textTransform:"uppercase",letterSpacing:"0.5px"});let Z=document.createElement("div");Object.assign(Z.style,{display:"flex",gap:"8px"});let _=(B,Y=!1)=>{let y=document.createElement("button");return y.textContent=B,Object.assign(y.style,{padding:"8px 14px",borderRadius:"10px",border:Y?"none":`1.5px solid ${r.primary}`,background:Y?r.primary:"transparent",color:Y?"#fff":r.primary,fontSize:"13px",fontWeight:"600",cursor:"pointer",transition:"all 0.2s cubic-bezier(0.25, 1, 0.5, 1)",boxShadow:Y?"0 4px 12px rgba(0, 122, 255, 0.3)":"none"}),y.onmouseenter=()=>{Y?(y.style.backgroundColor="#0062CC",y.style.transform="translateY(-1px)",y.style.boxShadow="0 6px 16px rgba(0, 122, 255, 0.4)"):y.style.backgroundColor="rgba(0, 122, 255, 0.05)"},y.onmouseleave=()=>{Y?(y.style.backgroundColor=r.primary,y.style.transform="translateY(0)",y.style.boxShadow="0 4px 12px rgba(0, 122, 255, 0.3)"):(y.style.backgroundColor="transparent",y.style.transform="translateY(0)")},y.onmousedown=()=>y.style.transform="scale(0.94)",y.onmouseup=()=>y.style.transform="scale(1)",y},F=_("Copiar HTML"),R=_("Preencher no CRM",!0),G=_("Smart CR");G.style.borderColor="#E67E22",G.style.color="#E67E22",G.style.display="none",Z.appendChild(G),Z.appendChild(F),Z.appendChild(R),$.appendChild(J),$.appendChild(Z);let I=document.createElement("div");I.contentEditable="true",Object.assign(I.style,{flex:"1",backgroundColor:r.bgSurface,border:`1px solid ${r.borderSubtle}`,borderRadius:"8px",padding:"20px",fontSize:"15px",lineHeight:"1.6",color:r.textPrimary,overflowY:"auto",outline:"none",boxShadow:"inset 0 1px 2px rgba(0,0,0,0.02)"}),z.appendChild($),z.appendChild(I),N(),E.appendChild(H),E.appendChild(z),u.appendChild(x),u.appendChild(E),c.appendChild(d),c.appendChild(u);let T=document.createElement("div");Object.assign(T.style,Ee),c.appendChild(T),Ae(c,T),document.body.appendChild(c);function C(){e=!e,e?(c.style.display="flex",Wt(c),o.length===0&&O()):c.style.display="none",ue(e,c,"cw-btn-email")}async function O(){m.innerHTML='<div style="padding: 20px; text-align: center; color: #5f6368;">Carregando...</div>',o=await xo.getTemplates(),f()}function f(){m.innerHTML="";let B=o.filter(w=>w.name.toLowerCase().includes(n.toLowerCase())||w.category.toLowerCase().includes(n.toLowerCase())),Y=Object.entries($e).filter(([w,l])=>l&&(w.toLowerCase().includes(n.toLowerCase())||l.toLowerCase().includes(n.toLowerCase()))).map(([w,l])=>({id:w,name:w.replace(/_/g," "),category:"\u26A1 Smart CRs",code:l,isSmartCR:!0})),y=xe.getSnippets("email").filter(w=>w.title.toLowerCase().includes(n.toLowerCase())||w.subject&&w.subject.toLowerCase().includes(n.toLowerCase())).map(w=>{let l=[],h=w.content.match(/\[([^\]]+)\]/g);return h&&[...new Set(h)].forEach(v=>{l.push({key:v,label:v.replace("[","").replace("]",""),type:v.toLowerCase().includes("data")?"date":"text",auto:v.toLowerCase().includes("nome")&&v.toLowerCase().includes("seu")?"agentName":null})}),{id:w.id||`snippet-${Math.random()}`,name:w.title,category:"\u{1F464} Pessoal",subject:w.subject||"Sem Assunto",template:w.content,placeholders:l}}),V=[...B,...Y,...y];if(V.length===0){m.innerHTML=`
                ...
                </div>`):H.style.display="none";return}let B=a.placeholders&&a.placeholders.length>0;if(H.style.display=B?"block":"none",!B)return;let Y=document.createElement("div");Object.assign(Y.style,{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"16px"}),(a.placeholders||[]).forEach(y=>{let V=document.createElement("div"),X=document.createElement("label");X.textContent=y.label,Object.assign(X.style,{display:"block",fontSize:"11px",fontWeight:"700",color:r.textSecondary,marginBottom:"8px",textTransform:"uppercase",letterSpacing:"0.5px"});let w=document.createElement("input");w.type=y.type||"text",w.dataset.key=y.key,Object.assign(w.style,{width:"100%",padding:"10px 12px",borderRadius:"8px",border:`1.5px solid ${r.borderSubtle}`,backgroundColor:"#FBFBFD",fontSize:"14px",boxSizing:"border-box",transition:"all 0.2s ease",outline:"none"}),w.onfocus=()=>{w.style.borderColor=r.primary,w.style.backgroundColor="#FFFFFF",w.style.boxShadow="0 0 0 4px rgba(0, 122, 255, 0.1)"},w.onblur=()=>{w.style.borderColor=r.borderSubtle,w.style.backgroundColor="#FBFBFD",w.style.boxShadow="none"},y.auto==="agentName"&&(w.value=Ye().split(" ")[0]),w.addEventListener("input",N),V.appendChild(X),V.appendChild(w),Y.appendChild(V)}),H.appendChild(Y)}function N(){if(!a){I.innerHTML=`
...
`),l=/\d{1,2}\/\d{1,2}/,h="\u{1F4C5}";if(w.forEach(D=>{/🇧🇷|🇵🇹|PT|BR|BRASIL|BRAZIL|PORTUGAL|LISBOA/i.test(D)?h="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM|ESPANHA|SPAIN|MEXICO|MÉXICO/i.test(D)&&(h="\u{1F1EA}\u{1F1F8}");let Q=D.match(l);if(Q){let oe=Q[0],ee=h;/🇧🇷|🇵🇹|PT|BR/i.test(D)?ee="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM/i.test(D)&&(ee="\u{1F1EA}\u{1F1F8}"),X.some(te=>te.flag===ee&&te.date===oe)||X.push({flag:ee,date:oe})}}),X.length===0){let D=(y.text||"").match(/\d{1,2}\/\d{1,2}/g);D&&[...new Set(D)].forEach(Q=>X.push({flag:"\u{1F4C5}",date:Q}))}let v="",k='<button id="cw-bau-toggle-btn" class="cw-btn-interactive" style="background:rgba(255,255,255,0.7); border:1px solid rgba(139, 92, 246, 0.4); border-radius:12px; padding:8px 12px; color:#6D28D9; font-size:12px; font-weight:600;">Detalhes</button>';window._cwIsAdmin&&(k=`
  ...
  `,document.head.appendChild(t)};var _e={add:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>',back:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z"/></svg>',wand:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M7.5 5.6L10 7 8.6 4.5 10 2 7.5 3.4 5 2l1.4 2.5L5 7zm12 9.8L17 14l1.4 2.5L17 19l2.5-1.4L22 19l-1.4-2.5L22 14zM22 2l-2.5 1.4L17 2l1.4 2.5L17 7l2.5-1.4L22 7l-1.4-2.5zm-7.63 5.29c-.39-.39-1.02-.39-1.41 0L1.29 18.96c-.39.39-.39 1.02 0 1.41l2.34 2.34c.39.39 1.02.39 1.41 0L16.7 11.05c.39-.39.39-1.02 0-1.41l-2.33-2.35zm-1.03 5.41l-2.12-2.12 2.44-2.44 2.12 2.12-2.44 2.44z"/></svg>',send:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/></svg>',check:'<svg width="64" height="64" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>',folder:'<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M10 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2h-8l-2-2z"/></svg>',empty:'<svg width="64" height="64" viewBox="0 0 24 24" fill="currentColor"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5v-3h3.56c.69 1.19 1.97 2 3.44 2s2.75-.81 3.44-2H19v3zm0-5h-4.99c0 1.1-.9 2-2 2s-2-.9-2-2H5V5h14v9z"/></svg>'};function Mo(){_o();let t=!1,e="dashboard",o=null,a=1,n=4,i=document.createElement("div");i.id="bau-form-popup",i.className="bau-popup cw-module-window",i.style.display="none";let s=pe(i,"BAU Central","v2.0.0","Dashboard de Casos BAU",{},()=>X());i.appendChild(s);let r=document.createElement("div");r.className="bau-view-container",i.appendChild(r);let c=document.createElement("div");c.id="bau-view-dashboard",c.className="bau-view active",c.innerHTML=`
    ...
    `,J.appendChild(Z),b.appendChild(J);let _=document.createElement("div");_.className="bau-step",_.id="bau-step-4",_.innerHTML=`
        <div class="bau-card">
