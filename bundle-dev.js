...
`+r;this.value=n.substring(0,o)+l+n.substring(a),this.selectionStart=this.selectionEnd=o+l.length}else if(e.key==="Tab")e.preventDefault(),e.shiftKey?s.startsWith("  ")&&(this.value=n.substring(0,i)+s.substring(2)+n.substring(o),this.selectionStart=this.selectionEnd=o-2):(this.value=n.substring(0,i)+"  "+s+n.substring(o),this.selectionStart=this.selectionEnd=o+2);else if(e.key==="Backspace"&&o===a&&o>0){let r=n.substring(0,o);r.endsWith("\u2022 ")?(e.preventDefault(),this.value=r.substring(0,o-2)+n.substring(a),this.selectionStart=this.selectionEnd=o-2):r.endsWith("  ")&&s.trim().startsWith("\u2022")&&(e.preventDefault(),this.value=r.substring(0,o-2)+n.substring(a),this.selectionStart=this.selectionEnd=o-2)}}))}function At(t,e,o){if(e.innerHTML="",!!ke[t]&&(o.activeFields.forEach(n=>{if(["TAGS_IMPLEMENTED","SCREENSHOTS_LIST","CONSENTIU_GRAVACAO","CASO_PORTUGAL","label_substatus"].includes(n))return;let i=`field-${n}`,s=document.createElement("label"),r=p=>Se[o.currentLang]?.[p]||Se.pt?.[p]||p;s.textContent=r(n.toLowerCase())!==n.toLowerCase()?r(n.toLowerCase()):n.replace(/_/g," ").replace(/\b\w/g,p=>p.toUpperCase())+":",Object.assign(s.style,{display:"flex",alignItems:"center",justifyContent:"space-between",fontSize:"13px",fontWeight:"700",color:R.textSub,marginBottom:"8px",marginTop:"24px",textTransform:"uppercase",letterSpacing:"0.5px"});let l=document.createElement("span");if(l.textContent=s.textContent,s.innerHTML="",s.appendChild(l),n==="SPEAKEASY_ID"){let p=document.createElement("button");p.innerHTML="\u2728 Auto Busca",p.style.cssText=`font-size: 11px; font-weight: 700; color: ${R.primary}; background-color: ${R.primaryBg}; border: none; border-radius: ${de.pill}; padding: 6px 14px; margin-left: 10px; cursor: pointer; transition: all 0.2s ${se};`,p.onmouseenter=()=>p.style.backgroundColor="#d2e3fc",p.onmouseleave=()=>p.style.backgroundColor=R.primaryBg,p.onclick=x=>{x.preventDefault(),Qt(i)},s.appendChild(p)}let d=document.createElement("button");d.innerHTML="\u2715",d.style.cssText=`font-size: 14px; background: ${R.bgInput}; border: none; color: ${R.textSub}; cursor: pointer; width: 24px; height: 24px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-left: auto; transition: all 0.2s ${se};`,d.onmouseenter=()=>{d.style.background=R.error,d.style.color=R.surface},d.onmouseleave=()=>{d.style.background=R.bgInput,d.style.color=R.textSub},d.onclick=async p=>{p.preventDefault(),await be(`Tem certeza que deseja remover o campo "${l.textContent.replace(":","")}"?`)&&(o.removeField(n),At(t,e,o))},s.appendChild(d);let c;He.includes(n)?(c=document.createElement("textarea"),c.classList.add("bullet-textarea","cw-textarea"),c.placeholder="Utilize marcadores para detalhar...",eo(c)):st.includes(n)?(c=document.createElement("textarea"),c.classList.add("cw-textarea"),c.placeholder="Descreva as considera\xE7\xF5es..."):(c=document.createElement("input"),c.type="text",c.classList.add("cw-input")),c.id=i,c.value=o.formData[i]||"",c.addEventListener("input",p=>o.updateField(i,p.target.value)),e.appendChild(s),e.appendChild(c)}),o.activeFields.includes("CONSENTIU_GRAVACAO"))){let n=r=>Se[o.currentLang]?.[r]||Se.pt?.[r]||r,i=document.createElement("label");i.textContent=n("consentiu_gravacao"),Object.assign(i.style,{display:"block",fontSize:"13px",fontWeight:"700",color:R.textSub,marginBottom:"8px",marginTop:"24px",textTransform:"uppercase",letterSpacing:"0.5px"});let s=document.createElement("select");s.className="cw-select",s.innerHTML=`
        ...
        `,s.value=o.consent?"true":"false",s.onchange=()=>o.setConsent(s.value==="true"),e.appendChild(i),e.appendChild(s)}}function Tt(t,e,o){let a=t.currentSubStatus;if(!a)return null;let n=ke[a],i=Se[t.currentLang]||Se.pt,s=d=>i[d]||Se.pt?.[d]||d,r='style="margin-bottom: 12px; padding-left: 30px;"',l="";if(t.activeFields.forEach(d=>{let c=s(d.toLowerCase()),p="N/A";if(d==="label_substatus")c=s("label_substatus"),p=n.name;else if(d==="TAGS_IMPLEMENTED"){c=s("tags_implemented");let x=[];e.getCheckedElements().forEach(g=>{let u=g.value,y=Ce[u],w=g.count||1,N=u==="ads_conversion_tracking"||u==="ads_enhanced_conversions";t.tagSupportUsed&&N&&!t.forcedScreenshots.has(u)?x.push(`${y.name} - ${s("ts_output_disclaimer")}`):x.push(w>1?`${y.name} (x${w})`:y.name)}),p=x.join(", ")||"N/A"}else if(d==="SCREENSHOTS_LIST"){c=s("screenshots_list");let x="",b=e.screenshotsElement;b&&Array.from(b.querySelectorAll('input[id^="name-"]')).forEach(u=>{let y=u.value,w=u.closest(".cw-screen-card");if(w){let N=w.querySelectorAll('input[id^="screen-"]'),z=!1,_="";N.forEach(Z=>{let T=Z.closest(".cw-input-group")?.querySelector(".cw-input-label")?.textContent||"Evid\xEAncia",A=Z.value.trim();A&&(_+=`<li>${T} - ${A}</li>`,z=!0)}),z&&(x+=`<div style="margin-bottom: 8px;"><b>${y}</b><ul ${r}>${_}</ul></div>`)}}),p=x||"N/A"}else if(d==="CASO_PORTUGAL")c=s("caso_portugal"),p=s("sim");else if(d==="CONSENTIU_GRAVACAO")c=s("consentiu_gravacao"),p=t.consent?s("sim"):s("nao");else{let x=`field-${d}`,b=t.formData[x],g="";if(n.fieldPrefixes&&n.fieldPrefixes[d]&&(g=n.fieldPrefixes[d]+" "),b&&b.trim()!==""&&b.trim()!=="\u2022"){let u=b.trim();if(He.includes(d)){let y=u.split(`
...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
    ...
    `;let x=d.querySelector(".cw-hero-grid"),b=d.querySelector(".cw-acc-container"),g=d.querySelector(".cw-results-container"),u=d.querySelector(".cw-search-input"),y=d.querySelector(".cw-status-bar"),w=d.querySelector(".cw-status-text"),N=d.querySelector(".cw-footer-icons");s.forEach(([T,A])=>{let q=i(A.name),M=document.createElement("div");M.className="cw-hero-card",M.id=`hero-${T}`,M.style.setProperty("--hero-color",q.color),M.innerHTML=`
        ...
        `,M.onclick=L=>{if(L.target.closest(".cw-step-btn"))return;let k=a[T]?a[T].count:0;_(T,k>0?-k:1,A)},M.querySelector(".minus").onclick=()=>_(T,-1,A),M.querySelector(".plus").onclick=()=>_(T,1,A),M.dataset.color=q.color,x.appendChild(M)});function z(T,A){let q=i(A.name),M=document.createElement("div");return M.className="cw-task-item",M.dataset.id=T,M.innerHTML=`
  ...
  `,u.onfocus=()=>{b.style.boxShadow="0 4px 12px rgba(26,115,232,0.15)",b.style.border="1px solid #1a73e8"},u.onblur=()=>{b.style.boxShadow="0 2px 6px rgba(0,0,0,0.04)",b.style.border="1px solid transparent"},b.appendChild(g),b.appendChild(u),x.appendChild(b),p.appendChild(x);let y=document.createElement("div");y.style.cssText="flex: 1; overflow-y: auto; padding: 0 24px 40px 24px; scroll-behavior: smooth;",p.appendChild(y);let w=null;function N(){if(w)return;w=document.createElement("div"),w.style.cssText=`
      ...
      `,f.title=v,f.dataset.key=k;let E=document.createElement("div");E.style.cssText="width: 24px; height: 24px; margin-bottom: 2px; transition: transform 0.2s;",E.innerHTML=F||Be.tasks;let O=document.createElement("div");return O.style.cssText="font-size: 9px; font-weight: 600; opacity: 0.7; letter-spacing: 0.3px;",O.textContent=v,f.appendChild(E),f.appendChild(O),f.onmouseenter=()=>{a!==k&&!(k==="history"&&n)&&(f.style.background="#F1F3F4",E.style.transform="scale(1.1)")},f.onmouseleave=()=>{a!==k&&!(k==="history"&&n)&&(f.style.background="transparent",E.style.transform="scale(1)")},f}function T(k){let v=document.getElementById(`cat-anchor-${k}`);v&&(v.scrollIntoView({behavior:"smooth",block:"start"}),a=k,A())}function A(){Object.keys(et).forEach(v=>{let F=c.querySelector(`#cw-sidebar-btn-${v}`);if(F)if(a===v&&!n){let f=ht[v];F.style.background=f.bg,F.style.color=f.color,F.querySelector("div:first-child").style.transform="scale(1.1)"}else F.style.background="transparent",F.style.color=i.textSecondary,F.querySelector("div:first-child").style.transform="scale(1)"});let k=c.querySelector("#cw-sidebar-btn-history");k&&(n?(k.style.background="#3C4043",k.style.color="#FFFFFF"):(k.style.background="transparent",k.style.color=i.textSecondary))}function q(){if(y.innerHTML="",e.trim()!==""){let v=[];if(Object.entries(et).forEach(([f,E])=>{let O=E.links.filter(I=>I.name.toLowerCase().includes(e.toLowerCase())||I.desc.toLowerCase().includes(e.toLowerCase()));v.push(...O.map(I=>({...I,_cat:f})))}),v.length===0){y.innerHTML='<div style="text-align:center; padding: 60px; color:#999; font-size:13px;">Nada encontrado.</div>';return}let F=document.createElement("div");F.innerHTML="Resultados da busca",F.style.cssText="font-size:12px; font-weight:700; color:#5f6368; margin:20px 0 10px; text-transform:uppercase; letter-spacing:0.5px;",y.appendChild(F),v.forEach(f=>{let E=M(f,Be[f._cat],!1,f._cat);y.appendChild(E)});return}Object.entries(et).forEach(([v,F])=>{let f=ht[v],E=document.createElement("div"),O=document.createElement("div");O.id=`cat-anchor-${v}`,O.style.cssText=`
          ...
          `,O.innerHTML=`
            ...
            ${F.label}
          `,E.appendChild(O);let I=document.createElement("div");I.style.cssText="display: grid; grid-template-columns: 1fr; gap: 8px;",F.links.forEach(D=>{let B=M(D,Be[v],!1,v);I.appendChild(B)}),E.appendChild(I),y.appendChild(E)});let k=document.createElement("div");k.style.height="80px",y.appendChild(k)}function M(k,v,F,f){let E=document.createElement("div"),O=ht[f]||ht.history;E.style.cssText=`
...
`),m=/\d{1,2}\/\d{1,2}/,S="\u{1F4C5}";if(W.forEach(H=>{/🇧🇷|🇵🇹|PT|BR|BRASIL|BRAZIL|PORTUGAL|LISBOA/i.test(H)?S="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM|ESPANHA|SPAIN|MEXICO|MÉXICO/i.test(H)&&(S="\u{1F1EA}\u{1F1F8}");let ee=H.match(m);if(ee){let oe=ee[0],Q=S;/🇧🇷|🇵🇹|PT|BR/i.test(H)?Q="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM/i.test(H)&&(Q="\u{1F1EA}\u{1F1F8}"),X.some(te=>te.flag===Q&&te.date===oe)||X.push({flag:Q,date:oe})}}),X.length===0){let H=(h.text||"").match(/\d{1,2}\/\d{1,2}/g);H&&[...new Set(H)].forEach(ee=>X.push({flag:"\u{1F4C5}",date:ee}))}let C="",$='<button id="cw-bau-toggle-btn" class="cw-btn-interactive" style="background:rgba(255,255,255,0.7); border:1px solid rgba(139, 92, 246, 0.4); border-radius:12px; padding:8px 12px; color:#6D28D9; font-size:12px; font-weight:600;">Detalhes</button>';window._cwIsAdmin&&($=`
        ...
        `,document.head.appendChild(v)}let i={bg:"#F0F2F5",surface:"#FFFFFF",primary:"#007AFF",primaryBg:"rgba(0, 122, 255, 0.1)",text:"#1C1C1E",textSub:"#8E8E93",border:"rgba(0, 0, 0, 0.08)",danger:"#FF3B30"},s={container:{display:"flex",flexDirection:"column",height:"100%",background:i.bg,fontFamily:"'Google Sans', Roboto, sans-serif"},tabHeader:{display:"flex",padding:"12px 16px 0 16px",background:i.surface,borderBottom:`1px solid ${i.border}`},tabBtn:{flex:1,padding:"12px",textAlign:"center",cursor:"pointer",fontSize:"13px",fontWeight:"500",color:i.textSub,borderBottom:"3px solid transparent",transition:"all 0.3s cubic-bezier(0.4, 0, 0.2, 1)",userSelect:"none"},tabActive:{color:i.primary,borderBottomColor:i.primary,fontWeight:"600"},listContainer:{flex:1,overflowY:"auto",padding:"16px",display:"flex",flexDirection:"column",gap:"12px"},emptyState:{padding:"40px 20px",textAlign:"center",color:"#BDC1C6",display:"flex",flexDirection:"column",alignItems:"center",gap:"10px"},card:{background:i.surface,borderRadius:"16px",padding:"16px",border:`1px solid ${i.border}`,boxShadow:"0 4px 12px rgba(0,0,0,0.05)",transition:"all 0.4s cubic-bezier(0.165, 0.84, 0.44, 1)",cursor:"default",position:"relative",overflow:"hidden"},cardHeader:{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"8px"},cardTitle:{fontSize:"14px",fontWeight:"600",color:i.text},cardPreview:{fontSize:"12px",color:i.textSub,lineHeight:"1.5",display:"-webkit-box",webkitLineClamp:"3",webkitBoxOrient:"vertical",overflow:"hidden"},cardActions:{display:"flex",justifyContent:"flex-end",gap:"8px",marginTop:"12px",paddingTop:"12px",borderTop:`1px dashed ${i.border}`},actionBtn:{padding:"6px 12px",borderRadius:"6px",fontSize:"12px",fontWeight:"600",cursor:"pointer",border:"none",background:"transparent",transition:"background 0.2s"},fab:{position:"absolute",bottom:"24px",right:"24px",width:"56px",height:"56px",borderRadius:"16px",background:i.primary,color:"#FFF",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 12px rgba(26, 115, 232, 0.4)",cursor:"pointer",transition:"transform 0.2s",zIndex:10},editorOverlay:{position:"absolute",top:0,left:0,width:"100%",height:"100%",background:"rgba(255, 255, 255, 0.85)",backdropFilter:"blur(25px) saturate(180%)",webkitBackdropFilter:"blur(25px) saturate(180%)",zIndex:20,transform:"translateY(100%)",transition:"transform 0.5s cubic-bezier(0.34, 1.56, 0.64, 1)",display:"flex",flexDirection:"column"},editorHeader:{padding:"16px 24px",background:i.surface,borderBottom:`1px solid ${i.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"},editorBody:{flex:1,padding:"24px",overflowY:"auto"},inputGroup:{marginBottom:"20px"},label:{display:"block",fontSize:"12px",fontWeight:"700",color:i.textSub,marginBottom:"8px",textTransform:"uppercase"},input:{width:"100%",padding:"12px",borderRadius:"8px",border:`1px solid ${i.border}`,fontSize:"14px",fontFamily:"inherit",outline:"none",background:i.surface,boxSizing:"border-box"}},r=document.createElement("div");r.id="library-popup",r.classList.add("cw-module-window"),Object.assign(r.style,me,{right:"auto",left:"50%",width:"400px",height:"600px",transform:"translateX(-50%) scale(0.05)",overflow:"hidden"});let d=pe(r,"Minha Biblioteca",t,"Gerencie seus snippets, textos e templates.",{popup:r},()=>k());r.appendChild(d);let c=document.createElement("div");Object.assign(c.style,s.container),r.appendChild(c);let p=document.createElement("div");Object.assign(p.style,s.tabHeader);let x=[{id:"general",label:"Geral",icon:"\u{1F4CB}"},{id:"note",label:"Notas",icon:"\u{1F4DD}"},{id:"email",label:"Emails",icon:"\u{1F4E7}"}];x.forEach(v=>{let F=document.createElement("div");F.innerHTML=`${v.icon} ${v.label}`,F.id=`lib-tab-${v.id}`,Object.assign(F.style,s.tabBtn),v.id===o&&Object.assign(F.style,s.tabActive),F.onmouseenter=()=>G.playHover(),F.onclick=()=>Z(v.id),p.appendChild(F)}),c.appendChild(p);let b=document.createElement("div");Object.assign(b.style,s.listContainer),c.appendChild(b);let g=document.createElement("div");g.className="cw-fab cw-tactile",Object.assign(g.style,s.fab),g.innerHTML='<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>',g.onmouseenter=()=>g.style.transform="scale(1.1)",g.onmouseleave=()=>g.style.transform="scale(1)",g.onclick=()=>T(),c.appendChild(g),a=document.createElement("div"),Object.assign(a.style,s.editorOverlay);let u=document.createElement("div");Object.assign(u.style,s.editorHeader),u.innerHTML='<span style="font-weight:700; font-size:16px;">Novo Item</span>';let y=document.createElement("button");y.innerHTML="Cancelar",y.style.cssText="background:none; border:none; color:#5f6368; font-weight:600; cursor:pointer;",y.onclick=A,u.appendChild(y),a.appendChild(u);let w=document.createElement("div");Object.assign(w.style,s.editorBody),a.appendChild(w);let N=document.createElement("div");N.style.cssText="padding:16px 24px; border-top:1px solid #DADCE0; background:#FFF; display:flex; justify-content:flex-end;";let z=document.createElement("button");z.textContent="Salvar",z.style.cssText="padding:10px 24px; background:#1a73e8; color:white; border:none; border-radius:20px; font-weight:600; cursor:pointer; box-shadow:0 2px 5px rgba(26,115,232,0.3);",z.onclick=q,N.appendChild(z),a.appendChild(N),c.appendChild(a);let _=document.createElement("div");Object.assign(_.style,Ae),_.className="no-drag",r.appendChild(_),Te(r,_),document.body.appendChild(r);function Z(v){G.playClick(),o=v,x.forEach(F=>{let f=document.getElementById(`lib-tab-${F.id}`);F.id===v?Object.assign(f.style,s.tabActive):Object.assign(f.style,s.tabBtn)}),K()}function K(){b.innerHTML="";let v=he.getSnippets(o);if(v.length===0){b.innerHTML=`
            ...
            `;return}v.forEach(F=>{let f=document.createElement("div");f.className="cw-lib-card",Object.assign(f.style,s.card),F.isCode&&(f.style.borderLeft=`4px solid ${i.primary}`,f.style.background="rgba(0, 122, 255, 0.02)");let E=F.content;if(F.isRich){let O=document.createElement("div");O.innerHTML=F.content;let I=O.querySelector("img");E=O.innerText.substring(0,150)+(O.innerText.length>150?"...":""),I&&(E="\u{1F5BC}\uFE0F [Cont\xE9m Imagens] "+E)}f.innerHTML=`
                ...
                <div style="${L(s.cardPreview)}; ${F.isCode?"font-family:'Roboto Mono', monospace; font-size:11px;":""}">${E}</div>
            ...
            `,f.onmouseenter=()=>{G.playHover()},f.querySelector(".cw-act-copy").onclick=O=>{if(O.stopPropagation(),G.playClick(),F.isRich){let I=new Blob([F.content],{type:"text/html"}),D=document.createElement("div");D.style.whiteSpace="pre-wrap",D.innerHTML=F.content;let B=new Blob([D.innerText],{type:"text/plain"}),U=[new ClipboardItem({"text/html":I,"text/plain":B})];navigator.clipboard.write(U)}else navigator.clipboard.writeText(F.content);Y("Copiado!")},f.querySelector(".cw-act-edit").onclick=O=>{O.stopPropagation(),G.playClick(),T(F)},f.querySelector(".cw-act-del").onclick=async O=>{O.stopPropagation(),G.playClick(),await be("Excluir este item?")&&(he.delete(F.id),K(),Y("Item exclu\xEDdo."))},b.appendChild(f)})}function T(v=null){n=v?v.id:null,w.innerHTML="",w.appendChild(M("title","T\xEDtulo / Nome",v?v.title:"")),o==="email"&&w.appendChild(M("subject","Assunto do Email",v?v.subject:""));let F="Conte\xFAdo";o==="email"&&(F="Corpo do Email (HTML)"),o==="note"&&(F="Texto da Nota (Reason)"),w.appendChild(M("content",F,v?v.content:"",{isRich:!0,isCode:v?v.isCode:!1})),u.querySelector("span").textContent=v?"Editar Item":"Novo Item",a.style.transform="translateY(0)",setTimeout(()=>{let f=w.querySelector("input");f&&f.focus()},300)}function A(){a.style.transform="translateY(100%)",setTimeout(()=>n=null,300)}async function q(){let v=w.querySelector("#cw-inp-title"),F=w.querySelector("#cw-inp-content"),f=v.value.trim(),E=F.contentEditable==="true"?F.innerHTML:F.value.trim(),O=F.getAttribute("data-is-code")==="true";if(!f||!E||E==="<br>"){Y("Preencha t\xEDtulo e conte\xFAdo.",{error:!0});return}let I={id:n,type:o,title:f,content:E,isCode:O,isRich:F.contentEditable==="true"};if(o==="email"){let D=w.querySelector("#cw-inp-subject").value.trim();if(!D){Y("Assunto \xE9 obrigat\xF3rio para emails.",{error:!0});return}I.subject=D}z.textContent="Salvando...",await he.save(I),z.textContent="Salvar",A(),K(),Y("Salvo com sucesso!"),G.playSuccess()}function M(v,F,f,E={}){let O=document.createElement("div");Object.assign(O.style,s.inputGroup);let I=document.createElement("label");I.textContent=F,Object.assign(I.style,s.label);let D;if(E.isRich){let B=document.createElement("div");B.style.cssText="display:flex; gap:6px; margin-bottom:12px; background:rgba(241, 243, 244, 0.8); padding:6px; border-radius:12px; border:1px solid #DADCE0; backdrop-filter: blur(10px);",B.innerHTML=`
            ...
            `,D=document.createElement("div"),D.contentEditable="true",Object.assign(D.style,s.input,{minHeight:"180px",maxHeight:"350px",overflowY:"auto",whiteSpace:"pre-wrap",lineHeight:"1.6",outline:"none"}),D.innerHTML=f||"",E.isCode&&(D.style.fontFamily="'Roboto Mono', monospace",D.style.backgroundColor="#F8F9FA",D.setAttribute("data-is-code","true")),B.querySelectorAll(".cw-toolbar-btn").forEach(U=>{U.onmouseenter=()=>G.playHover(),U.onmousedown=()=>G.playClick()}),B.querySelector(".cw-tb-bold").onclick=()=>{document.execCommand("bold"),D.focus()},B.querySelector(".cw-tb-italic").onclick=()=>{document.execCommand("italic"),D.focus()},B.querySelector(".cw-tb-code").onclick=U=>{let V=!(D.getAttribute("data-is-code")==="true");D.setAttribute("data-is-code",V),D.style.fontFamily=V?"'Roboto Mono', monospace":"inherit",D.style.backgroundColor=V?"rgba(0, 122, 255, 0.03)":i.surface,V?U.currentTarget.classList.add("active"):U.currentTarget.classList.remove("active"),D.focus()},B.querySelector(".cw-tb-img").onclick=async()=>{let U=await Yt("Cole a URL da imagem:");U&&(document.execCommand("insertImage",!1,U),D.querySelectorAll("img").forEach(V=>{V.style.maxWidth="100%",V.style.borderRadius="8px"}))},D.onpaste=U=>{let h=(U.clipboardData||U.originalEvent.clipboardData).items;for(let V of h)if(V.kind==="file"&&V.type.startsWith("image/")){U.preventDefault();let X=V.getAsFile(),W=new FileReader;W.onload=m=>{let S=`<img src="${m.target.result}" style="max-width:100%; border-radius:8px; margin:8px 0; display:block;">`;document.execCommand("insertHTML",!1,S)},W.readAsDataURL(X)}},O.appendChild(I),O.appendChild(B)}else D=document.createElement("input"),D.type="text",Object.assign(D.style,s.input),D.value=f||"",O.appendChild(I);return D.id=`cw-inp-${v}`,D.onfocus=()=>{D.style.borderColor=i.primary,D.style.boxShadow=`0 0 0 2px ${i.primaryBg}`},D.onblur=()=>{D.style.borderColor=i.border,D.style.boxShadow="none"},O.appendChild(D),O}function L(v){return Object.entries(v).map(([F,f])=>`${F.replace(/[A-Z]/g,E=>"-"+E.toLowerCase())}:${f}`).join(";")}function k(){e=!e,ue(e,r,"cw-btn-library"),e&&K()}return k}function Io(){let t="v1.0",e=!1,o={bg:"#F8F9FA",surface:"#FFFFFF",primary:"#1A73E8",text:"#202124",textSub:"#5F6368",border:"#DADCE0"},a="cw-configs-styles";if(!document.getElementById(a)){let x=document.createElement("style");x.id=a,x.innerHTML=`
