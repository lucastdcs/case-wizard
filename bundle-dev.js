...
`)+J:X.value=J}else X.value=J;X.dispatchEvent(new Event("input"))}}}}function q(g,h){let I=document.createElement("div");if(I.className="cw-actions-section",I.style.cssText=`
        ...
        `;let M=A.querySelector("#csa-back-btn");M.onmouseenter=()=>M.style.background="#f1f3f4",M.onmouseleave=()=>M.style.background="none",M.onclick=T;let V=A.querySelector("#csa-insert-btn");V.onmouseenter=()=>{V.style.transform="translateY(-1px)",V.style.boxShadow="0 6px 16px rgba(26,115,232,0.4)"},V.onmouseleave=()=>{V.style.transform="translateY(0)",V.style.boxShadow="0 4px 12px rgba(26,115,232,0.3)"},V.onclick=()=>{V.style.transform="scale(0.96)",q(L,"email"),setTimeout(()=>{V.style.transform="scale(1)",T()},300)}}function T(){i="list",$.style.transform="translateX(0)"}function x(L,E,z=null){let M=document.createElement("button"),V=z?`<span style="margin-right:6px; font-size:14px; opacity:0.9;">${z}</span>`:"";return M.innerHTML=`${V}${L}`,Object.assign(M.style,b),o===E&&n===""?Object.assign(M.style,p):(M.onmouseenter=()=>{M.style.background="#F1F3F4",M.style.borderColor="#DADCE0"},M.onmouseleave=()=>{M.style.background="#FFFFFF",M.style.borderColor="#DADCE0"}),M.onclick=()=>{o=E,n="",_.value="",S(),O()},M}function S(){G.innerHTML="",G.appendChild(x("Smart CRs",e,"\u26A1")),G.appendChild(x("Pessoal","PERSONAL_LIBRARY","\u{1F464}")),Object.keys(_e).forEach(L=>{G.appendChild(x(_e[L].title,L))})}function O(){N.innerHTML="";let L=[];if(n.trim()!==""){let g=n.toLowerCase();Object.values(_e).forEach(h=>{h.emails.forEach(I=>{(I.name.toLowerCase().includes(g)||I.subject.toLowerCase().includes(g))&&L.push({type:"email",data:I})})}),me.getSnippets("email").forEach(h=>{(h.title.toLowerCase().includes(g)||h.subject&&h.subject.toLowerCase().includes(g))&&L.push({type:"email",data:{name:h.title,subject:h.subject||"Sem Assunto",body:h.content}})}),Object.entries(qe).forEach(([h,I])=>{if(!I)return;(h.replace(/_/g," ").toLowerCase().includes(g)||I.toLowerCase().includes(g))&&L.push({type:"cr",key:h,code:I})})}else o===e?Object.entries(qe).forEach(([g,h])=>{h&&L.push({type:"cr",key:g,code:h})}):o==="PERSONAL_LIBRARY"?me.getSnippets("email").forEach(g=>{L.push({type:"email",data:{name:g.title,subject:g.subject||"Sem Assunto",body:g.content}})}):_e[o]&&_e[o].emails.forEach(g=>{L.push({type:"email",data:g})});if(L.length===0){N.innerHTML=`
