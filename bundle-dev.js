...
`),G=/\d{1,2}\/\d{1,2}/,A="\u{1F4C5}";if(B.forEach(D=>{/🇧🇷|🇵🇹|PT|BR|BRASIL|BRAZIL|PORTUGAL|LISBOA/i.test(D)?A="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM|ESPANHA|SPAIN|MEXICO|MÉXICO/i.test(D)&&(A="\u{1F1EA}\u{1F1F8}");let Y=D.match(G);if(Y){let Q=Y[0],j=A;/🇧🇷|🇵🇹|PT|BR/i.test(D)?j="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM/i.test(D)&&(j="\u{1F1EA}\u{1F1F8}"),_.some(U=>U.flag===j&&U.date===Q)||_.push({flag:j,date:Q})}}),_.length===0){let D=(S.text||"").match(/\d{1,2}\/\d{1,2}/g);D&&[...new Set(D)].forEach(Y=>_.push({flag:"\u{1F4C5}",date:Y}))}let K="",ae='<button id="cw-bau-toggle-btn" class="cw-btn-interactive" style="background:rgba(255,255,255,0.7); border:1px solid rgba(139, 92, 246, 0.4); border-radius:12px; padding:8px 12px; color:#6D28D9; font-size:12px; font-weight:600;">Detalhes</button>';window._cwIsAdmin&&(ae=`
    ...
    .bau-success-content::before {
        ...
        z-index: -1;
        opacity: 0;
        animation: bauAuraEntry 1.2s ${Ge} 0.2s forwards, bauAuraPulse 4s ease-in-out 1.4s infinite alternate;
    }
