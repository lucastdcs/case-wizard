...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
    ...
    `;let h=l.querySelector(".cw-hero-grid"),b=l.querySelector(".cw-acc-container"),g=l.querySelector(".cw-results-container"),u=l.querySelector(".cw-search-input"),w=l.querySelector(".cw-status-bar"),E=l.querySelector(".cw-status-text"),R=l.querySelector(".cw-footer-icons");r.forEach(([M,O])=>{let z=i(O.name),V=document.createElement("div");V.className="cw-hero-card",V.id=`hero-${M}`,V.style.setProperty("--hero-color",z.color),V.innerHTML=`
        ...
        `,V.onclick=()=>{b.querySelectorAll(".cw-acc-group.open").forEach(k=>{k!==z&&k.classList.remove("open")}),z.classList.toggle("open")};let F=document.createElement("div");F.className="cw-acc-body",O.tasks.forEach(k=>{let A=B(k.key,k);F.appendChild(A)}),z.appendChild(V),z.appendChild(F),b.appendChild(z)});function P(M,O,z){a[M]||(a[M]={count:0,data:z,brand:i(z.name)}),a[M].count+=O,a[M].count<=0&&delete a[M],J(),K(),t&&t()}function J(){let M=o.tagSupportUsed;r.forEach(([k])=>{let A=h.querySelector(`#hero-${k}`);if(!A)return;let I=a[k];I?(A.classList.add("active"),A.querySelector(".cw-step-val").textContent=I.count,A.querySelector(".cw-step-val").style.color=A.dataset.color,M&&(k==="ads_conversion_tracking"||k==="ads_enhanced_conversions")&&!o.forcedScreenshots.has(k)?A.classList.add("ts-success"):A.classList.remove("ts-success")):(A.classList.remove("active"),A.classList.remove("ts-success"))}),l.querySelectorAll(".cw-task-item").forEach(k=>{let A=k.dataset.id,I=a[A];I?(k.classList.add("selected"),k.querySelector(".cw-step-val").textContent=I.count,M&&(A==="ads_conversion_tracking"||A==="ads_enhanced_conversions")&&!o.forcedScreenshots.has(A)?k.classList.add("ts-success"):k.classList.remove("ts-success")):(k.classList.remove("selected"),k.classList.remove("ts-success"))});let z=Object.keys(a),V=0,F=[];if(z.forEach(k=>{let A=a[k];V+=A.count;for(let I=0;I<A.count;I++)F.length<6&&F.push(A.brand)}),V>0){w.classList.add("visible");let k=V>1?"A\xE7\xF5es":"A\xE7\xE3o",A=V>1?"definidas":"definida";E.textContent=`${V} ${k} ${A}`,R.innerHTML="",F.forEach(I=>{let f=document.createElement("div");f.className="cw-mini-icon",f.innerHTML=ze[I.icon]||ze.default;let T=f.querySelector("svg");T&&(T.style.width="14px",T.style.height="14px"),R.appendChild(f)})}else w.classList.remove("visible")}u.addEventListener("input",M=>{let O=M.target.value.toLowerCase();if(O.length>0){b.style.display="none",g.style.display="block",g.innerHTML="";let z=!1;Object.entries(Ce).forEach(([V,F])=>{if(F.name.toLowerCase().includes(O)){z=!0;let k=B(V,F);a[V]&&(k.classList.add("selected"),k.querySelector(".cw-step-val").textContent=a[V].count),g.appendChild(k)}}),z||(g.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else b.style.display="block",g.style.display="none"});function K(){p.innerHTML="";let M=Object.keys(a),O=!1;if(M.length===0){p.innerHTML=`<div class="cw-empty-state">${e("selecione_tarefas")}</div>`,d.style.display="none";return}let z=o.tagSupportUsed,V=document.createElement("div");V.className="cw-info-banner",V.innerHTML=`
...
CID: ${v.cid||"---"}
Case ID: ${v.caseId||"---"}
E-mail: ${v.clientEmail||"---"}`;$&&($.value=Z),D&&(D.style.display="block",D.style.maxHeight="300px",D.style.opacity="1",D.scrollIntoView({behavior:"smooth",block:"end"}))})}function b(){s=!s,ue(s,c,"cw-btn-script"),s?(p(),d||(d=setInterval(p,2e3))):d&&(clearInterval(d),d=null)}let g=pe(c,"Call Script",t,"Guia interativo para condu\xE7\xE3o de chamadas.",l,()=>{b()});c.appendChild(g);let u=document.createElement("div");Object.assign(u.style,a.contextBanner),u.innerHTML=`
  ...
  `,u.onfocus=()=>{b.style.boxShadow="0 4px 12px rgba(26,115,232,0.15)",b.style.border="1px solid #1a73e8"},u.onblur=()=>{b.style.boxShadow="0 2px 6px rgba(0,0,0,0.04)",b.style.border="1px solid transparent"},b.appendChild(g),b.appendChild(u),h.appendChild(b),p.appendChild(h);let w=document.createElement("div");w.style.cssText="flex: 1; overflow-y: auto; padding: 0 24px 40px 24px; scroll-behavior: smooth;",p.appendChild(w);let E=null;function R(){if(E)return;E=document.createElement("div"),E.style.cssText=`
      ...
      `,g.querySelectorAll('input[name="cw-bc-type"]').forEach(y=>{y.addEventListener("change",()=>{g.querySelectorAll(".cw-radio-option").forEach(S=>S.classList.remove("checked")),y.parentElement.classList.add("checked")})}),setTimeout(()=>{let y=g.querySelector(".cw-radio-option.info");y&&y.classList.add("checked")},100);let f=g.querySelector("#cw-bc-cancel"),T=g.querySelector("#cw-bc-close-x"),L=g.querySelector("#cw-bc-send");f.onclick=B,T.onclick=B,L.onclick=P,l.appendChild(g)}function R(f=null){if(!g)return;let T=g.querySelector("#cw-editor-title-label"),L=g.querySelector("#cw-bc-title"),y=g.querySelector("#cw-bc-text"),S=g.querySelector("#cw-bc-send");if(f){a=f.id,T.textContent="Editar Aviso",L.value=f.title||"",y.value=f.text||"",S.textContent="Salvar Altera\xE7\xF5es";let _=f.type||"info",q=g.querySelector(`input[name="cw-bc-type"][value="${_}"]`);q&&q.click()}else{a=null,T.textContent="Novo Aviso",L.value="",y.value="",S.textContent="Publicar";let _=g.querySelector('input[name="cw-bc-type"][value="info"]');_&&_.click()}g.classList.add("active"),setTimeout(()=>L.focus(),300)}function B(){g&&g.classList.remove("active"),a=null}async function P(){let f=g.querySelector("#cw-bc-send"),T=g.querySelector("#cw-bc-title"),L=g.querySelector("#cw-bc-text"),y=g.querySelector('input[name="cw-bc-type"]:checked'),S=y?y.value:"info";if(!T.value.trim()||!L.value.trim()){X("Preencha todos os campos!",{error:!0});return}f.textContent="Salvando...",f.style.opacity="0.7";let _=!1;a?_=await be.updateBroadcast(a,{title:T.value,text:L.value,type:S}):_=await be.sendBroadcast({title:T.value,text:L.value,type:S,author:window._cwCurrentUser||"admin"}),_?(X(a?"Atualizado!":"Publicado!"),G.playSuccess(),B(),setTimeout(()=>M(),1500)):(X("Erro ao salvar. Verifique a conex\xE3o.",{error:!0}),f.textContent=a?"Salvar Altera\xE7\xF5es":"Publicar",f.style.opacity="1")}async function J(f){if(await xe("Confirma a exclus\xE3o deste aviso?",{danger:!0}))if(await be.deleteBroadcast(f)){X("Aviso removido."),G.playClick();let y=_e.findIndex(S=>S.id===f);y>-1&&_e.splice(y,1),z(),setTimeout(()=>M(),1500)}else X("Erro ao excluir.",{error:!0})}let K=document.createElement("div");K.className="cw-nice-scroll",Object.assign(K.style,i.feedContainer),l.appendChild(K);async function M(){e&&(w.style.display="block",w.innerHTML="\u{1F504} Sincronizando...");try{let f=await be.fetchData();f&&f.broadcast&&(_t(f.broadcast),O(),e&&(z(),w.innerHTML='<span style="color:#137333">\u2713 Atualizado</span>',setTimeout(()=>{w.style.display="none"},1500)))}catch{e&&(w.innerHTML="\u26A0\uFE0F Offline")}}function O(){let f=document.getElementById("cw-btn-broadcast");if(!f)return;let T=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]");if(_e.some(y=>!T.includes(y.id))){if(f.classList.add("has-new"),!f.querySelector(".cw-badge")){let y=document.createElement("div");y.className="cw-badge",Object.assign(y.style,{position:"absolute",top:"8px",right:"8px",width:"8px",height:"8px",backgroundColor:"#d93025",borderRadius:"50%",border:"1px solid #fff",zIndex:"10"}),f.appendChild(y)}}else{f.classList.remove("has-new");let y=f.querySelector(".cw-badge");y&&y.remove()}}function z(){K.innerHTML="";let f=l.querySelector("#cw-bau-widget");f&&f.remove();let T=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]"),L=[..._e].sort((x,N)=>{let j=new Date(x.date).getTime()||0;return(new Date(N.date).getTime()||0)-j}),y=L.findIndex(x=>x.title&&x.title.toLowerCase().includes("disponibilidade bau"));if(y!==-1){let x=L[y];L.splice(y,1);let N=document.createElement("div");N.id="cw-bau-widget",Object.assign(N.style,i.bauContainer);let j=[],Y=(x.text||"").split(`
`),m=/\d{1,2}\/\d{1,2}/,v="\u{1F4C5}";if(Y.forEach(U=>{/🇧🇷|🇵🇹|PT|BR|BRASIL|BRAZIL|PORTUGAL|LISBOA/i.test(U)?v="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM|ESPANHA|SPAIN|MEXICO|MÉXICO/i.test(U)&&(v="\u{1F1EA}\u{1F1F8}");let ee=U.match(m);if(ee){let oe=ee[0],Q=v;/🇧🇷|🇵🇹|PT|BR/i.test(U)?Q="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM/i.test(U)&&(Q="\u{1F1EA}\u{1F1F8}"),j.some(te=>te.flag===Q&&te.date===oe)||j.push({flag:Q,date:oe})}}),j.length===0){let U=(x.text||"").match(/\d{1,2}\/\d{1,2}/g);U&&[...new Set(U)].forEach(ee=>j.push({flag:"\u{1F4C5}",date:ee}))}let C="",$='<button id="cw-bau-toggle-btn" class="cw-btn-interactive" style="background:rgba(255,255,255,0.7); border:1px solid rgba(139, 92, 246, 0.4); border-radius:12px; padding:8px 12px; color:#6D28D9; font-size:12px; font-weight:600;">Detalhes</button>';window._cwIsAdmin&&($=`
        ...
        `,document.head.appendChild(A)}let i={bg:"#F0F2F5",surface:"#FFFFFF",primary:"#007AFF",primaryBg:"rgba(0, 122, 255, 0.1)",text:"#1C1C1E",textSub:"#8E8E93",border:"rgba(0, 0, 0, 0.08)",danger:"#FF3B30"},r={container:{display:"flex",flexDirection:"column",height:"100%",background:i.bg,fontFamily:"'Google Sans', Roboto, sans-serif"},tabHeader:{display:"flex",padding:"12px 16px 0 16px",background:i.surface,borderBottom:`1px solid ${i.border}`},tabBtn:{flex:1,padding:"12px",textAlign:"center",cursor:"pointer",fontSize:"13px",fontWeight:"500",color:i.textSub,borderBottom:"3px solid transparent",transition:"all 0.3s cubic-bezier(0.4, 0, 0.2, 1)",userSelect:"none"},tabActive:{color:i.primary,borderBottomColor:i.primary,fontWeight:"600"},listContainer:{flex:1,overflowY:"auto",padding:"16px",display:"flex",flexDirection:"column",gap:"12px"},emptyState:{padding:"40px 20px",textAlign:"center",color:"#BDC1C6",display:"flex",flexDirection:"column",alignItems:"center",gap:"10px"},card:{background:i.surface,borderRadius:"16px",padding:"16px",border:`1px solid ${i.border}`,boxShadow:"0 4px 12px rgba(0,0,0,0.05)",transition:"all 0.4s cubic-bezier(0.165, 0.84, 0.44, 1)",cursor:"default",position:"relative",overflow:"hidden"},cardHeader:{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"8px"},cardTitle:{fontSize:"14px",fontWeight:"600",color:i.text},cardPreview:{fontSize:"12px",color:i.textSub,lineHeight:"1.5",display:"-webkit-box",webkitLineClamp:"3",webkitBoxOrient:"vertical",overflow:"hidden"},cardActions:{display:"flex",justifyContent:"flex-end",gap:"8px",marginTop:"12px",paddingTop:"12px",borderTop:`1px dashed ${i.border}`},actionBtn:{padding:"6px 12px",borderRadius:"6px",fontSize:"12px",fontWeight:"600",cursor:"pointer",border:"none",background:"transparent",transition:"background 0.2s"},fab:{position:"absolute",bottom:"24px",right:"24px",width:"56px",height:"56px",borderRadius:"16px",background:i.primary,color:"#FFF",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 12px rgba(26, 115, 232, 0.4)",cursor:"pointer",transition:"transform 0.2s",zIndex:10},editorOverlay:{position:"absolute",top:0,left:0,width:"100%",height:"100%",background:"rgba(255, 255, 255, 0.85)",backdropFilter:"blur(25px) saturate(180%)",webkitBackdropFilter:"blur(25px) saturate(180%)",zIndex:20,transform:"translateY(100%)",transition:"transform 0.5s cubic-bezier(0.34, 1.56, 0.64, 1)",display:"flex",flexDirection:"column"},editorHeader:{padding:"16px 24px",background:i.surface,borderBottom:`1px solid ${i.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"},editorBody:{flex:1,padding:"24px",overflowY:"auto"},inputGroup:{marginBottom:"20px"},label:{display:"block",fontSize:"12px",fontWeight:"700",color:i.textSub,marginBottom:"8px",textTransform:"uppercase"},input:{width:"100%",padding:"12px",borderRadius:"8px",border:`1px solid ${i.border}`,fontSize:"14px",fontFamily:"inherit",outline:"none",background:i.surface,boxSizing:"border-box"}},s=document.createElement("div");s.id="library-popup",s.classList.add("cw-module-window"),Object.assign(s.style,me,{right:"auto",left:"50%",width:"400px",height:"600px",transform:"translateX(-50%) scale(0.05)",overflow:"hidden"});let l=pe(s,"Minha Biblioteca",t,"Gerencie seus snippets, textos e templates.",{popup:s},()=>k());s.appendChild(l);let d=document.createElement("div");Object.assign(d.style,r.container),s.appendChild(d);let p=document.createElement("div");Object.assign(p.style,r.tabHeader);let h=[{id:"general",label:"Geral",icon:"\u{1F4CB}"},{id:"note",label:"Notas",icon:"\u{1F4DD}"},{id:"email",label:"Emails",icon:"\u{1F4E7}"}];h.forEach(A=>{let I=document.createElement("div");I.innerHTML=`${A.icon} ${A.label}`,I.id=`lib-tab-${A.id}`,Object.assign(I.style,r.tabBtn),A.id===o&&Object.assign(I.style,r.tabActive),I.onmouseenter=()=>G.playHover(),I.onclick=()=>J(A.id),p.appendChild(I)}),d.appendChild(p);let b=document.createElement("div");Object.assign(b.style,r.listContainer),d.appendChild(b);let g=document.createElement("div");g.className="cw-fab cw-tactile",Object.assign(g.style,r.fab),g.innerHTML='<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>',g.onmouseenter=()=>g.style.transform="scale(1.1)",g.onmouseleave=()=>g.style.transform="scale(1)",g.onclick=()=>M(),d.appendChild(g),a=document.createElement("div"),Object.assign(a.style,r.editorOverlay);let u=document.createElement("div");Object.assign(u.style,r.editorHeader),u.innerHTML='<span style="font-weight:700; font-size:16px;">Novo Item</span>';let w=document.createElement("button");w.innerHTML="Cancelar",w.style.cssText="background:none; border:none; color:#5f6368; font-weight:600; cursor:pointer;",w.onclick=O,u.appendChild(w),a.appendChild(u);let E=document.createElement("div");Object.assign(E.style,r.editorBody),a.appendChild(E);let R=document.createElement("div");R.style.cssText="padding:16px 24px; border-top:1px solid #DADCE0; background:#FFF; display:flex; justify-content:flex-end;";let B=document.createElement("button");B.textContent="Salvar",B.style.cssText="padding:10px 24px; background:#1a73e8; color:white; border:none; border-radius:20px; font-weight:600; cursor:pointer; box-shadow:0 2px 5px rgba(26,115,232,0.3);",B.onclick=z,R.appendChild(B),a.appendChild(R),d.appendChild(a);let P=document.createElement("div");Object.assign(P.style,Ae),P.className="no-drag",s.appendChild(P),Te(s,P),document.body.appendChild(s);function J(A){G.playClick(),o=A,h.forEach(I=>{let f=document.getElementById(`lib-tab-${I.id}`);I.id===A?Object.assign(f.style,r.tabActive):Object.assign(f.style,r.tabBtn)}),K()}function K(){b.innerHTML="";let A=he.getSnippets(o);if(A.length===0){b.innerHTML=`
            ...
            `,f.onmouseenter=()=>{G.playHover()},f.querySelector(".cw-act-copy").onclick=L=>{if(L.stopPropagation(),G.playClick(),I.isRich){let y=new Blob([I.content],{type:"text/html"}),S=document.createElement("div");S.style.whiteSpace="pre-wrap",S.innerHTML=I.content;let _=new Blob([S.innerText],{type:"text/plain"}),q=[new ClipboardItem({"text/html":y,"text/plain":_})];navigator.clipboard.write(q)}else navigator.clipboard.writeText(I.content);X("Copiado!")},f.querySelector(".cw-act-edit").onclick=L=>{L.stopPropagation(),G.playClick(),M(I)},f.querySelector(".cw-act-del").onclick=async L=>{L.stopPropagation(),G.playClick(),await xe("Excluir este item?")&&(he.delete(I.id),K(),X("Item exclu\xEDdo."))},b.appendChild(f)})}function M(A=null){n=A?A.id:null,E.innerHTML="",E.appendChild(V("title","T\xEDtulo / Nome",A?A.title:"")),o==="email"&&E.appendChild(V("subject","Assunto do Email",A?A.subject:""));let I="Conte\xFAdo";o==="email"&&(I="Corpo do Email (HTML)"),o==="note"&&(I="Texto da Nota (Reason)"),E.appendChild(V("content",I,A?A.content:"",{isRich:!0,isCode:A?A.isCode:!1})),u.querySelector("span").textContent=A?"Editar Item":"Novo Item",a.style.transform="translateY(0)",setTimeout(()=>{let f=E.querySelector("input");f&&f.focus()},300)}function O(){a.style.transform="translateY(100%)",setTimeout(()=>n=null,300)}async function z(){let A=E.querySelector("#cw-inp-title"),I=E.querySelector("#cw-inp-content"),f=A.value.trim(),T=I.contentEditable==="true"?I.innerHTML:I.value.trim(),L=I.getAttribute("data-is-code")==="true";if(!f||!T||T==="<br>"){X("Preencha t\xEDtulo e conte\xFAdo.",{error:!0});return}let y={id:n,type:o,title:f,content:T,isCode:L,isRich:I.contentEditable==="true"};if(o==="email"){let S=E.querySelector("#cw-inp-subject").value.trim();if(!S){X("Assunto \xE9 obrigat\xF3rio para emails.",{error:!0});return}y.subject=S}B.textContent="Salvando...",await he.save(y),B.textContent="Salvar",O(),K(),X("Salvo com sucesso!"),G.playSuccess()}function V(A,I,f,T={}){let L=document.createElement("div");Object.assign(L.style,r.inputGroup);let y=document.createElement("label");y.textContent=I,Object.assign(y.style,r.label);let S;if(T.isRich){let _=document.createElement("div");_.style.cssText="display:flex; gap:6px; margin-bottom:12px; background:rgba(241, 243, 244, 0.8); padding:6px; border-radius:12px; border:1px solid #DADCE0; backdrop-filter: blur(10px);",_.innerHTML=`
  ...
  `,document.head.appendChild(t)};function Mo(){_o();let t=!1,e=null,o=1,a=4,n=document.createElement("div");n.id="bau-form-popup",n.className="bau-popup cw-module-window",n.style.display="none";let r=pe(n,"BAU Form","v1.2.0","Solicite a abertura de casos BAU em um fluxo guiado.",{googleLine:null},()=>L());n.appendChild(r);let s=document.createElement("div");s.className="bau-progress-indicator",n.appendChild(s);let c=document.createElement("div");c.className="bau-content",n.appendChild(c);let l=document.createElement("form");l.id="bau-escalation-form",c.appendChild(l);let d=document.createElement("div");d.className="bau-step active",d.id="bau-step-1";let p=document.createElement("div");p.className="bau-card bau-context-card";let h=document.createElement("div");h.innerHTML=`
    ...
    `,p.appendChild(h),d.appendChild(p),l.appendChild(d);let b=document.createElement("div");b.className="bau-step",b.id="bau-step-2";let g=document.createElement("div");g.className="bau-card",g.innerHTML=`
