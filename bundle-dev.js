...
Irei solicitar descarte do mesmo, levando em conta a falta de acessos e solicita\xE7\xE3o de reagendamento do mesmo.`}};var Ue=t=>new Promise(e=>setTimeout(e,t));function Ke(t){t&&["mousedown","mouseup","click"].forEach(e=>t.dispatchEvent(new MouseEvent(e,{bubbles:!0,cancelable:!0,view:window})))}var eo="cw-automation-styles";if(!document.getElementById(eo)){let t=document.createElement("style");t.id=eo,t.innerHTML=`
...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
    ...
    `;let y=g.querySelector(".cw-hero-grid"),x=g.querySelector(".cw-acc-container"),d=g.querySelector(".cw-results-container"),p=g.querySelector(".cw-search-input"),w=g.querySelector(".cw-status-bar"),E=g.querySelector(".cw-status-text"),M=g.querySelector(".cw-footer-icons");s.forEach(([I,F])=>{let R=i(F.name),j=document.createElement("div");j.className="cw-hero-card",j.id=`hero-${I}`,j.style.setProperty("--hero-color",R.color),j.innerHTML=`
        ...
        `,j.onclick=_=>{if(_.target.closest(".cw-step-btn"))return;let A=a[I]?a[I].count:0;z(I,A>0?-A:1,F)},j.querySelector(".minus").onclick=()=>z(I,-1,F),j.querySelector(".plus").onclick=()=>z(I,1,F),j.dataset.color=R.color,y.appendChild(j)});function P(I,F){let R=i(F.name),j=document.createElement("div");return j.className="cw-task-item",j.dataset.id=I,j.innerHTML=`
        ...
        `,j.onclick=_=>{if(_.target.closest(".cw-step-btn"))return;let A=a[I]?a[I].count:0;z(I,A>0?-A:1,F)},j.querySelector(".minus").onclick=()=>z(I,-1,F),j.querySelector(".plus").onclick=()=>z(I,1,F),j}Object.entries(r).forEach(([I,F])=>{let R=document.createElement("div");R.className="cw-acc-group";let j=document.createElement("div");j.className="cw-acc-header",j.innerHTML=`
            <div class="cw-acc-title">
                <div class="cw-acc-dot" style="background:${F.brand.color}"></div>
                ${I}
            </div>
            <div class="cw-acc-icon">\u25BC</div>
        `,j.onclick=()=>{x.querySelectorAll(".cw-acc-group.open").forEach(A=>{A!==R&&A.classList.remove("open")}),R.classList.toggle("open")};let _=document.createElement("div");_.className="cw-acc-body",F.tasks.forEach(A=>{let N=P(A.key,A);_.appendChild(N)}),R.appendChild(j),R.appendChild(_),x.appendChild(R)});function z(I,F,R){a[I]||(a[I]={count:0,data:R,brand:i(R.name)}),a[I].count+=F,a[I].count<=0&&delete a[I],Z(),ee(),t&&t()}function Z(){let I=o.tagSupportUsed;s.forEach(([A])=>{let N=y.querySelector(`#hero-${A}`);if(!N)return;let q=a[A];q?(N.classList.add("active"),N.querySelector(".cw-step-val").textContent=q.count,N.querySelector(".cw-step-val").style.color=N.dataset.color,I&&(A==="ads_conversion_tracking"||A==="ads_enhanced_conversions")&&!o.forcedScreenshots.has(A)?N.classList.add("ts-success"):N.classList.remove("ts-success")):(N.classList.remove("active"),N.classList.remove("ts-success"))}),g.querySelectorAll(".cw-task-item").forEach(A=>{let N=A.dataset.id,q=a[N];q?(A.classList.add("selected"),A.querySelector(".cw-step-val").textContent=q.count,I&&(N==="ads_conversion_tracking"||N==="ads_enhanced_conversions")&&!o.forcedScreenshots.has(N)?A.classList.add("ts-success"):A.classList.remove("ts-success")):(A.classList.remove("selected"),A.classList.remove("ts-success"))});let R=Object.keys(a),j=0,_=[];if(R.forEach(A=>{let N=a[A];j+=N.count;for(let q=0;q<N.count;q++)_.length<6&&_.push(N.brand)}),j>0){w.classList.add("visible");let A=j>1?"A\xE7\xF5es":"A\xE7\xE3o",N=j>1?"definidas":"definida";E.textContent=`${j} ${A} ${N}`,M.innerHTML="",_.forEach(q=>{let f=document.createElement("div");f.className="cw-mini-icon",f.innerHTML=Pe[q.icon]||Pe.default;let S=f.querySelector("svg");S&&(S.style.width="14px",S.style.height="14px"),M.appendChild(f)})}else w.classList.remove("visible")}p.addEventListener("input",I=>{let F=I.target.value.toLowerCase();if(F.length>0){x.style.display="none",d.style.display="block",d.innerHTML="";let R=!1;Object.entries(Ce).forEach(([j,_])=>{if(_.name.toLowerCase().includes(F)){R=!0;let A=P(j,_);a[j]&&(A.classList.add("selected"),A.querySelector(".cw-step-val").textContent=a[j].count),d.appendChild(A)}}),R||(d.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else x.style.display="block",d.style.display="none"});function ee(){u.innerHTML="";let I=Object.keys(a),F=!1;if(I.length===0){u.innerHTML=`<div class="cw-empty-state">${e("selecione_tarefas")}</div>`,m.style.display="none";return}let R=o.tagSupportUsed,j=document.createElement("div");j.className="cw-info-banner",j.innerHTML=`
            <span style="font-size:14px">\u2139\uFE0F</span>
            <span>
                Os screenshots seguem as diretrizes atuais do Win Criteria e pol\xEDticas de Tag Support.
            </span>
        `,u.appendChild(j),I.forEach(_=>{let A=a[_].data,N=a[_].count,q=a[_].brand,S=R&&(_==="ads_conversion_tracking"||_==="ads_enhanced_conversions")&&!o.forcedScreenshots.has(_),D=o.screenshotMode||"implementation",L=A.screenshots?.[D]||[];if(L.length>0||S){F=!0;for(let Y=1;Y<=N;Y++){let k=document.createElement("div");k.className="cw-screen-card",S&&k.classList.add("ts-success"),k.style.setProperty("--brand-color",q.color),k.style.setProperty("--brand-bg",q.bg),k.style.setProperty("--brand-shadow",q.color+"40");let U=document.createElement("div");U.className="cw-card-header";let h=document.createElement("div");h.className="cw-card-icon",h.innerHTML=Pe[q.icon]||Pe.default;let K=document.createElement("div");K.style.cssText="flex:1; display:flex; align-items:center; gap:8px;";let V=document.createElement("input");V.className="cw-card-title-input",V.id=`name-${_}-${Y}`,V.value=`${A.name}${N>1?" #"+Y:""}`,V.title="Clique para renomear esta task";let X=document.createElement("span");if(X.className="cw-edit-hint",X.innerHTML="\u270E Renomear",K.appendChild(V),K.appendChild(X),U.appendChild(h),U.appendChild(K),k.appendChild(U),S){let b=document.createElement("div");b.className="cw-ts-disclaimer-box",b.innerHTML=`
                <span>${e("ts_disclaimer")}</span>
                <button class="cw-btn-ts-force">${e("incluir_mesmo_assim")}</button>
            `,b.querySelector("button").onclick=()=>{o.toggleForcedScreenshot(_,!0)},k.appendChild(b)}else L.forEach((b,C)=>{let l=document.createElement("div");l.className="cw-input-group";let v=document.createElement("label");v.className="cw-input-label",v.textContent=b;let T=document.createElement("input");T.className="cw-input-field",T.id=`screen-${_}-${Y}-${C}`,T.placeholder="Cole o link aqui...",T.setAttribute("autocomplete","off"),T.addEventListener("input",()=>{T.value.trim().length>5?T.classList.add("filled"):T.classList.remove("filled")});let $=document.createElement("div");$.className="cw-input-check",$.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>',l.appendChild(v),l.appendChild(T),l.appendChild($),k.appendChild(l)});u.appendChild(k)}}}),m.style.display=F?"block":"none"}return{selectionElement:g,screenshotsElement:m,updateSubStatus:()=>ee(),getCheckedElements:()=>Object.keys(a).map(I=>({value:I,count:a[I].count})),setTaskCount:(I,F)=>{a[I]&&delete a[I],F>0&&Ce[I]&&z(I,F,Ce[I])},toggleTask:(I,F=!0)=>{let R=a[I];F&&!R?z(I,1,Ce[I]):!F&&R&&z(I,-R.count,Ce[I])},setLanguage:I=>{e=I;let F=g.querySelector(".js-hero-title");F&&(F.textContent=e("acesso_rapido"));let R=g.querySelector(".cw-search-input");R&&(R.placeholder=e("buscar_catalogo")),ee()},reset:()=>{for(let I in a)delete a[I];p.value="",x.style.display="block",d.style.display="none",Z(),ee()}}}var ea={marginTop:"24px",marginBottom:"16px",padding:"20px",background:"#fff9e6",borderRadius:"20px",border:"1.5px solid #fbbc0540",display:"none",boxShadow:"0 4px 12px rgba(251, 188, 5, 0.05)"},ta={fontSize:"12px",color:"#b06000",marginTop:"8px",lineHeight:"1.4"},oa={width:"100%",padding:"12px 16px",borderRadius:"12px",border:"1.5px solid #dadce0",fontSize:"14px",marginBottom:"16px",boxSizing:"border-box",background:"#fff"},aa={display:"flex",gap:"20px",marginBottom:"12px"};function io(t){let e=document.createElement("div");e.id="tag-support-container",Object.assign(e.style,ea);let o=document.createElement("label");o.className="js-ts-main-label",o.textContent=t("utilizou_tag_support"),Object.assign(o.style,Ct,{marginTop:"0"});let a=document.createElement("div");Object.assign(a.style,aa);let n=document.createElement("input");n.type="radio",n.name="ts_usage_mod",n.value="Sim",Object.assign(n.style,Et);let i=document.createElement("label");i.textContent="Sim";let s=document.createElement("div");Object.assign(s.style,{display:"flex",alignItems:"center"}),s.appendChild(n),s.appendChild(i);let r=document.createElement("input");r.type="radio",r.name="ts_usage_mod",r.value="N\xE3o",r.checked=!0,Object.assign(r.style,Et);let c=document.createElement("label");c.textContent="N\xE3o";let g=document.createElement("div");Object.assign(g.style,{display:"flex",alignItems:"center"}),g.appendChild(r),g.appendChild(c),a.appendChild(s),a.appendChild(g);let m=document.createElement("div");m.style.display="block";let u=document.createElement("label");u.className="js-ts-reason-label",u.textContent=t("motivo_ts"),Object.assign(u.style,Ct,{fontSize:"12px"});let y=document.createElement("input");y.type="text",Object.assign(y.style,oa);let x=document.createElement("div");x.className="js-ts-warning",x.innerHTML=`\u26A0\uFE0F <strong>${t("lembre_preencher_form")}</strong> <a href="https://docs.google.com/forms/d/e/1FAIpQLSeP_JM8D-6qHa5ZC93aTzj38WiO5zx8nyrWNPvbZhjJj6CpkA/viewform" target="_blank" style="color:#e37400; text-decoration:underline;">Link aqui</a>`,Object.assign(x.style,ta),m.appendChild(u),m.appendChild(y),m.appendChild(x),e.appendChild(o),e.appendChild(a),e.appendChild(m),n.onchange=()=>{m.style.display="none",Promise.resolve().then(()=>(Xe(),at)).then(M=>M.notesState.setTagSupportUsed(!0))},r.onchange=()=>{m.style.display="block",Promise.resolve().then(()=>(Xe(),at)).then(M=>M.notesState.setTagSupportUsed(!1))};function d(M,P){if(e.style.display="none",!M||!P||P.length===0)return;P.some(Z=>Z==="ads_conversion_tracking"||Z==="ads_enhanced_conversions")?e.style.display="block":(E(),Promise.resolve().then(()=>(Xe(),at)).then(Z=>Z.notesState.setTagSupportUsed(!1)))}function p(){if(e.style.display==="none")return"";let M=`<br><b>Utilizou Tag Support?</b> ${n.checked?"\u2705 Sim":"\u274C N\xE3o"}`;return r.checked&&y.value.trim()!==""&&(M+=`<br><b>Motivo:</b> ${y.value}`),M+="<br>",M}function w(M){t=M,o.textContent=t("utilizou_tag_support"),u.textContent=t("motivo_ts"),x.innerHTML=`\u26A0\uFE0F <strong>${t("lembre_preencher_form")}</strong> <a href="https://docs.google.com/forms/d/e/1FAIpQLSeP_JM8D-6qHa5ZC93aTzj38WiO5zx8nyrWNPvbZhjJj6CpkA/viewform" target="_blank" style="color:#b06000; text-decoration:underline;">Link aqui</a>`}function E(){e.style.display="none",r.checked=!0,n.checked=!1,m.style.display="block",y.value=""}return{element:e,updateVisibility:d,getOutput:p,setLanguage:w,reset:E}}var Ft="cw_notes_parking_lot",rt="cw_notes_emergency_save";var ge={getAll:()=>{try{return JSON.parse(localStorage.getItem(Ft)||"[]")}catch{return[]}},save:t=>{let e=ge.getAll(),o={id:Date.now().toString(),timestamp:new Date().toISOString(),...t};return e.unshift(o),e.length>5&&e.pop(),localStorage.setItem(Ft,JSON.stringify(e)),o},delete:t=>{let e=ge.getAll();return e=e.filter(o=>o.id!==t),localStorage.setItem(Ft,JSON.stringify(e)),e},getCount:()=>ge.getAll().length,saveEmergency:t=>{let e={timestamp:Date.now(),data:t};localStorage.setItem(rt,JSON.stringify(e))},getEmergency:()=>{try{let t=localStorage.getItem(rt);if(!t)return null;let e=JSON.parse(t);return Date.now()-e.timestamp>432e5?(localStorage.removeItem(rt),null):!e.data||!e.data.subStatus?null:e.data}catch{return null}},clearEmergency:()=>{localStorage.removeItem(rt)}};var lt=["lucaste","ricardogi"];var se={glassBg:"rgba(61, 61, 61, 0.77)",glassBorder:"rgba(255, 255, 255, 0.15)",glassActive:"rgba(79, 79, 79, 0.89)",glassHighlight:"rgba(255, 255, 255, 0.08)",iconIdle:"#c2c5c8ff",iconActive:"#FFFFFF",blue:"#8AB4F8",red:"#F28B82",purple:"#C58AF9",green:"#81C995",orange:"#F9AB00",teal:"#00BFA5",pink:"#F48FB1",gray:"#9AA0A6"},ct=t=>new Promise(e=>setTimeout(e,t));function dt(t){let e=document.getElementById("cw-btn-notes");if(!e)return;let o=e.querySelector(".cw-dot-dirty");t?o||(o=document.createElement("div"),o.className="cw-dot-dirty",e.appendChild(o)):o&&o.remove()}function so(t){let e="cw-command-center-style";if(!document.getElementById(e)){let p=document.createElement("style");p.id=e,p.innerHTML=`
    ...
    `;let n=document.createElement("div");n.className="cw-focus-backdrop",document.body.appendChild(n),document.body.appendChild(a);let i=(p,w)=>{H.playClick(),a.querySelector(`.${p}`).classList.toggle("active"),w()};if(a.querySelector(".notes").onclick=p=>{p.stopPropagation(),i("notes",t.toggleNotes)},a.querySelector(".bauform").onclick=p=>{p.stopPropagation(),i("bauform",t.toggleBAUForm)},a.querySelector(".email").onclick=p=>{p.stopPropagation(),i("email",t.toggleEmail)},a.querySelector(".script").onclick=p=>{p.stopPropagation(),i("script",t.toggleScript)},a.querySelector(".links").onclick=p=>{p.stopPropagation(),i("links",t.toggleLinks)},a.querySelector(".library").onclick=p=>{p.stopPropagation(),i("library",t.toggleLibrary)},a.querySelector(".timezone").onclick=p=>{p.stopPropagation(),i("timezone",t.toggleTimezone)},a.querySelector(".configs").onclick=p=>{p.stopPropagation(),i("configs",t.toggleConfigs)},a.querySelector(".broadcast").onclick=p=>{p.stopPropagation(),i("broadcast",()=>{let w=p.currentTarget.querySelector(".cw-badge");w&&w.remove(),t.broadcastControl&&t.broadcastControl.toggle()})},a.querySelectorAll(".cw-btn").forEach(p=>{p.addEventListener("mouseenter",()=>H.playHover())}),t.broadcastControl&&t.broadcastControl.hasUnread){let p=document.createElement("div");p.className="cw-badge",a.querySelector(".broadcast").appendChild(p)}let s=null;a.onmouseleave=()=>{a.querySelector(".cw-btn.active")||a.classList.contains("processing-center")||(s=setTimeout(()=>{a.classList.add("collapsed")},3e3))},a.onmouseenter=()=>{s&&clearTimeout(s)},(async function(){let w=()=>{let M=fe();if(M){let P=M.split("@")[0].toLowerCase();if(lt.includes(P)){let z=a.querySelector("#cw-admin-tag");z&&z.classList.add("visible")}}else setTimeout(w,2e3)};w(),await ct(2800),a.classList.add("docked"),await ct(300);let E=a.querySelectorAll(".cw-btn");a.querySelectorAll(".cw-sep").forEach(M=>M.classList.add("visible"));for(let M=0;M<E.length;M++)E[M].classList.add("popped"),await ct(90);await ct(200),a.classList.add("system-check")})();let r=!1,c,g,m,u,y=3;a.onmousedown=p=>{if(p.target.closest("button"))return;p.preventDefault(),c=p.clientX,g=p.clientY;let w=a.getBoundingClientRect();m=w.left,u=w.top,document.addEventListener("mousemove",x),document.addEventListener("mouseup",d)};function x(p){let w=p.clientX-c,E=p.clientY-g;!r&&Math.sqrt(w*w+E*E)>y&&(r=!0,a.classList.add("dragging"),a.style.transition="none",s&&clearTimeout(s)),r&&(a.style.left=`${m+w}px`,a.style.top=`${u+E}px`,a.style.right="auto",a.style.bottom="auto",a.style.transform="none")}function d(p){if(document.removeEventListener("mousemove",x),document.removeEventListener("mouseup",d),r){r=!1,a.classList.remove("dragging");let w=window.innerWidth,E=window.innerHeight,M=a.getBoundingClientRect(),P=M.left+M.width/2,z;P<w/2?(z=24,a.classList.remove("side-right"),a.classList.add("side-left")):(z=w-M.width-24,a.classList.remove("side-left"),a.classList.add("side-right"));let Z=Math.max(24,Math.min(M.top,E-M.height-24));setTimeout(()=>{a.style.setProperty("transition","left 0.6s cubic-bezier(0.34, 1.56, 0.64, 1), top 0.6s cubic-bezier(0.34, 1.56, 0.64, 1)","important"),a.style.left=`${z}px`,a.style.top=`${Z}px`,a.style.bottom="auto",a.style.transform=""},10),setTimeout(()=>{a.style.transition="",a.style.removeProperty("transition")},700)}else{let w=a.querySelector(".cw-btn.active"),E=p.target.closest("button");if(a.classList.contains("collapsed")){let M=a.getBoundingClientRect(),P=window.innerHeight,z=M.top>P/2;if(a.style.setProperty("transition","none","important"),z){let Z=P-M.bottom;a.style.top="auto",a.style.bottom=`${Z}px`}else a.style.bottom="auto",a.style.top=`${M.top}px`;a.offsetWidth,a.style.removeProperty("transition"),a.classList.remove("collapsed"),H.playGenieOpen()}else!w&&!E&&(a.classList.add("collapsed"),H.playGenieOpen());E&&(E.style.transform="scale(0.9)",setTimeout(()=>E.style.transform="",150))}}}function Je(){let t=document.querySelector(".cw-pill"),e=document.querySelector(".cw-focus-backdrop");if(!t)return()=>{};t.classList.remove("collapsed"),window._CW_ABORT_PROCESS=!1;let o=document.createElement("div");o.className="cw-center-stage",o.innerHTML=`
...
`.replace(/\n/g,"<br>");Ze(I);let F=await mt();F?(F.innerText.trim()===""&&(F.innerHTML=""),document.execCommand("insertHTML",!1,I),ut(F),J("Nota gerada e inserida!")):J("Copiado! Abra uma nota para colar.")},e}var be=t=>new Promise(e=>setTimeout(e,t));function ve(t,e="info"){let o={info:"background: #e8f0fe; color: #1a73e8; padding: 2px 5px; border-radius: 3px;",warn:"background: #fef7e0; color: #b06000; padding: 2px 5px; border-radius: 3px;",error:"background: #fce8e6; color: #c5221f; padding: 2px 5px; border-radius: 3px;",success:"background: #e6f4ea; color: #137333; padding: 2px 5px; border-radius: 3px;"};console.log(`%c[EMAIL-BOT] ${t}`,o[e]||o.info)}function Fe(t){if(!t)return;let e={bubbles:!0,cancelable:!0,view:window};["mouseover","mousedown","mouseup","click"].forEach(o=>t.dispatchEvent(new MouseEvent(o,e)))}function gt(t,e){if(!t)return;let o=`cw-warning-${t.id||Math.random().toString(36).substr(2,9)}`,a=document.getElementById(o);a&&a.remove();let n=t.getBoundingClientRect(),i=document.createElement("div");i.id=o,i.style.cssText=`
    ...
    `,document.head.appendChild(t)}function ue(t,e,o){let a=document.getElementById(o);if(!e)return;let n=e.getAttribute("data-moved")==="true",i={x:0,y:0};if(a){let m=a.getBoundingClientRect();i.x=m.left+m.width/2,i.y=m.top+m.height/2}let s,r;if(!n)s=window.innerWidth/2,r=window.innerHeight/2;else{let m=e.getBoundingClientRect();s=m.left+m.width/2,r=m.top+m.height/2,s===0&&r===0&&(s=window.innerWidth/2,r=window.innerHeight/2)}let c=i.x-s,g=i.y-r;t?(H.playGenieOpen(),e.style.transition="none",e.style.opacity="0",e.style.pointerEvents="auto",n?e.style.transform=`translate(${c}px, ${g}px) scale(0.05)`:e.style.transform=`translate(calc(-50% + ${c}px), calc(-50% + ${g}px)) scale(0.05)`,e.offsetWidth,requestAnimationFrame(()=>{e.classList.add("open"),a&&a.classList.add("active"),e.style.transition="opacity 0.4s ease-out, transform 0.5s cubic-bezier(0.19, 1, 0.22, 1)",e.style.opacity="1",n?e.style.transform="translate(0, 0) scale(1)":e.style.transform="translate(-50%, -50%) scale(1)"}),typeof go=="function"&&go(e,o)):(H.playSwoosh(),e.style.transition="opacity 0.25s ease, transform 0.3s cubic-bezier(0.5, 0, 1, 1)",e.style.pointerEvents="none",requestAnimationFrame(()=>{e.style.opacity="0",n?e.style.transform=`translate(${c}px, ${g}px) scale(0.1)`:e.style.transform=`translate(calc(-50% + ${c}px), calc(-50% + ${g}px)) scale(0.1)`}),setTimeout(()=>{e.classList.remove("open"),a&&a.classList.remove("active"),e.style.transition="",e.style.transform=""},300),typeof Lt=="function"&&Lt(e))}function go(t,e){Lt(t);let o=a=>{if(!t.classList.contains("open"))return;let n=t.contains(a.target),i=document.querySelector(".cw-pill"),s=i&&i.contains(a.target);n?(t.classList.remove("idle"),t.style.zIndex="2147483648"):s||(t.classList.add("idle"),t.style.zIndex="2147483646")};t._idleHandler=o,document.addEventListener("mousedown",o)}function Lt(t){t._idleHandler&&(document.removeEventListener("mousedown",t._idleHandler),t._idleHandler=null)}function bo(){let t="v4.0.0",{popup:e,content:o,header:a,animRefs:n,credit:i}=Qt(t,ee),s=io(h),r=no(()=>{A(),G.setActiveTasks(r.getCheckedElements())},h,G),c=document.createElement("div");c.style.display="none";let g=ao((b,C)=>{N(b,C)});c.appendChild(g);let m=ro({onSaveCurrent:async()=>{let b=await Y();return L(),b},onLoadDraft:b=>{U(b)},t:b=>h(b)}),u=F(),y=R(),x=document.createElement("div"),d=V(),p=q(m,h);o.appendChild(u),o.appendChild(y),o.appendChild(d),o.appendChild(c),o.appendChild(x),r.selectionElement.style.display="none",r.screenshotsElement.style.display="none";let w=document.createElement("button");w.id="manual-task-toggle",w.textContent=h("gostaria_de_adicionar_uma_task")||"Gostaria de adicionar uma task",w.style.cssText=`display: none; width: 100%; padding: 14px; border: 2px dashed ${B.primary}; background: ${B.surface}; color: ${B.primary}; border-radius: ${de.medium}; cursor: pointer; font-size: 13px; font-weight: 700; margin-top: 8px; transition: all 0.2s ${re}; text-transform: uppercase; letter-spacing: 0.5px;`,w.onmouseenter=()=>{w.style.background=B.primaryBg},w.onmouseleave=()=>{w.style.background=B.surface},w.onclick=()=>{r.selectionElement.style.display="block",r.screenshotsElement.style.display="block",w.style.display="none"},o.appendChild(w),o.appendChild(r.selectionElement),o.appendChild(s.element),o.appendChild(r.screenshotsElement),o.appendChild(p);let E=document.createElement("div");E.style.display="none",E.style.flexGrow="1",E.style.minHeight="0",E.style.overflow="hidden";let M=po(()=>I());M.style.height="100%",E.appendChild(M),e.insertBefore(E,i);let P=a.lastElementChild;P&&(P.insertBefore(m.historyBtnWrapper,P.firstChild),P.insertBefore(K(),P.firstChild)),e.appendChild(m.drawer);let z=null;G.subscribe(b=>{X(b),Z(),b.isDirty?(z&&clearTimeout(z),z=setTimeout(async()=>{let C=await Y(!0);C.subStatus?ge.saveEmergency(C):ge.clearEmergency(),b.isDirty=!1},2e3)):z&&(clearTimeout(z),z=null)});function Z(){let b=ge.getCount()>0,C=!!G.currentSubStatus;dt(b||C)}function ee(){G.visible=!G.visible,ue(G.visible,e,"cw-btn-notes")}function I(){G.isSplitView=!G.isSplitView,G.isSplitView?(o.style.display="none",E.style.display="flex",E.style.flexDirection="column",n.googleLine&&(n.googleLine.style.background="linear-gradient(to right, #8e24aa, #7b1fa2)")):(o.style.display="flex",E.style.display="none",n.googleLine&&(n.googleLine.style.background="linear-gradient(to right, #4285F4, #EA4335, #FBBC05, #34A853)"))}function F(){let b=document.createElement("div");if(b.innerHTML=`
        ...
        `,!document.getElementById("cw-segmented-styles")){let l=document.createElement("style");l.id="cw-segmented-styles",l.innerHTML=`
            ...
            `,document.head.appendChild(l)}let C=(l,v)=>{let $=b.querySelector(`#${l}`).querySelector(".cw-segmented-indicator");$&&($.style.transform=`translateX(${v*100}%) translateX(${v*2}px)`)};return b.querySelectorAll("#lang-selector button").forEach((l,v)=>{l.onclick=()=>{G.setLanguage(l.dataset.lang),b.querySelectorAll("#lang-selector button").forEach(T=>T.classList.remove("active")),l.classList.add("active"),C("lang-selector",v),H.playHover(),G.currentSubStatus&&_(G.currentSubStatus)}}),b.querySelectorAll("#type-selector button").forEach((l,v)=>{l.onclick=()=>{G.setCaseType(l.dataset.type),b.querySelectorAll("#type-selector button").forEach(T=>T.classList.remove("active")),l.classList.add("active"),C("type-selector",v),H.playHover(),G.currentSubStatus&&_(G.currentSubStatus)}}),b.querySelectorAll("#portugal-selector button").forEach((l,v)=>{l.onclick=()=>{G.setPortugalCase(l.dataset.val==="true"),b.querySelectorAll("#portugal-selector button").forEach(T=>T.classList.remove("active")),l.classList.add("active"),C("portugal-selector",v),H.playHover(),G.currentSubStatus&&_(G.currentSubStatus)}}),b}function R(){let b=document.createElement("div");b.className="cw-status-section",b.style.cssText="display: flex; flex-direction: column; gap: 8px;",b.innerHTML=`
        ...
        `;let C=b.querySelector("#main-status-select"),l=b.querySelector("#sub-status-select");return C.onchange=()=>{G.setStatus(C.value),j(C.value,l),G.setSubStatus(""),_("")},l.onchange=()=>{G.setSubStatus(l.value),_(l.value)},b}function j(b,C){if(C.innerHTML=`<option value="">${h("select_substatus")}</option>`,!b){C.disabled=!0;return}for(let l in Te)if(Te[l].status===b){let v=document.createElement("option");v.value=l,v.textContent=Te[l].name,C.appendChild(v)}C.disabled=!1}function _(b){if(g.render&&g.render(b,G.currentCaseType),!b){c.style.display="none",x.style.display="none",document.getElementById("manual-task-toggle").style.display="none",r.selectionElement.style.display="none",r.screenshotsElement.style.display="none",d.style.display="flex",d.style.opacity="1",p.style.display="none";return}d.style.opacity="0",setTimeout(()=>{G.currentSubStatus&&(d.style.display="none")},400),p.style.display="grid";let C=Te[b];C&&C.templateFields&&G.setActiveFields(C.templateFields),D(),kt(b,x,G),x.style.display="block",c.style.display="block";let l=b.startsWith("SO_"),v=b==="NI_Awaiting_Validation",T=document.getElementById("manual-task-toggle");l||v?(r.selectionElement.style.display="block",T.style.display="none"):(r.selectionElement.style.display="none",r.screenshotsElement.style.display="none",T.style.display="block");let $=b==="SO_Education_Only"?"education":"implementation";G.setScreenshotMode($),G.currentCaseType==="lm"?G.toggleFieldExclusion("field-ON_CALL",!0):G.toggleFieldExclusion("field-ON_CALL",!1),r.updateSubStatus(b),A();let O=document.getElementById("email-automation-toggle-row");O&&(O.style.display=je[b]?"flex":"none")}function A(){let b=r.getCheckedElements().map(C=>C.value);s.updateVisibility(G.currentSubStatus,b)}function N(b,C){let l=it[b];if(l){for(let v in l)if(v==="linkedTask")r.toggleTask(l.linkedTask,C);else if(v==="activeTasks")l.activeTasks.forEach(T=>{C?r.setTaskCount(T.value,T.count):r.setTaskCount(T.value,0)});else if(v.startsWith("field-")){let T=v,$=l[v],O=document.getElementById(T);if(O){let W=Ge.includes(T.replace("field-",""));if(C)if(W){let te=O.value.trim();te.includes($.trim())||(O.value=te?te+`
`+$.trim():$.trim())}else O.value=$;else if(W){let te=O.value.trim(),Q=$.trim();te.includes(Q)&&(O.value=te.replace(Q,"").trim().replace(/\n{3,}/g,`

`))}else O.value.trim()===$.trim()&&(O.value="");G.updateField(T,O.value),O.dispatchEvent(new Event("input"))}}}}function q(b,C){let l=document.createElement("div");if(l.className="cw-actions-section",l.style.cssText=`
            display: grid;
        ...
        `,!document.getElementById("cw-actions-hover-styles")){let te=document.createElement("style");te.id="cw-actions-hover-styles",te.innerHTML=`
            ...
            `,document.head.appendChild(te)}let v=document.createElement("div");v.id="email-automation-toggle-row",v.style.cssText="grid-column: span 2; display: none; align-items: center; justify-content: center; padding-bottom: 6px; border-bottom: 1px solid rgba(0,0,0,0.05); margin-bottom: 2px;",v.innerHTML=`
        ...
        `;let T=b.parkButton;T.classList.add("js-btn-park"),T.style.cssText="width: 100%; margin: 0; border-radius: 10px; height: 34px; font-weight: 600; font-size: 11.5px;";let $=document.createElement("button");$.className="cw-btn-secondary js-btn-reset",$.textContent=C("limpar"),$.style.cssText=`width: 100%; height: 34px; background: ${B.surface}; color: ${B.textSub}; border: 1px solid ${B.border}; border-radius: 10px; font-weight: 600; cursor: pointer; font-size: 11.5px;`,$.onclick=()=>L();let O=document.createElement("button");O.className="cw-btn-secondary js-btn-copy",O.textContent=C("copiar"),O.style.cssText=`width: 100%; height: 34px; background: ${B.surface}; color: ${B.primary}; border: 1px solid ${B.primary}; border-radius: 10px; font-weight: 600; cursor: pointer; font-size: 11.5px;`,O.onclick=()=>f();let W=document.createElement("button");return W.className="cw-btn-primary js-btn-generate",W.textContent=C("preencher"),W.style.cssText=`width: 100%; height: 38px; background: ${B.primary}; color: #fff; border: none; border-radius: 10px; font-weight: 600; cursor: pointer; grid-column: span 2; font-size: 12.5px; box-shadow: 0 4px 10px rgba(26, 115, 232, 0.2); margin-top: 0px;`,W.onclick=()=>S(),l.appendChild(v),l.appendChild(T),l.appendChild($),l.appendChild(O),l.appendChild(W),l}async function f(){if(!G.currentSubStatus){J(h("select_substatus"),{error:!0});return}let b=Tt(G,r,s);b?(Ze(b),J(h("copiado_sucesso")),H.playClick()):J(h("select_substatus"),{error:!0})}async function S(){if(!G.currentSubStatus){J(h("select_substatus"),{error:!0});return}let b=Tt(G,r,s);Ze(b),ee();let C=Je(),l=await mt();if(l){l.focus(),document.execCommand("insertHTML",!1,b),ut(l);let v=document.getElementById("email-automation-checkbox");(!v||v.checked)&&G.currentSubStatus&&je[G.currentSubStatus]&&await ft(je[G.currentSubStatus]),J(h("inserido_copiado")),H.playSuccess(),L()}C()}function D(){if(G.currentSubStatus){if(G.currentCaseType==="lm")G.removeField("ON_CALL");else{let b=Te[G.currentSubStatus];b&&b.templateFields.includes("ON_CALL")&&G.addFieldAt("ON_CALL",1)}G.isPortugalCase?(G.addFieldAt("CASO_PORTUGAL",1),G.addFieldAt("CONSENTIU_GRAVACAO",2)):(G.removeField("CASO_PORTUGAL"),G.removeField("CONSENTIU_GRAVACAO"))}}function L(){G.reset(),r.reset(),s.reset(),Z(),ge.clearEmergency(),o.querySelectorAll("select").forEach(C=>C.value=""),o.querySelector("#sub-status-select").disabled=!0;let b=document.getElementById("email-automation-toggle-row");b&&(b.style.display="none"),x.innerHTML="",c.style.display="none",d.style.display="flex",d.style.opacity="1",p.style.display="none",document.getElementById("manual-task-toggle")&&(document.getElementById("manual-task-toggle").style.display="none"),r.selectionElement.style.display="none",r.screenshotsElement.style.display="none"}async function Y(b=!1){let C={};x.querySelectorAll("input, textarea, select").forEach(O=>{(O.id.startsWith("field-")||O.id==="consent-select")&&(C[O.id]=O.value)});let l="Cliente",v="---";if(!b)try{let O=await xe();l=O.advertiserName,v=O.cid}catch(O){console.warn("Erro ao coletar pageData:",O)}let T=r.getCheckedElements().map(O=>({key:O.value,count:O.count})),$=T.map(O=>{let W=Ce[O.key];return W?W.name:O.key});return{currentCaseType:G.currentCaseType,currentLang:G.currentLang,isPortugalCase:G.isPortugalCase,consent:G.consent,tagSupportUsed:G.tagSupportUsed,forcedScreenshots:[...G.forcedScreenshots],excludedFields:[...G.excludedFields],activeFields:G.activeFields,status:G.currentStatus,subStatus:G.currentSubStatus,formData:C,activeTasks:T,summaryTags:$,clientName:l,cid:v,timestamp:new Date().toISOString()}}let k=b=>new Promise(C=>setTimeout(C,b));async function U(b){G.setLanguage(b.currentLang||"pt"),G.setCaseType(b.currentCaseType||"bau"),G.setPortugalCase(b.isPortugalCase||!1),G.setConsent(b.consent||!1),G.setExcludedFields(b.excludedFields||[]),b.activeFields&&G.setActiveFields(b.activeFields);let C=o.querySelector(`#lang-selector button[data-lang="${G.currentLang}"]`);C&&C.classList.add("active"),o.querySelectorAll("#lang-selector button").forEach(T=>{T!==C&&T.classList.remove("active")});let l=o.querySelector(`#type-selector button[data-type="${G.currentCaseType}"]`);l&&l.classList.add("active"),o.querySelectorAll("#type-selector button").forEach(T=>{T!==l&&T.classList.remove("active")});let v=o.querySelector(`#portugal-selector button[data-val="${G.isPortugalCase}"]`);if(v&&v.classList.add("active"),o.querySelectorAll("#portugal-selector button").forEach(T=>{T!==v&&T.classList.remove("active")}),b.status){let T=o.querySelector("#main-status-select");T.value=b.status,G.setStatus(b.status);let $=o.querySelector("#sub-status-select");if(j(b.status,$),await k(50),b.subStatus){if($.value=b.subStatus,G.setSubStatus(b.subStatus),_(b.subStatus),await k(100),b.tagSupportUsed!==void 0){G.setTagSupportUsed(b.tagSupportUsed);let O=s.element.querySelector('input[value="Sim"]'),W=s.element.querySelector('input[value="N\xE3o"]');b.tagSupportUsed&&O?O.checked=!0:W&&(W.checked=!0),s.element.querySelector("div:last-child").style.display=b.tagSupportUsed?"none":"block"}b.forcedScreenshots&&G.setForcedScreenshots(b.forcedScreenshots);for(let O in b.formData){let W=document.getElementById(O);W&&(W.value=b.formData[O],G.updateField(O,W.value))}b.activeTasks&&(b.activeTasks.forEach(O=>r.setTaskCount(O.key,O.count)),G.setActiveTasks(r.getCheckedElements()))}}G.isDirty=!1}function h(b){return Se[G.currentLang]?.[b]||Se.pt?.[b]||b}function K(){let b=document.createElement("div");return b.innerHTML='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="16 3 21 3 21 8"></polyline><line x1="4" y1="20" x2="21" y2="3"></line><polyline points="21 16 21 21 16 21"></polyline><line x1="15" y1="15" x2="21" y2="21"></line><line x1="4" y1="4" x2="9" y2="9"></line></svg>',b.style.cssText="width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; border-radius: 50%; cursor: pointer; color: #9AA0A6; transition: all 0.2s;",b.onclick=C=>{C.stopPropagation(),I()},b.title="Alternar para Split & Transfer",b}function V(){let b=document.createElement("div");return b.id="notes-empty-state",b.style.cssText=`
            display: flex;
            flex-direction: column;
            align-items: center;
            justify-content: center;
    ...
    `,document.head.appendChild(g),Object.assign(c.style,me,{width:"850px",height:"650px",display:"none",flexDirection:"column",fontFamily:"'-apple-system', 'BlinkMacSystemFont', 'Segoe UI', 'Roboto', 'Helvetica Neue', 'Arial', sans-serif",borderRadius:"12px",overflow:"hidden"});let m=pe(c,"Email Assistant",t,"Refatora\xE7\xE3o completa do m\xF3dulo de e-mail para uma experi\xEAncia moderna e eficiente.",{popup:c},()=>N()),u=document.createElement("div");Object.assign(u.style,{display:"flex",flex:"1",overflow:"hidden",backgroundColor:r.bgApp});let y=document.createElement("div");Object.assign(y.style,{width:"320px",backgroundColor:"#EFEFF0",borderRight:`1px solid ${r.borderSubtle}`,display:"flex",flexDirection:"column",flexShrink:"0"});let x=document.createElement("div");Object.assign(x.style,{padding:"16px",borderBottom:`1px solid ${r.borderSubtle}`,position:"relative"});let d=document.createElement("input");d.placeholder="Buscar templates...",Object.assign(d.style,{width:"100%",padding:"10px 14px 10px 36px",borderRadius:"10px",border:"1.5px solid transparent",backgroundColor:"#E3E3E8",fontSize:"15px",outline:"none",boxSizing:"border-box",color:r.textPrimary,backgroundImage:`url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="%238A8A8E" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>')`,backgroundRepeat:"no-repeat",backgroundPosition:"12px center",transition:"all 0.2s ease-in-out"}),d.onfocus=()=>{d.style.backgroundColor="#FFFFFF",d.style.borderColor=r.primary,d.style.boxShadow="0 0 0 4px rgba(0, 122, 255, 0.1)",d.style.transform="scale(1.02)"},d.onblur=()=>{d.style.backgroundColor="#E3E3E8",d.style.borderColor="transparent",d.style.boxShadow="none",d.style.transform="scale(1)"};let p=document.createElement("div");p.id="email-template-list",Object.assign(p.style,{flex:"1",overflowY:"auto",padding:"8px",scrollBehavior:"smooth"});let w=document.createElement("div");w.innerHTML="\u2715",Object.assign(w.style,{position:"absolute",right:"26px",top:"50%",transform:"translateY(-50%)",fontSize:"10px",color:"#fff",cursor:"pointer",display:"none",backgroundColor:"#C7C7CC",width:"16px",height:"16px",borderRadius:"50%",textAlign:"center",lineHeight:"16px",fontWeight:"bold"}),w.onclick=()=>{d.value="",n="",w.style.display="none",f(),d.focus()},x.appendChild(d),x.appendChild(w),y.appendChild(x),y.appendChild(p);let E=document.createElement("div");Object.assign(E.style,{flex:"1",display:"flex",flexDirection:"column",overflow:"hidden",backgroundColor:r.bgApp,transition:"all 0.3s cubic-bezier(0.25, 1, 0.5, 1)"});let M=document.createElement("div");Object.assign(M.style,{padding:"20px",borderBottom:`1px solid ${r.borderSubtle}`,backgroundColor:r.bgSurface,maxHeight:"250px",overflowY:"auto",display:"none"});let P=document.createElement("div");Object.assign(P.style,{flex:"1",display:"flex",flexDirection:"column",padding:"20px",backgroundColor:r.bgApp,overflow:"hidden"});let z=document.createElement("div");Object.assign(z.style,{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px"});let Z=document.createElement("span");Z.textContent="Preview do E-mail",Object.assign(Z.style,{fontSize:"12px",fontWeight:"600",color:r.textSecondary,textTransform:"uppercase",letterSpacing:"0.5px"});let ee=document.createElement("div");Object.assign(ee.style,{display:"flex",gap:"8px"});let I=(k,U=!1)=>{let h=document.createElement("button");return h.textContent=k,Object.assign(h.style,{padding:"8px 14px",borderRadius:"10px",border:U?"none":`1.5px solid ${r.primary}`,background:U?r.primary:"transparent",color:U?"#fff":r.primary,fontSize:"13px",fontWeight:"600",cursor:"pointer",transition:"all 0.2s cubic-bezier(0.25, 1, 0.5, 1)",boxShadow:U?"0 4px 12px rgba(0, 122, 255, 0.3)":"none"}),h.onmouseenter=()=>{U?(h.style.backgroundColor="#0062CC",h.style.transform="translateY(-1px)",h.style.boxShadow="0 6px 16px rgba(0, 122, 255, 0.4)"):h.style.backgroundColor="rgba(0, 122, 255, 0.05)"},h.onmouseleave=()=>{U?(h.style.backgroundColor=r.primary,h.style.transform="translateY(0)",h.style.boxShadow="0 4px 12px rgba(0, 122, 255, 0.3)"):(h.style.backgroundColor="transparent",h.style.transform="translateY(0)")},h.onmousedown=()=>h.style.transform="scale(0.94)",h.onmouseup=()=>h.style.transform="scale(1)",h},F=I("Copiar HTML"),R=I("Preencher no CRM",!0),j=I("Smart CR");j.style.borderColor="#E67E22",j.style.color="#E67E22",j.style.display="none",ee.appendChild(j),ee.appendChild(F),ee.appendChild(R),z.appendChild(Z),z.appendChild(ee);let _=document.createElement("div");_.contentEditable="true",Object.assign(_.style,{flex:"1",backgroundColor:r.bgSurface,border:`1px solid ${r.borderSubtle}`,borderRadius:"8px",padding:"20px",fontSize:"15px",lineHeight:"1.6",color:r.textPrimary,overflowY:"auto",outline:"none",boxShadow:"inset 0 1px 2px rgba(0,0,0,0.02)"}),P.appendChild(z),P.appendChild(_),Y(),E.appendChild(M),E.appendChild(P),u.appendChild(y),u.appendChild(E),c.appendChild(m),c.appendChild(u);let A=document.createElement("div");Object.assign(A.style,Ee),c.appendChild(A),ke(c,A),document.body.appendChild(c);function N(){e=!e,e?(c.style.display="flex",Wt(c),o.length===0&&q()):c.style.display="none",ue(e,c,"cw-btn-email")}async function q(){p.innerHTML='<div style="padding: 20px; text-align: center; color: #5f6368;">Carregando...</div>',o=await xo.getTemplates(),f()}function f(){p.innerHTML="";let k=o.filter(X=>X.name.toLowerCase().includes(n.toLowerCase())||X.category.toLowerCase().includes(n.toLowerCase())),U=Object.entries(je).filter(([X,b])=>b&&(X.toLowerCase().includes(n.toLowerCase())||b.toLowerCase().includes(n.toLowerCase()))).map(([X,b])=>({id:X,name:X.replace(/_/g," "),category:"\u26A1 Smart CRs",code:b,isSmartCR:!0})),h=ye.getSnippets("email").filter(X=>X.title.toLowerCase().includes(n.toLowerCase())||X.subject&&X.subject.toLowerCase().includes(n.toLowerCase())).map(X=>{let b=[],C=X.content.match(/\[([^\]]+)\]/g);return C&&[...new Set(C)].forEach(l=>{b.push({key:l,label:l.replace("[","").replace("]",""),type:l.toLowerCase().includes("data")?"date":"text",auto:l.toLowerCase().includes("nome")&&l.toLowerCase().includes("seu")?"agentName":null})}),{id:X.id||`snippet-${Math.random()}`,name:X.title,category:"\u{1F464} Pessoal",subject:X.subject||"Sem Assunto",template:X.content,placeholders:b}}),K=[...k,...U,...h];if(K.length===0){p.innerHTML=`
                ...
                </div>`;return}[...new Set(K.map(X=>X.category))].sort((X,b)=>X.localeCompare(b)).forEach(X=>{let b=s.has(X)||n.length>0,C=K.filter(W=>W.category===X),l=document.createElement("div");Object.assign(l.style,{padding:"12px 16px 12px 24px",fontSize:"11px",fontWeight:"700",color:r.textSecondary,textTransform:"uppercase",letterSpacing:"0.8px",position:"sticky",top:"-8px",backgroundColor:"rgba(239, 239, 240, 0.9)",zIndex:"10",backdropFilter:"blur(20px)",margin:"0 -8px 8px -8px",borderBottom:`0.5px solid ${r.borderSubtle}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between",userSelect:"none",transition:"background-color 0.2s ease"}),l.onmouseenter=()=>l.style.backgroundColor="rgba(230, 230, 232, 0.9)",l.onmouseleave=()=>l.style.backgroundColor="rgba(239, 239, 240, 0.9)";let v=document.createElement("span");v.textContent=X,l.appendChild(v);let T=document.createElement("span");T.textContent=C.length,Object.assign(T.style,{backgroundColor:"rgba(0, 0, 0, 0.05)",padding:"2px 8px",borderRadius:"10px",fontSize:"10px",color:r.textSecondary});let $=document.createElement("span");$.innerHTML=b?"\u{10012A}":"\u{10012B}",$.innerHTML=b?"\u25BE":"\u25B8",$.style.marginLeft="8px",$.style.transition="transform 0.3s ease";let O=document.createElement("div");O.style.display="flex",O.style.alignItems="center",O.appendChild(T),O.appendChild($),l.appendChild(O),l.onclick=()=>{s.has(X)?s.delete(X):s.add(X),f()},p.appendChild(l),b&&C.forEach(W=>{let te=a&&a.id===W.id,Q=document.createElement("div");if(Object.assign(Q.style,{padding:"12px 14px",fontSize:"14px",cursor:"pointer",transition:"all 0.3s cubic-bezier(0.25, 1, 0.5, 1)",borderRadius:"10px",color:r.textPrimary,margin:"4px 6px",display:"flex",alignItems:"center",gap:"12px",backgroundColor:te?r.primary:r.bgSurface,boxShadow:te?"0 4px 12px rgba(0, 122, 255, 0.3)":"0 1px 2px rgba(0,0,0,0.05)",border:te?"none":`1px solid ${r.borderSubtle}`,position:"relative",overflow:"hidden"}),te){let ie=document.createElement("div");Object.assign(ie.style,{position:"absolute",left:"0",top:"0",bottom:"0",width:"4px",backgroundColor:"#fff",borderRadius:"0 4px 4px 0"}),Q.appendChild(ie)}let oe=document.createElement("span");oe.innerHTML=W.isSmartCR?"\u26A1":W.category==="\u{1F464} Pessoal"?"\u{1F464}":"\u{1F4C4}",oe.style.fontSize="12px",oe.style.opacity="0.7",oe.style.flexShrink="0",Q.appendChild(oe);let ae=document.createElement("span");ae.textContent=W.name,ae.style.overflow="hidden",ae.style.textOverflow="ellipsis",ae.style.whiteSpace="nowrap",ae.style.flex="1",Q.appendChild(ae),te&&(Q.style.color="#fff",Q.style.fontWeight="600",oe.style.opacity="1"),Q.onmouseenter=()=>{te||(Q.style.backgroundColor="#f8f8f9",Q.style.transform="translateY(-1px) scale(1.01)",Q.style.boxShadow="0 4px 8px rgba(0,0,0,0.08)",Q.style.borderColor="rgba(0, 122, 255, 0.2)")},Q.onmouseleave=()=>{te||(Q.style.backgroundColor=r.bgSurface,Q.style.transform="translateY(0) scale(1)",Q.style.boxShadow="0 1px 2px rgba(0,0,0,0.05)",Q.style.borderColor=r.borderSubtle)},Q.onmousedown=()=>{Q.style.transform=te?"scale(0.97)":"scale(0.98)"},Q.onmouseup=()=>{Q.style.transform=te?"scale(1)":"translateY(-1px) scale(1.01)"},Q.onclick=()=>{D(W)},p.appendChild(Q)})})}let S=null;async function D(k){a?.id!==k.id&&(a=k,S&&clearTimeout(S),E.style.opacity="0",E.style.transform="translateY(5px)",S=setTimeout(()=>{j.style.display=k.isSmartCR?"block":"none",R.style.display=k.isSmartCR?"none":"block",F.style.display=k.isSmartCR?"none":"block",f(),L(),Y(),E.style.opacity="1",E.style.transform="translateY(0)",S=null},150))}function L(){if(M.innerHTML="",!a||a.isSmartCR){a?.isSmartCR?(M.style.display="block",M.innerHTML=`<div style="padding: 12px; font-size: 13px; color: #856404; background: #FFF3CD; border: 1px solid #FFEEBA; border-radius: 8px; display: flex; align-items: center; gap: 8px;">
...
CID: ${C.cid||"---"}
Case ID: ${C.caseId||"---"}
E-mail: ${C.clientEmail||"---"}`;v&&(v.value=$),T&&(T.style.display="block",T.style.maxHeight="300px",T.style.opacity="1",T.scrollIntoView({behavior:"smooth",block:"end"}))})}function x(){r=!r,ue(r,c,"cw-btn-script"),r?(u(),m||(m=setInterval(u,2e3))):m&&(clearInterval(m),m=null)}let d=pe(c,"Call Script",t,"Guia interativo para condu\xE7\xE3o de chamadas.",g,()=>{x()});c.appendChild(d);let p=document.createElement("div");Object.assign(p.style,a.contextBanner),p.innerHTML=`
  ...
  `;let w=p.querySelector("#csa-toggle-options"),E=p.querySelector("#csa-options-content"),M=p.querySelector("#csa-options-arrow"),P=!1;w.onclick=()=>{P=!P,M.style.transform=P?"rotate(180deg)":"rotate(0deg)",E.style.maxHeight=P?"400px":"0",E.style.opacity=P?"1":"0",E.style.marginTop=P?"8px":"0",H.playClick()};let z=p.querySelector("#cw-pill-message"),Z=p.querySelector("#cw-am-copy-final"),ee=p.querySelector("#cw-am-message-area");z.onmouseenter=()=>{z.style.borderColor="#007AFF",z.style.boxShadow="0 2px 8px rgba(0,0,0,0.05)"},z.onmouseleave=()=>{z.style.borderColor="#DADCE0",z.style.boxShadow="0 1px 2px rgba(0,0,0,0.02)"},z.addEventListener("click",()=>{y()}),Z.addEventListener("click",()=>{ee.value&&(navigator.clipboard.writeText(ee.value),J("Mensagem copiada!"),H.playSuccess(),Z.style.background="#34A853",Z.textContent="Copiado!",setTimeout(()=>{Z.style.background="#1A73E8",Z.textContent="Copiar Mensagem Final"},2e3))});let I=(C,l)=>{let v=p.querySelector(C),T=p.querySelector(l);v.onclick=()=>{let $=T.textContent;!$||$.includes("---")||$.includes("N\xE3o encontrado")||(navigator.clipboard.writeText($),H.playSuccess(),v.classList.add("copied"),setTimeout(()=>v.classList.remove("copied"),1500))}};c.appendChild(p);let F=document.createElement("div");Object.assign(F.style,a.progressBarContainer);let R=document.createElement("div");R.className="csa-progress-fill",Object.assign(R.style,a.progressBarFill),F.appendChild(R),c.appendChild(F);let j=document.createElement("div");j.id="csa-content",Object.assign(j.style,a.contentArea),c.appendChild(j);let _=document.createElement("div");Object.assign(_.style,a.footer);let A=document.createElement("span");A.textContent="by lucaste@",Object.assign(A.style,{fontSize:"10px",color:"#bdc1c6"});let N=document.createElement("button");N.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"></path><path d="M3 3v5h5"></path></svg> Resetar Script',Object.assign(N.style,a.resetBtn),N.onmouseenter=()=>N.style.background="#fce8e6",N.onmouseleave=()=>N.style.background="transparent",N.onclick=()=>{N.style.transform="scale(0.9)",setTimeout(()=>N.style.transform="scale(1)",150);for(let C in n)delete n[C];V()},_.appendChild(A),_.appendChild(N),c.appendChild(_);let q=document.createElement("div");Object.assign(q.style,{display:"flex",flexDirection:"column",gap:"8px",marginBottom:"16px"});let f=document.createElement("div");f.className="csa-segmented-control",f.innerHTML=`
  ...
  `,p.onfocus=()=>{x.style.boxShadow="0 4px 12px rgba(26,115,232,0.15)",x.style.border="1px solid #1a73e8"},p.onblur=()=>{x.style.boxShadow="0 2px 6px rgba(0,0,0,0.04)",x.style.border="1px solid transparent"},x.appendChild(d),x.appendChild(p),y.appendChild(x),u.appendChild(y);let w=document.createElement("div");w.style.cssText="flex: 1; overflow-y: auto; padding: 0 24px 40px 24px; scroll-behavior: smooth;",u.appendChild(w);let E=null;function M(){if(E)return;E=document.createElement("div"),E.style.cssText=`
          position: absolute; bottom: 0; left: 0; width: 100%; height: 100%;
          background: rgba(255,255,255,0.98); z-index: 20;
          display: flex; flex-direction: column;
          transform: translateY(100%); transition: transform 0.3s cubic-bezier(0.2, 0.8, 0.2, 1);
          box-shadow: 0 -4px 20px rgba(0,0,0,0.1);
      `;let A=document.createElement("div");A.style.cssText="padding: 16px 24px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #F1F3F4;",A.innerHTML='<span style="font-size: 16px; font-weight: 700; color: #202124;">\u{1F552} Recentes</span>';let N=document.createElement("button");N.innerHTML='<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>',N.style.cssText="background: none; border: none; cursor: pointer; color: #5f6368;",N.onclick=()=>{z(),n=!1,F()},A.appendChild(N),E.appendChild(A);let q=document.createElement("div");q.id="cw-history-list",q.style.cssText="flex: 1; overflow-y: auto; padding: 20px; background: #F8F9FA;",E.appendChild(q),u.appendChild(E)}function P(){E||M();let A=E.querySelector("#cw-history-list");A.innerHTML="";let N=na();N.length===0?A.innerHTML='<div style="text-align: center; color: #999; margin-top: 60px; font-size:13px;">Nada por aqui ainda.</div>':N.forEach(q=>{let f=j(q,$e[q._originalCat],!0,q._originalCat);A.appendChild(f)}),requestAnimationFrame(()=>E.style.transform="translateY(0)")}function z(){E&&(E.style.transform="translateY(100%)")}function Z(){m.innerHTML="";let A=ee("history","Recentes",$e.history);A.id="cw-sidebar-btn-history",A.onclick=()=>{H.playClick(),n=!n,n?P():z(),F()},m.appendChild(A);let N=document.createElement("div");N.style.cssText="width: 32px; height: 1px; background: rgba(0,0,0,0.08); margin: 4px 0;",m.appendChild(N),Object.keys(Qe).forEach(q=>{let f=Qe[q],S=ee(q,f.label,$e[q]);S.id=`cw-sidebar-btn-${q}`,S.onclick=()=>{H.playClick(),n&&(n=!1,z()),I(q)},m.appendChild(S)})}function ee(A,N,q){let f=document.createElement("div");f.style.cssText=`
          width: 56px; height: 56px; border-radius: 16px;
          display: flex; flex-direction: column; align-items: center; justify-content: center;
          cursor: pointer; color: ${i.textSecondary}; 
          transition: all 0.2s cubic-bezier(0.2, 0.0, 0.2, 1);
          position: relative;
      `,f.title=N,f.dataset.key=A;let S=document.createElement("div");S.style.cssText="width: 24px; height: 24px; margin-bottom: 2px; transition: transform 0.2s;",S.innerHTML=q||$e.tasks;let D=document.createElement("div");return D.style.cssText="font-size: 9px; font-weight: 600; opacity: 0.7; letter-spacing: 0.3px;",D.textContent=N,f.appendChild(S),f.appendChild(D),f.onmouseenter=()=>{a!==A&&!(A==="history"&&n)&&(f.style.background="#F1F3F4",S.style.transform="scale(1.1)")},f.onmouseleave=()=>{a!==A&&!(A==="history"&&n)&&(f.style.background="transparent",S.style.transform="scale(1)")},f}function I(A){let N=document.getElementById(`cat-anchor-${A}`);N&&(N.scrollIntoView({behavior:"smooth",block:"start"}),a=A,F())}function F(){Object.keys(Qe).forEach(N=>{let q=m.querySelector(`#cw-sidebar-btn-${N}`);if(q)if(a===N&&!n){let f=xt[N];q.style.background=f.bg,q.style.color=f.color,q.querySelector("div:first-child").style.transform="scale(1.1)"}else q.style.background="transparent",q.style.color=i.textSecondary,q.querySelector("div:first-child").style.transform="scale(1)"});let A=m.querySelector("#cw-sidebar-btn-history");A&&(n?(A.style.background="#3C4043",A.style.color="#FFFFFF"):(A.style.background="transparent",A.style.color=i.textSecondary))}function R(){if(w.innerHTML="",e.trim()!==""){let N=[];if(Object.entries(Qe).forEach(([f,S])=>{let D=S.links.filter(L=>L.name.toLowerCase().includes(e.toLowerCase())||L.desc.toLowerCase().includes(e.toLowerCase()));N.push(...D.map(L=>({...L,_cat:f})))}),N.length===0){w.innerHTML='<div style="text-align:center; padding: 60px; color:#999; font-size:13px;">Nada encontrado.</div>';return}let q=document.createElement("div");q.innerHTML="Resultados da busca",q.style.cssText="font-size:12px; font-weight:700; color:#5f6368; margin:20px 0 10px; text-transform:uppercase; letter-spacing:0.5px;",w.appendChild(q),N.forEach(f=>{let S=j(f,$e[f._cat],!1,f._cat);w.appendChild(S)});return}Object.entries(Qe).forEach(([N,q])=>{let f=xt[N],S=document.createElement("div"),D=document.createElement("div");D.id=`cat-anchor-${N}`,D.style.cssText=`
              display: flex; align-items: center; gap: 8px;
          ...
          `,D.innerHTML=`
            ...
            ${q.label}
          `,S.appendChild(D);let L=document.createElement("div");L.style.cssText="display: grid; grid-template-columns: 1fr; gap: 8px;",q.links.forEach(Y=>{let k=j(Y,$e[N],!1,N);L.appendChild(k)}),S.appendChild(L),w.appendChild(S)});let A=document.createElement("div");A.style.height="80px",w.appendChild(A)}function j(A,N,q,f){let S=document.createElement("div"),D=xt[f]||xt.history;S.style.cssText=`
      ...
      `;let L=document.createElement("div");L.style.cssText=`
          ...
          display: flex; align-items: center; justify-content: center;
          flex-shrink: 0; transition: all 0.2s;
      `,L.innerHTML=N||$e.tasks;let Y=L.querySelector("svg");Y&&(Y.style.width="22px",Y.style.height="22px");let k=document.createElement("div");k.style.cssText="flex: 1; display: flex; flex-direction: column; gap: 2px; overflow: hidden;";let U=document.createElement("div");U.style.cssText=`font-size: 14px; font-weight: 600; color: ${i.textPrimary}; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;`,U.textContent=A.name;let h=document.createElement("div");h.style.cssText=`font-size: 12px; color: ${i.textSecondary}; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;`,h.textContent=A.desc,k.appendChild(U),k.appendChild(h);let K=document.createElement("div");return K.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>',K.style.cssText=`
          width: 32px; height: 32px; border-radius: 8px;
          display: flex; align-items: center; justify-content: center;
          color: #9AA0A6; transition: all 0.2s; opacity: 0;
      `,K.title="Copiar URL",S.onmouseenter=()=>{S.style.transform="translateY(-2px)",S.style.boxShadow="0 8px 20px rgba(0,0,0,0.08)",S.style.borderColor="rgba(0,0,0,0.05)",S.style.borderLeft=`4px solid ${D.color}`,K.style.opacity="1",K.style.background="#F1F3F4"},S.onmouseleave=()=>{S.style.transform="translateY(0)",S.style.boxShadow="0 1px 3px rgba(0,0,0,0.05)",S.style.border="1px solid transparent",K.style.opacity="0",K.style.background="transparent"},S.onclick=()=>{!q&&f&&So(A,f),window.open(A.url,"_blank")},K.onclick=V=>{V.stopPropagation(),H.playClick(),navigator.clipboard.writeText(A.url),!q&&f&&So(A,f),J("Link copiado!")},S.appendChild(L),S.appendChild(k),S.appendChild(K),S}p.addEventListener("input",A=>{e=A.target.value,R()});function _(){o=!o,ue(o,s,"cw-btn-links")}return document.body.appendChild(s),Z(),R(),_}var Ne=[];function qt(t){Ne=t}var ia=60*1e3;window._cwIsAdmin=!1;window._cwCurrentUser=null;function Ao(){let t="v4.9",e=!1,o=null,a=null;function n(f){if(!f)return"";try{let S=new Date(f);return isNaN(S.getTime())?String(f):S.toLocaleString("pt-BR",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}).replace(","," \xE0s")}catch{return String(f)}}if(!document.getElementById("cw-broadcast-hd-css")){let f=document.createElement("style");f.id="cw-broadcast-hd-css",f.innerHTML=`
        @keyframes cw-pulse {
      ...
      `,d.querySelectorAll('input[name="cw-bc-type"]').forEach(L=>{L.addEventListener("change",()=>{d.querySelectorAll(".cw-radio-option").forEach(Y=>Y.classList.remove("checked")),L.parentElement.classList.add("checked")})}),setTimeout(()=>{let L=d.querySelector(".cw-radio-option.info");L&&L.classList.add("checked")},100);let f=d.querySelector("#cw-bc-cancel"),S=d.querySelector("#cw-bc-close-x"),D=d.querySelector("#cw-bc-send");f.onclick=P,S.onclick=P,D.onclick=z,g.appendChild(d)}function M(f=null){if(!d)return;let S=d.querySelector("#cw-editor-title-label"),D=d.querySelector("#cw-bc-title"),L=d.querySelector("#cw-bc-text"),Y=d.querySelector("#cw-bc-send");if(f){a=f.id,S.textContent="Editar Aviso",D.value=f.title||"",L.value=f.text||"",Y.textContent="Salvar Altera\xE7\xF5es";let k=f.type||"info",U=d.querySelector(`input[name="cw-bc-type"][value="${k}"]`);U&&U.click()}else{a=null,S.textContent="Novo Aviso",D.value="",L.value="",Y.textContent="Publicar";let k=d.querySelector('input[name="cw-bc-type"][value="info"]');k&&k.click()}d.classList.add("active"),setTimeout(()=>D.focus(),300)}function P(){d&&d.classList.remove("active"),a=null}async function z(){let f=d.querySelector("#cw-bc-send"),S=d.querySelector("#cw-bc-title"),D=d.querySelector("#cw-bc-text"),L=d.querySelector('input[name="cw-bc-type"]:checked'),Y=L?L.value:"info";if(!S.value.trim()||!D.value.trim()){J("Preencha todos os campos!",{error:!0});return}f.textContent="Salvando...",f.style.opacity="0.7";let k=!1;a?k=await le.updateBroadcast(a,{title:S.value,text:D.value,type:Y}):k=await le.sendBroadcast({title:S.value,text:D.value,type:Y,author:window._cwCurrentUser||"admin"}),k?(J(a?"Atualizado!":"Publicado!"),H.playSuccess(),P(),setTimeout(()=>I(),1500)):(J("Erro ao salvar. Verifique a conex\xE3o.",{error:!0}),f.textContent=a?"Salvar Altera\xE7\xF5es":"Publicar",f.style.opacity="1")}async function Z(f){if(await he("Confirma a exclus\xE3o deste aviso?",{danger:!0}))if(await le.deleteBroadcast(f)){J("Aviso removido."),H.playClick();let L=Ne.findIndex(Y=>Y.id===f);L>-1&&Ne.splice(L,1),R(),setTimeout(()=>I(),1500)}else J("Erro ao excluir.",{error:!0})}let ee=document.createElement("div");ee.className="cw-nice-scroll",Object.assign(ee.style,i.feedContainer),g.appendChild(ee);async function I(){e&&(w.style.display="block",w.innerHTML="\u{1F504} Sincronizando...");try{let f=await le.fetchData();f&&f.broadcast&&(qt(f.broadcast),F(),e&&(R(),w.innerHTML='<span style="color:#137333">\u2713 Atualizado</span>',setTimeout(()=>{w.style.display="none"},1500)))}catch{e&&(w.innerHTML="\u26A0\uFE0F Offline")}}function F(){let f=document.getElementById("cw-btn-broadcast");if(!f)return;let S=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]");if(Ne.some(L=>!S.includes(L.id))){if(f.classList.add("has-new"),!f.querySelector(".cw-badge")){let L=document.createElement("div");L.className="cw-badge",Object.assign(L.style,{position:"absolute",top:"8px",right:"8px",width:"8px",height:"8px",backgroundColor:"#d93025",borderRadius:"50%",border:"1px solid #fff",zIndex:"10"}),f.appendChild(L)}}else{f.classList.remove("has-new");let L=f.querySelector(".cw-badge");L&&L.remove()}}function R(){ee.innerHTML="";let f=g.querySelector("#cw-bau-widget");f&&f.remove();let S=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]"),D=[...Ne].sort((h,K)=>{let V=new Date(h.date).getTime()||0;return(new Date(K.date).getTime()||0)-V}),L=D.findIndex(h=>h.title&&h.title.toLowerCase().includes("disponibilidade bau"));if(L!==-1){let h=D[L];D.splice(L,1);let K=document.createElement("div");K.id="cw-bau-widget",Object.assign(K.style,i.bauContainer);let V=[],X=(h.text||"").split(`
`),b=/\d{1,2}\/\d{1,2}/,C="\u{1F4C5}";if(X.forEach(O=>{/🇧🇷|🇵🇹|PT|BR|BRASIL|BRAZIL|PORTUGAL|LISBOA/i.test(O)?C="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM|ESPANHA|SPAIN|MEXICO|MÉXICO/i.test(O)&&(C="\u{1F1EA}\u{1F1F8}");let W=O.match(b);if(W){let te=W[0],Q=C;/🇧🇷|🇵🇹|PT|BR/i.test(O)?Q="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM/i.test(O)&&(Q="\u{1F1EA}\u{1F1F8}"),V.some(ae=>ae.flag===Q&&ae.date===te)||V.push({flag:Q,date:te})}}),V.length===0){let O=(h.text||"").match(/\d{1,2}\/\d{1,2}/g);O&&[...new Set(O)].forEach(W=>V.push({flag:"\u{1F4C5}",date:W}))}let l="",v='<button id="cw-bau-toggle-btn" class="cw-btn-interactive" style="background:rgba(255,255,255,0.7); border:1px solid rgba(139, 92, 246, 0.4); border-radius:12px; padding:8px 12px; color:#6D28D9; font-size:12px; font-weight:600;">Detalhes</button>';window._cwIsAdmin&&(v=`
          ...
          `,w.after(K);let T=K.querySelector("#cw-bau-toggle-btn"),$=K.querySelector("#cw-bau-full");if(T&&$&&(T.onclick=()=>{let O=$.style.display==="none";$.style.display=O?"block":"none",T.textContent=O?"Ocultar":"Detalhes"}),window._cwIsAdmin){let O=K.querySelector(".cw-bau-edit");O&&(O.onclick=()=>M(h))}}let Y=D.sort((h,K)=>{let V=S.includes(h.id),X=S.includes(K.id);return V===X?0:V?1:-1});if(Y.length===0&&!L){let h=document.createElement("div");Object.assign(h.style,i.emptyState),h.innerHTML=`
           ...
           `,ee.appendChild(h)}let k=Y.filter(h=>!S.includes(h.id)),U=Y.filter(h=>S.includes(h.id));if(k.forEach(h=>ee.appendChild(j(h,!1))),U.length>0){let h=document.createElement("div");Object.assign(h.style,i.historyDivider),h.innerHTML=`<span>Hist\xF3rico (${U.length})</span><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>`;let K=document.createElement("div");Object.assign(K.style,i.historyContainer),U.forEach(X=>K.appendChild(j(X,!0)));let V=!1;h.onclick=()=>{H.playClick(),V=!V,K.style.display=V?"flex":"none",h.querySelector("svg").style.transform=V?"rotate(180deg)":"rotate(0deg)"},ee.appendChild(h),ee.appendChild(K)}}function j(f,S){let D=document.createElement("div");Object.assign(D.style,S?i.cardHistory:i.card);let L=s[f.type]||s.info,Y=document.createElement("div");Object.assign(Y.style,i.cardHeader);let k=document.createElement("div");Object.assign(k.style,i.typeTag,{color:L.color,background:L.bg}),k.innerHTML=`${L.icon} <span>${f.type}</span>`;let U=document.createElement("span");if(Object.assign(U.style,i.dateTag),U.textContent=n(f.date),Y.appendChild(k),S)Y.appendChild(U);else{let b=document.createElement("button");b.className="cw-btn-interactive",Object.assign(b.style,i.dismissBtn),b.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"></polyline></svg>',b.onmouseenter=()=>{b.style.color="#1e8e3e",b.style.background="#e6f4ea",b.style.borderColor="#1e8e3e"},b.onmouseleave=()=>{b.style.color="#5f6368",b.style.background="#fff",b.style.borderColor="rgba(0,0,0,0.1)"},b.onclick=C=>{C.stopPropagation(),H.playClick(),D.style.transform="translateX(20px)",D.style.opacity="0",setTimeout(()=>{let l=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]");l.push(f.id),localStorage.setItem("cw_read_broadcasts",JSON.stringify(l)),R(),F()},200)},Y.appendChild(b)}let h=document.createElement("div");Object.assign(h.style,i.cardContent);let K=document.createElement("div");Object.assign(K.style,i.msgTitle),K.textContent=f.title;let V=document.createElement("div");Object.assign(V.style,i.msgBody),V.innerHTML=c(f.text);let X=document.createElement("div");if(Object.assign(X.style,i.msgMeta),X.innerHTML=`Publicado por <b>${f.author||"Sistema"}</b>`,S||(X.innerHTML+=` \u2022 ${n(f.date)}`),h.appendChild(K),h.appendChild(V),h.appendChild(X),D.appendChild(Y),D.appendChild(h),window._cwIsAdmin){let b=document.createElement("div");b.className="cw-card-actions";let C=document.createElement("button");C.className="cw-action-btn edit",C.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg> Editar',C.onclick=()=>M(f);let l=document.createElement("button");l.className="cw-action-btn delete",l.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg> Excluir',l.onclick=()=>Z(f.id),b.appendChild(C),b.appendChild(l),D.appendChild(b)}return D}let _=le.getCachedBroadcasts();_.length>0&&(qt(_),R()),setTimeout(p,500),I(),o||(o=setInterval(I,ia));let A=document.createElement("div");Object.assign(A.style,Ee),A.className="no-drag",g.appendChild(A),ke(g,A),document.body.appendChild(g);let N=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]"),q=Ne.some(f=>!N.includes(f.id));return{toggle:u,hasUnread:q}}function Eo(){if(localStorage.getItem("cw_onboarding_seen_v1"))return;let t=[{icon:"\u{1F680}",title:"Bem-vindo ao TechSol Suite",text:"Sua nova central de opera\xE7\xF5es para maximizar produtividade e padroniza\xE7\xE3o no CRM."},{icon:"\u{1F4DD}",title:"Notas Autom\xE1ticas",text:"Gere notas de caso (BAU/LM) perfeitas em segundos. Selecione o Status, as Tasks e deixe o wizard escrever o texto t\xE9cnico para voc\xEA."},{icon:"\u26A1",title:"Quick Email & Scripts",text:"Responda e-mails com templates inteligentes que detectam o contexto e use scripts de chamada interativos que guiam seu atendimento."},{icon:"\u{1F4E2}",title:"Fique Informado",text:"O m\xF3dulo Broadcast traz avisos importantes e disponibilidade BAU direto na sua tela, sem precisar abrir planilhas externas."},{icon:"\u2705",title:"Tudo Pronto!",text:"Explore o Menu Flutuante para come\xE7ar. Bom trabalho!",isLast:!0}],e=0,o={overlay:{position:"fixed",top:0,left:0,width:"100vw",height:"100vh",backgroundColor:"rgba(0,0,0,0.6)",backdropFilter:"blur(4px)",zIndex:"2147483647",display:"flex",alignItems:"center",justifyContent:"center",opacity:"0",transition:"opacity 0.3s ease"},card:{width:"380px",background:"#fff",borderRadius:"24px",padding:"32px",textAlign:"center",position:"relative",boxShadow:"0 20px 50px rgba(0,0,0,0.3)",fontFamily:"'Google Sans', Roboto, sans-serif",transform:"translateY(20px)",transition:"all 0.4s cubic-bezier(0.19, 1, 0.22, 1)"},icon:{fontSize:"48px",marginBottom:"20px",display:"block"},title:{fontSize:"22px",fontWeight:"700",color:"#202124",marginBottom:"12px"},text:{fontSize:"15px",color:"#5f6368",lineHeight:"1.6",marginBottom:"32px"},dotsContainer:{display:"flex",justifyContent:"center",gap:"8px",marginBottom:"24px"},dot:{width:"8px",height:"8px",borderRadius:"50%",background:"#dadce0",transition:"all 0.3s"},dotActive:{background:"#1a73e8",width:"24px",borderRadius:"4px"},btnContainer:{display:"flex",justifyContent:"space-between",alignItems:"center"},btn:{padding:"10px 24px",borderRadius:"20px",border:"none",cursor:"pointer",fontSize:"14px",fontWeight:"600",transition:"background 0.2s"},btnSkip:{background:"transparent",color:"#5f6368"},btnNext:{background:"#1a73e8",color:"#fff",boxShadow:"0 4px 12px rgba(26,115,232,0.3)"}},a=document.createElement("div");Object.assign(a.style,o.overlay);let n=document.createElement("div");Object.assign(n.style,o.card);let i=document.createElement("div");Object.assign(i.style,o.icon);let s=document.createElement("div");Object.assign(s.style,o.title);let r=document.createElement("div");Object.assign(r.style,o.text);let c=document.createElement("div");Object.assign(c.style,o.dotsContainer);let g=document.createElement("div");Object.assign(g.style,o.btnContainer);let m=document.createElement("button");m.textContent="Pular",Object.assign(m.style,o.btn,o.btnSkip),m.onmouseover=()=>m.style.color="#202124",m.onmouseout=()=>m.style.color="#5f6368";let u=document.createElement("button");u.textContent="Pr\xF3ximo",Object.assign(u.style,o.btn,o.btnNext),u.onmouseover=()=>u.style.transform="scale(1.05)",u.onmouseout=()=>u.style.transform="scale(1)",g.appendChild(m),g.appendChild(u),n.appendChild(i),n.appendChild(s),n.appendChild(r),n.appendChild(c),n.appendChild(g),a.appendChild(n),document.body.appendChild(a);function y(d){let p=t[d];i.textContent=p.icon,s.textContent=p.title,r.textContent=p.text,c.innerHTML="",t.forEach((w,E)=>{let M=document.createElement("div");Object.assign(M.style,o.dot),E===d&&Object.assign(M.style,o.dotActive),c.appendChild(M)}),p.isLast?(m.style.display="none",u.textContent="Come\xE7ar \u{1F680}",u.style.width="100%"):(m.style.display="block",u.textContent="Pr\xF3ximo",u.style.width="auto")}function x(){localStorage.setItem("cw_onboarding_seen_v1","true"),a.style.opacity="0",n.style.transform="translateY(20px)",setTimeout(()=>a.remove(),400),H.playSuccess(),J("Tudo pronto! Use o menu flutuante.")}u.onclick=()=>{H.playClick(),e<t.length-1?(e++,y(e)):x()},m.onclick=async()=>{await he("Pular o tutorial?")&&x()},y(0),requestAnimationFrame(()=>{a.style.opacity="1",n.style.transform="translateY(0)"})}var ko={version:"v5.1",title:"Atualiza\xE7\xE3o: v5.1 - Produtividade Blindada \u{1F6E1}\uFE0F",slides:[{icon:"\u{1F17F}\uFE0F",title:"Estacionamento de Casos",text:"Interrup\xE7\xE3o urgente? Agora voc\xEA pode 'Estacionar' seu atendimento atual (Notas + Tasks) com um clique e retomar depois exatamente de onde parou."},{icon:"\u{1F6DF}",title:"Sistema 'Airbag'",text:"Caiu a internet? Fechou a aba sem querer? O TechSol agora possui Auto-Save de emerg\xEAncia a cada 5 segundos. Seu texto est\xE1 salvo, sempre."},{icon:"\u{1F7E0}",title:"Indicador de Progresso",text:"Nunca mais esque\xE7a uma nota aberta. Um indicador laranja ('Dirty State') avisa na P\xEDlula principal se h\xE1 trabalho n\xE3o salvo/estacionado."},{icon:"\u{1F50D}",title:"Time Zone Pro",text:"O m\xF3dulo de fusos hor\xE1rios ganhou superpoderes: nova barra de pesquisa global, filtros r\xE1pidos por regi\xE3o e corre\xE7\xE3o de visualiza\xE7\xE3o."},{icon:"\u{1F916}",title:"Leitura de BAU Aprimorada",text:"O sistema de Broadcast agora \xE9 mais inteligente ao ler avisos de disponibilidade, detectando datas e bandeiras mesmo quando quebradas em v\xE1rias linhas."},{icon:"\u{1F3A8}",title:"Refinamento Visual",text:"Bot\xF5es padronizados, sombras suavizadas e micro-intera\xE7\xF5es t\xE1teis em todo o sistema para uma experi\xEAncia mais fluida e profissional."}]};function To(t){let e=localStorage.getItem("cw_last_version");if(!e){localStorage.setItem("cw_last_version",t);return}e!==t&&sa(t)}function sa(t){let e=ko.slides,o=0,a={overlay:{position:"fixed",top:0,left:0,width:"100vw",height:"100vh",backgroundColor:"rgba(0,0,0,0.7)",backdropFilter:"blur(5px)",zIndex:"2147483647",display:"flex",alignItems:"center",justifyContent:"center",opacity:"0",transition:"opacity 0.3s ease"},card:{width:"400px",background:"#fff",borderRadius:"24px",padding:"32px",textAlign:"center",position:"relative",boxShadow:"0 24px 60px rgba(0,0,0,0.4)",fontFamily:"'Google Sans', Roboto, sans-serif",transform:"translateY(30px)",transition:"all 0.4s cubic-bezier(0.19, 1, 0.22, 1)"},badge:{display:"inline-block",padding:"4px 12px",borderRadius:"12px",background:"#E8F0FE",color:"#1967D2",fontSize:"11px",fontWeight:"700",textTransform:"uppercase",marginBottom:"16px",letterSpacing:"0.5px"},icon:{fontSize:"42px",marginBottom:"16px",display:"block"},title:{fontSize:"20px",fontWeight:"700",color:"#202124",marginBottom:"8px"},text:{fontSize:"14px",color:"#5f6368",lineHeight:"1.5",marginBottom:"32px",minHeight:"42px"},dotsContainer:{display:"flex",justifyContent:"center",gap:"6px",marginBottom:"24px"},dot:{width:"6px",height:"6px",borderRadius:"50%",background:"#dadce0",transition:"all 0.3s"},dotActive:{background:"#1a73e8",width:"18px",borderRadius:"4px"},btn:{width:"100%",padding:"12px",borderRadius:"12px",border:"none",cursor:"pointer",fontSize:"14px",fontWeight:"600",transition:"all 0.2s",background:"#1a73e8",color:"#fff",boxShadow:"0 4px 12px rgba(26,115,232,0.3)"}},n=document.createElement("div");Object.assign(n.style,a.overlay);let i=document.createElement("div");Object.assign(i.style,a.card);let s=document.createElement("div");Object.assign(s.style,a.badge),s.textContent=`Atualiza\xE7\xE3o ${t}`;let r=document.createElement("div");Object.assign(r.style,a.icon);let c=document.createElement("div");Object.assign(c.style,a.title);let g=document.createElement("div");Object.assign(g.style,a.text);let m=document.createElement("div");Object.assign(m.style,a.dotsContainer);let u=document.createElement("button");Object.assign(u.style,a.btn),u.onmouseover=()=>u.style.transform="scale(1.02)",u.onmouseout=()=>u.style.transform="scale(1)",i.appendChild(s),i.appendChild(r),i.appendChild(c),i.appendChild(g),i.appendChild(m),i.appendChild(u),n.appendChild(i),document.body.appendChild(n);function y(d){let p=e[d];r.textContent=p.icon,c.textContent=p.title,g.textContent=p.text,m.innerHTML="",e.forEach((w,E)=>{let M=document.createElement("div");Object.assign(M.style,a.dot),E===d&&Object.assign(M.style,a.dotActive),m.appendChild(M)}),d===e.length-1?u.textContent="Entendi, vamos l\xE1! \u{1F44D}":u.textContent="Pr\xF3ximo"}function x(){localStorage.setItem("cw_last_version",t),n.style.opacity="0",i.style.transform="translateY(30px)",setTimeout(()=>n.remove(),400),H.playSuccess(),J(`TechSol atualizado para ${t}!`)}u.onclick=()=>{H.playClick(),o<e.length-1?(o++,y(o)):x()},y(0),requestAnimationFrame(()=>{n.style.opacity="1",i.style.transform="translateY(0)"})}var Fo="cw_timezone_pinned",_t=[{id:"pt",name:"Portugal",flag:"\u{1F1F5}\u{1F1F9}",zone:"Europe/Lisbon",label:"Lisboa",region:"eu"},{id:"es",name:"Espanha",flag:"\u{1F1EA}\u{1F1F8}",zone:"Europe/Madrid",label:"Madrid",region:"eu"},{id:"ar",name:"Argentina",flag:"\u{1F1E6}\u{1F1F7}",zone:"America/Argentina/Buenos_Aires",label:"Buenos Aires",region:"sa"},{id:"bo",name:"Bol\xEDvia",flag:"\u{1F1E7}\u{1F1F4}",zone:"America/La_Paz",label:"La Paz",region:"sa"},{id:"cl",name:"Chile",flag:"\u{1F1E8}\u{1F1F1}",zone:"America/Santiago",label:"Santiago",region:"sa"},{id:"co",name:"Col\xF4mbia",flag:"\u{1F1E8}\u{1F1F4}",zone:"America/Bogota",label:"Bogot\xE1",region:"sa"},{id:"ec",name:"Equador",flag:"\u{1F1EA}\u{1F1E8}",zone:"America/Guayaquil",label:"Guayaquil",region:"sa"},{id:"py",name:"Paraguai",flag:"\u{1F1F5}\u{1F1FE}",zone:"America/Asuncion",label:"Assun\xE7\xE3o",region:"sa"},{id:"pe",name:"Peru",flag:"\u{1F1F5}\u{1F1EA}",zone:"America/Lima",label:"Lima",region:"sa"},{id:"uy",name:"Uruguai",flag:"\u{1F1FA}\u{1F1FE}",zone:"America/Montevideo",label:"Montevid\xE9u",region:"sa"},{id:"ve",name:"Venezuela",flag:"\u{1F1FB}\u{1F1EA}",zone:"America/Caracas",label:"Caracas",region:"sa"},{id:"mx",name:"M\xE9xico",flag:"\u{1F1F2}\u{1F1FD}",zone:"America/Mexico_City",label:"CDMX",region:"na"},{id:"cr",name:"Costa Rica",flag:"\u{1F1E8}\u{1F1F7}",zone:"America/Costa_Rica",label:"San Jos\xE9",region:"na"},{id:"sv",name:"El Salvador",flag:"\u{1F1F8}\u{1F1FB}",zone:"America/El_Salvador",label:"San Salvador",region:"na"},{id:"gt",name:"Guatemala",flag:"\u{1F1EC}\u{1F1F9}",zone:"America/Guatemala",label:"C. da Guatemala",region:"na"},{id:"hn",name:"Honduras",flag:"\u{1F1ED}\u{1F1F3}",zone:"America/Tegucigalpa",label:"Tegucigalpa",region:"na"},{id:"ni",name:"Nicar\xE1gua",flag:"\u{1F1F3}\u{1F1EE}",zone:"America/Managua",label:"Man\xE1gua",region:"na"},{id:"pa",name:"Panam\xE1",flag:"\u{1F1F5}\u{1F1E6}",zone:"America/Panama",label:"C. do Panam\xE1",region:"na"},{id:"do",name:"Rep. Dominicana",flag:"\u{1F1E9}\u{1F1F4}",zone:"America/Santo_Domingo",label:"Santo Domingo",region:"na"},{id:"pr",name:"Porto Rico",flag:"\u{1F1F5}\u{1F1F7}",zone:"America/Puerto_Rico",label:"San Juan",region:"na"}],ra=[{id:"all",label:"Todos"},{id:"sa",label:"Am\xE9rica do Sul"},{id:"na",label:"Norte & Central"},{id:"eu",label:"Europa"}];function Oo(){let t="v2.2 Pro",e=!1,o=null,a="mx",n=JSON.parse(localStorage.getItem(Fo)||"[]"),i="",s="all",r=new Date;r.setHours(14,0,0,0);let c={bg:"#F8F9FA",surface:"#FFFFFF",primary:"#1A73E8",primaryBg:"#E8F0FE",text:"#202124",textSub:"#5F6368",border:"#DADCE0",success:"#1E8E3E",successBg:"#E6F4EA",warning:"#E37400",warningBg:"#FEF7E0",error:"#D93025",errorBg:"#FCE8E6"},g={container:{display:"flex",flexDirection:"column",height:"100%",background:c.bg,fontFamily:"'Google Sans', Roboto, sans-serif"},tabHeader:{display:"flex",background:c.surface,borderBottom:`1px solid ${c.border}`,padding:"8px 16px 0 16px"},tabBtn:{flex:1,padding:"12px",textAlign:"center",cursor:"pointer",fontSize:"13px",fontWeight:"500",color:c.textSub,borderBottom:"3px solid transparent",transition:"all 0.2s ease",userSelect:"none"},tabActive:{color:c.primary,borderBottomColor:c.primary,fontWeight:"600"},toolbar:{padding:"12px 16px 8px 16px",background:c.bg,display:"flex",flexDirection:"column",gap:"12px",borderBottom:"1px solid rgba(0,0,0,0.03)"},searchInputWrapper:{position:"relative",width:"100%"},searchInput:{width:"100%",boxSizing:"border-box",padding:"10px 12px 10px 38px",borderRadius:"10px",border:"1px solid transparent",background:"#FFFFFF",fontSize:"14px",color:c.text,outline:"none",boxShadow:"0 1px 3px rgba(0,0,0,0.05)",transition:"all 0.2s",fontFamily:"'Google Sans', Roboto, sans-serif"},searchIcon:{position:"absolute",left:"12px",top:"50%",transform:"translateY(-50%)",width:"16px",height:"16px",color:"#9AA0A6",pointerEvents:"none"},chipsRow:{display:"flex",gap:"8px",overflowX:"auto",paddingBottom:"4px",scrollbarWidth:"none",msOverflowStyle:"none"},chip:{whiteSpace:"nowrap",padding:"6px 12px",borderRadius:"16px",fontSize:"12px",fontWeight:"500",cursor:"pointer",border:`1px solid ${c.border}`,background:c.surface,color:c.textSub,transition:"all 0.2s"},chipActive:{background:c.primaryBg,color:c.primary,borderColor:c.primaryBg,fontWeight:"600"},listContainer:{padding:"16px 16px 40px 16px",overflowY:"auto",flex:1,display:"flex",flexDirection:"column",gap:"12px",scrollbarWidth:"none"},hubCard:{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px 20px",background:c.surface,borderRadius:"16px",border:"1px solid transparent",boxShadow:"0 2px 6px rgba(60,64,67,0.05)",transition:"transform 0.2s cubic-bezier(0.25, 0.8, 0.25, 1), box-shadow 0.2s ease",cursor:"pointer",position:"relative"},hubCardPinned:{borderLeft:`4px solid ${c.primary}`,paddingLeft:"16px"},plannerWrapper:{padding:"24px",display:"flex",flexDirection:"column",gap:"24px",flex:1,overflowY:"auto"},timeComparisonRow:{display:"flex",gap:"16px",alignItems:"stretch"},timeCard:{flex:1,padding:"20px",borderRadius:"20px",background:c.surface,border:`1px solid ${c.border}`,display:"flex",flexDirection:"column",alignItems:"center",gap:"8px",boxShadow:"0 4px 12px rgba(60,64,67,0.05)"},timelineContainer:{position:"relative",height:"60px",marginTop:"16px",userSelect:"none"},timelineTrack:{position:"absolute",top:"26px",left:"0",right:"0",height:"6px",borderRadius:"3px",background:"#E0E0E0",overflow:"hidden"},dayZone:{position:"absolute",top:"0",bottom:"0",left:"37.5%",width:"37.5%",background:"rgba(52, 168, 83, 0.3)",pointerEvents:"none"},hdInput:{fontSize:"28px",fontWeight:"700",color:c.text,border:"none",background:"transparent",width:"100%",textAlign:"center",outline:"none",fontFamily:"'Google Sans', sans-serif",cursor:"text"},statusBadge:{padding:"8px 16px",borderRadius:"50px",fontSize:"13px",fontWeight:"600",display:"inline-flex",alignItems:"center",gap:"8px",marginTop:"16px",alignSelf:"center",transition:"background-color 0.3s"}},m=document.createElement("div");m.id="timezone-popup",m.classList.add("cw-module-window"),Object.assign(m.style,me,{right:"100px",width:"450px",height:"720px",overflow:"hidden",borderRadius:"24px"});let y=pe(m,"Time Zone Traveler",t,"Monitoramento global e planejamento de chamadas.",{popup:m},()=>S());m.appendChild(y);let x=document.createElement("div");Object.assign(x.style,g.container),m.appendChild(x);let d=document.createElement("div");Object.assign(d.style,g.tabHeader);let p=document.createElement("div");p.textContent="Monitoramento",Object.assign(p.style,g.tabBtn,g.tabActive);let w=document.createElement("div");w.textContent="Planejador",Object.assign(w.style,g.tabBtn),d.appendChild(p),d.appendChild(w),x.appendChild(d);let E=document.createElement("div");Object.assign(E.style,g.toolbar);let M=document.createElement("div");Object.assign(M.style,g.searchInputWrapper);let P=document.createElement("div");P.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>',Object.assign(P.style,g.searchIcon);let z=document.createElement("input");z.placeholder="Buscar cidade ou pa\xEDs...",Object.assign(z.style,g.searchInput),z.onfocus=()=>{z.style.boxShadow="0 2px 8px rgba(26,115,232,0.15)",z.style.borderColor="rgba(26,115,232,0.3)"},z.onblur=()=>{z.style.boxShadow="0 1px 3px rgba(0,0,0,0.05)",z.style.borderColor="transparent"},z.oninput=D=>{i=D.target.value.toLowerCase(),A()},M.appendChild(P),M.appendChild(z),E.appendChild(M);let Z=document.createElement("div");Object.assign(Z.style,g.chipsRow),ra.forEach(D=>{let L=document.createElement("div");L.textContent=D.label,L.id=`tz-filter-${D.id}`,Object.assign(L.style,g.chip),D.id===s&&Object.assign(L.style,g.chipActive),L.onclick=()=>{H.playClick(),s=D.id,Array.from(Z.children).forEach(Y=>{Object.assign(Y.style,g.chip)}),Object.assign(L.style,g.chipActive),A()},Z.appendChild(L)}),E.appendChild(Z),x.appendChild(E);let ee=document.createElement("div");Object.assign(ee.style,g.listContainer);let I=document.createElement("style");I.textContent="#timezone-popup ::-webkit-scrollbar { display: none; }",x.appendChild(I);let F=document.createElement("div");Object.assign(F.style,g.plannerWrapper,{display:"none"}),x.appendChild(ee),x.appendChild(F),p.onclick=()=>R("live"),w.onclick=()=>R("plan");function R(D){H.playClick(),D==="live"?(Object.assign(p.style,g.tabActive),Object.assign(w.style,g.tabBtn),w.style.borderBottomColor="transparent",ee.style.display="flex",E.style.display="flex",F.style.display="none",q()):(Object.assign(w.style,g.tabActive),Object.assign(p.style,g.tabBtn),p.style.borderBottomColor="transparent",F.style.display="flex",ee.style.display="none",E.style.display="none",f(),N())}function j(D){return D>=9&&D<17?{color:c.success,bg:c.successBg,label:"Aberto",icon:"\u{1F7E2}"}:D>=8&&D<9?{color:c.warning,bg:c.warningBg,label:"Abrindo",icon:"\u{1F7E1}"}:D>=17&&D<19?{color:c.warning,bg:c.warningBg,label:"Fechando",icon:"\u{1F7E1}"}:{color:c.textSub,bg:"#F1F3F4",label:"Fechado",icon:"\u{1F534}"}}function _(D){n.includes(D)?n=n.filter(L=>L!==D):n.push(D),localStorage.setItem(Fo,JSON.stringify(n)),A(),H.playClick()}function A(){ee.innerHTML="";let D=new Date,L=_t.filter(k=>{let U=k.name.toLowerCase().includes(i)||k.label.toLowerCase().includes(i),h=s==="all"||k.region===s;return U&&h});if(L.sort((k,U)=>{let h=n.includes(k.id),K=n.includes(U.id);return h&&!K?-1:!h&&K?1:k.name.localeCompare(U.name)}),L.length===0){ee.innerHTML=`
            ...
            `,b.onmouseenter=()=>{b.style.transform="translateY(-2px)",b.style.boxShadow="0 6px 12px rgba(60,64,67,0.1)"},b.onmouseleave=()=>{b.style.transform="translateY(0)",b.style.boxShadow="0 2px 6px rgba(60,64,67,0.05)"};let v=b.querySelector(".cw-pin-btn");v.onmouseenter=()=>{v.style.backgroundColor="#F1F3F4"},v.onmouseleave=()=>{v.style.backgroundColor="transparent"},v.onclick=T=>{T.stopPropagation(),_(k.id)},b.onclick=()=>{a=k.id,R("plan")},ee.appendChild(b)});let Y=document.createElement("div");Y.style.height="20px",Y.style.width="100%",ee.appendChild(Y)}function N(){F.innerHTML="";let D=document.createElement("div"),L=document.createElement("label");L.textContent="Onde est\xE1 o cliente?",L.style.cssText="display:block; font-size:12px; font-weight:700; color:#5F6368; margin-bottom:8px; text-transform:uppercase; letter-spacing:0.5px;";let Y=document.createElement("select");Object.assign(Y.style,Ut),Y.style.padding="14px",[..._t].sort((oe,ae)=>oe.name.localeCompare(ae.name)).forEach(oe=>{let ae=document.createElement("option");ae.value=oe.id,ae.textContent=`${oe.flag} ${oe.name} (${oe.zone})`,oe.id===a&&(ae.selected=!0),Y.appendChild(ae)}),Y.onchange=oe=>{a=oe.target.value,Q(),H.playClick()},D.appendChild(L),D.appendChild(Y),F.appendChild(D);let U=document.createElement("div");Object.assign(U.style,g.timeComparisonRow);let h=document.createElement("div");Object.assign(h.style,g.timeCard),h.style.backgroundColor="#F8FAFF",h.style.borderColor="#E8F0FE",h.innerHTML=`
        ...
        `,document.head.appendChild(q)}let s={bg:"linear-gradient(180deg, #F8FAFD 0%, #EEF2F8 100%)",surface:"rgba(255, 255, 255, 0.85)",glass:"rgba(255, 255, 255, 0.7)",primary:"#1a73e8",primaryLight:"rgba(26, 115, 232, 0.1)",text:"#1f1f1f",textSub:"#474747",border:"rgba(0, 0, 0, 0.06)",danger:"#d93025"},r={container:{display:"flex",flexDirection:"column",height:"100%",background:s.bg,fontFamily:"'Google Sans', Roboto, sans-serif"},tabHeader:{display:"flex",padding:"16px 24px 0 24px",background:"transparent",borderBottom:`1px solid ${s.border}`,gap:"8px"},tabBtn:{flex:1,padding:"14px 16px",textAlign:"center",cursor:"pointer",fontSize:"14px",fontWeight:"500",color:s.textSub,borderBottom:"3px solid transparent",transition:"all 0.3s ease",userSelect:"none",display:"flex",alignItems:"center",justifyContent:"center",gap:"8px",borderRadius:"12px 12px 0 0"},tabActive:{color:s.primary,borderBottomColor:s.primary,fontWeight:"600",background:"rgba(26, 115, 232, 0.04)"},listContainer:{flex:1,overflowY:"auto",padding:"24px",display:"flex",flexDirection:"column",gap:"16px"},emptyState:{padding:"60px 24px",textAlign:"center",color:s.textSub,display:"flex",flexDirection:"column",alignItems:"center",gap:"16px"},card:{background:s.surface,borderRadius:"24px",padding:"20px",border:`1px solid ${s.border}`,boxShadow:"0 4px 12px rgba(0,0,0,0.03)",transition:"all 0.4s cubic-bezier(0.165, 0.84, 0.44, 1)",cursor:"default",position:"relative"},cardHeader:{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"12px"},cardTitle:{fontSize:"15px",fontWeight:"600",color:s.text,letterSpacing:"-0.01em"},cardPreview:{fontSize:"13px",color:s.textSub,lineHeight:"1.6",display:"-webkit-box",webkitLineClamp:"3",webkitBoxOrient:"vertical",overflow:"hidden"},cardActions:{display:"flex",justifyContent:"flex-end",gap:"12px",marginTop:"16px",paddingTop:"16px",borderTop:`1px solid ${s.border}`},actionBtn:{padding:"8px 14px",borderRadius:"10px",fontSize:"13px",fontWeight:"500",cursor:"pointer",border:"none",background:"transparent",transition:"all 0.2s",display:"flex",alignItems:"center",gap:"6px"},fab:{position:"absolute",bottom:"32px",right:"32px",width:"64px",height:"64px",borderRadius:"20px",background:`linear-gradient(135deg, ${s.primary}, #0059c1)`,color:"#FFF",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 8px 24px rgba(26, 115, 232, 0.4)",cursor:"pointer",transition:"all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1)",zIndex:10},editorOverlay:{position:"absolute",top:0,left:0,width:"100%",height:"100%",background:"rgba(255, 255, 255, 0.9)",backdropFilter:"blur(30px) saturate(180%)",webkitBackdropFilter:"blur(30px) saturate(180%)",zIndex:20,transform:"translateY(100%)",transition:"transform 0.6s cubic-bezier(0.16, 1, 0.3, 1)",display:"flex",flexDirection:"column"},editorHeader:{padding:"24px 32px",background:"transparent",borderBottom:`1px solid ${s.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"},editorBody:{flex:1,padding:"32px",overflowY:"auto"},inputGroup:{marginBottom:"24px"},label:{display:"block",fontSize:"13px",fontWeight:"600",color:s.textSub,marginBottom:"10px",letterSpacing:"0.02em"},input:{width:"100%",padding:"14px 18px",borderRadius:"14px",border:`1px solid ${s.border}`,fontSize:"15px",fontFamily:"inherit",outline:"none",background:s.surface,transition:"all 0.2s ease",boxSizing:"border-box"}},c=document.createElement("div");c.id="library-popup",c.classList.add("cw-module-window"),Object.assign(c.style,me,{right:"auto",left:"50%",width:"400px",height:"600px",transform:"translateX(-50%) scale(0.05)"});let m=pe(c,"Minha Biblioteca",t,"Gerencie seus snippets, textos e templates.",{popup:c},()=>N());c.appendChild(m);let u=document.createElement("div");Object.assign(u.style,r.container),c.appendChild(u);let y=document.createElement("div");Object.assign(y.style,r.tabHeader);let x=[{id:"general",label:"Geral",icon:i.tabs.general},{id:"note",label:"Notas",icon:i.tabs.note},{id:"email",label:"Emails",icon:i.tabs.email}];x.forEach(q=>{let f=document.createElement("div");f.innerHTML=`${q.icon} <span>${q.label}</span>`,f.id=`lib-tab-${q.id}`,Object.assign(f.style,r.tabBtn),q.id===o&&Object.assign(f.style,r.tabActive),f.onmouseenter=()=>H.playHover(),f.onclick=()=>ee(q.id),y.appendChild(f)}),u.appendChild(y);let d=document.createElement("div");Object.assign(d.style,r.listContainer),u.appendChild(d);let p=document.createElement("div");p.className="cw-fab cw-tactile",Object.assign(p.style,r.fab),p.innerHTML=i.actions.add,p.onmouseenter=()=>{p.style.transform="scale(1.1)",p.style.boxShadow="0 12px 32px rgba(26, 115, 232, 0.5)"},p.onmouseleave=()=>{p.style.transform="scale(1)",p.style.boxShadow="0 8px 24px rgba(26, 115, 232, 0.4)"},p.onclick=()=>F(),u.appendChild(p),a=document.createElement("div"),Object.assign(a.style,r.editorOverlay);let w=document.createElement("div");Object.assign(w.style,r.editorHeader),w.innerHTML='<span style="font-weight:700; font-size:16px;">Novo Item</span>';let E=document.createElement("button");E.innerHTML="Cancelar",E.style.cssText="background:none; border:none; color:#5f6368; font-weight:600; cursor:pointer;",E.onclick=R,w.appendChild(E),a.appendChild(w);let M=document.createElement("div");Object.assign(M.style,r.editorBody),a.appendChild(M);let P=document.createElement("div");P.style.cssText="padding:24px 32px; border-top:1px solid rgba(0,0,0,0.06); background:transparent; display:flex; justify-content:flex-end;";let z=document.createElement("button");z.textContent="Salvar",z.style.cssText="padding:12px 32px; background:linear-gradient(135deg, #1a73e8, #0059c1); color:white; border:none; border-radius:14px; font-weight:600; cursor:pointer; box-shadow:0 4px 12px rgba(26,115,232,0.3); transition: all 0.2s;",z.onclick=j,P.appendChild(z),a.appendChild(P),u.appendChild(a);let Z=document.createElement("div");Object.assign(Z.style,Ee),Z.className="no-drag",c.appendChild(Z),ke(c,Z),document.body.appendChild(c);function ee(q){H.playClick(),o=q,x.forEach(f=>{let S=document.getElementById(`lib-tab-${f.id}`);f.id===q?Object.assign(S.style,r.tabActive):Object.assign(S.style,r.tabBtn)}),I()}function I(){d.innerHTML="";let q=ye.getSnippets(o);if(q.length===0){d.innerHTML=`
            ...
            `;return}q.forEach(f=>{let S=document.createElement("div");S.className="cw-aura-card",Object.assign(S.style,r.card),f.isCode&&(S.style.borderLeft=`4px solid ${s.primary}`,S.style.background="rgba(26, 115, 232, 0.02)");let D=f.content,L="";if(f.isRich){let Y=document.createElement("div");Y.innerHTML=f.content;let k=Y.querySelector("img");D=Y.innerText.substring(0,150)+(Y.innerText.length>150?"...":""),k&&(L=`<span style="display:inline-flex; align-items:center; background:rgba(26, 115, 232, 0.1); color:#1a73e8; padding:2px 8px; border-radius:10px; font-size:11px; margin-bottom:8px; font-weight:600;">${i.media} Media</span>`)}S.innerHTML=`
                <div style="${A(r.cardHeader)}">
                    <div style="${A(r.cardTitle)}">${f.title}</div>
                ...
                ${L}
                <div style="${A(r.cardPreview)}; ${f.isCode?"font-family:'Roboto Mono', monospace; font-size:12px; background:rgba(0,0,0,0.02); padding:12px; border-radius:12px;":""}">${D}</div>
                <div style="${A(r.cardActions)}">
                    <button class="cw-act-copy cw-tactile" title="Copiar" style="${A(r.actionBtn)}; color:#1a73e8;">
                        ${i.actions.copy}
                        <span>Copiar</span>
                    </button>
                    <button class="cw-act-edit cw-tactile" title="Editar" style="${A(r.actionBtn)}; color:#474747;">
                        ${i.actions.edit}
                        <span>Editar</span>
                    </button>
                    <button class="cw-act-del cw-tactile" title="Excluir" style="${A(r.actionBtn)}; color:#d93025;">
                        ${i.actions.delete}
                        <span>Excluir</span>
                    </button>
                </div>
            `,S.onmouseenter=()=>{H.playHover()},S.querySelector(".cw-act-copy").onclick=Y=>{if(Y.stopPropagation(),H.playClick(),f.isRich){let k=new Blob([f.content],{type:"text/html"}),U=document.createElement("div");U.style.whiteSpace="pre-wrap",U.innerHTML=f.content;let h=new Blob([U.innerText],{type:"text/plain"}),K=[new ClipboardItem({"text/html":k,"text/plain":h})];navigator.clipboard.write(K)}else navigator.clipboard.writeText(f.content);J("Copiado!")},S.querySelector(".cw-act-edit").onclick=Y=>{Y.stopPropagation(),H.playClick(),F(f)},S.querySelector(".cw-act-del").onclick=async Y=>{Y.stopPropagation(),H.playClick(),await he("Excluir este item?")&&(ye.delete(f.id),I(),J("Item exclu\xEDdo."))},d.appendChild(S)})}function F(q=null){n=q?q.id:null,M.innerHTML="",M.appendChild(_("title","T\xEDtulo / Nome",q?q.title:"")),o==="email"&&M.appendChild(_("subject","Assunto do Email",q?q.subject:""));let f="Conte\xFAdo";o==="email"&&(f="Corpo do Email (HTML)"),o==="note"&&(f="Texto da Nota (Reason)"),M.appendChild(_("content",f,q?q.content:"",{isRich:!0,isCode:q?q.isCode:!1})),w.querySelector("span").textContent=q?"Editar Item":"Novo Item",a.style.transform="translateY(0)",setTimeout(()=>{let S=M.querySelector("input");S&&S.focus()},300)}function R(){a.style.transform="translateY(100%)",setTimeout(()=>n=null,300)}async function j(){let q=M.querySelector("#cw-inp-title"),f=M.querySelector("#cw-inp-content"),S=q.value.trim(),D=f.contentEditable==="true"?f.innerHTML:f.value.trim(),L=f.getAttribute("data-is-code")==="true";if(!S||!D||D==="<br>"){J("Preencha t\xEDtulo e conte\xFAdo.",{error:!0});return}let Y={id:n,type:o,title:S,content:D,isCode:L,isRich:f.contentEditable==="true"};if(o==="email"){let k=M.querySelector("#cw-inp-subject").value.trim();if(!k){J("Assunto \xE9 obrigat\xF3rio para emails.",{error:!0});return}Y.subject=k}z.textContent="Salvando...",await ye.save(Y),z.textContent="Salvar",R(),I(),J("Salvo com sucesso!"),H.playSuccess()}function _(q,f,S,D={}){let L=document.createElement("div");Object.assign(L.style,r.inputGroup);let Y=document.createElement("label");Y.textContent=f,Object.assign(Y.style,r.label);let k;if(D.isRich){let U=document.createElement("div");U.style.cssText="display:flex; gap:8px; margin-bottom:14px; background:rgba(255, 255, 255, 0.5); padding:8px; border-radius:14px; border:1px solid rgba(0,0,0,0.06); backdrop-filter: blur(10px); width: fit-content;",U.innerHTML=`
            ...
            `,k=document.createElement("div"),k.contentEditable="true",Object.assign(k.style,r.input,{minHeight:"180px",maxHeight:"350px",overflowY:"auto",whiteSpace:"pre-wrap",lineHeight:"1.6",outline:"none"}),k.innerHTML=S||"",D.isCode&&(k.style.fontFamily="'Roboto Mono', monospace",k.style.backgroundColor="#F8F9FA",k.setAttribute("data-is-code","true")),U.querySelectorAll(".cw-toolbar-btn").forEach(h=>{h.onmouseenter=()=>H.playHover(),h.onmousedown=()=>H.playClick()}),U.querySelector(".cw-tb-bold").onclick=()=>{document.execCommand("bold"),k.focus()},U.querySelector(".cw-tb-italic").onclick=()=>{document.execCommand("italic"),k.focus()},U.querySelector(".cw-tb-code").onclick=h=>{let V=!(k.getAttribute("data-is-code")==="true");k.setAttribute("data-is-code",V),k.style.fontFamily=V?"'Roboto Mono', monospace":"inherit",k.style.backgroundColor=V?"rgba(0, 122, 255, 0.03)":s.surface,V?h.currentTarget.classList.add("active"):h.currentTarget.classList.remove("active"),k.focus()},U.querySelector(".cw-tb-img").onclick=async()=>{let h=await Jt("Cole a URL da imagem:");h&&(document.execCommand("insertImage",!1,h),k.querySelectorAll("img").forEach(V=>{V.style.maxWidth="100%",V.style.borderRadius="8px"}))},k.onpaste=h=>{let K=(h.clipboardData||h.originalEvent.clipboardData).items;for(let V of K)if(V.kind==="file"&&V.type.startsWith("image/")){h.preventDefault();let X=V.getAsFile(),b=new FileReader;b.onload=C=>{let l=`<img src="${C.target.result}" style="max-width:100%; border-radius:8px; margin:8px 0; display:block;">`;document.execCommand("insertHTML",!1,l)},b.readAsDataURL(X)}},L.appendChild(Y),L.appendChild(U)}else k=document.createElement("input"),k.type="text",Object.assign(k.style,r.input),k.value=S||"",L.appendChild(Y);return k.id=`cw-inp-${q}`,k.onfocus=()=>{k.style.borderColor=s.primary,k.style.boxShadow=`0 0 0 2px ${s.primaryBg}`},k.onblur=()=>{k.style.borderColor=s.border,k.style.boxShadow="none"},L.appendChild(k),L}function A(q){return Object.entries(q).map(([f,S])=>`${f.replace(/[A-Z]/g,D=>"-"+D.toLowerCase())}:${S}`).join(";")}function N(){e=!e,ue(e,c,"cw-btn-library"),e&&I()}return N}function Io(){let t="v1.0",e=!1,o={bg:"#F8F9FA",surface:"#FFFFFF",primary:"#1A73E8",text:"#202124",textSub:"#5F6368",border:"#DADCE0"},a="cw-configs-styles";if(!document.getElementById(a)){let d=document.createElement("style");d.id=a,d.innerHTML=`
        ...
        `;try{let d=await xe(),p=d.userProfile,w=d.agentEmail?d.agentEmail.split("@")[0]:"user";if(!p){c.innerHTML=`
  ...
  `,document.head.appendChild(t)};var Oe={add:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z"/></svg>',back:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z"/></svg>',wand:'<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M7.5 5.6L10 7 8.6 4.5 10 2 7.5 3.4 5 2l1.4 2.5L5 7zm12 9.8L17 14l1.4 2.5L17 19l2.5-1.4L22 19l-1.4-2.5L22 14zM22 2l-2.5 1.4L17 2l1.4 2.5L17 7l2.5-1.4L22 7l-1.4-2.5zm-7.63 5.29c-.39-.39-1.02-.39-1.41 0L1.29 18.96c-.39.39-.39 1.02 0 1.41l2.34 2.34c.39.39 1.02.39 1.41 0L16.7 11.05c.39-.39.39-1.02 0-1.41l-2.33-2.35zm-1.03 5.41l-2.12-2.12 2.44-2.44 2.12 2.12-2.44 2.44z"/></svg>',send:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z"/></svg>',check:'<svg width="64" height="64" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>',folder:'<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M10 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2h-8l-2-2z"/></svg>',empty:'<svg width="64" height="64" viewBox="0 0 24 24" fill="currentColor"><path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5v-3h3.56c.69 1.19 1.97 2 3.44 2s2.75-.81 3.44-2H19v3zm0-5h-4.99c0 1.1-.9 2-2 2s-2-.9-2-2H5V5h14v9z"/></svg>',refresh:'<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M17.65 6.35C16.2 4.9 14.21 4 12 4c-4.42 0-7.99 3.58-7.99 8s3.57 8 7.99 8c3.73 0 6.84-2.55 7.73-6h-2.08c-.82 2.33-3.04 4-5.65 4-3.31 0-6-2.69-6-6s2.69-6 6-6c1.66 0 3.14.69 4.22 1.78L13 11h7V4l-2.35 2.35z"/></svg>',expand:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M16.59 8.59L12 13.17 7.41 8.59 6 10l6 6 6-6z"/></svg>'};function Mo(){_o();let t=!1,e="dashboard",o=null,a=1,n=4,i=document.createElement("div");i.id="bau-form-popup",i.className="bau-popup cw-module-window",i.style.display="none";let s=pe(i,"BAU Central","v2.1.0","Dashboard de Casos BAU",{},()=>C()),r=s.querySelector("div:last-child");if(r){let l=document.createElement("div");l.className="bau-refresh-btn no-drag",l.innerHTML=Oe.refresh,l.title="Atualizar Dashboard",l.onclick=async v=>{v.stopPropagation(),!l.classList.contains("spinning")&&(l.classList.add("spinning"),H.playClick(),await f(),setTimeout(()=>l.classList.remove("spinning"),1e3))},r.insertBefore(l,r.firstChild)}i.appendChild(s);let c=document.createElement("div");c.className="bau-view-container",i.appendChild(c);let g=document.createElement("div");g.id="bau-view-dashboard",g.className="bau-view active",g.innerHTML=`
    ...
    `,m.appendChild(u);let y=document.createElement("div");y.className="bau-content",m.appendChild(y);let x=document.createElement("div");x.className="bau-progress-indicator",y.appendChild(x);let d=document.createElement("form");d.id="bau-escalation-form",y.appendChild(d);let p=document.createElement("div");p.className="bau-step active",p.id="bau-step-1";let w=document.createElement("div");w.className="bau-card bau-context-card",w.innerHTML=`
        <div id="bau-vital-highlights" class="bau-highlight-panel"></div>
        <div id="bau-all-data"></div>
    `;let E=document.createElement("div");E.className="bau-dynamic-inputs-container",E.innerHTML=`
    ...
    `,P.querySelectorAll(".bau-task-item").forEach(l=>{let v=l.querySelector("input");l.addEventListener("click",()=>{v.checked=!v.checked,l.classList.toggle("active",v.checked),H.playClick()})}),M.appendChild(P),d.appendChild(M);let Z=document.createElement("div");Z.className="bau-step",Z.id="bau-step-3";let ee=document.createElement("div");ee.className="bau-card",ee.innerHTML=`
    ...
    `,c.appendChild(A),document.body.appendChild(i);function N(l){e=l,i.querySelectorAll(".bau-view").forEach(O=>O.classList.remove("active"));let v=i.querySelector(`#bau-view-${l}`);v&&v.classList.add("active");let T=s.querySelector(".cw-module-header-title")||s.querySelector("h2"),$=s.querySelector(".cw-module-header-subtitle")||s.querySelector("p");T&&(T.textContent=l==="form"?"Novo Caso BAU":"BAU Central"),$&&($.textContent=l==="form"?"Preencha os detalhes abaixo":"Dashboard de Casos BAU")}function q(){let l=i.querySelector("#bau-case-list-container"),v=i.querySelector("#bau-dashboard-metrics");v&&(v.innerHTML=`
        ...
        `).join(""))}async function f(){let l=i.querySelector("#bau-case-list-container");if(l){q();try{let v=await Rt();D(v)}catch(v){l.innerHTML='<p class="bau-error-text">Erro ao carregar casos.</p>',console.error("Error loading BAU cases:",v)}}}function S(l){let T=(ie=>{switch(ie){case"PENDING_TL_CREATION":return{text:"Aguardando TL",class:"status-yellow",aura:"status-yellow-aura"};case"CREATED":return{text:"Aprovado / Criado",class:"status-green",aura:"status-green-aura"};case"DISCARDED":return{text:"Descartado pelo TL",class:"status-red",aura:"status-red-aura"};case"CANCELED_BY_AGENT":return{text:"Cancelado",class:"status-gray",aura:""};default:return{text:ie||"Pendente",class:"status-gray",aura:""}}})(l.status),$=l.date?new Date(l.date).toLocaleDateString("pt-BR"):"",O="",W="";if(l.status==="PENDING_TL_CREATION"&&l.availability_1){let ie=new Date(l.availability_1),Ae=new Date;(ie<=Ae||ie-Ae<36e5*2)&&(O='<span class="bau-sla-badge">Urgente</span>',W="bau-pulse-attention")}let te=l.reason&&l.reason.trim()?l.reason:"Nenhum contexto adicional fornecido pelo agente.",oe=/^(\d{3}-\d{3}-\d{4}|\d{10})$/.test(l.cid||""),ae=!l.caseId||l.caseId==="N/A"||!oe;return ae&&l.status==="PENDING_TL_CREATION"&&(W="bau-pulse-attention"),`
            <li class="bau-case-card ${T.aura} ${W}" data-case-id="${l.id}">
                <div class="bau-case-main">
                    ...
                    <div class="bau-case-info">
                        <div class="bau-case-header">
                            ...
                            ${O}
                            <span class="bau-case-date">${$}</span>
                        </div>
                        <p class="bau-case-details">
                            <span data-tooltip="Customer ID do Anunciante">Case: ${l.caseId||"N/A"}</span> \u2022
                            <span data-tooltip="CID do Anunciante (Formato: 000-000-0000)" class="${oe?"":"bau-error-text"}">CID: ${l.cid||"N/A"}</span> \u2022
                            <span data-tooltip="O que deve ser feito em BAU">Motivo: ${te}</span>
                        </p>
                        ${ae?`<div class="bau-data-error-hint">${l.caseId?"CID Inv\xE1lido":"Dados Incompletos"} - Contate o Suporte</div>`:""}
                    </div>
                </div>
        ...
        `}function D(l){let v=i.querySelector("#bau-case-list-container"),T=i.querySelector("#bau-dashboard-metrics");if(!v||!T)return;if(!l||l.length===0){T.innerHTML="",v.innerHTML=`
            ...
            `;return}let $=l.filter(Q=>Q.status==="PENDING_TL_CREATION").length,O=l.filter(Q=>Q.status==="CREATED").length;T.innerHTML=`
            <div class="bau-metric-card">
                <span class="bau-metric-value">${$}</span>
        ...
        `,v.innerHTML="";let W=l.slice(0,5),te=l.slice(5);if(W.forEach(Q=>{v.insertAdjacentHTML("beforeend",S(Q))}),te.length>0){let Q=document.createElement("li");Q.className="bau-accordion-container";let oe=document.createElement("button");oe.className="bau-accordion-toggle",oe.innerHTML=`${Oe.expand} <span>Mostrar ${te.length} casos mais antigos</span>`;let ae=document.createElement("ul");ae.className="bau-case-list bau-accordion-content",ae.style.display="none",te.forEach(ie=>{ae.insertAdjacentHTML("beforeend",S(ie))}),oe.onclick=()=>{let ie=ae.style.display==="none";ae.style.display=ie?"block":"none",oe.classList.toggle("expanded",ie);let Ae=oe.querySelector("span");Ae&&(Ae.textContent=ie?"Esconder casos mais antigos":`Mostrar ${te.length} casos mais antigos`),H.playClick()},Q.appendChild(oe),Q.appendChild(ae),v.appendChild(Q)}}function L(){d.querySelectorAll(".bau-step").forEach((l,v)=>{l.classList.toggle("active",v+1===a)}),x.innerHTML="";for(let l=1;l<=n;l++){let v=document.createElement("div");v.className=`bau-progress-step ${l===a?"active":l<a?"completed":""}`,v.textContent=l,x.appendChild(v)}R.style.display=a>1?"inline-block":"none",j.style.display=a<n?"inline-block":"none",_.style.display=a===n?"flex":"none",a===4&&h()}function Y(l){let v=i.querySelector(`#bau-step-${l}`);if(!v)return!0;if(l===1){let T=i.querySelector("#bau-cid-input"),$=T.value.trim(),W=/^(\d{3}-\d{3}-\d{4}|\d{10})$/.test($),te=i.querySelector("#bau-cid-error");if(!W&&T.parentElement.style.display!=="none")return T.classList.add("invalid-cid"),te&&(te.style.display="flex"),J("Erro: O formato do CID \xE9 inv\xE1lido.",{error:!0}),!1;T.classList.remove("invalid-cid"),te&&(te.style.display="none")}if(l===2)return d.querySelector('textarea[name="reason"]').value.trim()?d.querySelector('input[name="taskType"]:checked')?!0:(J("Erro: Selecione pelo menos uma Task.",{error:!0}),!1):(J("Erro: A descri\xE7\xE3o do que deve ser feito \xE9 obrigat\xF3ria.",{error:!0}),!1);for(let T of v.querySelectorAll("[required]"))if(!T.value.trim())return J(`Erro: O campo '${T.name||T.placeholder}' \xE9 obrigat\xF3rio.`,{error:!0}),!1;return!0}j.onclick=()=>{Y(a)&&(a++,L(),H.playClick())},R.onclick=()=>{a>1&&(a--,L(),H.playClick())};async function k(){let l=await xe()||{};(!l.amName||l.amName==="N/A")&&(l.amName=l.internalEmail||"N/A"),o=l;let v=d.querySelector("#bau-vital-highlights");if(v){let O=[{label:"Anunciante",value:l.advName},{label:"CID",value:l.cid},{label:"Website",value:l.site||l.website},{label:"Case ID",value:l.caseId}];v.innerHTML=O.map(W=>{let te=W.value&&W.value!=="N/A"&&W.value!=="undefined"&&W.value!=="null"?W.value:"N\xE3o capturado";return`
                    <div class="bau-highlight-item">
                        <span class="bau-highlight-label">${W.label}</span>
                        <span class="bau-highlight-value">${te}</span>
                    </div>
                `}).join("")}["advName","cid","amName","seId"].forEach(O=>{let W=l[O],te=d.querySelector(`[name="${O}"]`),Q=i.querySelector(`#wrapper-${O}`);if(te&&(te.value=W&&W!=="N/A"?W:""),Q){let oe=W&&W!==""&&W!=="N/A"&&W!=="undefined"&&W!=="null";Q.style.display=oe?"none":"block"}});let $=i.querySelector("#bau-all-data");if($){let O=[{label:"Anunciante",value:l.advName},{label:"CID",value:l.cid},{label:"AM",value:l.amName},{label:"SE ID",value:l.seId},{label:"Site",value:l.site},{label:"Email",value:l.email},{label:"Timezone",value:l.timezone},{label:"Case ID",value:l.caseId},{label:"Programa",value:l.salesProgram},{label:"Idioma",value:l.language}];$.innerHTML=`
            ...
            `}}i.querySelector("#bau-top-se-search").onclick=l=>{l.preventDefault(),st("bau-context-se-id-input")};let U=i.querySelector("#bau-cid-input");U&&U.addEventListener("input",()=>{let l=U.value.trim(),T=/^(\d{3}-\d{3}-\d{4}|\d{10})$/.test(l),$=i.querySelector("#bau-cid-error");!T&&l.length>0?(U.classList.add("invalid-cid"),$&&($.style.display="flex")):(U.classList.remove("invalid-cid"),$&&($.style.display="none"))});function h(){let l=new FormData(d),v=Object.fromEntries(l.entries()),T=l.getAll("taskType"),$=i.querySelector("#bau-confirmation-details");if(!$)return;let O=T.length>0?T.join(", "):"Nenhuma";$.innerHTML=`
