...
* **Tasks e screenshots (`task_screenshots`):** o catálogo de tasks — do Case Notes **e** da grade do form BAU, que consome só o nome (ADR-0014) — e, por task, a lista de evidências que o Win Criteria exige em cada modo (`implementation`/`education`). Um item por task, em `lang: 'ALL'`, com o espanhol no próprio valor (`screenshots_es`) — **posicional e do mesmo tamanho da lista base**, porque a quantidade de evidências não muda com o idioma; linha em branco cai no texto original. A chave do item é identidade (os modelos de nota, os rascunhos do agente e os atalhos do Ctrl+K a guardam) e por isso não muda depois de criada. O `TASKS_DB` do bundle segue como fallback embutido. Ver `docs/decisions/0012-tasks-e-screenshots-na-central.md` e `docs/decisions/0014-tasks-do-form-bau-vem-da-central.md`.
* **A navegação da Central** é um trilho lateral agrupado pelo regime de publicação — catálogo (passa pela fila), operação (publica direto) e governança —, precedido de uma home "Hoje". O grupo carrega o regime, então a tela deixou de explicá-lo por escrito em cada painel. Cada destino tem rota própria (`#/links`, `#/aprovacoes`), e publicação direta exige um passo de confirmação que declara o alcance. Ver `docs/decisions/0007-arquitetura-da-central-de-conteudo.md`.
* **Permissão:** `Content_Access` diz LDAP → papel; o que o papel PODE mora na aba `Content_Roles`, numa matriz módulo × ação (`ver`, `propor`, `aprovar`, `publicar direto`, `reverter`) mais quatro permissões globais (`gerenciar acessos`, `gerenciar papéis`, `ver auditoria`, `aprovar a própria proposta`). Ajustar um papel deixou de custar deploy. `assertContentRole_()` segue sendo a porta única de verificação — só a origem do dado mudou. Quatro regras não são editáveis por checkbox e vivem no servidor com teste: anti-lockout, escalação declarada, revogação imediata do cache e "aprovar autorização exige controlar autorização". Aba ausente ou ilegível cai no preset do código, que é a matriz de hoje; e um módulo acrescentado ao código DEPOIS de a linha do papel ter sido gravada herda o preset naquela casa, em vez de nascer fechado para todos — ausência não é decisão de ninguém (`docs/decisions/0013-modulo-novo-herda-o-preset.md`). Ver `docs/decisions/0009-rbac-editavel-da-central.md`.
* **Auditoria:** toda ação da Central escreve uma linha em `Content_Log`, com coluna própria para módulo, chave, item e detalhe — o formato anterior era uma linha na aba `Logs` genérica, com tudo concatenado num campo só, legível mas não filtrável. A tela mostra as últimas em uma barra lateral "Atividade recente" (com a foto de quem fez, como no TL Dashboard), **filtrada no servidor** pelo papel de quem pergunta: mudança de acesso só para quem gerencia acesso, e linha do módulo `people` só para quem propõe nele. `backfillContentLog()` traz o histórico que ficou na `Logs`; simula por padrão e copia sem apagar. Esquema e retenção em `specs/data-models/db-schema.md`.
