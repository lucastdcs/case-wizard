...
`),m=/\d{1,2}\/\d{1,2}/,v="\u{1F4C5}";if(M.forEach(U=>{/🇧🇷|🇵🇹|PT|BR|BRASIL|BRAZIL|PORTUGAL|LISBOA/i.test(U)?v="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM|ESPANHA|SPAIN|MEXICO|MÉXICO/i.test(U)&&(v="\u{1F1EA}\u{1F1F8}");let ee=U.match(m);if(ee){let oe=ee[0],Q=v;/🇧🇷|🇵🇹|PT|BR/i.test(U)?Q="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM/i.test(U)&&(Q="\u{1F1EA}\u{1F1F8}"),_.some(te=>te.flag===Q&&te.date===oe)||_.push({flag:Q,date:oe})}}),_.length===0){let U=(h.text||"").match(/\d{1,2}\/\d{1,2}/g);U&&[...new Set(U)].forEach(ee=>_.push({flag:"\u{1F4C5}",date:ee}))}let C="",$='<button id="cw-bau-toggle-btn" class="cw-btn-interactive" style="background:rgba(255,255,255,0.7); border:1px solid rgba(139, 92, 246, 0.4); border-radius:12px; padding:8px 12px; color:#6D28D9; font-size:12px; font-weight:600;">Detalhes</button>';window._cwIsAdmin&&($=`
    ...
    .bau-popup {
      ...
      animation: cw-genie-effect-in 0.4s ${Io};
    }
