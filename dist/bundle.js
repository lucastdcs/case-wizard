...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
    ...
    `;let x=p.querySelector(".cw-hero-grid"),g=p.querySelector(".cw-acc-container"),f=p.querySelector(".cw-results-container"),u=p.querySelector(".cw-search-input"),y=p.querySelector(".cw-status-bar"),S=p.querySelector(".cw-status-text"),z=p.querySelector(".cw-footer-icons");s.forEach(([I,T])=>{let M=i(T.name),V=document.createElement("div");V.className="cw-hero-card",V.id=`hero-${I}`,V.style.setProperty("--hero-color",M.color),V.innerHTML=`
        ...
        `,V.onclick=q=>{if(q.target.closest(".cw-step-btn"))return;let k=a[I]?a[I].count:0;P(I,k>0?-k:1,T)},V.querySelector(".minus").onclick=()=>P(I,-1,T),V.querySelector(".plus").onclick=()=>P(I,1,T),V.dataset.color=M.color,x.appendChild(V)});function B(I,T){let M=i(T.name),V=document.createElement("div");return V.className="cw-task-item",V.dataset.id=I,V.innerHTML=`
...
`.replace(/\n/g,"<br>");Je(I);let T=await ut();T?(T.innerText.trim()===""&&(T.innerHTML=""),document.execCommand("insertHTML",!1,I),pt(T),X("Nota gerada e inserida!")):X("Copiado! Abra uma nota para colar.")},e}var fe=t=>new Promise(e=>setTimeout(e,t));function he(t,e="info"){let o={info:"background: #e8f0fe; color: #1a73e8; padding: 2px 5px; border-radius: 3px;",warn:"background: #fef7e0; color: #b06000; padding: 2px 5px; border-radius: 3px;",error:"background: #fce8e6; color: #c5221f; padding: 2px 5px; border-radius: 3px;",success:"background: #e6f4ea; color: #137333; padding: 2px 5px; border-radius: 3px;"};console.log(`%c[EMAIL-BOT] ${t}`,o[e]||o.info)}function ke(t){if(!t)return;let e={bubbles:!0,cancelable:!0,view:window};["mouseover","mousedown","mouseup","click"].forEach(o=>t.dispatchEvent(new MouseEvent(o,e)))}function mt(t,e){if(!t)return;let o=`cw-warning-${t.id||Math.random().toString(36).substr(2,9)}`,a=document.getElementById(o);a&&a.remove();let n=t.getBoundingClientRect(),i=document.createElement("div");i.id=o,i.style.cssText=`
    ...
    `,document.head.appendChild(t)}function ue(t,e,o){let a=document.getElementById(o);if(!e)return;let n=e.getAttribute("data-moved")==="true",i={x:0,y:0};if(a){let d=a.getBoundingClientRect();i.x=d.left+d.width/2,i.y=d.top+d.height/2}let s,r;if(!n)s=window.innerWidth/2,r=window.innerHeight/2;else{let d=e.getBoundingClientRect();s=d.left+d.width/2,r=d.top+d.height/2,s===0&&r===0&&(s=window.innerWidth/2,r=window.innerHeight/2)}let l=i.x-s,p=i.y-r;t?(G.playGenieOpen(),e.style.transition="none",e.style.opacity="0",e.style.pointerEvents="auto",n?e.style.transform=`translate(${l}px, ${p}px) scale(0.05)`:e.style.transform=`translate(calc(-50% + ${l}px), calc(-50% + ${p}px)) scale(0.05)`,e.offsetWidth,requestAnimationFrame(()=>{e.classList.add("open"),a&&a.classList.add("active"),e.style.transition="opacity 0.4s ease-out, transform 0.5s cubic-bezier(0.19, 1, 0.22, 1)",e.style.opacity="1",n?e.style.transform="translate(0, 0) scale(1)":e.style.transform="translate(-50%, -50%) scale(1)"}),typeof po=="function"&&po(e,o)):(G.playSwoosh(),e.style.transition="opacity 0.25s ease, transform 0.3s cubic-bezier(0.5, 0, 1, 1)",e.style.pointerEvents="none",requestAnimationFrame(()=>{e.style.opacity="0",n?e.style.transform=`translate(${l}px, ${p}px) scale(0.1)`:e.style.transform=`translate(calc(-50% + ${l}px), calc(-50% + ${p}px)) scale(0.1)`}),setTimeout(()=>{e.classList.remove("open"),a&&a.classList.remove("active"),e.style.transition="",e.style.transform=""},300),typeof kt=="function"&&kt(e))}function po(t,e){kt(t);let o=a=>{if(!t.classList.contains("open"))return;let n=t.contains(a.target),i=document.querySelector(".cw-pill"),s=i&&i.contains(a.target);n?(t.classList.remove("idle"),t.style.zIndex="2147483648"):s||(t.classList.add("idle"),t.style.zIndex="2147483646")};t._idleHandler=o,document.addEventListener("mousedown",o)}function kt(t){t._idleHandler&&(document.removeEventListener("mousedown",t._idleHandler),t._idleHandler=null)}function uo(){let t="v4.0.0",{popup:e,content:o,header:a,animRefs:n,credit:i}=Wt(t,J),s=Qt(h),r=Zt(()=>{k(),W.setActiveTasks(r.getCheckedElements())},h,W),l=document.createElement("div");l.style.display="none";let p=Kt((m,w)=>{C(m,w)});l.appendChild(p);let d=no({onSaveCurrent:async()=>{let m=await N();return L(),m},onLoadDraft:m=>{Y(m)},t:m=>h(m)}),c=T(),x=M(),g=document.createElement("div"),f=_(),u=F(d,h);o.appendChild(c),o.appendChild(x),o.appendChild(f),o.appendChild(l),o.appendChild(g),r.selectionElement.style.display="none",r.screenshotsElement.style.display="none";let y=document.createElement("button");y.id="manual-task-toggle",y.textContent=h("gostaria_de_adicionar_uma_task")||"Gostaria de adicionar uma task",y.style.cssText=`display: none; width: 100%; padding: 14px; border: 2px dashed ${H.primary}; background: ${H.surface}; color: ${H.primary}; border-radius: ${de.medium}; cursor: pointer; font-size: 13px; font-weight: 700; margin-top: 8px; transition: all 0.2s ${se}; text-transform: uppercase; letter-spacing: 0.5px;`,y.onmouseenter=()=>{y.style.background=H.primaryBg},y.onmouseleave=()=>{y.style.background=H.surface},y.onclick=()=>{r.selectionElement.style.display="block",r.screenshotsElement.style.display="block",y.style.display="none"},o.appendChild(y),o.appendChild(r.selectionElement),o.appendChild(s.element),o.appendChild(r.screenshotsElement),o.appendChild(u);let S=document.createElement("div");S.style.display="none",S.style.flexGrow="1",S.style.minHeight="0",S.style.overflow="hidden";let z=ro(()=>I());z.style.height="100%",S.appendChild(z),e.insertBefore(S,i);let B=a.lastElementChild;B&&(B.insertBefore(d.historyBtnWrapper,B.firstChild),B.insertBefore(v(),B.firstChild)),e.appendChild(d.drawer);let P=null;W.subscribe(m=>{D(m),K(),m.isDirty?(P&&clearTimeout(P),P=setTimeout(async()=>{let w=await N(!0);w.subStatus?ge.saveEmergency(w):ge.clearEmergency(),m.isDirty=!1},2e3)):P&&(clearTimeout(P),P=null)});function K(){let m=ge.getCount()>0,w=!!W.currentSubStatus;ct(m||w)}function J(){W.visible=!W.visible,ue(W.visible,e,"cw-btn-notes")}function I(){W.isSplitView=!W.isSplitView,W.isSplitView?(o.style.display="none",S.style.display="flex",S.style.flexDirection="column",n.googleLine&&(n.googleLine.style.background="linear-gradient(to right, #8e24aa, #7b1fa2)")):(o.style.display="flex",S.style.display="none",n.googleLine&&(n.googleLine.style.background="linear-gradient(to right, #4285F4, #EA4335, #FBBC05, #34A853)"))}function T(){let m=document.createElement("div");if(m.innerHTML=`
        ...
        `,!document.getElementById("cw-segmented-styles")){let E=document.createElement("style");E.id="cw-segmented-styles",E.innerHTML=`
            ...
            `,document.head.appendChild(E)}let w=(E,$)=>{let Z=m.querySelector(`#${E}`).querySelector(".cw-segmented-indicator");Z&&(Z.style.transform=`translateX(${$*100}%) translateX(${$*2}px)`)};return m.querySelectorAll("#lang-selector button").forEach((E,$)=>{E.onclick=()=>{W.setLanguage(E.dataset.lang),m.querySelectorAll("#lang-selector button").forEach(j=>j.classList.remove("active")),E.classList.add("active"),w("lang-selector",$),G.playHover(),W.currentSubStatus&&q(W.currentSubStatus)}}),m.querySelectorAll("#type-selector button").forEach((E,$)=>{E.onclick=()=>{W.setCaseType(E.dataset.type),m.querySelectorAll("#type-selector button").forEach(j=>j.classList.remove("active")),E.classList.add("active"),w("type-selector",$),G.playHover(),W.currentSubStatus&&q(W.currentSubStatus)}}),m.querySelectorAll("#portugal-selector button").forEach((E,$)=>{E.onclick=()=>{W.setPortugalCase(E.dataset.val==="true"),m.querySelectorAll("#portugal-selector button").forEach(j=>j.classList.remove("active")),E.classList.add("active"),w("portugal-selector",$),G.playHover(),W.currentSubStatus&&q(W.currentSubStatus)}}),m}function M(){let m=document.createElement("div");m.className="cw-status-section",m.style.cssText="display: flex; flex-direction: column; gap: 8px;",m.innerHTML=`
