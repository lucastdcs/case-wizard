...
E-mail: ${data.clientEmail || "---"}`;
  ...
  var injectStyles2 = () => {
    ...
    .bau-step { display: none; animation: fadeIn 0.5s ${EASE2}; opacity: 0; }
  ...
  function initBAUForm() {
    ...
    confirmCard.innerHTML = `
        <h3 style="margin-top: 0; color: ${COLORS3.blue}; font-size: 16px; margin-bottom: 20px;">Confirme os dados antes de enviar</h3>
    ...
    function renderConfirmation() {
      ...
      const availabilities = [data.availability_1, data.availability_2, data.availability_3].filter((v) => v && v.trim() !== "").map((v) => v.replace("T", " ")).join(" | ");
      ...
      container.innerHTML = `
            ...
            <div class="bau-confirm-row" style="flex-direction: column; align-items: flex-start; gap: 8px;">
                ...
                <div style="font-size: 13px; color: ${COLORS3.textPrimary}; background: #f8f9fa; padding: 12px; border-radius: 8px; width: 100%; box-sizing: border-box; white-space: pre-wrap;">${data.description}</div>
            </div>
    ...
    function validateStep(step) {
      if (step === 2) {
        ...
        if (!reason) {
          showToast("Erro: O Motivo da Abertura \xE9 obrigat\xF3rio.", "error");
          return false;
        }
        if (tasks.length === 0) {
      ...
      for (const input of inputs) {
        if (!input.value.trim()) {
          ...
          showToast(`Erro: O campo '${labelText.replace(":", "")}' \xE9 obrigat\xF3rio.`, "error");
    ...
    function renderData(pd) {
      ...
      if (allDataEl) {
        ...
        const fields = [
          ...
          { label: "Idioma", name: "language", value: pd.language, type: "select", options: ["Portugu\xEAs", "Espanhol"] },
      ...
      if (missingFields.length > 0) {
        banner.innerHTML = `<span>\u26A0\uFE0F</span> Alguns dados n\xE3o foram encontrados. Por favor, complete os campos edit\xE1veis acima.`;
      } else {
        banner.innerHTML = `<span>\u2705</span> Dados do CRM carregados. Voc\xEA pode editar qualquer campo clicando nele.`;
      }
    }
