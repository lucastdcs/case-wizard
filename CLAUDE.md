...
- Test: `npm run test:content` · `test:call-script` · `test:emails` · `test:note-templates` · `test:tasks` · `test:shortcuts` · `test:prefs` · `test:people` · `test:deployment-env` · `test:timezones` · `test:jsonp` · `test:backup` · `test:dash-changelog` · `test:tl-decision` — node harnesses that stub the browser/Apps Script globals (there is no runner: each script exits non-zero on failure). `npm run test:scraping` runs the real scrapers with Playwright against two redacted captures of a real CRM case screen — one translated, one in the original language (`specs/fixtures/`); `npm run smoke:shortcuts`, `smoke:wizards`, `smoke:env-badge`, `smoke:broadcast`, `smoke:bau-scraping`, `smoke:tasks` and `smoke:bau-tasks` drive the agent UI with Playwright over `mock-crm.html`; `smoke:content`, `smoke:people` and `smoke:tl-dash` drive the Apps Script dashboards, loaded from disk with `google.script.run` doubled. **Every Playwright script honours `CW_CHROMIUM`**: set it to a Chromium binary (e.g. `CW_CHROMIUM=/opt/pw-browsers/chromium`) when the environment has a browser but not the exact build Playwright pins, otherwise they die at launch — and a launch failure reads like "suite green" if you only check the exit of the node tests. Anything not covered by these is still verified by hand via the Dev bookmarklet on the CRM plus the Apps Script execution log (see `README.md` → Testing)
