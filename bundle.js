...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
    ...
    `;let x=p.querySelector(".cw-hero-grid"),b=p.querySelector(".cw-acc-container"),g=p.querySelector(".cw-results-container"),u=p.querySelector(".cw-search-input"),w=p.querySelector(".cw-status-bar"),S=p.querySelector(".cw-status-text"),R=p.querySelector(".cw-footer-icons");s.forEach(([L,k])=>{let z=i(k.name),j=document.createElement("div");j.className="cw-hero-card",j.id=`hero-${L}`,j.style.setProperty("--hero-color",z.color),j.innerHTML=`
        ...
        `,j.onclick=q=>{if(q.target.closest(".cw-step-btn"))return;let F=a[L]?a[L].count:0;M(L,F>0?-F:1,k)},j.querySelector(".minus").onclick=()=>M(L,-1,k),j.querySelector(".plus").onclick=()=>M(L,1,k),j.dataset.color=z.color,x.appendChild(j)});function _(L,k){let z=i(k.name),j=document.createElement("div");return j.className="cw-task-item",j.dataset.id=L,j.innerHTML=`
        ...
        `,j.onclick=q=>{if(q.target.closest(".cw-step-btn"))return;let F=a[L]?a[L].count:0;M(L,F>0?-F:1,k)},j.querySelector(".minus").onclick=()=>M(L,-1,k),j.querySelector(".plus").onclick=()=>M(L,1,k),j}Object.entries(r).forEach(([L,k])=>{let z=document.createElement("div");z.className="cw-acc-group";let j=document.createElement("div");j.className="cw-acc-header",j.innerHTML=`
            <div class="cw-acc-title">
                <div class="cw-acc-dot" style="background:${k.brand.color}"></div>
                ${L}
            </div>
            <div class="cw-acc-icon">\u25BC</div>
        `,j.onclick=()=>{b.querySelectorAll(".cw-acc-group.open").forEach(F=>{F!==z&&F.classList.remove("open")}),z.classList.toggle("open")};let q=document.createElement("div");q.className="cw-acc-body",k.tasks.forEach(F=>{let h=_(F.key,F);q.appendChild(h)}),z.appendChild(j),z.appendChild(q),b.appendChild(z)});function M(L,k,z){a[L]||(a[L]={count:0,data:z,brand:i(z.name)}),a[L].count+=k,a[L].count<=0&&delete a[L],J(),K(),t&&t()}function J(){let L=o.tagSupportUsed;s.forEach(([F])=>{let h=x.querySelector(`#hero-${F}`);if(!h)return;let T=a[F];T?(h.classList.add("active"),h.querySelector(".cw-step-val").textContent=T.count,h.querySelector(".cw-step-val").style.color=h.dataset.color,L&&(F==="ads_conversion_tracking"||F==="ads_enhanced_conversions")&&!o.forcedScreenshots.has(F)?h.classList.add("ts-success"):h.classList.remove("ts-success")):(h.classList.remove("active"),h.classList.remove("ts-success"))}),p.querySelectorAll(".cw-task-item").forEach(F=>{let h=F.dataset.id,T=a[h];T?(F.classList.add("selected"),F.querySelector(".cw-step-val").textContent=T.count,L&&(h==="ads_conversion_tracking"||h==="ads_enhanced_conversions")&&!o.forcedScreenshots.has(h)?F.classList.add("ts-success"):F.classList.remove("ts-success")):(F.classList.remove("selected"),F.classList.remove("ts-success"))});let z=Object.keys(a),j=0,q=[];if(z.forEach(F=>{let h=a[F];j+=h.count;for(let T=0;T<h.count;T++)q.length<6&&q.push(h.brand)}),j>0){w.classList.add("visible");let F=j>1?"A\xE7\xF5es":"A\xE7\xE3o",h=j>1?"definidas":"definida";S.textContent=`${j} ${F} ${h}`,R.innerHTML="",q.forEach(T=>{let f=document.createElement("div");f.className="cw-mini-icon",f.innerHTML=ze[T.icon]||ze.default;let y=f.querySelector("svg");y&&(y.style.width="14px",y.style.height="14px"),R.appendChild(f)})}else w.classList.remove("visible")}u.addEventListener("input",L=>{let k=L.target.value.toLowerCase();if(k.length>0){b.style.display="none",g.style.display="block",g.innerHTML="";let z=!1;Object.entries(Ce).forEach(([j,q])=>{if(q.name.toLowerCase().includes(k)){z=!0;let F=_(j,q);a[j]&&(F.classList.add("selected"),F.querySelector(".cw-step-val").textContent=a[j].count),g.appendChild(F)}}),z||(g.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else b.style.display="block",g.style.display="none"});function K(){c.innerHTML="";let L=Object.keys(a),k=!1;if(L.length===0){c.innerHTML=`<div class="cw-empty-state">${e("selecione_tarefas")}</div>`,d.style.display="none";return}let z=o.tagSupportUsed,j=document.createElement("div");j.className="cw-info-banner",j.innerHTML=`
            <span style="font-size:14px">\u2139\uFE0F</span>
            <span>
                Os screenshots seguem as diretrizes atuais do Win Criteria e pol\xEDticas de Tag Support.
            </span>
        `,c.appendChild(j),L.forEach(q=>{let F=a[q].data,h=a[q].count,T=a[q].brand,y=z&&(q==="ads_conversion_tracking"||q==="ads_enhanced_conversions")&&!o.forcedScreenshots.has(q),C=o.screenshotMode||"implementation",O=F.screenshots?.[C]||[];if(O.length>0||y){k=!0;for(let I=1;I<=h;I++){let N=document.createElement("div");N.className="cw-screen-card",y&&N.classList.add("ts-success"),N.style.setProperty("--brand-color",T.color),N.style.setProperty("--brand-bg",T.bg),N.style.setProperty("--brand-shadow",T.color+"40");let $=document.createElement("div");$.className="cw-card-header";let v=document.createElement("div");v.className="cw-card-icon",v.innerHTML=ze[T.icon]||ze.default;let U=document.createElement("div");U.style.cssText="flex:1; display:flex; align-items:center; gap:8px;";let X=document.createElement("input");X.className="cw-card-title-input",X.id=`name-${q}-${I}`,X.value=`${F.name}${h>1?" #"+I:""}`,X.title="Clique para renomear esta task";let W=document.createElement("span");if(W.className="cw-edit-hint",W.innerHTML="\u270E Renomear",U.appendChild(X),U.appendChild(W),$.appendChild(v),$.appendChild(U),N.appendChild($),y){let m=document.createElement("div");m.className="cw-ts-disclaimer-box",m.innerHTML=`
                <span>${e("ts_disclaimer")}</span>
                <button class="cw-btn-ts-force">${e("incluir_mesmo_assim")}</button>
            `,m.querySelector("button").onclick=()=>{o.toggleForcedScreenshot(q,!0)},N.appendChild(m)}else O.forEach((m,E)=>{let A=document.createElement("div");A.className="cw-input-group";let H=document.createElement("label");H.className="cw-input-label",H.textContent=m;let B=document.createElement("input");B.className="cw-input-field",B.id=`screen-${q}-${I}-${E}`,B.placeholder="Cole o link aqui...",B.setAttribute("autocomplete","off"),B.addEventListener("input",()=>{B.value.trim().length>5?B.classList.add("filled"):B.classList.remove("filled")});let Z=document.createElement("div");Z.className="cw-input-check",Z.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>',A.appendChild(H),A.appendChild(B),A.appendChild(Z),N.appendChild(A)});c.appendChild(N)}}}),d.style.display=k?"block":"none"}return{selectionElement:p,screenshotsElement:d,updateSubStatus:()=>K(),getCheckedElements:()=>Object.keys(a).map(L=>({value:L,count:a[L].count})),setTaskCount:(L,k)=>{a[L]&&delete a[L],k>0&&Ce[L]&&M(L,k,Ce[L])},toggleTask:(L,k=!0)=>{let z=a[L];k&&!z?M(L,1,Ce[L]):!k&&z&&M(L,-z.count,Ce[L])},setLanguage:L=>{e=L;let k=p.querySelector(".js-hero-title");k&&(k.textContent=e("acesso_rapido"));let z=p.querySelector(".cw-search-input");z&&(z.placeholder=e("buscar_catalogo")),K()},reset:()=>{for(let L in a)delete a[L];u.value="",b.style.display="block",g.style.display="none",J(),K()}}}var Zo={marginTop:"24px",marginBottom:"16px",padding:"20px",background:"#fff9e6",borderRadius:"20px",border:"1.5px solid #fbbc0540",display:"none",boxShadow:"0 4px 12px rgba(251, 188, 5, 0.05)"},Qo={fontSize:"12px",color:"#b06000",marginTop:"8px",lineHeight:"1.4"},ea={width:"100%",padding:"12px 16px",borderRadius:"12px",border:"1.5px solid #dadce0",fontSize:"14px",marginBottom:"16px",boxSizing:"border-box",background:"#fff"},ta={display:"flex",gap:"20px",marginBottom:"12px"};function ao(t){let e=document.createElement("div");e.id="tag-support-container",Object.assign(e.style,Zo);let o=document.createElement("label");o.className="js-ts-main-label",o.textContent=t("utilizou_tag_support"),Object.assign(o.style,St,{marginTop:"0"});let a=document.createElement("div");Object.assign(a.style,ta);let n=document.createElement("input");n.type="radio",n.name="ts_usage_mod",n.value="Sim",Object.assign(n.style,Et);let i=document.createElement("label");i.textContent="Sim";let s=document.createElement("div");Object.assign(s.style,{display:"flex",alignItems:"center"}),s.appendChild(n),s.appendChild(i);let r=document.createElement("input");r.type="radio",r.name="ts_usage_mod",r.value="N\xE3o",r.checked=!0,Object.assign(r.style,Et);let l=document.createElement("label");l.textContent="N\xE3o";let p=document.createElement("div");Object.assign(p.style,{display:"flex",alignItems:"center"}),p.appendChild(r),p.appendChild(l),a.appendChild(s),a.appendChild(p);let d=document.createElement("div");d.style.display="block";let c=document.createElement("label");c.className="js-ts-reason-label",c.textContent=t("motivo_ts"),Object.assign(c.style,St,{fontSize:"12px"});let x=document.createElement("input");x.type="text",Object.assign(x.style,ea);let b=document.createElement("div");b.className="js-ts-warning",b.innerHTML=`\u26A0\uFE0F <strong>${t("lembre_preencher_form")}</strong> <a href="https://docs.google.com/forms/d/e/1FAIpQLSeP_JM8D-6qHa5ZC93aTzj38WiO5zx8nyrWNPvbZhjJj6CpkA/viewform" target="_blank" style="color:#e37400; text-decoration:underline;">Link aqui</a>`,Object.assign(b.style,Qo),d.appendChild(c),d.appendChild(x),d.appendChild(b),e.appendChild(o),e.appendChild(a),e.appendChild(d),n.onchange=()=>{d.style.display="none",Promise.resolve().then(()=>(Xe(),it)).then(R=>R.notesState.setTagSupportUsed(!0))},r.onchange=()=>{d.style.display="block",Promise.resolve().then(()=>(Xe(),it)).then(R=>R.notesState.setTagSupportUsed(!1))};function g(R,_){if(e.style.display="none",!R||!_||_.length===0)return;_.some(J=>J==="ads_conversion_tracking"||J==="ads_enhanced_conversions")?e.style.display="block":(S(),Promise.resolve().then(()=>(Xe(),it)).then(J=>J.notesState.setTagSupportUsed(!1)))}function u(){if(e.style.display==="none")return"";let R=`<br><b>Utilizou Tag Support?</b> ${n.checked?"\u2705 Sim":"\u274C N\xE3o"}`;return r.checked&&x.value.trim()!==""&&(R+=`<br><b>Motivo:</b> ${x.value}`),R+="<br>",R}function w(R){t=R,o.textContent=t("utilizou_tag_support"),c.textContent=t("motivo_ts"),b.innerHTML=`\u26A0\uFE0F <strong>${t("lembre_preencher_form")}</strong> <a href="https://docs.google.com/forms/d/e/1FAIpQLSeP_JM8D-6qHa5ZC93aTzj38WiO5zx8nyrWNPvbZhjJj6CpkA/viewform" target="_blank" style="color:#b06000; text-decoration:underline;">Link aqui</a>`}function S(){e.style.display="none",r.checked=!0,n.checked=!1,d.style.display="block",x.value=""}return{element:e,updateVisibility:g,getOutput:u,setLanguage:w,reset:S}}var kt="cw_notes_parking_lot",lt="cw_notes_emergency_save";var ge={getAll:()=>{try{return JSON.parse(localStorage.getItem(kt)||"[]")}catch{return[]}},save:t=>{let e=ge.getAll(),o={id:Date.now().toString(),timestamp:new Date().toISOString(),...t};return e.unshift(o),e.length>5&&e.pop(),localStorage.setItem(kt,JSON.stringify(e)),o},delete:t=>{let e=ge.getAll();return e=e.filter(o=>o.id!==t),localStorage.setItem(kt,JSON.stringify(e)),e},getCount:()=>ge.getAll().length,saveEmergency:t=>{let e={timestamp:Date.now(),data:t};localStorage.setItem(lt,JSON.stringify(e))},getEmergency:()=>{try{let t=localStorage.getItem(lt);if(!t)return null;let e=JSON.parse(t);return Date.now()-e.timestamp>432e5?(localStorage.removeItem(lt),null):!e.data||!e.data.subStatus?null:e.data}catch{return null}},clearEmergency:()=>{localStorage.removeItem(lt)}};var oa="https://script.google.com/a/macros/google.com/s/AKfycbw2oPrfcDrXoKWT25hCAetnJ132SKFasTbywfwCVNT-qjwnO6Ip-yRp0w89eyyceziaDQ/exec",Ft="cw_data_broadcast",no="cw_data_tips",aa=["Processando...","Mantenha o foco!","Aguarde..."];function Je(t,e={}){return new Promise((o,a)=>{let n="cw_cb_"+Math.round(1e5*Math.random()),i=document.createElement("script");window[n]=l=>{document.body.contains(i)&&document.body.removeChild(i),delete window[n],o(l)};let s=Object.keys(e).map(l=>encodeURIComponent(l)+"="+encodeURIComponent(e[l])).join("&"),r=`${oa}?op=${t}&callback=${n}&t=${Date.now()}&${s}`;i.src=r,i.onerror=()=>{document.body.contains(i)&&document.body.removeChild(i),delete window[n],a(new Error("JSONP Error (Check Corp Login)"))},document.body.appendChild(i)})}async function je(t,e){try{console.log(`\u{1F4E4} Executando ${t}...`,e);let o=await Je(t,e);if(o&&o.status==="success")return console.log("\u2705 Sucesso:",t),o;throw console.warn("\u26A0\uFE0F Falha:",o),new Error(o.message||`Opera\xE7\xE3o ${t} falhou.`)}catch(o){throw console.error(`\u274C Erro na opera\xE7\xE3o ${t}:`,o),o}}var be={fetchTips:async()=>{try{let t=await Je("tips");t?.tips&&localStorage.setItem(no,JSON.stringify(t.tips))}catch(t){console.warn("Tips offline",t)}},fetchData:async()=>{try{let t=await Je("broadcast");if(t?.broadcast)return localStorage.setItem(Ft,JSON.stringify(t.broadcast)),t}catch(t){console.warn("Broadcast offline",t)}return{broadcast:JSON.parse(localStorage.getItem(Ft)||"[]")}},getCachedBroadcasts:()=>JSON.parse(localStorage.getItem(Ft)||"[]"),getRandomTip:()=>{let t=aa,e=localStorage.getItem(no);if(e)try{t=JSON.parse(e)}catch{}return t[Math.floor(Math.random()*t.length)]},sendBroadcast:async t=>{let e={...t,date:new Date().toISOString(),id:Date.now().toString()};return await je("new_broadcast",e)},updateBroadcast:async(t,e)=>{let o={id:t,...e};return await je("update_broadcast",o)},deleteBroadcast:async t=>await je("delete_broadcast",{id:t}),logEvent:(t,e,o="",a=null)=>{try{let n="anon";try{let s=Ee();s&&(n=s.split("@")[0].toLowerCase())}catch{}let i={timestamp:new Date().toISOString(),user:n,version:"v5.1",category:t,action:e,label:o,value:a||""};Je("log",i).catch(s=>{})}catch(n){console.warn("Analytics error",n)}},logUsage:()=>{},getUserSnippets:async t=>{try{return await Je("get_user_snippets",{user:t})}catch(e){return console.warn("Erro ao buscar snippets:",e),null}},saveSnippet:async(t,e)=>{let o={...t,user:e};return await je("save_snippet",o)},deleteSnippet:async(t,e)=>await je("delete_snippet",{id:t,user:e})},io=async(t,e)=>{let o={...t,user:e,date:new Date().toISOString()};return await je("create_bau_escalation",o),!0};var ct=["lucaste","ricardogi"];var se={glassBg:"rgba(61, 61, 61, 0.77)",glassBorder:"rgba(255, 255, 255, 0.15)",glassActive:"rgba(79, 79, 79, 0.89)",glassHighlight:"rgba(255, 255, 255, 0.08)",iconIdle:"#c2c5c8ff",iconActive:"#FFFFFF",blue:"#8AB4F8",red:"#F28B82",purple:"#C58AF9",green:"#81C995",orange:"#F9AB00",teal:"#00BFA5",pink:"#F48FB1",gray:"#9AA0A6"},dt=t=>new Promise(e=>setTimeout(e,t));function pt(t){let e=document.getElementById("cw-btn-notes");if(!e)return;let o=e.querySelector(".cw-dot-dirty");t?o||(o=document.createElement("div"),o.className="cw-dot-dirty",e.appendChild(o)):o&&o.remove()}function so(t){let e="cw-command-center-style";if(!document.getElementById(e)){let u=document.createElement("style");u.id=e,u.innerHTML=`
    ...
    `;let n=document.createElement("div");n.className="cw-focus-backdrop",document.body.appendChild(n),document.body.appendChild(a);let i=(u,w)=>{P.playClick(),a.querySelector(`.${u}`).classList.toggle("active"),w()};if(a.querySelector(".notes").onclick=u=>{u.stopPropagation(),i("notes",t.toggleNotes)},a.querySelector(".bauform").onclick=u=>{u.stopPropagation(),i("bauform",t.toggleBAUForm)},a.querySelector(".email").onclick=u=>{u.stopPropagation(),i("email",t.toggleEmail)},a.querySelector(".script").onclick=u=>{u.stopPropagation(),i("script",t.toggleScript)},a.querySelector(".links").onclick=u=>{u.stopPropagation(),i("links",t.toggleLinks)},a.querySelector(".library").onclick=u=>{u.stopPropagation(),i("library",t.toggleLibrary)},a.querySelector(".timezone").onclick=u=>{u.stopPropagation(),i("timezone",t.toggleTimezone)},a.querySelector(".configs").onclick=u=>{u.stopPropagation(),i("configs",t.toggleConfigs)},a.querySelector(".broadcast").onclick=u=>{u.stopPropagation(),i("broadcast",()=>{let w=u.currentTarget.querySelector(".cw-badge");w&&w.remove(),t.broadcastControl&&t.broadcastControl.toggle()})},a.querySelectorAll(".cw-btn").forEach(u=>{u.addEventListener("mouseenter",()=>P.playHover())}),t.broadcastControl&&t.broadcastControl.hasUnread){let u=document.createElement("div");u.className="cw-badge",a.querySelector(".broadcast").appendChild(u)}let s=null;a.onmouseleave=()=>{a.querySelector(".cw-btn.active")||a.classList.contains("processing-center")||(s=setTimeout(()=>{a.classList.add("collapsed")},3e3))},a.onmouseenter=()=>{s&&clearTimeout(s)},(async function(){let w=()=>{let R=Ee();if(R){let _=R.split("@")[0].toLowerCase();if(ct.includes(_)){let M=a.querySelector("#cw-admin-tag");M&&M.classList.add("visible")}}else setTimeout(w,2e3)};w(),await dt(2800),a.classList.add("docked"),await dt(300);let S=a.querySelectorAll(".cw-btn");a.querySelectorAll(".cw-sep").forEach(R=>R.classList.add("visible"));for(let R=0;R<S.length;R++)S[R].classList.add("popped"),await dt(90);await dt(200),a.classList.add("system-check")})();let r=!1,l,p,d,c,x=3;a.onmousedown=u=>{if(u.target.closest("button"))return;u.preventDefault(),l=u.clientX,p=u.clientY;let w=a.getBoundingClientRect();d=w.left,c=w.top,document.addEventListener("mousemove",b),document.addEventListener("mouseup",g)};function b(u){let w=u.clientX-l,S=u.clientY-p;!r&&Math.sqrt(w*w+S*S)>x&&(r=!0,a.classList.add("dragging"),a.style.transition="none",s&&clearTimeout(s)),r&&(a.style.left=`${d+w}px`,a.style.top=`${c+S}px`,a.style.right="auto",a.style.bottom="auto",a.style.transform="none")}function g(u){if(document.removeEventListener("mousemove",b),document.removeEventListener("mouseup",g),r){r=!1,a.classList.remove("dragging");let w=window.innerWidth,S=window.innerHeight,R=a.getBoundingClientRect(),_=R.left+R.width/2,M;_<w/2?(M=24,a.classList.remove("side-right"),a.classList.add("side-left")):(M=w-R.width-24,a.classList.remove("side-left"),a.classList.add("side-right"));let J=Math.max(24,Math.min(R.top,S-R.height-24));setTimeout(()=>{a.style.setProperty("transition","left 0.6s cubic-bezier(0.34, 1.56, 0.64, 1), top 0.6s cubic-bezier(0.34, 1.56, 0.64, 1)","important"),a.style.left=`${M}px`,a.style.top=`${J}px`,a.style.bottom="auto",a.style.transform=""},10),setTimeout(()=>{a.style.transition="",a.style.removeProperty("transition")},700)}else{let w=a.querySelector(".cw-btn.active"),S=u.target.closest("button");if(a.classList.contains("collapsed")){let R=a.getBoundingClientRect(),_=window.innerHeight,M=R.top>_/2;if(a.style.setProperty("transition","none","important"),M){let J=_-R.bottom;a.style.top="auto",a.style.bottom=`${J}px`}else a.style.bottom="auto",a.style.top=`${R.top}px`;a.offsetWidth,a.style.removeProperty("transition"),a.classList.remove("collapsed"),P.playGenieOpen()}else!w&&!S&&(a.classList.add("collapsed"),P.playGenieOpen());S&&(S.style.transform="scale(0.9)",setTimeout(()=>S.style.transform="",150))}}}function Ze(){let t=document.querySelector(".cw-pill"),e=document.querySelector(".cw-focus-backdrop");if(!t)return()=>{};t.classList.remove("collapsed"),window._CW_ABORT_PROCESS=!1;let o=document.createElement("div");o.className="cw-center-stage",o.innerHTML=`
  ...
  `;let a=document.createElement("div");a.className="cw-abort-btn",a.textContent="Cancelar",a.onclick=i=>{i.stopPropagation(),window._CW_ABORT_PROCESS=!0,Y("Cancelado!",{duration:3e3}),o.remove(),t.classList.remove("processing-center"),t.classList.remove("success"),t.classList.add("collapsed"),e&&e.classList.remove("active")},o.appendChild(a),t.appendChild(o);let n=Date.now();return t.classList.add("processing-center"),e&&e.classList.add("active"),function(){if(window._CW_ABORT_PROCESS||!t.contains(o))return;let s=Date.now()-n,r=Math.max(0,2e3-s);setTimeout(()=>{if(window._CW_ABORT_PROCESS||!t.contains(o))return;let l=o.querySelector(".cw-center-dots"),p=o.querySelector(".cw-center-text"),d=o.querySelector(".cw-center-success"),c=o.querySelector(".cw-abort-btn");l&&(l.style.display="none"),p&&(p.style.display="none"),c&&(c.style.display="none"),d&&d.classList.add("show"),t.classList.add("success"),setTimeout(()=>{t.classList.remove("processing-center"),setTimeout(()=>{o.remove(),t.classList.remove("success"),t.classList.add("collapsed"),e&&e.classList.remove("active")},400)},1e3)},r)}}function ro(t){let{onSaveCurrent:e,onLoadDraft:o,t:a}=t,n=document.createElement("button");n.className="js-btn-park",n.innerHTML=`
    ...
    `;let p=document.createElement("div");p.style.cssText="padding: 16px 24px; border-bottom: 1px solid #F1F3F4; display: flex; justify-content: space-between; align-items: center; background: #fff;",p.innerHTML=`<span class="js-drawer-title" style="font-size:16px; font-weight:700; color:#202124;">${a("rascunhos_salvos")}</span>`;let d=document.createElement("button");d.innerHTML='<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#5f6368" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>',d.style.cssText="background:none; border:none; padding:4px; cursor:pointer; display:flex; align-items:center; justify-content:center; border-radius:50%; transition:background 0.2s;",d.onmouseenter=()=>d.style.background="#F1F3F4",d.onmouseleave=()=>d.style.background="transparent",d.onclick=()=>x(!1),p.appendChild(d);let c=document.createElement("div");c.style.cssText="flex: 1; overflow-y: auto; padding: 16px 24px; background: #F8F9FA; display: flex; flex-direction: column; gap: 12px;",l.appendChild(p),l.appendChild(c);function x(g){let u=l.style.transform==="translateY(0%)";(g!==void 0?g:!u)?(b(),l.style.transform="translateY(0%)"):l.style.transform="translateY(110%)"}function b(){let g=ge.getAll();if(c.innerHTML="",g.length===0){c.innerHTML=`
                ...
                border: 1.5px solid ${D.bgInput}; box-shadow: ${Re.subtle};
...
`.replace(/\n/g,"<br>");Qe(L);let k=await gt();k?(k.innerText.trim()===""&&(k.innerHTML=""),document.execCommand("insertHTML",!1,L),mt(k),Y("Nota gerada e inserida!")):Y("Copiado! Abra uma nota para colar.")},e}var fe=t=>new Promise(e=>setTimeout(e,t));function ye(t,e="info"){let o={info:"background: #e8f0fe; color: #1a73e8; padding: 2px 5px; border-radius: 3px;",warn:"background: #fef7e0; color: #b06000; padding: 2px 5px; border-radius: 3px;",error:"background: #fce8e6; color: #c5221f; padding: 2px 5px; border-radius: 3px;",success:"background: #e6f4ea; color: #137333; padding: 2px 5px; border-radius: 3px;"};console.log(`%c[EMAIL-BOT] ${t}`,o[e]||o.info)}function Fe(t){if(!t)return;let e={bubbles:!0,cancelable:!0,view:window};["mouseover","mousedown","mouseup","click"].forEach(o=>t.dispatchEvent(new MouseEvent(o,e)))}function ft(t,e){if(!t)return;let o=`cw-warning-${t.id||Math.random().toString(36).substr(2,9)}`,a=document.getElementById(o);a&&a.remove();let n=t.getBoundingClientRect(),i=document.createElement("div");i.id=o,i.style.cssText=`
    ...
    `,document.head.appendChild(t)}function ue(t,e,o){let a=document.getElementById(o);if(!e)return;let n=e.getAttribute("data-moved")==="true",i={x:0,y:0};if(a){let d=a.getBoundingClientRect();i.x=d.left+d.width/2,i.y=d.top+d.height/2}let s,r;if(!n)s=window.innerWidth/2,r=window.innerHeight/2;else{let d=e.getBoundingClientRect();s=d.left+d.width/2,r=d.top+d.height/2,s===0&&r===0&&(s=window.innerWidth/2,r=window.innerHeight/2)}let l=i.x-s,p=i.y-r;t?(P.playGenieOpen(),e.style.transition="none",e.style.opacity="0",e.style.pointerEvents="auto",n?e.style.transform=`translate(${l}px, ${p}px) scale(0.05)`:e.style.transform=`translate(calc(-50% + ${l}px), calc(-50% + ${p}px)) scale(0.05)`,e.offsetWidth,requestAnimationFrame(()=>{e.classList.add("open"),a&&a.classList.add("active"),e.style.transition="opacity 0.4s ease-out, transform 0.5s cubic-bezier(0.19, 1, 0.22, 1)",e.style.opacity="1",n?e.style.transform="translate(0, 0) scale(1)":e.style.transform="translate(-50%, -50%) scale(1)"}),typeof go=="function"&&go(e,o)):(P.playSwoosh(),e.style.transition="opacity 0.25s ease, transform 0.3s cubic-bezier(0.5, 0, 1, 1)",e.style.pointerEvents="none",requestAnimationFrame(()=>{e.style.opacity="0",n?e.style.transform=`translate(${l}px, ${p}px) scale(0.1)`:e.style.transform=`translate(calc(-50% + ${l}px), calc(-50% + ${p}px)) scale(0.1)`}),setTimeout(()=>{e.classList.remove("open"),a&&a.classList.remove("active"),e.style.transition="",e.style.transform=""},300),typeof It=="function"&&It(e))}function go(t,e){It(t);let o=a=>{if(!t.classList.contains("open"))return;let n=t.contains(a.target),i=document.querySelector(".cw-pill"),s=i&&i.contains(a.target);n?(t.classList.remove("idle"),t.style.zIndex="2147483648"):s||(t.classList.add("idle"),t.style.zIndex="2147483646")};t._idleHandler=o,document.addEventListener("mousedown",o)}function It(t){t._idleHandler&&(document.removeEventListener("mousedown",t._idleHandler),t._idleHandler=null)}function fo(){let t="v4.0.0",{popup:e,content:o,header:a,animRefs:n,credit:i}=Kt(t,K),s=ao(v),r=oo(()=>{F(),V.setActiveTasks(r.getCheckedElements())},v,V),l=document.createElement("div");l.style.display="none";let p=to((m,E)=>{h(m,E)});l.appendChild(p);let d=ro({onSaveCurrent:async()=>{let m=await I();return O(),m},onLoadDraft:m=>{$(m)},t:m=>v(m)}),c=k(),x=z(),b=document.createElement("div"),g=X(),u=T(d,v);o.appendChild(c),o.appendChild(x),o.appendChild(g),o.appendChild(l),o.appendChild(b),r.selectionElement.style.display="none",r.screenshotsElement.style.display="none";let w=document.createElement("button");w.id="manual-task-toggle",w.textContent=v("gostaria_de_adicionar_uma_task")||"Gostaria de adicionar uma task",w.style.cssText=`display: none; width: 100%; padding: 14px; border: 2px dashed ${D.primary}; background: ${D.surface}; color: ${D.primary}; border-radius: ${de.medium}; cursor: pointer; font-size: 13px; font-weight: 700; margin-top: 8px; transition: all 0.2s ${re}; text-transform: uppercase; letter-spacing: 0.5px;`,w.onmouseenter=()=>{w.style.background=D.primaryBg},w.onmouseleave=()=>{w.style.background=D.surface},w.onclick=()=>{r.selectionElement.style.display="block",r.screenshotsElement.style.display="block",w.style.display="none"},o.appendChild(w),o.appendChild(r.selectionElement),o.appendChild(s.element),o.appendChild(r.screenshotsElement),o.appendChild(u);let S=document.createElement("div");S.style.display="none",S.style.flexGrow="1",S.style.minHeight="0",S.style.overflow="hidden";let R=po(()=>L());R.style.height="100%",S.appendChild(R),e.insertBefore(S,i);let _=a.lastElementChild;_&&(_.insertBefore(d.historyBtnWrapper,_.firstChild),_.insertBefore(U(),_.firstChild)),e.appendChild(d.drawer);let M=null;V.subscribe(m=>{W(m),J(),m.isDirty?(M&&clearTimeout(M),M=setTimeout(async()=>{let E=await I(!0);E.subStatus?ge.saveEmergency(E):ge.clearEmergency(),m.isDirty=!1},2e3)):M&&(clearTimeout(M),M=null)});function J(){let m=ge.getCount()>0,E=!!V.currentSubStatus;pt(m||E)}function K(){V.visible=!V.visible,ue(V.visible,e,"cw-btn-notes")}function L(){V.isSplitView=!V.isSplitView,V.isSplitView?(o.style.display="none",S.style.display="flex",S.style.flexDirection="column",n.googleLine&&(n.googleLine.style.background="linear-gradient(to right, #8e24aa, #7b1fa2)")):(o.style.display="flex",S.style.display="none",n.googleLine&&(n.googleLine.style.background="linear-gradient(to right, #4285F4, #EA4335, #FBBC05, #34A853)"))}function k(){let m=document.createElement("div");if(m.innerHTML=`
            <div style="display: flex; gap: 12px; margin-bottom: 8px;">
                ...
                <div style="flex: 1;">
                    <div class="cw-section-title js-label-fluxo" style="font-size: 10px; margin-bottom: 6px;">${v("fluxo")}</div>
                ...
                <div style="flex: 1;">
                    <div class="cw-section-title js-label-portugal" style="font-size: 10px; margin-bottom: 6px;">${v("caso_portugal")}</div>
                    <div class="cw-segmented-control" id="portugal-selector">
                        <div class="cw-segmented-indicator"></div>
                        <button data-val="false" class="active" style="z-index:2">${v("nao")}</button>
                        <button data-val="true" style="z-index:2">${v("sim")}</button>
                    </div>
                </div>
            </div>
        `,!document.getElementById("cw-segmented-styles")){let A=document.createElement("style");A.id="cw-segmented-styles",A.innerHTML=`
            ...
            `,document.head.appendChild(A)}let E=(A,H)=>{let Z=m.querySelector(`#${A}`).querySelector(".cw-segmented-indicator");Z&&(Z.style.transform=`translateX(${H*100}%) translateX(${H*2}px)`)};return m.querySelectorAll("#lang-selector button").forEach((A,H)=>{A.onclick=()=>{V.setLanguage(A.dataset.lang),m.querySelectorAll("#lang-selector button").forEach(B=>B.classList.remove("active")),A.classList.add("active"),E("lang-selector",H),P.playHover(),V.currentSubStatus&&q(V.currentSubStatus)}}),m.querySelectorAll("#type-selector button").forEach((A,H)=>{A.onclick=()=>{V.setCaseType(A.dataset.type),m.querySelectorAll("#type-selector button").forEach(B=>B.classList.remove("active")),A.classList.add("active"),E("type-selector",H),P.playHover(),V.currentSubStatus&&q(V.currentSubStatus)}}),m.querySelectorAll("#portugal-selector button").forEach((A,H)=>{A.onclick=()=>{V.setPortugalCase(A.dataset.val==="true"),m.querySelectorAll("#portugal-selector button").forEach(B=>B.classList.remove("active")),A.classList.add("active"),E("portugal-selector",H),P.playHover(),V.currentSubStatus&&q(V.currentSubStatus)}}),m}function z(){let m=document.createElement("div");m.className="cw-status-section",m.style.cssText="display: flex; flex-direction: column; gap: 8px;",m.innerHTML=`
            <div class="cw-section-title js-label-status" style="margin-top: 8px;">${v("status_principal")}</div>
            <select id="main-status-select" class="cw-select">
                <option value="" disabled selected>${v("select_status")}</option>
            ...
            <div class="cw-section-title js-label-substatus" style="margin-top: 8px;">${v("substatus")}</div>
            <select id="sub-status-select" class="cw-select" disabled>
                <option value="">${v("select_substatus")}</option>
            </select>
        `;let E=m.querySelector("#main-status-select"),A=m.querySelector("#sub-status-select");return E.onchange=()=>{V.setStatus(E.value),j(E.value,A),V.setSubStatus(""),q("")},A.onchange=()=>{V.setSubStatus(A.value),q(A.value)},m}function j(m,E){if(E.innerHTML=`<option value="">${v("select_substatus")}</option>`,!m){E.disabled=!0;return}for(let A in ke)if(ke[A].status===m){let H=document.createElement("option");H.value=A,H.textContent=ke[A].name,E.appendChild(H)}E.disabled=!1}function q(m){if(p.render&&p.render(m,V.currentCaseType),!m){l.style.display="none",b.style.display="none",document.getElementById("manual-task-toggle").style.display="none",r.selectionElement.style.display="none",r.screenshotsElement.style.display="none",g.style.display="flex",g.style.opacity="1",u.style.display="none";return}g.style.opacity="0",setTimeout(()=>{V.currentSubStatus&&(g.style.display="none")},400),u.style.display="grid";let E=ke[m];E&&E.templateFields&&V.setActiveFields(E.templateFields),C(),At(m,b,V),b.style.display="block",l.style.display="block";let A=m.startsWith("SO_"),H=m==="NI_Awaiting_Validation",B=document.getElementById("manual-task-toggle");A||H?(r.selectionElement.style.display="block",B.style.display="none"):(r.selectionElement.style.display="none",r.screenshotsElement.style.display="none",B.style.display="block");let Z=m==="SO_Education_Only"?"education":"implementation";V.setScreenshotMode(Z),V.currentCaseType==="lm"?V.toggleFieldExclusion("field-ON_CALL",!0):V.toggleFieldExclusion("field-ON_CALL",!1),r.updateSubStatus(m),F();let G=document.getElementById("email-automation-toggle-row");G&&(G.style.display=$e[m]?"flex":"none")}function F(){let m=r.getCheckedElements().map(E=>E.value);s.updateVisibility(V.currentSubStatus,m)}function h(m,E){let A=rt[m];if(A){for(let H in A)if(H==="linkedTask")r.toggleTask(A.linkedTask,E);else if(H==="activeTasks")A.activeTasks.forEach(B=>{E?r.setTaskCount(B.value,B.count):r.setTaskCount(B.value,0)});else if(H.startsWith("field-")){let B=H,Z=A[H],G=document.getElementById(B);if(G){let ee=He.includes(B.replace("field-",""));if(E)if(ee){let oe=G.value.trim();oe.includes(Z.trim())||(G.value=oe?oe+`
`+Z.trim():Z.trim())}else G.value=Z;else if(ee){let oe=G.value.trim(),Q=Z.trim();oe.includes(Q)&&(G.value=oe.replace(Q,"").trim().replace(/\n{3,}/g,`

`))}else G.value.trim()===Z.trim()&&(G.value="");V.updateField(B,G.value),G.dispatchEvent(new Event("input"))}}}}function T(m,E){let A=document.createElement("div");if(A.className="cw-actions-section",A.style.cssText=`
            display: grid;
        ...
        `;let B=m.parkButton;B.classList.add("js-btn-park"),B.style.cssText="width: 100%; margin: 0; border-radius: 10px; height: 34px; font-weight: 600; font-size: 11.5px;";let Z=document.createElement("button");Z.className="cw-btn-secondary js-btn-reset",Z.textContent=E("limpar"),Z.style.cssText=`width: 100%; height: 34px; background: ${D.surface}; color: ${D.textSub}; border: 1px solid ${D.border}; border-radius: 10px; font-weight: 600; cursor: pointer; font-size: 11.5px;`,Z.onclick=()=>O();let G=document.createElement("button");G.className="cw-btn-secondary js-btn-copy",G.textContent=E("copiar"),G.style.cssText=`width: 100%; height: 34px; background: ${D.surface}; color: ${D.primary}; border: 1px solid ${D.primary}; border-radius: 10px; font-weight: 600; cursor: pointer; font-size: 11.5px;`,G.onclick=()=>f();let ee=document.createElement("button");return ee.className="cw-btn-primary js-btn-generate",ee.textContent=E("preencher"),ee.style.cssText=`width: 100%; height: 38px; background: ${D.primary}; color: #fff; border: none; border-radius: 10px; font-weight: 600; cursor: pointer; grid-column: span 2; font-size: 12.5px; box-shadow: 0 4px 10px rgba(26, 115, 232, 0.2); margin-top: 0px;`,ee.onclick=()=>y(),A.appendChild(H),A.appendChild(B),A.appendChild(Z),A.appendChild(G),A.appendChild(ee),A}async function f(){if(!V.currentSubStatus){Y(v("select_substatus"),{error:!0});return}let m=Tt(V,r,s);m?(Qe(m),Y(v("copiado_sucesso")),P.playClick()):Y(v("select_substatus"),{error:!0})}async function y(){if(!V.currentSubStatus){Y(v("select_substatus"),{error:!0});return}let m=Tt(V,r,s);Qe(m),K();let E=Ze(),A=await gt();if(A){A.focus(),document.execCommand("insertHTML",!1,m),mt(A);let H=document.getElementById("email-automation-checkbox");(!H||H.checked)&&V.currentSubStatus&&$e[V.currentSubStatus]&&await xt($e[V.currentSubStatus]),Y(v("inserido_copiado")),P.playSuccess(),O()}E()}function C(){if(V.currentSubStatus){if(V.currentCaseType==="lm")V.removeField("ON_CALL");else{let m=ke[V.currentSubStatus];m&&m.templateFields.includes("ON_CALL")&&V.addFieldAt("ON_CALL",1)}V.isPortugalCase?(V.addFieldAt("CASO_PORTUGAL",1),V.addFieldAt("CONSENTIU_GRAVACAO",2)):(V.removeField("CASO_PORTUGAL"),V.removeField("CONSENTIU_GRAVACAO"))}}function O(){V.reset(),r.reset(),s.reset(),J(),ge.clearEmergency(),o.querySelectorAll("select").forEach(E=>E.value=""),o.querySelector("#sub-status-select").disabled=!0;let m=document.getElementById("email-automation-toggle-row");m&&(m.style.display="none"),b.innerHTML="",l.style.display="none",g.style.display="flex",g.style.opacity="1",u.style.display="none",document.getElementById("manual-task-toggle")&&(document.getElementById("manual-task-toggle").style.display="none"),r.selectionElement.style.display="none",r.screenshotsElement.style.display="none"}async function I(m=!1){let E={};b.querySelectorAll("input, textarea, select").forEach(G=>{(G.id.startsWith("field-")||G.id==="consent-select")&&(E[G.id]=G.value)});let A="Cliente",H="---";if(!m)try{let G=await we();A=G.advertiserName,H=G.cid}catch(G){console.warn("Erro ao coletar pageData:",G)}let B=r.getCheckedElements().map(G=>({key:G.value,count:G.count})),Z=B.map(G=>{let ee=Ce[G.key];return ee?ee.name:G.key});return{currentCaseType:V.currentCaseType,currentLang:V.currentLang,isPortugalCase:V.isPortugalCase,consent:V.consent,tagSupportUsed:V.tagSupportUsed,forcedScreenshots:[...V.forcedScreenshots],excludedFields:[...V.excludedFields],activeFields:V.activeFields,status:V.currentStatus,subStatus:V.currentSubStatus,formData:E,activeTasks:B,summaryTags:Z,clientName:A,cid:H,timestamp:new Date().toISOString()}}let N=m=>new Promise(E=>setTimeout(E,m));async function $(m){V.setLanguage(m.currentLang||"pt"),V.setCaseType(m.currentCaseType||"bau"),V.setPortugalCase(m.isPortugalCase||!1),V.setConsent(m.consent||!1),V.setExcludedFields(m.excludedFields||[]),m.activeFields&&V.setActiveFields(m.activeFields);let E=o.querySelector(`#lang-selector button[data-lang="${V.currentLang}"]`);E&&E.classList.add("active"),o.querySelectorAll("#lang-selector button").forEach(B=>{B!==E&&B.classList.remove("active")});let A=o.querySelector(`#type-selector button[data-type="${V.currentCaseType}"]`);A&&A.classList.add("active"),o.querySelectorAll("#type-selector button").forEach(B=>{B!==A&&B.classList.remove("active")});let H=o.querySelector(`#portugal-selector button[data-val="${V.isPortugalCase}"]`);if(H&&H.classList.add("active"),o.querySelectorAll("#portugal-selector button").forEach(B=>{B!==H&&B.classList.remove("active")}),m.status){let B=o.querySelector("#main-status-select");B.value=m.status,V.setStatus(m.status);let Z=o.querySelector("#sub-status-select");if(j(m.status,Z),await N(50),m.subStatus){if(Z.value=m.subStatus,V.setSubStatus(m.subStatus),q(m.subStatus),await N(100),m.tagSupportUsed!==void 0){V.setTagSupportUsed(m.tagSupportUsed);let G=s.element.querySelector('input[value="Sim"]'),ee=s.element.querySelector('input[value="N\xE3o"]');m.tagSupportUsed&&G?G.checked=!0:ee&&(ee.checked=!0),s.element.querySelector("div:last-child").style.display=m.tagSupportUsed?"none":"block"}m.forcedScreenshots&&V.setForcedScreenshots(m.forcedScreenshots);for(let G in m.formData){let ee=document.getElementById(G);ee&&(ee.value=m.formData[G],V.updateField(G,ee.value))}m.activeTasks&&(m.activeTasks.forEach(G=>r.setTaskCount(G.key,G.count)),V.setActiveTasks(r.getCheckedElements()))}}V.isDirty=!1}function v(m){return Se[V.currentLang]?.[m]||Se.pt?.[m]||m}function U(){let m=document.createElement("div");return m.innerHTML='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="16 3 21 3 21 8"></polyline><line x1="4" y1="20" x2="21" y2="3"></line><polyline points="21 16 21 21 16 21"></polyline><line x1="15" y1="15" x2="21" y2="21"></line><line x1="4" y1="4" x2="9" y2="9"></line></svg>',m.style.cssText="width: 32px; height: 32px; display: flex; align-items: center; justify-content: center; border-radius: 50%; cursor: pointer; color: #9AA0A6; transition: all 0.2s;",m.onclick=E=>{E.stopPropagation(),L()},m.title="Alternar para Split & Transfer",m}function X(){let m=document.createElement("div");return m.id="notes-empty-state",m.style.cssText=`
            display: flex;
            flex-direction: column;
            align-items: center;
            justify-content: center;
    ...
    `,document.head.appendChild(p),Object.assign(l.style,me,{width:"850px",height:"650px",display:"none",flexDirection:"column",fontFamily:"'-apple-system', 'BlinkMacSystemFont', 'Segoe UI', 'Roboto', 'Helvetica Neue', 'Arial', sans-serif",borderRadius:"12px",overflow:"hidden"});let d=pe(l,"Email Assistant",t,"Refatora\xE7\xE3o completa do m\xF3dulo de e-mail para uma experi\xEAncia moderna e eficiente.",{popup:l},()=>h()),c=document.createElement("div");Object.assign(c.style,{display:"flex",flex:"1",overflow:"hidden",backgroundColor:r.bgApp});let x=document.createElement("div");Object.assign(x.style,{width:"320px",backgroundColor:"#EFEFF0",borderRight:`1px solid ${r.borderSubtle}`,display:"flex",flexDirection:"column",flexShrink:"0"});let b=document.createElement("div");Object.assign(b.style,{padding:"16px",borderBottom:`1px solid ${r.borderSubtle}`,position:"relative"});let g=document.createElement("input");g.placeholder="Buscar templates...",Object.assign(g.style,{width:"100%",padding:"10px 14px 10px 36px",borderRadius:"10px",border:"1.5px solid transparent",backgroundColor:"#E3E3E8",fontSize:"15px",outline:"none",boxSizing:"border-box",color:r.textPrimary,backgroundImage:`url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="%238A8A8E" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>')`,backgroundRepeat:"no-repeat",backgroundPosition:"12px center",transition:"all 0.2s ease-in-out"}),g.onfocus=()=>{g.style.backgroundColor="#FFFFFF",g.style.borderColor=r.primary,g.style.boxShadow="0 0 0 4px rgba(0, 122, 255, 0.1)",g.style.transform="scale(1.02)"},g.onblur=()=>{g.style.backgroundColor="#E3E3E8",g.style.borderColor="transparent",g.style.boxShadow="none",g.style.transform="scale(1)"};let u=document.createElement("div");u.id="email-template-list",Object.assign(u.style,{flex:"1",overflowY:"auto",padding:"8px",scrollBehavior:"smooth"});let w=document.createElement("div");w.innerHTML="\u2715",Object.assign(w.style,{position:"absolute",right:"26px",top:"50%",transform:"translateY(-50%)",fontSize:"10px",color:"#fff",cursor:"pointer",display:"none",backgroundColor:"#C7C7CC",width:"16px",height:"16px",borderRadius:"50%",textAlign:"center",lineHeight:"16px",fontWeight:"bold"}),w.onclick=()=>{g.value="",n="",w.style.display="none",f(),g.focus()},b.appendChild(g),b.appendChild(w),x.appendChild(b),x.appendChild(u);let S=document.createElement("div");Object.assign(S.style,{flex:"1",display:"flex",flexDirection:"column",overflow:"hidden",backgroundColor:r.bgApp,transition:"all 0.3s cubic-bezier(0.25, 1, 0.5, 1)"});let R=document.createElement("div");Object.assign(R.style,{padding:"20px",borderBottom:`1px solid ${r.borderSubtle}`,backgroundColor:r.bgSurface,maxHeight:"250px",overflowY:"auto",display:"none"});let _=document.createElement("div");Object.assign(_.style,{flex:"1",display:"flex",flexDirection:"column",padding:"20px",backgroundColor:r.bgApp,overflow:"hidden"});let M=document.createElement("div");Object.assign(M.style,{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"12px"});let J=document.createElement("span");J.textContent="Preview do E-mail",Object.assign(J.style,{fontSize:"12px",fontWeight:"600",color:r.textSecondary,textTransform:"uppercase",letterSpacing:"0.5px"});let K=document.createElement("div");Object.assign(K.style,{display:"flex",gap:"8px"});let L=(N,$=!1)=>{let v=document.createElement("button");return v.textContent=N,Object.assign(v.style,{padding:"8px 14px",borderRadius:"10px",border:$?"none":`1.5px solid ${r.primary}`,background:$?r.primary:"transparent",color:$?"#fff":r.primary,fontSize:"13px",fontWeight:"600",cursor:"pointer",transition:"all 0.2s cubic-bezier(0.25, 1, 0.5, 1)",boxShadow:$?"0 4px 12px rgba(0, 122, 255, 0.3)":"none"}),v.onmouseenter=()=>{$?(v.style.backgroundColor="#0062CC",v.style.transform="translateY(-1px)",v.style.boxShadow="0 6px 16px rgba(0, 122, 255, 0.4)"):v.style.backgroundColor="rgba(0, 122, 255, 0.05)"},v.onmouseleave=()=>{$?(v.style.backgroundColor=r.primary,v.style.transform="translateY(0)",v.style.boxShadow="0 4px 12px rgba(0, 122, 255, 0.3)"):(v.style.backgroundColor="transparent",v.style.transform="translateY(0)")},v.onmousedown=()=>v.style.transform="scale(0.94)",v.onmouseup=()=>v.style.transform="scale(1)",v},k=L("Copiar HTML"),z=L("Preencher no CRM",!0),j=L("Smart CR");j.style.borderColor="#E67E22",j.style.color="#E67E22",j.style.display="none",K.appendChild(j),K.appendChild(k),K.appendChild(z),M.appendChild(J),M.appendChild(K);let q=document.createElement("div");q.contentEditable="true",Object.assign(q.style,{flex:"1",backgroundColor:r.bgSurface,border:`1px solid ${r.borderSubtle}`,borderRadius:"8px",padding:"20px",fontSize:"15px",lineHeight:"1.6",color:r.textPrimary,overflowY:"auto",outline:"none",boxShadow:"inset 0 1px 2px rgba(0,0,0,0.02)"}),_.appendChild(M),_.appendChild(q),I(),S.appendChild(R),S.appendChild(_),c.appendChild(x),c.appendChild(S),l.appendChild(d),l.appendChild(c);let F=document.createElement("div");Object.assign(F.style,Ae),l.appendChild(F),Te(l,F),document.body.appendChild(l);function h(){e=!e,e?(l.style.display="flex",jt(l),o.length===0&&T()):l.style.display="none",ue(e,l,"cw-btn-email")}async function T(){u.innerHTML='<div style="padding: 20px; text-align: center; color: #5f6368;">Carregando...</div>',o=await xo.getTemplates(),f()}function f(){u.innerHTML="";let N=o.filter(W=>W.name.toLowerCase().includes(n.toLowerCase())||W.category.toLowerCase().includes(n.toLowerCase())),$=Object.entries($e).filter(([W,m])=>m&&(W.toLowerCase().includes(n.toLowerCase())||m.toLowerCase().includes(n.toLowerCase()))).map(([W,m])=>({id:W,name:W.replace(/_/g," "),category:"\u26A1 Smart CRs",code:m,isSmartCR:!0})),v=he.getSnippets("email").filter(W=>W.title.toLowerCase().includes(n.toLowerCase())||W.subject&&W.subject.toLowerCase().includes(n.toLowerCase())).map(W=>{let m=[],E=W.content.match(/\[([^\]]+)\]/g);return E&&[...new Set(E)].forEach(A=>{m.push({key:A,label:A.replace("[","").replace("]",""),type:A.toLowerCase().includes("data")?"date":"text",auto:A.toLowerCase().includes("nome")&&A.toLowerCase().includes("seu")?"agentName":null})}),{id:W.id||`snippet-${Math.random()}`,name:W.title,category:"\u{1F464} Pessoal",subject:W.subject||"Sem Assunto",template:W.content,placeholders:m}}),U=[...N,...$,...v];if(U.length===0){u.innerHTML=`
                ...
                </div>`):R.style.display="none";return}let N=a.placeholders&&a.placeholders.length>0;if(R.style.display=N?"block":"none",!N)return;let $=document.createElement("div");Object.assign($.style,{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"16px"}),(a.placeholders||[]).forEach(v=>{let U=document.createElement("div"),X=document.createElement("label");X.textContent=v.label,Object.assign(X.style,{display:"block",fontSize:"11px",fontWeight:"700",color:r.textSecondary,marginBottom:"8px",textTransform:"uppercase",letterSpacing:"0.5px"});let W=document.createElement("input");W.type=v.type||"text",W.dataset.key=v.key,Object.assign(W.style,{width:"100%",padding:"10px 12px",borderRadius:"8px",border:`1.5px solid ${r.borderSubtle}`,backgroundColor:"#FBFBFD",fontSize:"14px",boxSizing:"border-box",transition:"all 0.2s ease",outline:"none"}),W.onfocus=()=>{W.style.borderColor=r.primary,W.style.backgroundColor="#FFFFFF",W.style.boxShadow="0 0 0 4px rgba(0, 122, 255, 0.1)"},W.onblur=()=>{W.style.borderColor=r.borderSubtle,W.style.backgroundColor="#FBFBFD",W.style.boxShadow="none"},v.auto==="agentName"&&(W.value=Ye().split(" ")[0]),W.addEventListener("input",I),U.appendChild(X),U.appendChild(W),$.appendChild(U)}),R.appendChild($)}function I(){if(!a){q.innerHTML=`
...
E-mail: ${E.clientEmail||"---"}`;H&&(H.value=Z),B&&(B.style.display="block",B.style.maxHeight="300px",B.style.opacity="1",B.scrollIntoView({behavior:"smooth",block:"end"}))})}function b(){r=!r,ue(r,l,"cw-btn-script"),r?(c(),d||(d=setInterval(c,2e3))):d&&(clearInterval(d),d=null)}let g=pe(l,"Call Script",t,"Guia interativo para condu\xE7\xE3o de chamadas.",p,()=>{b()});l.appendChild(g);let u=document.createElement("div");Object.assign(u.style,a.contextBanner),u.innerHTML=`
  ...
  `;let w=u.querySelector("#csa-toggle-options"),S=u.querySelector("#csa-options-content"),R=u.querySelector("#csa-options-arrow"),_=!1;w.onclick=()=>{_=!_,R.style.transform=_?"rotate(180deg)":"rotate(0deg)",S.style.maxHeight=_?"400px":"0",S.style.opacity=_?"1":"0",S.style.marginTop=_?"8px":"0",P.playClick()};let M=u.querySelector("#cw-pill-message"),J=u.querySelector("#cw-am-copy-final"),K=u.querySelector("#cw-am-message-area");M.onmouseenter=()=>{M.style.borderColor="#007AFF",M.style.boxShadow="0 2px 8px rgba(0,0,0,0.05)"},M.onmouseleave=()=>{M.style.borderColor="#DADCE0",M.style.boxShadow="0 1px 2px rgba(0,0,0,0.02)"},M.addEventListener("click",()=>{x()}),J.addEventListener("click",()=>{K.value&&(navigator.clipboard.writeText(K.value),Y("Mensagem copiada!"),P.playSuccess(),J.style.background="#34A853",J.textContent="Copiado!",setTimeout(()=>{J.style.background="#1A73E8",J.textContent="Copiar Mensagem Final"},2e3))});let L=(E,A)=>{let H=u.querySelector(E),B=u.querySelector(A);H.onclick=()=>{let Z=B.textContent;!Z||Z.includes("---")||Z.includes("N\xE3o encontrado")||(navigator.clipboard.writeText(Z),P.playSuccess(),H.classList.add("copied"),setTimeout(()=>H.classList.remove("copied"),1500))}};l.appendChild(u);let k=document.createElement("div");Object.assign(k.style,a.progressBarContainer);let z=document.createElement("div");z.className="csa-progress-fill",Object.assign(z.style,a.progressBarFill),k.appendChild(z),l.appendChild(k);let j=document.createElement("div");j.id="csa-content",Object.assign(j.style,a.contentArea),l.appendChild(j);let q=document.createElement("div");Object.assign(q.style,a.footer);let F=document.createElement("span");F.textContent="by lucaste@",Object.assign(F.style,{fontSize:"10px",color:"#bdc1c6"});let h=document.createElement("button");h.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"></path><path d="M3 3v5h5"></path></svg> Resetar Script',Object.assign(h.style,a.resetBtn),h.onmouseenter=()=>h.style.background="#fce8e6",h.onmouseleave=()=>h.style.background="transparent",h.onclick=()=>{h.style.transform="scale(0.9)",setTimeout(()=>h.style.transform="scale(1)",150);for(let E in n)delete n[E];X()},q.appendChild(F),q.appendChild(h),l.appendChild(q);let T=document.createElement("div");Object.assign(T.style,{display:"flex",flexDirection:"column",gap:"8px",marginBottom:"16px"});let f=document.createElement("div");f.className="csa-segmented-control",f.innerHTML=`
  ...
  `,u.onfocus=()=>{b.style.boxShadow="0 4px 12px rgba(26,115,232,0.15)",b.style.border="1px solid #1a73e8"},u.onblur=()=>{b.style.boxShadow="0 2px 6px rgba(0,0,0,0.04)",b.style.border="1px solid transparent"},b.appendChild(g),b.appendChild(u),x.appendChild(b),c.appendChild(x);let w=document.createElement("div");w.style.cssText="flex: 1; overflow-y: auto; padding: 0 24px 40px 24px; scroll-behavior: smooth;",c.appendChild(w);let S=null;function R(){if(S)return;S=document.createElement("div"),S.style.cssText=`
          position: absolute; bottom: 0; left: 0; width: 100%; height: 100%;
          background: rgba(255,255,255,0.98); z-index: 20;
          display: flex; flex-direction: column;
          transform: translateY(100%); transition: transform 0.3s cubic-bezier(0.2, 0.8, 0.2, 1);
          box-shadow: 0 -4px 20px rgba(0,0,0,0.1);
      `;let F=document.createElement("div");F.style.cssText="padding: 16px 24px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #F1F3F4;",F.innerHTML='<span style="font-size: 16px; font-weight: 700; color: #202124;">\u{1F552} Recentes</span>';let h=document.createElement("button");h.innerHTML='<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>',h.style.cssText="background: none; border: none; cursor: pointer; color: #5f6368;",h.onclick=()=>{M(),n=!1,k()},F.appendChild(h),S.appendChild(F);let T=document.createElement("div");T.id="cw-history-list",T.style.cssText="flex: 1; overflow-y: auto; padding: 20px; background: #F8F9FA;",S.appendChild(T),c.appendChild(S)}function _(){S||R();let F=S.querySelector("#cw-history-list");F.innerHTML="";let h=na();h.length===0?F.innerHTML='<div style="text-align: center; color: #999; margin-top: 60px; font-size:13px;">Nada por aqui ainda.</div>':h.forEach(T=>{let f=j(T,Be[T._originalCat],!0,T._originalCat);F.appendChild(f)}),requestAnimationFrame(()=>S.style.transform="translateY(0)")}function M(){S&&(S.style.transform="translateY(100%)")}function J(){d.innerHTML="";let F=K("history","Recentes",Be.history);F.id="cw-sidebar-btn-history",F.onclick=()=>{P.playClick(),n=!n,n?_():M(),k()},d.appendChild(F);let h=document.createElement("div");h.style.cssText="width: 32px; height: 1px; background: rgba(0,0,0,0.08); margin: 4px 0;",d.appendChild(h),Object.keys(et).forEach(T=>{let f=et[T],y=K(T,f.label,Be[T]);y.id=`cw-sidebar-btn-${T}`,y.onclick=()=>{P.playClick(),n&&(n=!1,M()),L(T)},d.appendChild(y)})}function K(F,h,T){let f=document.createElement("div");f.style.cssText=`
          width: 56px; height: 56px; border-radius: 16px;
          display: flex; flex-direction: column; align-items: center; justify-content: center;
          cursor: pointer; color: ${i.textSecondary};
          transition: all 0.2s cubic-bezier(0.2, 0.0, 0.2, 1);
          position: relative;
      `,f.title=h,f.dataset.key=F;let y=document.createElement("div");y.style.cssText="width: 24px; height: 24px; margin-bottom: 2px; transition: transform 0.2s;",y.innerHTML=T||Be.tasks;let C=document.createElement("div");return C.style.cssText="font-size: 9px; font-weight: 600; opacity: 0.7; letter-spacing: 0.3px;",C.textContent=h,f.appendChild(y),f.appendChild(C),f.onmouseenter=()=>{a!==F&&!(F==="history"&&n)&&(f.style.background="#F1F3F4",y.style.transform="scale(1.1)")},f.onmouseleave=()=>{a!==F&&!(F==="history"&&n)&&(f.style.background="transparent",y.style.transform="scale(1)")},f}function L(F){let h=document.getElementById(`cat-anchor-${F}`);h&&(h.scrollIntoView({behavior:"smooth",block:"start"}),a=F,k())}function k(){Object.keys(et).forEach(h=>{let T=d.querySelector(`#cw-sidebar-btn-${h}`);if(T)if(a===h&&!n){let f=ht[h];T.style.background=f.bg,T.style.color=f.color,T.querySelector("div:first-child").style.transform="scale(1.1)"}else T.style.background="transparent",T.style.color=i.textSecondary,T.querySelector("div:first-child").style.transform="scale(1)"});let F=d.querySelector("#cw-sidebar-btn-history");F&&(n?(F.style.background="#3C4043",F.style.color="#FFFFFF"):(F.style.background="transparent",F.style.color=i.textSecondary))}function z(){if(w.innerHTML="",e.trim()!==""){let h=[];if(Object.entries(et).forEach(([f,y])=>{let C=y.links.filter(O=>O.name.toLowerCase().includes(e.toLowerCase())||O.desc.toLowerCase().includes(e.toLowerCase()));h.push(...C.map(O=>({...O,_cat:f})))}),h.length===0){w.innerHTML='<div style="text-align:center; padding: 60px; color:#999; font-size:13px;">Nada encontrado.</div>';return}let T=document.createElement("div");T.innerHTML="Resultados da busca",T.style.cssText="font-size:12px; font-weight:700; color:#5f6368; margin:20px 0 10px; text-transform:uppercase; letter-spacing:0.5px;",w.appendChild(T),h.forEach(f=>{let y=j(f,Be[f._cat],!1,f._cat);w.appendChild(y)});return}Object.entries(et).forEach(([h,T])=>{let f=ht[h],y=document.createElement("div"),C=document.createElement("div");C.id=`cat-anchor-${h}`,C.style.cssText=`
              display: flex; align-items: center; gap: 8px;
          ...
          `,C.innerHTML=`
            ...
            ${T.label}
          `,y.appendChild(C);let O=document.createElement("div");O.style.cssText="display: grid; grid-template-columns: 1fr; gap: 8px;",T.links.forEach(I=>{let N=j(I,Be[h],!1,h);O.appendChild(N)}),y.appendChild(O),w.appendChild(y)});let F=document.createElement("div");F.style.height="80px",w.appendChild(F)}function j(F,h,T,f){let y=document.createElement("div"),C=ht[f]||ht.history;y.style.cssText=`
      ...
      `;let O=document.createElement("div");O.style.cssText=`
          ...
          display: flex; align-items: center; justify-content: center;
          flex-shrink: 0; transition: all 0.2s;
      `,O.innerHTML=h||Be.tasks;let I=O.querySelector("svg");I&&(I.style.width="22px",I.style.height="22px");let N=document.createElement("div");N.style.cssText="flex: 1; display: flex; flex-direction: column; gap: 2px; overflow: hidden;";let $=document.createElement("div");$.style.cssText=`font-size: 14px; font-weight: 600; color: ${i.textPrimary}; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;`,$.textContent=F.name;let v=document.createElement("div");v.style.cssText=`font-size: 12px; color: ${i.textSecondary}; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;`,v.textContent=F.desc,N.appendChild($),N.appendChild(v);let U=document.createElement("div");return U.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>',U.style.cssText=`
          width: 32px; height: 32px; border-radius: 8px;
          display: flex; align-items: center; justify-content: center;
          color: #9AA0A6; transition: all 0.2s; opacity: 0;
      `,U.title="Copiar URL",y.onmouseenter=()=>{y.style.transform="translateY(-2px)",y.style.boxShadow="0 8px 20px rgba(0,0,0,0.08)",y.style.borderColor="rgba(0,0,0,0.05)",y.style.borderLeft=`4px solid ${C.color}`,U.style.opacity="1",U.style.background="#F1F3F4"},y.onmouseleave=()=>{y.style.transform="translateY(0)",y.style.boxShadow="0 1px 3px rgba(0,0,0,0.05)",y.style.border="1px solid transparent",U.style.opacity="0",U.style.background="transparent"},y.onclick=()=>{!T&&f&&So(F,f),window.open(F.url,"_blank")},U.onclick=X=>{X.stopPropagation(),P.playClick(),navigator.clipboard.writeText(F.url),!T&&f&&So(F,f),Y("Link copiado!")},y.appendChild(O),y.appendChild(N),y.appendChild(U),y}u.addEventListener("input",F=>{e=F.target.value,z()});function q(){o=!o,ue(o,s,"cw-btn-links")}return document.body.appendChild(s),J(),z(),q}var _e=[];function qt(t){_e=t}var ia=60*1e3;window._cwIsAdmin=!1;window._cwCurrentUser=null;function Eo(){let t="v4.9",e=!1,o=null,a=null;function n(f){if(!f)return"";try{let y=new Date(f);return isNaN(y.getTime())?String(f):y.toLocaleString("pt-BR",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}).replace(","," \xE0s")}catch{return String(f)}}if(!document.getElementById("cw-broadcast-hd-css")){let f=document.createElement("style");f.id="cw-broadcast-hd-css",f.innerHTML=`
        @keyframes cw-pulse {
      ...
      `,g.querySelectorAll('input[name="cw-bc-type"]').forEach(O=>{O.addEventListener("change",()=>{g.querySelectorAll(".cw-radio-option").forEach(I=>I.classList.remove("checked")),O.parentElement.classList.add("checked")})}),setTimeout(()=>{let O=g.querySelector(".cw-radio-option.info");O&&O.classList.add("checked")},100);let f=g.querySelector("#cw-bc-cancel"),y=g.querySelector("#cw-bc-close-x"),C=g.querySelector("#cw-bc-send");f.onclick=_,y.onclick=_,C.onclick=M,p.appendChild(g)}function R(f=null){if(!g)return;let y=g.querySelector("#cw-editor-title-label"),C=g.querySelector("#cw-bc-title"),O=g.querySelector("#cw-bc-text"),I=g.querySelector("#cw-bc-send");if(f){a=f.id,y.textContent="Editar Aviso",C.value=f.title||"",O.value=f.text||"",I.textContent="Salvar Altera\xE7\xF5es";let N=f.type||"info",$=g.querySelector(`input[name="cw-bc-type"][value="${N}"]`);$&&$.click()}else{a=null,y.textContent="Novo Aviso",C.value="",O.value="",I.textContent="Publicar";let N=g.querySelector('input[name="cw-bc-type"][value="info"]');N&&N.click()}g.classList.add("active"),setTimeout(()=>C.focus(),300)}function _(){g&&g.classList.remove("active"),a=null}async function M(){let f=g.querySelector("#cw-bc-send"),y=g.querySelector("#cw-bc-title"),C=g.querySelector("#cw-bc-text"),O=g.querySelector('input[name="cw-bc-type"]:checked'),I=O?O.value:"info";if(!y.value.trim()||!C.value.trim()){Y("Preencha todos os campos!",{error:!0});return}f.textContent="Salvando...",f.style.opacity="0.7";let N=!1;a?N=await be.updateBroadcast(a,{title:y.value,text:C.value,type:I}):N=await be.sendBroadcast({title:y.value,text:C.value,type:I,author:window._cwCurrentUser||"admin"}),N?(Y(a?"Atualizado!":"Publicado!"),P.playSuccess(),_(),setTimeout(()=>L(),1500)):(Y("Erro ao salvar. Verifique a conex\xE3o.",{error:!0}),f.textContent=a?"Salvar Altera\xE7\xF5es":"Publicar",f.style.opacity="1")}async function J(f){if(await xe("Confirma a exclus\xE3o deste aviso?",{danger:!0}))if(await be.deleteBroadcast(f)){Y("Aviso removido."),P.playClick();let O=_e.findIndex(I=>I.id===f);O>-1&&_e.splice(O,1),z(),setTimeout(()=>L(),1500)}else Y("Erro ao excluir.",{error:!0})}let K=document.createElement("div");K.className="cw-nice-scroll",Object.assign(K.style,i.feedContainer),p.appendChild(K);async function L(){e&&(w.style.display="block",w.innerHTML="\u{1F504} Sincronizando...");try{let f=await be.fetchData();f&&f.broadcast&&(qt(f.broadcast),k(),e&&(z(),w.innerHTML='<span style="color:#137333">\u2713 Atualizado</span>',setTimeout(()=>{w.style.display="none"},1500)))}catch{e&&(w.innerHTML="\u26A0\uFE0F Offline")}}function k(){let f=document.getElementById("cw-btn-broadcast");if(!f)return;let y=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]");if(_e.some(O=>!y.includes(O.id))){if(f.classList.add("has-new"),!f.querySelector(".cw-badge")){let O=document.createElement("div");O.className="cw-badge",Object.assign(O.style,{position:"absolute",top:"8px",right:"8px",width:"8px",height:"8px",backgroundColor:"#d93025",borderRadius:"50%",border:"1px solid #fff",zIndex:"10"}),f.appendChild(O)}}else{f.classList.remove("has-new");let O=f.querySelector(".cw-badge");O&&O.remove()}}function z(){K.innerHTML="";let f=p.querySelector("#cw-bau-widget");f&&f.remove();let y=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]"),C=[..._e].sort((v,U)=>{let X=new Date(v.date).getTime()||0;return(new Date(U.date).getTime()||0)-X}),O=C.findIndex(v=>v.title&&v.title.toLowerCase().includes("disponibilidade bau"));if(O!==-1){let v=C[O];C.splice(O,1);let U=document.createElement("div");U.id="cw-bau-widget",Object.assign(U.style,i.bauContainer);let X=[],W=(v.text||"").split(`
`),m=/\d{1,2}\/\d{1,2}/,E="\u{1F4C5}";if(W.forEach(G=>{/🇧🇷|🇵🇹|PT|BR|BRASIL|BRAZIL|PORTUGAL|LISBOA/i.test(G)?E="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM|ESPANHA|SPAIN|MEXICO|MÉXICO/i.test(G)&&(E="\u{1F1EA}\u{1F1F8}");let ee=G.match(m);if(ee){let oe=ee[0],Q=E;/🇧🇷|🇵🇹|PT|BR/i.test(G)?Q="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM/i.test(G)&&(Q="\u{1F1EA}\u{1F1F8}"),X.some(te=>te.flag===Q&&te.date===oe)||X.push({flag:Q,date:oe})}}),X.length===0){let G=(v.text||"").match(/\d{1,2}\/\d{1,2}/g);G&&[...new Set(G)].forEach(ee=>X.push({flag:"\u{1F4C5}",date:ee}))}let A="",H='<button id="cw-bau-toggle-btn" class="cw-btn-interactive" style="background:rgba(255,255,255,0.7); border:1px solid rgba(139, 92, 246, 0.4); border-radius:12px; padding:8px 12px; color:#6D28D9; font-size:12px; font-weight:600;">Detalhes</button>';window._cwIsAdmin&&(H=`
              ...
              `).join("")}</div>
                  ...
                  <div id="cw-bau-full" style="display:none; margin-top:12px; padding-top:12px; border-top:1px dashed rgba(139, 92, 246, 0.3); font-size:13px; line-height:1.5; color:#581C87;">${l(v.text)}</div>
              `:A=`
                <div style="display:flex; justify-content:space-between; align-items:start;">
                    <div style="font-size:13px; color:#581C87; line-height:1.5; flex:1;">${l(v.text)}</div>
           ...
           `,K.appendChild(v)}let N=I.filter(v=>!y.includes(v.id)),$=I.filter(v=>y.includes(v.id));if(N.forEach(v=>K.appendChild(j(v,!1))),$.length>0){let v=document.createElement("div");Object.assign(v.style,i.historyDivider),v.innerHTML=`<span>Hist\xF3rico (${$.length})</span><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg>`;let U=document.createElement("div");Object.assign(U.style,i.historyContainer),$.forEach(W=>U.appendChild(j(W,!0)));let X=!1;v.onclick=()=>{P.playClick(),X=!X,U.style.display=X?"flex":"none",v.querySelector("svg").style.transform=X?"rotate(180deg)":"rotate(0deg)"},K.appendChild(v),K.appendChild(U)}}function j(f,y){let C=document.createElement("div");Object.assign(C.style,y?i.cardHistory:i.card);let O=s[f.type]||s.info,I=document.createElement("div");Object.assign(I.style,i.cardHeader);let N=document.createElement("div");Object.assign(N.style,i.typeTag,{color:O.color,background:O.bg}),N.innerHTML=`${O.icon} <span>${f.type}</span>`;let $=document.createElement("span");if(Object.assign($.style,i.dateTag),$.textContent=n(f.date),I.appendChild(N),y)I.appendChild($);else{let m=document.createElement("button");m.className="cw-btn-interactive",Object.assign(m.style,i.dismissBtn),m.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="20 6 9 17 4 12"></polyline></svg>',m.onmouseenter=()=>{m.style.color="#1e8e3e",m.style.background="#e6f4ea",m.style.borderColor="#1e8e3e"},m.onmouseleave=()=>{m.style.color="#5f6368",m.style.background="#fff",m.style.borderColor="rgba(0,0,0,0.1)"},m.onclick=E=>{E.stopPropagation(),P.playClick(),C.style.transform="translateX(20px)",C.style.opacity="0",setTimeout(()=>{let A=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]");A.push(f.id),localStorage.setItem("cw_read_broadcasts",JSON.stringify(A)),z(),k()},200)},I.appendChild(m)}let v=document.createElement("div");Object.assign(v.style,i.cardContent);let U=document.createElement("div");Object.assign(U.style,i.msgTitle),U.textContent=f.title;let X=document.createElement("div");Object.assign(X.style,i.msgBody),X.innerHTML=l(f.text);let W=document.createElement("div");if(Object.assign(W.style,i.msgMeta),W.innerHTML=`Publicado por <b>${f.author||"Sistema"}</b>`,y||(W.innerHTML+=` \u2022 ${n(f.date)}`),v.appendChild(U),v.appendChild(X),v.appendChild(W),C.appendChild(I),C.appendChild(v),window._cwIsAdmin){let m=document.createElement("div");m.className="cw-card-actions";let E=document.createElement("button");E.className="cw-action-btn edit",E.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg> Editar',E.onclick=()=>R(f);let A=document.createElement("button");A.className="cw-action-btn delete",A.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg> Excluir',A.onclick=()=>J(f.id),m.appendChild(E),m.appendChild(A),C.appendChild(m)}return C}let q=be.getCachedBroadcasts();q.length>0&&(qt(q),z()),setTimeout(u,500),L(),o||(o=setInterval(L,ia));let F=document.createElement("div");Object.assign(F.style,Ae),F.className="no-drag",p.appendChild(F),Te(p,F),document.body.appendChild(p);let h=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]"),T=_e.some(f=>!h.includes(f.id));return{toggle:c,hasUnread:T}}function Ao(){if(localStorage.getItem("cw_onboarding_seen_v1"))return;let t=[{icon:"\u{1F680}",title:"Bem-vindo ao TechSol Suite",text:"Sua nova central de opera\xE7\xF5es para maximizar produtividade e padroniza\xE7\xE3o no CRM."},{icon:"\u{1F4DD}",title:"Notas Autom\xE1ticas",text:"Gere notas de caso (BAU/LM) perfeitas em segundos. Selecione o Status, as Tasks e deixe o wizard escrever o texto t\xE9cnico para voc\xEA."},{icon:"\u26A1",title:"Quick Email & Scripts",text:"Responda e-mails com templates inteligentes que detectam o contexto e use scripts de chamada interativos que guiam seu atendimento."},{icon:"\u{1F4E2}",title:"Fique Informado",text:"O m\xF3dulo Broadcast traz avisos importantes e disponibilidade BAU direto na sua tela, sem precisar abrir planilhas externas."},{icon:"\u2705",title:"Tudo Pronto!",text:"Explore o Menu Flutuante para come\xE7ar. Bom trabalho!",isLast:!0}],e=0,o={overlay:{position:"fixed",top:0,left:0,width:"100vw",height:"100vh",backgroundColor:"rgba(0,0,0,0.6)",backdropFilter:"blur(4px)",zIndex:"2147483647",display:"flex",alignItems:"center",justifyContent:"center",opacity:"0",transition:"opacity 0.3s ease"},card:{width:"380px",background:"#fff",borderRadius:"24px",padding:"32px",textAlign:"center",position:"relative",boxShadow:"0 20px 50px rgba(0,0,0,0.3)",fontFamily:"'Google Sans', Roboto, sans-serif",transform:"translateY(20px)",transition:"all 0.4s cubic-bezier(0.19, 1, 0.22, 1)"},icon:{fontSize:"48px",marginBottom:"20px",display:"block"},title:{fontSize:"22px",fontWeight:"700",color:"#202124",marginBottom:"12px"},text:{fontSize:"15px",color:"#5f6368",lineHeight:"1.6",marginBottom:"32px"},dotsContainer:{display:"flex",justifyContent:"center",gap:"8px",marginBottom:"24px"},dot:{width:"8px",height:"8px",borderRadius:"50%",background:"#dadce0",transition:"all 0.3s"},dotActive:{background:"#1a73e8",width:"24px",borderRadius:"4px"},btnContainer:{display:"flex",justifyContent:"space-between",alignItems:"center"},btn:{padding:"10px 24px",borderRadius:"20px",border:"none",cursor:"pointer",fontSize:"14px",fontWeight:"600",transition:"background 0.2s"},btnSkip:{background:"transparent",color:"#5f6368"},btnNext:{background:"#1a73e8",color:"#fff",boxShadow:"0 4px 12px rgba(26,115,232,0.3)"}},a=document.createElement("div");Object.assign(a.style,o.overlay);let n=document.createElement("div");Object.assign(n.style,o.card);let i=document.createElement("div");Object.assign(i.style,o.icon);let s=document.createElement("div");Object.assign(s.style,o.title);let r=document.createElement("div");Object.assign(r.style,o.text);let l=document.createElement("div");Object.assign(l.style,o.dotsContainer);let p=document.createElement("div");Object.assign(p.style,o.btnContainer);let d=document.createElement("button");d.textContent="Pular",Object.assign(d.style,o.btn,o.btnSkip),d.onmouseover=()=>d.style.color="#202124",d.onmouseout=()=>d.style.color="#5f6368";let c=document.createElement("button");c.textContent="Pr\xF3ximo",Object.assign(c.style,o.btn,o.btnNext),c.onmouseover=()=>c.style.transform="scale(1.05)",c.onmouseout=()=>c.style.transform="scale(1)",p.appendChild(d),p.appendChild(c),n.appendChild(i),n.appendChild(s),n.appendChild(r),n.appendChild(l),n.appendChild(p),a.appendChild(n),document.body.appendChild(a);function x(g){let u=t[g];i.textContent=u.icon,s.textContent=u.title,r.textContent=u.text,l.innerHTML="",t.forEach((w,S)=>{let R=document.createElement("div");Object.assign(R.style,o.dot),S===g&&Object.assign(R.style,o.dotActive),l.appendChild(R)}),u.isLast?(d.style.display="none",c.textContent="Come\xE7ar \u{1F680}",c.style.width="100%"):(d.style.display="block",c.textContent="Pr\xF3ximo",c.style.width="auto")}function b(){localStorage.setItem("cw_onboarding_seen_v1","true"),a.style.opacity="0",n.style.transform="translateY(20px)",setTimeout(()=>a.remove(),400),P.playSuccess(),Y("Tudo pronto! Use o menu flutuante.")}c.onclick=()=>{P.playClick(),e<t.length-1?(e++,x(e)):b()},d.onclick=async()=>{await xe("Pular o tutorial?")&&b()},x(0),requestAnimationFrame(()=>{a.style.opacity="1",n.style.transform="translateY(0)"})}var To={version:"v5.1",title:"Atualiza\xE7\xE3o: v5.1 - Produtividade Blindada \u{1F6E1}\uFE0F",slides:[{icon:"\u{1F17F}\uFE0F",title:"Estacionamento de Casos",text:"Interrup\xE7\xE3o urgente? Agora voc\xEA pode 'Estacionar' seu atendimento atual (Notas + Tasks) com um clique e retomar depois exatamente de onde parou."},{icon:"\u{1F6DF}",title:"Sistema 'Airbag'",text:"Caiu a internet? Fechou a aba sem querer? O TechSol agora possui Auto-Save de emerg\xEAncia a cada 5 segundos. Seu texto est\xE1 salvo, sempre."},{icon:"\u{1F7E0}",title:"Indicador de Progresso",text:"Nunca mais esque\xE7a uma nota aberta. Um indicador laranja ('Dirty State') avisa na P\xEDlula principal se h\xE1 trabalho n\xE3o salvo/estacionado."},{icon:"\u{1F50D}",title:"Time Zone Pro",text:"O m\xF3dulo de fusos hor\xE1rios ganhou superpoderes: nova barra de pesquisa global, filtros r\xE1pidos por regi\xE3o e corre\xE7\xE3o de visualiza\xE7\xE3o."},{icon:"\u{1F916}",title:"Leitura de BAU Aprimorada",text:"O sistema de Broadcast agora \xE9 mais inteligente ao ler avisos de disponibilidade, detectando datas e bandeiras mesmo quando quebradas em v\xE1rias linhas."},{icon:"\u{1F3A8}",title:"Refinamento Visual",text:"Bot\xF5es padronizados, sombras suavizadas e micro-intera\xE7\xF5es t\xE1teis em todo o sistema para uma experi\xEAncia mais fluida e profissional."}]};function ko(t){let e=localStorage.getItem("cw_last_version");if(!e){localStorage.setItem("cw_last_version",t);return}e!==t&&sa(t)}function sa(t){let e=To.slides,o=0,a={overlay:{position:"fixed",top:0,left:0,width:"100vw",height:"100vh",backgroundColor:"rgba(0,0,0,0.7)",backdropFilter:"blur(5px)",zIndex:"2147483647",display:"flex",alignItems:"center",justifyContent:"center",opacity:"0",transition:"opacity 0.3s ease"},card:{width:"400px",background:"#fff",borderRadius:"24px",padding:"32px",textAlign:"center",position:"relative",boxShadow:"0 24px 60px rgba(0,0,0,0.4)",fontFamily:"'Google Sans', Roboto, sans-serif",transform:"translateY(30px)",transition:"all 0.4s cubic-bezier(0.19, 1, 0.22, 1)"},badge:{display:"inline-block",padding:"4px 12px",borderRadius:"12px",background:"#E8F0FE",color:"#1967D2",fontSize:"11px",fontWeight:"700",textTransform:"uppercase",marginBottom:"16px",letterSpacing:"0.5px"},icon:{fontSize:"42px",marginBottom:"16px",display:"block"},title:{fontSize:"20px",fontWeight:"700",color:"#202124",marginBottom:"8px"},text:{fontSize:"14px",color:"#5f6368",lineHeight:"1.5",marginBottom:"32px",minHeight:"42px"},dotsContainer:{display:"flex",justifyContent:"center",gap:"6px",marginBottom:"24px"},dot:{width:"6px",height:"6px",borderRadius:"50%",background:"#dadce0",transition:"all 0.3s"},dotActive:{background:"#1a73e8",width:"18px",borderRadius:"4px"},btn:{width:"100%",padding:"12px",borderRadius:"12px",border:"none",cursor:"pointer",fontSize:"14px",fontWeight:"600",transition:"all 0.2s",background:"#1a73e8",color:"#fff",boxShadow:"0 4px 12px rgba(26,115,232,0.3)"}},n=document.createElement("div");Object.assign(n.style,a.overlay);let i=document.createElement("div");Object.assign(i.style,a.card);let s=document.createElement("div");Object.assign(s.style,a.badge),s.textContent=`Atualiza\xE7\xE3o ${t}`;let r=document.createElement("div");Object.assign(r.style,a.icon);let l=document.createElement("div");Object.assign(l.style,a.title);let p=document.createElement("div");Object.assign(p.style,a.text);let d=document.createElement("div");Object.assign(d.style,a.dotsContainer);let c=document.createElement("button");Object.assign(c.style,a.btn),c.onmouseover=()=>c.style.transform="scale(1.02)",c.onmouseout=()=>c.style.transform="scale(1)",i.appendChild(s),i.appendChild(r),i.appendChild(l),i.appendChild(p),i.appendChild(d),i.appendChild(c),n.appendChild(i),document.body.appendChild(n);function x(g){let u=e[g];r.textContent=u.icon,l.textContent=u.title,p.textContent=u.text,d.innerHTML="",e.forEach((w,S)=>{let R=document.createElement("div");Object.assign(R.style,a.dot),S===g&&Object.assign(R.style,a.dotActive),d.appendChild(R)}),g===e.length-1?c.textContent="Entendi, vamos l\xE1! \u{1F44D}":c.textContent="Pr\xF3ximo"}function b(){localStorage.setItem("cw_last_version",t),n.style.opacity="0",i.style.transform="translateY(30px)",setTimeout(()=>n.remove(),400),P.playSuccess(),Y(`TechSol atualizado para ${t}!`)}c.onclick=()=>{P.playClick(),o<e.length-1?(o++,x(o)):b()},x(0),requestAnimationFrame(()=>{n.style.opacity="1",i.style.transform="translateY(0)"})}var Fo="cw_timezone_pinned",_t=[{id:"pt",name:"Portugal",flag:"\u{1F1F5}\u{1F1F9}",zone:"Europe/Lisbon",label:"Lisboa",region:"eu"},{id:"es",name:"Espanha",flag:"\u{1F1EA}\u{1F1F8}",zone:"Europe/Madrid",label:"Madrid",region:"eu"},{id:"ar",name:"Argentina",flag:"\u{1F1E6}\u{1F1F7}",zone:"America/Argentina/Buenos_Aires",label:"Buenos Aires",region:"sa"},{id:"bo",name:"Bol\xEDvia",flag:"\u{1F1E7}\u{1F1F4}",zone:"America/La_Paz",label:"La Paz",region:"sa"},{id:"cl",name:"Chile",flag:"\u{1F1E8}\u{1F1F1}",zone:"America/Santiago",label:"Santiago",region:"sa"},{id:"co",name:"Col\xF4mbia",flag:"\u{1F1E8}\u{1F1F4}",zone:"America/Bogota",label:"Bogot\xE1",region:"sa"},{id:"ec",name:"Equador",flag:"\u{1F1EA}\u{1F1E8}",zone:"America/Guayaquil",label:"Guayaquil",region:"sa"},{id:"py",name:"Paraguai",flag:"\u{1F1F5}\u{1F1FE}",zone:"America/Asuncion",label:"Assun\xE7\xE3o",region:"sa"},{id:"pe",name:"Peru",flag:"\u{1F1F5}\u{1F1EA}",zone:"America/Lima",label:"Lima",region:"sa"},{id:"uy",name:"Uruguai",flag:"\u{1F1FA}\u{1F1FE}",zone:"America/Montevideo",label:"Montevid\xE9u",region:"sa"},{id:"ve",name:"Venezuela",flag:"\u{1F1FB}\u{1F1EA}",zone:"America/Caracas",label:"Caracas",region:"sa"},{id:"mx",name:"M\xE9xico",flag:"\u{1F1F2}\u{1F1FD}",zone:"America/Mexico_City",label:"CDMX",region:"na"},{id:"cr",name:"Costa Rica",flag:"\u{1F1E8}\u{1F1F7}",zone:"America/Costa_Rica",label:"San Jos\xE9",region:"na"},{id:"sv",name:"El Salvador",flag:"\u{1F1F8}\u{1F1FB}",zone:"America/El_Salvador",label:"San Salvador",region:"na"},{id:"gt",name:"Guatemala",flag:"\u{1F1EC}\u{1F1F9}",zone:"America/Guatemala",label:"C. da Guatemala",region:"na"},{id:"hn",name:"Honduras",flag:"\u{1F1ED}\u{1F1F3}",zone:"America/Tegucigalpa",label:"Tegucigalpa",region:"na"},{id:"ni",name:"Nicar\xE1gua",flag:"\u{1F1F3}\u{1F1EE}",zone:"America/Managua",label:"Man\xE1gua",region:"na"},{id:"pa",name:"Panam\xE1",flag:"\u{1F1F5}\u{1F1E6}",zone:"America/Panama",label:"C. do Panam\xE1",region:"na"},{id:"do",name:"Rep. Dominicana",flag:"\u{1F1E9}\u{1F1F4}",zone:"America/Santo_Domingo",label:"Santo Domingo",region:"na"},{id:"pr",name:"Porto Rico",flag:"\u{1F1F5}\u{1F1F7}",zone:"America/Puerto_Rico",label:"San Juan",region:"na"}],ra=[{id:"all",label:"Todos"},{id:"sa",label:"Am\xE9rica do Sul"},{id:"na",label:"Norte & Central"},{id:"eu",label:"Europa"}];function Oo(){let t="v2.2 Pro",e=!1,o=null,a="mx",n=JSON.parse(localStorage.getItem(Fo)||"[]"),i="",s="all",r=new Date;r.setHours(14,0,0,0);let l={bg:"#F8F9FA",surface:"#FFFFFF",primary:"#1A73E8",primaryBg:"#E8F0FE",text:"#202124",textSub:"#5F6368",border:"#DADCE0",success:"#1E8E3E",successBg:"#E6F4EA",warning:"#E37400",warningBg:"#FEF7E0",error:"#D93025",errorBg:"#FCE8E6"},p={container:{display:"flex",flexDirection:"column",height:"100%",background:l.bg,fontFamily:"'Google Sans', Roboto, sans-serif"},tabHeader:{display:"flex",background:l.surface,borderBottom:`1px solid ${l.border}`,padding:"8px 16px 0 16px"},tabBtn:{flex:1,padding:"12px",textAlign:"center",cursor:"pointer",fontSize:"13px",fontWeight:"500",color:l.textSub,borderBottom:"3px solid transparent",transition:"all 0.2s ease",userSelect:"none"},tabActive:{color:l.primary,borderBottomColor:l.primary,fontWeight:"600"},toolbar:{padding:"12px 16px 8px 16px",background:l.bg,display:"flex",flexDirection:"column",gap:"12px",borderBottom:"1px solid rgba(0,0,0,0.03)"},searchInputWrapper:{position:"relative",width:"100%"},searchInput:{width:"100%",boxSizing:"border-box",padding:"10px 12px 10px 38px",borderRadius:"10px",border:"1px solid transparent",background:"#FFFFFF",fontSize:"14px",color:l.text,outline:"none",boxShadow:"0 1px 3px rgba(0,0,0,0.05)",transition:"all 0.2s",fontFamily:"'Google Sans', Roboto, sans-serif"},searchIcon:{position:"absolute",left:"12px",top:"50%",transform:"translateY(-50%)",width:"16px",height:"16px",color:"#9AA0A6",pointerEvents:"none"},chipsRow:{display:"flex",gap:"8px",overflowX:"auto",paddingBottom:"4px",scrollbarWidth:"none",msOverflowStyle:"none"},chip:{whiteSpace:"nowrap",padding:"6px 12px",borderRadius:"16px",fontSize:"12px",fontWeight:"500",cursor:"pointer",border:`1px solid ${l.border}`,background:l.surface,color:l.textSub,transition:"all 0.2s"},chipActive:{background:l.primaryBg,color:l.primary,borderColor:l.primaryBg,fontWeight:"600"},listContainer:{padding:"16px 16px 40px 16px",overflowY:"auto",flex:1,display:"flex",flexDirection:"column",gap:"12px",scrollbarWidth:"none"},hubCard:{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px 20px",background:l.surface,borderRadius:"16px",border:"1px solid transparent",boxShadow:"0 2px 6px rgba(60,64,67,0.05)",transition:"transform 0.2s cubic-bezier(0.25, 0.8, 0.25, 1), box-shadow 0.2s ease",cursor:"pointer",position:"relative"},hubCardPinned:{borderLeft:`4px solid ${l.primary}`,paddingLeft:"16px"},plannerWrapper:{padding:"24px",display:"flex",flexDirection:"column",gap:"24px",flex:1,overflowY:"auto"},timeComparisonRow:{display:"flex",gap:"16px",alignItems:"stretch"},timeCard:{flex:1,padding:"20px",borderRadius:"20px",background:l.surface,border:`1px solid ${l.border}`,display:"flex",flexDirection:"column",alignItems:"center",gap:"8px",boxShadow:"0 4px 12px rgba(60,64,67,0.05)"},timelineContainer:{position:"relative",height:"60px",marginTop:"16px",userSelect:"none"},timelineTrack:{position:"absolute",top:"26px",left:"0",right:"0",height:"6px",borderRadius:"3px",background:"#E0E0E0",overflow:"hidden"},dayZone:{position:"absolute",top:"0",bottom:"0",left:"37.5%",width:"37.5%",background:"rgba(52, 168, 83, 0.3)",pointerEvents:"none"},hdInput:{fontSize:"28px",fontWeight:"700",color:l.text,border:"none",background:"transparent",width:"100%",textAlign:"center",outline:"none",fontFamily:"'Google Sans', sans-serif",cursor:"text"},statusBadge:{padding:"8px 16px",borderRadius:"50px",fontSize:"13px",fontWeight:"600",display:"inline-flex",alignItems:"center",gap:"8px",marginTop:"16px",alignSelf:"center",transition:"background-color 0.3s"}},d=document.createElement("div");d.id="timezone-popup",d.classList.add("cw-module-window"),Object.assign(d.style,me,{right:"100px",width:"450px",height:"720px",overflow:"hidden",borderRadius:"24px"});let x=pe(d,"Time Zone Traveler",t,"Monitoramento global e planejamento de chamadas.",{popup:d},()=>y());d.appendChild(x);let b=document.createElement("div");Object.assign(b.style,p.container),d.appendChild(b);let g=document.createElement("div");Object.assign(g.style,p.tabHeader);let u=document.createElement("div");u.textContent="Monitoramento",Object.assign(u.style,p.tabBtn,p.tabActive);let w=document.createElement("div");w.textContent="Planejador",Object.assign(w.style,p.tabBtn),g.appendChild(u),g.appendChild(w),b.appendChild(g);let S=document.createElement("div");Object.assign(S.style,p.toolbar);let R=document.createElement("div");Object.assign(R.style,p.searchInputWrapper);let _=document.createElement("div");_.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>',Object.assign(_.style,p.searchIcon);let M=document.createElement("input");M.placeholder="Buscar cidade ou pa\xEDs...",Object.assign(M.style,p.searchInput),M.onfocus=()=>{M.style.boxShadow="0 2px 8px rgba(26,115,232,0.15)",M.style.borderColor="rgba(26,115,232,0.3)"},M.onblur=()=>{M.style.boxShadow="0 1px 3px rgba(0,0,0,0.05)",M.style.borderColor="transparent"},M.oninput=C=>{i=C.target.value.toLowerCase(),F()},R.appendChild(_),R.appendChild(M),S.appendChild(R);let J=document.createElement("div");Object.assign(J.style,p.chipsRow),ra.forEach(C=>{let O=document.createElement("div");O.textContent=C.label,O.id=`tz-filter-${C.id}`,Object.assign(O.style,p.chip),C.id===s&&Object.assign(O.style,p.chipActive),O.onclick=()=>{P.playClick(),s=C.id,Array.from(J.children).forEach(I=>{Object.assign(I.style,p.chip)}),Object.assign(O.style,p.chipActive),F()},J.appendChild(O)}),S.appendChild(J),b.appendChild(S);let K=document.createElement("div");Object.assign(K.style,p.listContainer);let L=document.createElement("style");L.textContent="#timezone-popup ::-webkit-scrollbar { display: none; }",b.appendChild(L);let k=document.createElement("div");Object.assign(k.style,p.plannerWrapper,{display:"none"}),b.appendChild(K),b.appendChild(k),u.onclick=()=>z("live"),w.onclick=()=>z("plan");function z(C){P.playClick(),C==="live"?(Object.assign(u.style,p.tabActive),Object.assign(w.style,p.tabBtn),w.style.borderBottomColor="transparent",K.style.display="flex",S.style.display="flex",k.style.display="none",T()):(Object.assign(w.style,p.tabActive),Object.assign(u.style,p.tabBtn),u.style.borderBottomColor="transparent",k.style.display="flex",K.style.display="none",S.style.display="none",f(),h())}function j(C){return C>=9&&C<17?{color:l.success,bg:l.successBg,label:"Aberto",icon:"\u{1F7E2}"}:C>=8&&C<9?{color:l.warning,bg:l.warningBg,label:"Abrindo",icon:"\u{1F7E1}"}:C>=17&&C<19?{color:l.warning,bg:l.warningBg,label:"Fechando",icon:"\u{1F7E1}"}:{color:l.textSub,bg:"#F1F3F4",label:"Fechado",icon:"\u{1F534}"}}function q(C){n.includes(C)?n=n.filter(O=>O!==C):n.push(C),localStorage.setItem(Fo,JSON.stringify(n)),F(),P.playClick()}function F(){K.innerHTML="";let C=new Date,O=_t.filter(N=>{let $=N.name.toLowerCase().includes(i)||N.label.toLowerCase().includes(i),v=s==="all"||N.region===s;return $&&v});if(O.sort((N,$)=>{let v=n.includes(N.id),U=n.includes($.id);return v&&!U?-1:!v&&U?1:N.name.localeCompare($.name)}),O.length===0){K.innerHTML=`
            ...
            `;return}O.forEach(N=>{let $=n.includes(N.id),v=C.toLocaleTimeString("pt-BR",{timeZone:N.zone,hour:"2-digit",minute:"2-digit"}),U=parseInt(v.split(":")[0]),X=j(U),W=U<6||U>18,m=document.createElement("div");Object.assign(m.style,p.hubCard),$&&Object.assign(m.style,p.hubCardPinned);let E=$?"\u2605":"\u2606",A=$?"#F9AB00":"#DADCE0";m.innerHTML=`
                ...
                <div style="text-align:right;">
                    <div style="font-size:24px; font-weight:700; color:${l.text}; font-family:'Google Sans', sans-serif;">${v}</div>
            ...
            `,m.onmouseenter=()=>{m.style.transform="translateY(-2px)",m.style.boxShadow="0 6px 12px rgba(60,64,67,0.1)"},m.onmouseleave=()=>{m.style.transform="translateY(0)",m.style.boxShadow="0 2px 6px rgba(60,64,67,0.05)"};let H=m.querySelector(".cw-pin-btn");H.onmouseenter=()=>{H.style.backgroundColor="#F1F3F4"},H.onmouseleave=()=>{H.style.backgroundColor="transparent"},H.onclick=B=>{B.stopPropagation(),q(N.id)},m.onclick=()=>{a=N.id,z("plan")},K.appendChild(m)});let I=document.createElement("div");I.style.height="20px",I.style.width="100%",K.appendChild(I)}function h(){k.innerHTML="";let C=document.createElement("div"),O=document.createElement("label");O.textContent="Onde est\xE1 o cliente?",O.style.cssText="display:block; font-size:12px; font-weight:700; color:#5F6368; margin-bottom:8px; text-transform:uppercase; letter-spacing:0.5px;";let I=document.createElement("select");Object.assign(I.style,Ht),I.style.padding="14px",[..._t].sort((ne,te)=>ne.name.localeCompare(te.name)).forEach(ne=>{let te=document.createElement("option");te.value=ne.id,te.textContent=`${ne.flag} ${ne.name} (${ne.zone})`,ne.id===a&&(te.selected=!0),I.appendChild(te)}),I.onchange=ne=>{a=ne.target.value,Q(),P.playClick()},C.appendChild(O),C.appendChild(I),k.appendChild(C);let $=document.createElement("div");Object.assign($.style,p.timeComparisonRow);let v=document.createElement("div");Object.assign(v.style,p.timeCard),v.style.backgroundColor="#F8FAFF",v.style.borderColor="#E8F0FE",v.innerHTML=`
        ...
        `,$.appendChild(v),$.appendChild(U),k.appendChild($);let X=document.createElement("div");X.id="cw-planner-status",Object.assign(X.style,p.statusBadge),k.appendChild(X);let W=document.createElement("div");Object.assign(W.style,{padding:"0 4px",marginTop:"12px"});let m=document.createElement("div");m.textContent="Arraste para simular o hor\xE1rio:",m.style.cssText="font-size:12px; color:#5F6368; text-align:center; margin-bottom:12px;";let E=document.createElement("div");Object.assign(E.style,p.timelineContainer);let A=document.createElement("div");Object.assign(A.style,p.timelineTrack);let H=document.createElement("div");Object.assign(H.style,p.dayZone),A.appendChild(H);let B=document.createElement("input");B.type="range",B.min="0",B.max="1439",B.step="15",B.style.cssText="position:absolute; top:20px; left:0; width:100%; -webkit-appearance:none; background:transparent; z-index:2; cursor:pointer;";let Z=document.createElement("div");Z.style.cssText="position:absolute; top:36px; width:100%; display:flex; justify-content:space-between; font-size:10px; font-weight:600; color:#9AA0A6; padding:0 2px;",Z.innerHTML="<span>00h</span><span>06h</span><span>12h</span><span>18h</span><span>24h</span>",E.appendChild(A),E.appendChild(B),E.appendChild(Z),W.appendChild(m),W.appendChild(E),k.appendChild(W);let G=v.querySelector("#cw-time-input-br"),ee=U.querySelector("#cw-time-display-client"),oe=U.querySelector("#cw-client-label");function Q(){let ne=_t.find(Me=>Me.id===a);oe.textContent=`${ne.flag} ${ne.label} (${ne.zone})`;let te=r.getHours(),le=r.getMinutes(),We=`${String(te).padStart(2,"0")}:${String(le).padStart(2,"0")}`;G.value=We,B.value=te*60+le;let tt=r.toLocaleTimeString("pt-BR",{timeZone:ne.zone,hour:"2-digit",minute:"2-digit"});ee.textContent=tt;let qe=parseInt(tt.split(":")[0]);qe>=9&&qe<17?(X.style.background=l.successBg,X.style.color=l.success,X.innerHTML='<span style="font-size:16px">\u2705</span> Hor\xE1rio Comercial Ideal'):qe>=8&&qe<9||qe>=17&&qe<19?(X.style.background=l.warningBg,X.style.color=l.warning,X.innerHTML='<span style="font-size:16px">\u26A0\uFE0F</span> Hor\xE1rio Limite (Aten\xE7\xE3o)'):(X.style.background=l.errorBg,X.style.color=l.error,X.innerHTML='<span style="font-size:16px">\u26D4</span> Fora de Hor\xE1rio')}B.oninput=ne=>{let te=parseInt(ne.target.value);r.setHours(Math.floor(te/60)),r.setMinutes(te%60),Q()},G.oninput=ne=>{let[te,le]=ne.target.value.split(":");te&&le&&(r.setHours(parseInt(te)),r.setMinutes(parseInt(le)),Q())},Q()}function T(){F(),o||(o=setInterval(F,6e4))}function f(){o&&(clearInterval(o),o=null)}function y(){e=!e,ue(e,d,"cw-btn-timezone"),e?z("live"):f()}return document.body.appendChild(d),y}function Io(){let t="v1.1",e=!1,o="general",a=null,n=null;if(!document.getElementById("cw-lib-styles")){let h=document.createElement("style");h.id="cw-lib-styles",h.innerHTML=`
        ...
        `,document.head.appendChild(h)}let i={bg:"#F0F2F5",surface:"#FFFFFF",primary:"#007AFF",primaryBg:"rgba(0, 122, 255, 0.1)",text:"#1C1C1E",textSub:"#8E8E93",border:"rgba(0, 0, 0, 0.08)",danger:"#FF3B30"},s={container:{display:"flex",flexDirection:"column",height:"100%",background:i.bg,fontFamily:"'Google Sans', Roboto, sans-serif"},tabHeader:{display:"flex",padding:"12px 16px 0 16px",background:i.surface,borderBottom:`1px solid ${i.border}`},tabBtn:{flex:1,padding:"12px",textAlign:"center",cursor:"pointer",fontSize:"13px",fontWeight:"500",color:i.textSub,borderBottom:"3px solid transparent",transition:"all 0.3s cubic-bezier(0.4, 0, 0.2, 1)",userSelect:"none"},tabActive:{color:i.primary,borderBottomColor:i.primary,fontWeight:"600"},listContainer:{flex:1,overflowY:"auto",padding:"16px",display:"flex",flexDirection:"column",gap:"12px"},emptyState:{padding:"40px 20px",textAlign:"center",color:"#BDC1C6",display:"flex",flexDirection:"column",alignItems:"center",gap:"10px"},card:{background:i.surface,borderRadius:"16px",padding:"16px",border:`1px solid ${i.border}`,boxShadow:"0 4px 12px rgba(0,0,0,0.05)",transition:"all 0.4s cubic-bezier(0.165, 0.84, 0.44, 1)",cursor:"default",position:"relative",overflow:"hidden"},cardHeader:{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"8px"},cardTitle:{fontSize:"14px",fontWeight:"600",color:i.text},cardPreview:{fontSize:"12px",color:i.textSub,lineHeight:"1.5",display:"-webkit-box",webkitLineClamp:"3",webkitBoxOrient:"vertical",overflow:"hidden"},cardActions:{display:"flex",justifyContent:"flex-end",gap:"8px",marginTop:"12px",paddingTop:"12px",borderTop:`1px dashed ${i.border}`},actionBtn:{padding:"6px 12px",borderRadius:"6px",fontSize:"12px",fontWeight:"600",cursor:"pointer",border:"none",background:"transparent",transition:"background 0.2s"},fab:{position:"absolute",bottom:"24px",right:"24px",width:"56px",height:"56px",borderRadius:"16px",background:i.primary,color:"#FFF",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 12px rgba(26, 115, 232, 0.4)",cursor:"pointer",transition:"transform 0.2s",zIndex:10},editorOverlay:{position:"absolute",top:0,left:0,width:"100%",height:"100%",background:"rgba(255, 255, 255, 0.85)",backdropFilter:"blur(25px) saturate(180%)",webkitBackdropFilter:"blur(25px) saturate(180%)",zIndex:20,transform:"translateY(100%)",transition:"transform 0.5s cubic-bezier(0.34, 1.56, 0.64, 1)",display:"flex",flexDirection:"column"},editorHeader:{padding:"16px 24px",background:i.surface,borderBottom:`1px solid ${i.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"},editorBody:{flex:1,padding:"24px",overflowY:"auto"},inputGroup:{marginBottom:"20px"},label:{display:"block",fontSize:"12px",fontWeight:"700",color:i.textSub,marginBottom:"8px",textTransform:"uppercase"},input:{width:"100%",padding:"12px",borderRadius:"8px",border:`1px solid ${i.border}`,fontSize:"14px",fontFamily:"inherit",outline:"none",background:i.surface,boxSizing:"border-box"}},r=document.createElement("div");r.id="library-popup",r.classList.add("cw-module-window"),Object.assign(r.style,me,{right:"auto",left:"50%",width:"400px",height:"600px",transform:"translateX(-50%) scale(0.05)",overflow:"hidden"});let p=pe(r,"Minha Biblioteca",t,"Gerencie seus snippets, textos e templates.",{popup:r},()=>F());r.appendChild(p);let d=document.createElement("div");Object.assign(d.style,s.container),r.appendChild(d);let c=document.createElement("div");Object.assign(c.style,s.tabHeader);let x=[{id:"general",label:"Geral",icon:"\u{1F4CB}"},{id:"note",label:"Notas",icon:"\u{1F4DD}"},{id:"email",label:"Emails",icon:"\u{1F4E7}"}];x.forEach(h=>{let T=document.createElement("div");T.innerHTML=`${h.icon} ${h.label}`,T.id=`lib-tab-${h.id}`,Object.assign(T.style,s.tabBtn),h.id===o&&Object.assign(T.style,s.tabActive),T.onmouseenter=()=>P.playHover(),T.onclick=()=>J(h.id),c.appendChild(T)}),d.appendChild(c);let b=document.createElement("div");Object.assign(b.style,s.listContainer),d.appendChild(b);let g=document.createElement("div");g.className="cw-fab cw-tactile",Object.assign(g.style,s.fab),g.innerHTML='<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>',g.onmouseenter=()=>g.style.transform="scale(1.1)",g.onmouseleave=()=>g.style.transform="scale(1)",g.onclick=()=>L(),d.appendChild(g),a=document.createElement("div"),Object.assign(a.style,s.editorOverlay);let u=document.createElement("div");Object.assign(u.style,s.editorHeader),u.innerHTML='<span style="font-weight:700; font-size:16px;">Novo Item</span>';let w=document.createElement("button");w.innerHTML="Cancelar",w.style.cssText="background:none; border:none; color:#5f6368; font-weight:600; cursor:pointer;",w.onclick=k,u.appendChild(w),a.appendChild(u);let S=document.createElement("div");Object.assign(S.style,s.editorBody),a.appendChild(S);let R=document.createElement("div");R.style.cssText="padding:16px 24px; border-top:1px solid #DADCE0; background:#FFF; display:flex; justify-content:flex-end;";let _=document.createElement("button");_.textContent="Salvar",_.style.cssText="padding:10px 24px; background:#1a73e8; color:white; border:none; border-radius:20px; font-weight:600; cursor:pointer; box-shadow:0 2px 5px rgba(26,115,232,0.3);",_.onclick=z,R.appendChild(_),a.appendChild(R),d.appendChild(a);let M=document.createElement("div");Object.assign(M.style,Ae),M.className="no-drag",r.appendChild(M),Te(r,M),document.body.appendChild(r);function J(h){P.playClick(),o=h,x.forEach(T=>{let f=document.getElementById(`lib-tab-${T.id}`);T.id===h?Object.assign(f.style,s.tabActive):Object.assign(f.style,s.tabBtn)}),K()}function K(){b.innerHTML="";let h=he.getSnippets(o);if(h.length===0){b.innerHTML=`
            ...
            `;return}h.forEach(T=>{let f=document.createElement("div");f.className="cw-lib-card",Object.assign(f.style,s.card),T.isCode&&(f.style.borderLeft=`4px solid ${i.primary}`,f.style.background="rgba(0, 122, 255, 0.02)");let y=T.content;if(T.isRich){let C=document.createElement("div");C.innerHTML=T.content;let O=C.querySelector("img");y=C.innerText.substring(0,150)+(C.innerText.length>150?"...":""),O&&(y="\u{1F5BC}\uFE0F [Cont\xE9m Imagens] "+y)}f.innerHTML=`
                <div style="${q(s.cardHeader)}">
                    <div style="${q(s.cardTitle)}">${T.title}</div>
                ...
                <div style="${q(s.cardPreview)}; ${T.isCode?"font-family:'Roboto Mono', monospace; font-size:11px;":""}">${y}</div>
            ...
            `,f.onmouseenter=()=>{P.playHover()},f.querySelector(".cw-act-copy").onclick=C=>{if(C.stopPropagation(),P.playClick(),T.isRich){let O=new Blob([T.content],{type:"text/html"}),I=document.createElement("div");I.style.whiteSpace="pre-wrap",I.innerHTML=T.content;let N=new Blob([I.innerText],{type:"text/plain"}),$=[new ClipboardItem({"text/html":O,"text/plain":N})];navigator.clipboard.write($)}else navigator.clipboard.writeText(T.content);Y("Copiado!")},f.querySelector(".cw-act-edit").onclick=C=>{C.stopPropagation(),P.playClick(),L(T)},f.querySelector(".cw-act-del").onclick=async C=>{C.stopPropagation(),P.playClick(),await xe("Excluir este item?")&&(he.delete(T.id),K(),Y("Item exclu\xEDdo."))},b.appendChild(f)})}function L(h=null){n=h?h.id:null,S.innerHTML="",S.appendChild(j("title","T\xEDtulo / Nome",h?h.title:"")),o==="email"&&S.appendChild(j("subject","Assunto do Email",h?h.subject:""));let T="Conte\xFAdo";o==="email"&&(T="Corpo do Email (HTML)"),o==="note"&&(T="Texto da Nota (Reason)"),S.appendChild(j("content",T,h?h.content:"",{isRich:!0,isCode:h?h.isCode:!1})),u.querySelector("span").textContent=h?"Editar Item":"Novo Item",a.style.transform="translateY(0)",setTimeout(()=>{let f=S.querySelector("input");f&&f.focus()},300)}function k(){a.style.transform="translateY(100%)",setTimeout(()=>n=null,300)}async function z(){let h=S.querySelector("#cw-inp-title"),T=S.querySelector("#cw-inp-content"),f=h.value.trim(),y=T.contentEditable==="true"?T.innerHTML:T.value.trim(),C=T.getAttribute("data-is-code")==="true";if(!f||!y||y==="<br>"){Y("Preencha t\xEDtulo e conte\xFAdo.",{error:!0});return}let O={id:n,type:o,title:f,content:y,isCode:C,isRich:T.contentEditable==="true"};if(o==="email"){let I=S.querySelector("#cw-inp-subject").value.trim();if(!I){Y("Assunto \xE9 obrigat\xF3rio para emails.",{error:!0});return}O.subject=I}_.textContent="Salvando...",await he.save(O),_.textContent="Salvar",k(),K(),Y("Salvo com sucesso!"),P.playSuccess()}function j(h,T,f,y={}){let C=document.createElement("div");Object.assign(C.style,s.inputGroup);let O=document.createElement("label");O.textContent=T,Object.assign(O.style,s.label);let I;if(y.isRich){let N=document.createElement("div");N.style.cssText="display:flex; gap:6px; margin-bottom:12px; background:rgba(241, 243, 244, 0.8); padding:6px; border-radius:12px; border:1px solid #DADCE0; backdrop-filter: blur(10px);",N.innerHTML=`
            ...
            `,I=document.createElement("div"),I.contentEditable="true",Object.assign(I.style,s.input,{minHeight:"180px",maxHeight:"350px",overflowY:"auto",whiteSpace:"pre-wrap",lineHeight:"1.6",outline:"none"}),I.innerHTML=f||"",y.isCode&&(I.style.fontFamily="'Roboto Mono', monospace",I.style.backgroundColor="#F8F9FA",I.setAttribute("data-is-code","true")),N.querySelectorAll(".cw-toolbar-btn").forEach($=>{$.onmouseenter=()=>P.playHover(),$.onmousedown=()=>P.playClick()}),N.querySelector(".cw-tb-bold").onclick=()=>{document.execCommand("bold"),I.focus()},N.querySelector(".cw-tb-italic").onclick=()=>{document.execCommand("italic"),I.focus()},N.querySelector(".cw-tb-code").onclick=$=>{let U=!(I.getAttribute("data-is-code")==="true");I.setAttribute("data-is-code",U),I.style.fontFamily=U?"'Roboto Mono', monospace":"inherit",I.style.backgroundColor=U?"rgba(0, 122, 255, 0.03)":i.surface,U?$.currentTarget.classList.add("active"):$.currentTarget.classList.remove("active"),I.focus()},N.querySelector(".cw-tb-img").onclick=async()=>{let $=await Yt("Cole a URL da imagem:");$&&(document.execCommand("insertImage",!1,$),I.querySelectorAll("img").forEach(U=>{U.style.maxWidth="100%",U.style.borderRadius="8px"}))},I.onpaste=$=>{let v=($.clipboardData||$.originalEvent.clipboardData).items;for(let U of v)if(U.kind==="file"&&U.type.startsWith("image/")){$.preventDefault();let X=U.getAsFile(),W=new FileReader;W.onload=m=>{let E=`<img src="${m.target.result}" style="max-width:100%; border-radius:8px; margin:8px 0; display:block;">`;document.execCommand("insertHTML",!1,E)},W.readAsDataURL(X)}},C.appendChild(O),C.appendChild(N)}else I=document.createElement("input"),I.type="text",Object.assign(I.style,s.input),I.value=f||"",C.appendChild(O);return I.id=`cw-inp-${h}`,I.onfocus=()=>{I.style.borderColor=i.primary,I.style.boxShadow=`0 0 0 2px ${i.primaryBg}`},I.onblur=()=>{I.style.borderColor=i.border,I.style.boxShadow="none"},C.appendChild(I),C}function q(h){return Object.entries(h).map(([T,f])=>`${T.replace(/[A-Z]/g,y=>"-"+y.toLowerCase())}:${f}`).join(";")}function F(){e=!e,ue(e,r,"cw-btn-library"),e&&K()}return F}function Lo(){let t="v1.0",e=!1,o={bg:"#F8F9FA",surface:"#FFFFFF",primary:"#1A73E8",text:"#202124",textSub:"#5F6368",border:"#DADCE0"},a="cw-configs-styles";if(!document.getElementById(a)){let x=document.createElement("style");x.id=a,x.innerHTML=`
    ...
    `;let p=l.querySelector("#cw-config-sound-toggle");p.onchange=x=>{P.setMuted(!x.target.checked),x.target.checked&&P.playClick()},r.appendChild(l);let d=document.createElement("div");d.className="cw-configs-section",d.innerHTML=`
    ...
    `,r.appendChild(d);function c(){e=!e,ue(e,n,"cw-btn-configs"),e&&P.playClick()}return document.body.appendChild(n),c}var ie={blue:"#1A73E8",red:"#D93025",yellow:"#F9AB00",green:"#1E8E3E",blueLight:"#E8F0FE",redLight:"#FCE8E6",yellowLight:"#FEF7E0",greenLight:"#E6F4EA",textPrimary:"#202124",textSecondary:"#5F6368",border:"#DADCE0",surface:"rgba(255, 255, 255, 0.8)",white:"#FFFFFF"},Ue={small:"8px",medium:"12px",large:"16px",pill:"100px"},qo={deep:"0 12px 40px rgba(0,0,0,0.12)",subtle:"0 4px 12px rgba(0,0,0,0.05)"},la="cubic-bezier(0.4, 0, 0.2, 1)",yt=`all 0.2s ${la}`,_o=()=>{if(document.getElementById("bau-form-global-styles"))return;let t=document.createElement("style");t.id="bau-form-global-styles",t.innerHTML=`
    .bau-popup {
  ...
  `,document.head.appendChild(t)};function Mo(){_o();let t=!1,e={},o=document.createElement("div");o.id="bau-form-popup",o.className="bau-popup cw-module-window",o.style.display="none";let n=pe(o,"BAU Form","v1.0.0","Solicite a abertura de casos BAU rapidamente.",{googleLine:null},()=>F());o.appendChild(n);let i=document.createElement("div");i.className="bau-content",o.appendChild(i);let s=document.createElement("form");i.appendChild(s);let r=document.createElement("div");r.className="bau-card";let l=document.createElement("div");l.className="bau-header-banner",l.innerHTML="<span>\u26A0\uFE0F</span> Verifique os dados capturados do CRM",r.appendChild(l);let p=document.createElement("div");p.innerHTML=`
    ...
    `,r.appendChild(p),s.appendChild(r);let d=document.createElement("div");d.id="bau-dynamic-fallback",d.className="bau-card bau-fallback-card",d.style.display="none",s.appendChild(d);let c=document.createElement("div");c.className="bau-card";let x=document.createElement("label");x.className="bau-label",x.textContent="A\xE7\xF5es R\xE1pidas (Preenche Motivo e Task)",c.appendChild(x);let b=document.createElement("div");b.className="bau-chips-container",[{id:"gtm",text:"\u{1F680} Instala\xE7\xE3o GTM",reason:"Nova Implementa\xE7\xE3o",task:"Setup GTM"},{id:"ecw4",text:"\u{1F6D2} ECW4 Purchase",reason:"Nova Implementa\xE7\xE3o",task:"Google Ads Conversion"},{id:"consent",text:"\u{1F6E1}\uFE0F Consent Mode",reason:"Corre\xE7\xE3o de Tag",task:"GA4 Events"}].forEach(h=>{let T=document.createElement("div");T.className="bau-chip",T.textContent=h.text,T.dataset.id=h.id,T.onclick=()=>{b.querySelectorAll(".bau-chip").forEach(C=>C.classList.remove("active")),T.classList.add("active"),P.playClick();let f=s.querySelector('select[name="reason"]'),y=s.querySelector('select[name="taskType"]');f&&(f.value=h.reason),y&&(y.value=h.task)},b.appendChild(T)}),c.appendChild(b);let u=document.createElement("div");u.className="bau-grid-2",u.style.marginTop="16px",u.innerHTML=`
        <div>
    ...
    `,c.appendChild(u);let w=document.createElement("label");w.className="bau-label",w.textContent="Justificativa / Descri\xE7\xE3o",w.style.marginTop="16px",c.appendChild(w);let S=document.createElement("textarea");S.name="description",S.required=!0,S.className="bau-textarea",S.placeholder="Descreva detalhadamente o que precisa ser feito...",S.style.minHeight="100px",c.appendChild(S);let R=document.createElement("label");R.className="bau-label",R.textContent="Disponibilidade (3 op\xE7\xF5es para reagendamento)",R.style.marginTop="16px",c.appendChild(R);let _=document.createElement("div");_.className="bau-grid-2",_.style.gap="12px";let M=document.createElement("input");M.type="datetime-local",M.name="availability_1",M.required=!0,M.className="bau-input";let J=document.createElement("input");J.type="datetime-local",J.name="availability_2",J.required=!0,J.className="bau-input";let K=document.createElement("input");K.type="datetime-local",K.name="availability_3",K.required=!0,K.className="bau-input",_.appendChild(M),_.appendChild(J),_.appendChild(K),c.appendChild(_),s.appendChild(c);let L=document.createElement("div");L.className="bau-footer";let k=document.createElement("button");k.type="submit",k.className="bau-btn-submit",k.innerHTML="<span>\u26A1</span> Enviar para o TL abrir o Caso",L.appendChild(k),s.appendChild(L),document.body.appendChild(o);let z=document.getElementById("bau-toggle-data"),j=document.getElementById("bau-hidden-data");z&&j&&(z.onclick=()=>{let h=j.style.display==="none";j.style.display=h?"block":"none",z.textContent=h?"Ocultar dados \u25B2":"Ver todos os dados capturados \u25BC",P.playClick()});async function q(){e=await we()||{};let h=e;if(h){document.getElementById("bau-adv-name").textContent=h.advName||"Anunciante Desconhecido",document.getElementById("bau-adv-details").textContent=`CID: ${h.cid||"N/A"} \u2022 AM: ${h.amName||"N/A"}`;let T=document.getElementById("bau-hidden-data");T&&(T.innerHTML=`
                    <b>Email:</b> ${h.email||"N/A"}<br>
                    <b>Idioma:</b> ${h.language||"N/A"}<br>
                    <b>Programa:</b> ${h.salesProgram||"N/A"}<br>
                    <b>Speakeasy ID:</b> ${h.seId||"N/A"}
                `);let y=[{key:"advName",label:"Nome do Anunciante"},{key:"cid",label:"Customer ID (CID)"},{key:"amName",label:"Account Manager"},{key:"email",label:"Email de Contato"},{key:"language",label:"Idioma"},{key:"salesProgram",label:"Sales Program"},{key:"seId",label:"Speakeasy ID"},{key:"site",label:"Site / URL"},{key:"timezone",label:"Fuso Hor\xE1rio"}].filter(C=>!h[C.key]||h[C.key]==="N/A"||h[C.key]==="---");if(y.length>0){d.innerHTML=`
                ...
                `;let C=d.querySelector("#bau-fallback-grid");y.forEach(O=>{let I=document.createElement("div");I.innerHTML=`
                        <label class="bau-label">${O.label}</label>
                        <input type="text" name="${O.key}" class="bau-input" placeholder="Preencher ${O.label}...">
                    `,C.appendChild(I)}),d.style.display="block",l.style.background=ie.yellowLight,l.style.color="#E37400",l.style.borderBottomColor="#FEF1D1",l.innerHTML="<span>\u26A0\uFE0F</span> Dados ausentes. Verifique o fallback abaixo."}else d.style.display="none",l.style.background=ie.greenLight,l.style.color=ie.green,l.style.borderBottomColor=ie.green,l.innerHTML="<span>\u2705</span> Dados capturados com sucesso!"}}s.onsubmit=async h=>{h.preventDefault();let T=k.innerHTML;k.disabled=!0,k.innerHTML="Carregando...";let f=new FormData(s),y=Object.fromEntries(f.entries()),C=e,O=`${y.availability_1} | ${y.availability_2} | ${y.availability_3}`,I={caseId:C.caseId||"",cid:y.cid||C.cid||"",seId:y.seId||C.seId||"",advName:y.advName||C.advName||"",email:y.email||C.email||"",language:y.language||C.language||"",amName:y.amName||C.amName||"",salesProgram:y.salesProgram||C.salesProgram||"",site:y.site||C.site||"",timezone:y.timezone||C.timezone||"",reason:y.reason,taskType:y.taskType,description:y.description,availability:O},N=Object.keys(I).filter($=>!I[$]||I[$]==="N/A"||I[$]==="---");if(N.length>0){k.disabled=!1,k.innerHTML=T,P.playClick(),Y(`Erro: Preencha todos os campos (${N.join(", ")})`,"error");return}try{await io(I,C.agentEmail||"anon"),P.playSuccess(),Y("Escalonamento enviado com sucesso!","success"),s.reset(),b.querySelectorAll(".bau-chip.active").forEach($=>$.classList.remove("active")),F()}catch($){console.error("Erro BAU:",$),Y("Falha ao enviar: "+($.message||"Erro desconhecido"),"error")}finally{k.disabled=!1,k.innerHTML=T}};async function F(){t=!t,t&&(o.style.display="flex",await q()),ue(t,o,"cw-btn-bauform")}return F}function ca(){if(window.techSolInitialized){Ct();return}window.techSolInitialized=!0;let t="v5.2";console.log(`\u{1F680} TechSol Suite Initializing (${t})...`);try{Pt();try{P.initGlobalListeners(),P.playStartup()}catch(d){console.warn("\xC1udio bloqueado:",d)}be.fetchTips(),Ct();let e=fo(),o=yo(),a=wo(),n=Co(),i=Oo(),s=Io(),r=Lo(),l=Mo(),p=Eo();so({toggleNotes:e,toggleEmail:o,toggleScript:a,toggleLinks:n,toggleTimezone:i,toggleLibrary:s,toggleConfigs:r,toggleBAUForm:l,broadcastControl:p}),setTimeout(()=>{be.logEvent("App","Start","Session Start"),Ao(),setTimeout(()=>{ko(t)},500)},2500)}catch(e){console.error("Erro fatal na inicializa\xE7\xE3o:",e),Y("Erro cr\xEDtico ao iniciar o Case Wizard.",{error:!0})}}ca();})();
